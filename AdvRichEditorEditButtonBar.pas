...
const
  ...
  // v1.0.1.0 : New : Added support to open RTF files directly from TAdvRichEditorEditButtonBar
  // v1.0.1.1 : Fixed : Issue with removing TAdvRichEditor at design time
  // v1.0.1.2 : Images for copy and paste were changed.
...
begin
  ...
  // paste
  btn := AddSpeedButton(Self, TAdvRichEditorPaste, I, 'Paste text from clipboard', 'TMSEDBBPASTE');
  btn.Parent := Self;
  Inc(I);

  // copy
  btn := AddSpeedButton(Self, TAdvRichEditorCopy, I, 'Copy selected text', 'TMSEDBBCOPY');
  btn.Parent := Self;
  Inc(I);
...
procedure TAdvRichEditorEditButtonBar.Notification(AComponent: TComponent;
