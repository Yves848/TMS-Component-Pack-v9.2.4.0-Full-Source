...
const
  ...
  // 1.2.2.0 : Improved : Label positioning in high DPI
  // 1.2.3.0 : New : Label positions lTopRight, lpBottomRight added
  // 1.2.4.0 : Improved : Support for High DPI
  // 1.2.4.1 : Fixed : Small memory leak
...
begin
  ...
  FTreeViewFont := TFont.Create;
  FTreeViewFont.Assign(FTreeView.Font);
...
procedure TAdvTreeComboBox.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  lblmargin: integer;
begin
  if Assigned(FLabel) then
...
begin
  ...
  FDPIScale := GetDPIScale(Self, FDropTreeForm.Canvas);


  FSelectedNode := FTreeView.Selected;

  FOldCaption := Caption;
  FDropTreeForm.Scaled := true;
  FDropTreeForm.Left := self.Left;
  FDropTreeForm.Top := self.Top;
  FDropTreeForm.Visible := true;
  FDropTreeForm.Width := Round(FDPIScale * FDropWidth);
  FDropTreeForm.Height := Round(FDPIScale * FDropHeight);

  FTreeView.Font.Assign(FTreeViewFont);
  FTreeView.Font.Height := Round(FDPIScale * FTreeViewFont.Height);

  P := Point(0, 0);
  P := ClientToScreen(P);
  case FDropPosition of
    dpAuto: begin
        if P.y + Round(FDPIScale * FDropHeight) >= GetSystemMetrics(SM_CYSCREEN) then
        begin //Up
          FdropTreeForm.Left := P.x;
          FdropTreeForm.Top := p.y - Round(FDPIScale * FDropHeight);
        end
        else
        begin //Down
          FdropTreeForm.Left := P.x;
