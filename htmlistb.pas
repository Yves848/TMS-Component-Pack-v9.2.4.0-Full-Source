...
const
  ...
  // 2.2.3.0 : Improved : HTML engine drawing in high DPI mode
  // 2.2.4.0 : New : Support for LINE-HEIGHT attribute for <P> tag
  // 2.2.4.1 : Improved : Handling for high DPI with Form.Scaled = false
  // 2.3.0.0 : Support for handling &#id; characters in HTML
  // 2.3.1.0 : Improved : Image align & float right aligned paragraph handling added
  // 2.4.0.0 : New : VCL Styles support added
  // 2.4.0.1 : Improved : VCL Style handling
  // 2.4.1.0 : New : Per monitor high DPI support added
    ...
    try
      ...
      begin
        ...
        if FUseVCLStyles then
          ACanvas.Font.Color := FStyleFontColor
        else
      ...
      if (Index <= Items.Count - 1) then
        HTMLDrawEx(ACanvas, Items[Index], hrect, FImages, pt.x, pt.y, -1, -1, FShadowOffset, False, False, False, not SelectionHTMLColors and (odSelected in State) and ShowSelection,
          True, False, not FEllipsis, FFormScaled, 1.0, urlcol, clNone, clNone, FShadowColor, a, s, f, XSize, YSize, hl, ml, hr, FImageCache, FContainer, FLineSpacing, BidiMode, clHighlight, clHighlightText, [], FDPIScale);
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMListBox.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}

procedure THTMListBox.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Remeasure;
end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure THTMListBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure THTMListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  FUseVCLStyles := False;
  FDPIScale := 1;
  FStyleFontColor := clNone;
end;

procedure THTMListBox.CreateWnd;
var
  frm: TCustomForm;
begin
  inherited;
...
begin
  ...
  begin
    if items.Count * ItemHeight > Round(FDPIScale * Height) then
  ...
  r.Bottom := r.Top + Round(FDPIScale * Height);

  for i := 0 to Items.Count - 1 do
  begin
    if FAutoItemHeight then
    begin
      HTMLDrawEx(Canvas, Items[i], r, FImages, 0, 0, -1, -1, FShadowOffset, True, True, False, True, True, False, not FEllipsis, FFormScaled, 1.0,
        FURLColor, clNone, clNone, FShadowColor, a, s, f, xsize, ysize, hl, ml, hr, FImageCache, FContainer, FLineSpacing, BidiMode, clHighlight, clHighlightText, [], FDPIScale);
...
begin
  ...
  begin
    ...
    begin
      ...
      HTMLDrawEx(Canvas, Items[message.Result], r, FImages, 0, 0, -1, -1, FShadowOffset, True, True, False, True, True, False, not FEllipsis, FFormScaled, 1.0,
                 FURLColor, clNone, clNone, FShadowColor, a, s, f, xsize, ysize, hl, ml, hr, FImageCache, FContainer, FLineSpacing, BidiMode, clHighlight, clHighlightText, [], FDPIScale);
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, Items[i - 1], r, fImages, 0, 0, -1, -1, fShadowOffset, true, true, false, true, true, false, not FEllipsis, FFormScaled, 1.0,
               FURLColor, clNone, clNone, fShadowColor, a, s, f, xsize, ysize, hl, ml, hr, FImageCache, FContainer, 0, BidiMode, clHighlight, clHighlightText, [], FDPIScale);
...
procedure THTMListBox.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if HTMLDrawEx(Canvas, Self.Items[res], r, FImages, X, Y, -1, -1, FShadowOffset, true, false, false, true, true, false, not FEllipsis, FFormScaled, 1.0,
                  FURLColor, clNone, clNone, fShadowColor, anchor, stripped, f, xsize, ysize, hl, ml, hr, FImageCache, FContainer, 0, BidiMode, clHighlight, clHighlightText, [], FDPIScale) then
      Result := Anchor;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      HTMLDrawEx(Canvas, Items[i], r, FImages, 0, 0, -1, -1, FShadowOffset, False, False, False, IsSelect, FBlinking, False, not FEllipsis, FFormScaled, 1.0, fURLColor, clNone, clNone, FShadowColor,
                 a, s, f, xsize, ysize, hl, ml, hr, FImageCache, FContainer, 0, BidiMode, clHighlight, clHighlightText, [], FDPIScale);
    end;
  end;
