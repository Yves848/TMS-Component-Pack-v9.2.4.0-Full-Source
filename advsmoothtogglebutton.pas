...
const
  ...
  // v1.5.1.2 : Fixed : Issue with arrow keys not changing focus
  // v1.5.2.0 : New : ColorDownDisabled and BevelColorDownDisabled properties added
  // v1.5.3.0 : Improved : property TextRendering to change the rendering of the text
  // v1.5.4.0 : Improved : Support for High DPI
  // v1.6.0.0 : New : VCL Styles support
...
type
  ...
  TAdvSmoothToggleButtonTextRenderingHint = (tbtAntiAlias, tbtAntiAliasGridFit, tbtClearType);
  ...
  private
    ...
    FColorDownDisabled: TColor;
    FBevelColorDownDisabled: TColor;
    FTextRendering: TAdvSmoothToggleButtonTextRenderingHint;
    ...
    procedure SetBevelColorDownDisabled(const Value: TColor);
    procedure SetColorDownDisabled(const Value: TColor);
    procedure SetTextRendering(
      const Value: TAdvSmoothToggleButtonTextRenderingHint);
  protected
  ...
  published
    ...
    property ColorDownDisabled: TColor read FColorDownDisabled write SetColorDownDisabled default clGray;
    ...
    property BevelColorDownDisabled: TColor read FBevelColorDownDisabled write SetBevelColorDownDisabled default clGray;
    ...
    property TextRendering: TAdvSmoothToggleButtonTextRenderingHint read FTextRendering write SetTextRendering default tbtClearType;
...
begin
  ...
  begin
    ...
    FColorDownDisabled := (Source as TAdvSmoothToggleButton).ColorDownDisabled;
    FBevelColorDownDisabled := (Source as TAdvSmoothToggleButton).BevelColorDownDisabled;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothToggleButton.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothToggleButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  Appearance.Font.Size := MulDiv(Appearance.Font.Size, M, D);
end;
...
procedure TAdvSmoothToggleButton.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  FColorDown := clNone;
  FColorDownDisabled := clGray;
  FBevelColorDownDisabled := clGray;
...
procedure TAdvSmoothToggleButton.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
...
//  if FTransparent and not FMouseEnter then
  ...
  case TextRendering of
    tbtAntiAlias: g.SetTextRenderingHint(TextRenderingHintAntiAlias);
    tbtAntiAliasGridFit: g.SetTextRenderingHint(TextRenderingHintAntiAliasGridFit);
    tbtClearType: g.SetTextRenderingHint(TextRenderingHintClearTypeGridFit);
  end;

  DPIScale := GetDPIScale(Self, Canvas);
  cRounding := Round(DPIScale * Appearance.Rounding);
  cBevelWidth := Round(DPIScale * BevelWidth);

  if Status.Visible and ((Status.Caption <> '') or not Status.Appearance.Fill.Picture.Empty) then
  begin
    vs := Round(VerticalSpacing * DPIScale);
    hs := Round(HorizontalSpacing * DPIScale);
  end
  else
  begin
    vs := 0;
  ...
  begin
    if not PictureDisabled.Empty then
      pc := PictureDisabled;
    if Down then
      bvc := BevelColorDownDisabled
    else
      bvc := BevelColorDisabled;
    c := ColorDisabled;
    cd := ColorDownDisabled;
  end
  else if Down then
  begin
  ...
  begin
    ...
    DrawRadialBackGround(g, MakeRect(x, y, w, cBevelWidth), bvc, bvcto, 255, 255, rtTop, cRounding - 1);
    DrawRadialBackGround(g, MakeRect(x, y + cBevelWidth - 1, cBevelWidth, h - (cBevelWidth * 2) + 2), bvc, bvcto, 255, 255, rtNone, cRounding - 1);
    DrawRadialBackGround(g, MakeRect(x, h - cBevelWidth + vs, w, cBevelWidth), bvc, bvcto, 255, 255, rtBottom, cRounding - 1);
    DrawRadialBackGround(g, MakeRect(w - cBevelWidth + hs, y + cBevelWidth - 1, cBevelWidth, h - (cBevelWidth * 2) + 2), bvc, bvcto, 255, 255, rtNone, cRounding - 1);
  end
  else if BevelWidth > 0 then
  begin
    path := GDIPFill.CreateRoundRectangle(MakeRect(x, y, w, h), cRounding - 1, rtBoth, false);
  ...
  begin
    ...
    begin
      x := x + (cBevelWidth div 2);
      y := y + (cBevelWidth div 2);
      w := w - cBevelWidth;
      h := h - cBevelWidth;
    end;
  end;
  ...
  FAppearance.Draw(g, Caption, x, y, w, h, 0, 0, c, cd, clNone, fc, False, (FDown and FMouseInside) or (FDown and AutoToggle = true), false, false, false, rtBoth, pc, 0, 0, true,
    picidx, picname, g.GetTextRenderingHint);

  if BorderInnerColor <> clNone then
  begin
    path := GDIPFill.CreateRoundRectangle(MakeRect(x, y, w - 1, h - 1), cRounding - 1, rtBoth, false);
  ...
  begin
    with Status do
    begin
      Appearance.CalculateSize(g, Status.Caption, DPIScale);
      xs := Self.Width + Round(DPISCale * Status.OffsetLeft) - Status.Appearance.GetWidth;
      ys := Round(Status.OffsetTop * DPIScale);
      Appearance.Draw(g, Round(DPISCale * Status.OffsetLeft) + xs, ys, 0, 0, true,Status.Caption, DPIScale);
    end;
  end;

  if TabStop and FFocused and ShowFocus then
    DrawFocus(g, MakeRect(0,0, Width - 1 , Height - 1), cRounding);
  ...
  begin
    ...
    arrowSize := Round(DPIScale * 3);
//    DrawArrow(Canvas, ap, DropDownArrowColor, DropDownArrowColor);

    pts[0].X := am.X - arrowSize;
    pts[0].Y := am.Y -arrowSize;
    pts[1].X := am.X + arrowSize;
    pts[1].Y := am.Y - arrowSize;
    pts[2].X := am.X;
    pts[2].Y := am.Y + arrowSize;

    path := TGPGraphicsPath.Create;

    path.AddPolygon(PGPPointF(@pts), 3);

    b := TGPSolidBrush.Create(MakeColor(255, DropDownArrowColor));
    g.FillPath(b, path);
...
procedure TAdvSmoothToggleButton.SetBevelColorDownDisabled(const Value: TColor);
begin
  if FBevelColorDownDisabled <> Value then
  begin
    FBevelColorDownDisabled := Value;
    Changed;
  end;
end;
...
procedure TAdvSmoothToggleButton.SetColorDownDisabled(const Value: TColor);
begin
  if FColorDownDisabled <> Value then
  begin
    FColorDownDisabled := Value;
    Changed;
  end;
end;
...
procedure TAdvSmoothToggleButton.SetTextRendering(
  const Value: TAdvSmoothToggleButtonTextRenderingHint);
begin
