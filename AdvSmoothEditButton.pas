...
const
  ...
  // v1.3.3.0 : Improved : Support for High DPI (button width)
  //          : Improved : Up,Down,Left,Right key handling
...
var
  ...
  DPIScale: single;
begin
  DPIScale := GetDPIScale(Self, FButton.FButton.Canvas);

  SendMessage(Handle, EM_GETRECT, 0, LParam(@Loc));
  Loc.Bottom := ClientHeight + 1;  {+1 is workaround for windows paint bug}
  Loc.Right := ClientWidth - Round((FButton.Width) * DPIScale) - FUnitSize - 2;
...
begin
  ...
  if (FButton <> nil) then
   begin
     DPIScale := GetDPIScale(Self, FButton.FButton.Canvas);
     cBtnW := Round(FButton.FBWidth * DPIScale);

     if NewStyleControls and Ctl3D then
       FButton.SetBounds(Width - cBtnW - Dist - Offs,1 + FlatCorr, cBtnW, Height - Dist)
     else
       FButton.SetBounds (Width - cBtnW - Offs, 1, cBtnW, Height - 2);
...
begin
  inherited;
  DPIScale := GetDPIScale(Self, FButton.FButton.Canvas);
  Button.PaintTo(DC, X + Width - Round(Button.Width * DPIScale) - 2 - BorderWidth, Y + BorderWidth + 2 );
end;
...
begin
  inherited;
  DPIScale := GetDPIScale(Self, FButton.FButton.Canvas);
  Button.PaintTo(Canvas, X + Width - Round(Button.Width * DPIScale) - 2 - BorderWidth, Y + BorderWidth + 2 );
end;
