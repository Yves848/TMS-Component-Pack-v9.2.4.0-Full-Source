...
uses
  ...
  GDIPFill, Math, GDIPPictureContainer, ImgList, AdvStyleIF, Dialogs,
  AdvGDIP, Types
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
const
  ...
  // v1.5.0.1 : Fixed : Issue setting progressbar maximum
  // v1.5.1.0 : New: Per monitor support for high DPI
  // v1.5.1.1 : Fixed : Issue with use on a datamodule
...
type
  ...
  protected
    ...
    procedure GetTextSize(g: TGPGraphics; r: TRect; s: String; ft: TFont; var sizer: TGPRectF; ADPIScale: single = 1);
...
begin
  ...
  begin
    Result.Width := Round(SplashScreen.FDPIScale * SplashScreen.Width);
    Result.Height := Round(SplashScreen.FDPIScale * SplashScreen.Height);
  end;
  ...
  begin
    ...
    Result.FSplashProgressBar.FDPIScale := SplashScreen.FDPIScale;
    Result.FSplashProgressBar.Parent := Result;
    Result.FSplashProgressBar.Visible := False;
    Result.FSplashProgressBar.Appearance.FDPIScale := SplashScreen.FDPIScale;
    ...
    Result.FSplashProgressBar.Left := Round(SplashScreen.ProgressBar.Left * SplashScreen.FDPIScale);
    Result.FSplashProgressBar.Top := Round(SplashScreen.ProgressBar.Top * SplashScreen.FDPIScale);
    Result.FSplashProgressBar.Width := Round(SplashScreen.ProgressBar.Width * SplashScreen.FDPIScale);
    Result.FSplashProgressBar.Height := Round(SplashScreen.ProgressBar.Height * SplashScreen.FDPIScale);
  end;
...
function DrawGDIPText(g: TGPGraphics; Text: string; P: TPoint; Font: TFont; GradType: TAdvGradientType; Pic: TAdvGDIPPicture; HatchStyle: THatchStyle; OpacityStart, OpacityEnd: Byte; ColorStart, ColorEnd: TColor; CalcSize: Boolean; Angle: integer; TextRendering: TAdvSmoothSplashScreenTextRenderingHint; ADPIScale: single = 1): TSize;
var
  ff: TGPFontFamily;
  fs, x, y: Integer;
  sf: TGPStringFormat;
...
begin
  ...
  begin
    ...
    f := TGPFont.Create(ff, Font.Size * ADPIScale, fs, UnitPoint);
...
begin
  ...
  GetTextSize(g, r, s, Font, sizerect, FDPIScale);
  tw := sizerect.Width;
  th := sizerect.Height;
  ...
  bn := GetInsideRectF(r);
  sf := TGPStringFormat.Create;
  f := TGPFont.Create(ff, ft.Size * FDPIScale, fs, UnitPoint);
...
procedure TGDIPSplashProgress.GetTextSize(g: TGPGraphics; r: TRect; s: String; ft: TFont; var sizer: TGPRectF; ADPIScale: single = 1);
var
  ff: TGPFontFamily;
  fs: integer;
  bn: TGPRectF;
...
begin
  ...
  bn := GetInsideRectF(r);
  sf := TGPStringFormat.Create;
  f := TGPFont.Create(ff, ft.Size * ADPIScale , fs, UnitPoint);
...
begin
  ...
  begin
    ...
    FSplashWindow.FSplashProgressBar.Left := Round(FProgressBar.Left * FDPIScale);
    FSplashWindow.FSplashProgressBar.Top := Round(FProgressBar.Top * FDPIScale);
    FSplashWindow.FSplashProgressBar.Width := Round(FProgressBar.Width * FDPIScale);
    FSplashWindow.FSplashProgressBar.Height := Round(FProgressBar.Height * FDPIScale);
...
begin
  ...
  if Assigned(FMainForm) then
    FDPIScale := GetDPIScale(FMainForm, FMainForm.Canvas)
  else
    FDPIScale := CalcDPIScale(0, true);
...
begin
  ...
  R := Rect(Round(FDPIScale * Item.PosX), Round(FDPIScale * Item.PosY), Width, Item.PosY + Round(FDPIScale * 30)); // TODO: proper height
  RefreshRect(R, Fast);
end;
...
begin
  ...
  X := Round(SplashScreen.FDPIScale * Item.PosX);
  Y := Round(SplashScreen.FDPIScale * Item.PosY);

  // Draw Image
  if Assigned(Item.Picture) and (not Item.Picture.Empty) then
  begin
    DrawGDIPImage(g, Point(Round(SplashScreen.FDPIScale * Item.PosX), Round(SplashScreen.FDPIScale * Item.PosY)), Item.Picture);
    X := X + Item.Picture.Width + Round(SplashScreen.FDPIScale * SplashScreen.PictureGap);
  end
  ...
  begin
    ...
    X := X + SplashScreen.Images.Width + Round(SplashScreen.FDPIScale * SplashScreen.PictureGap);
  end;

  if (Item.Text <> '') then
    DrawGDIPText(g, Item.Text, Point(X, Y), Item.Font, gtSolid, nil, HatchStyleHorizontal, 255, 255, Item.Font.Color, Item.Font.Color, False, 0, SplashScreen.TextRendering, SplashScreen.FDPIScale);
  ...
  if not Assigned(SplashScreen) or not Assigned(ListItem) then
    ...
    begin
      ...
      HTMLDrawGDIP(g, FSplashScreen.FListItemsSettings.HTMLFont, ListItem.HTMLText, R, FSplashScreen.Images, 0, 0, -1, -1, FSplashScreen.FListItemsSettings.HTMLShadowOffset, False, True, False, False,
        False, False, True, 1.0, FSplashScreen.FListItemsSettings.HTMLURLColor, clNone, clNone, FSplashScreen.FListItemsSettings.HTMLShadowColor, a, s, k, XSize, YSize, l, m, hr, nil, FSplashScreen.FContainer, 2, FSplashScreen.FDPIScale);
...
begin
  ...
  begin
    ...
    R1.Bottom := R1.Bottom + Round(SplashScreen.FDPIScale * SplashScreen.ListItemsSettings.Space);
  end;
  ...
  begin
    ...
    begin
      ...
      d := d - (IR.Bottom - IR.Top) - Round(SplashScreen.FDPIScale * SplashScreen.ListItemsSettings.Space);
      if (d <= 0) then
      begin
...
begin
  ...
  InflatRounding(Round(FSplashScreen.FDPIScale * FSplashScreen.Fill.Rounding), R);
  R.Right := R.Right - 5;
  sp := 1;
  spy := 3;
  ...
  with SplashScreen.BasicProgramInfo.ProgramName do
    PNS := DrawGDIPText(g, Text, Point(0, 0), Font, GradientType, Picture, HatchStyle, OpacityStart, OpacityEnd, ColorStart, ColorEnd, True, Angle, SplashScreen.TextRendering, SplashScreen.FDPIScale);

  with SplashScreen.BasicProgramInfo.ProgramVersion do
    PVS := DrawGDIPText(g, Text, Point(0, 0), Font, GradientType, Picture, HatchStyle, OpacityStart, OpacityEnd, ColorStart, ColorEnd, True, Angle, SplashScreen.TextRendering, SplashScreen.FDPIScale);
  ...
  with SplashScreen.BasicProgramInfo.ProgramName do
    DrawGDIPText(g, Text, Point(X, Y), Font, GradientType, Picture, HatchStyle, OpacityStart, OpacityEnd, ColorStart, ColorEnd, False, Angle, SplashScreen.TextRendering, SplashScreen.FDPIScale);
  ...
  DrawGDIPText(g, SplashScreen.BasicProgramInfo.CopyRight, Point(X, Y), SplashScreen.BasicProgramInfo.CopyRightFont,
               gtSolid, nil, HatchStyleHorizontal, 255, 255, SplashScreen.BasicProgramInfo.CopyRightFont.Color, SplashScreen.BasicProgramInfo.CopyRightFont.Color, False, 0, SplashScreen.TextRendering, SplashScreen.FDPIScale);
...
begin
  ...
  begin
    rgn := 0;
    R := ClientRect;
    rounding := Round(SplashScreen.FDPIScale * FSplashScreen.Fill.Rounding) shl 1; // because of smooth gradient implementation
...
begin
  ...
  begin
    ...
    FSplashProgressBar.Appearance.Draw(g, Rect(Round(FSplashScreen.FDPIScale * FSplashProgressBar.Left), Round(FSplashScreen.FDPIScale * FSplashProgressBar.Top), Round(FSplashScreen.FDPIScale * (FSplashProgressBar.Left + FSplashProgressBar.Width)), Round(FSplashScreen.FDPIScale * (FSplashProgressBar.Top + FSplashProgressBar.Height))), FSplashProgressBar.Minimum, FSplashProgressBar.Maximum, FSplashProgressBar.Position);
    if not Assigned(graphics) then
      g.Free;
  end;
end;
...
begin
  ...
  FAdvSplashScreen.FDPIScale := GetDPIScale(FAdvSplashScreen.FMainForm, FAdvSplashScreen.FMainForm.Canvas);
