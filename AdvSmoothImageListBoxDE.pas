{**************************************************************************}
{ TAdvSmoothImageListBoxDE DESIGN TIME EDITOR                              }
...
uses
  Classes, AdvSmoothImageListBox, DesignIntf, DesignEditors;

type
  TAdvSmoothImageListBoxEditor = class(TDefaultEditor)
  protected
...
procedure TAdvSmoothImageListBoxEditor.EditProperty(const PropertyEditor: IProperty; var Continue: Boolean);
var
  PropName: string;
begin
...
procedure TAdvSmoothImageListBoxEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothImageListBox).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothImageListBox) then
         (Component as TAdvSmoothImageListBox).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothImageListBoxEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothImageListBoxEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
