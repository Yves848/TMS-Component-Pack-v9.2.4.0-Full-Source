...
const
  ...
  // v1.6.0.2  : Improved : Support for high DPI
  // v1.6.0.3  : Fixed : Warning in 64bit code
  // v1.6.0.4  : Fixed : Issue with tool item size calculation
  // v1.6.0.5  : Improved : Drawing of TAdvOfficeColorSelector when transparent component icon is used
  // v1.6.1.0  : Improved : Color selector selected item & color drawing + high DPI adaption
  // v1.6.1.1  : Fixed : Issue with color selector when imagelist image is used
  // v1.6.1.2  : Fixed : Small drawing issues in high DPI mode of selected color bar
  // v1.6.2.0  : Improved : Office 2016 black style
...
type
  ...
  protected
    ...
    procedure GetToolImage(bmp: TBitmap); override;
    procedure GetToolPicture(var pic: TGDIPPicture); override;
    procedure DrawCustom(ACanvas: TCanvas; R: TRect); override;
    procedure DrawGlyphAndCaption(Pic: TGDIPPicture; R: TRect); override;
    procedure DoSelect(Index: integer; Item: TAdvSelectorItem); override;
    function GetActionLinkClass: TControlActionLinkClass; override;
  public
...
begin
  ...
  begin
    ...
    FScale := CalcDPIScale(Canvas, GetParentForm(Self));

    case Style of
    ssButton:
      begin
        if DropDownButton and
          ( ((DropDownPosition = dpRight) and (X >= (Width - Round(FScale * FDropDownBtnWidth)))) or
           ((DropDownPosition = dpBottom) and (Y >= (Height - Round(FScale * FDropDownBtnWidth)))) or not DropDownSplit) then
        begin
          PopupBtnDown;
        end
        else
...
begin
  ...
  begin
    ...
    Appearance.TextColor := clWindowText;
   ...
   tsOffice2016White:
    ...
    tsOffice2016Black:
      begin
        ...
        ColorDropDown := $363636;
        ColorDropDownTo := clNone;
        Font.Color := clSilver;
        CaptionAppearance.TextColor := clSilver;
        Appearance.TextColor := clSilver;
        Appearance.TextColorHot := clSilver;
        Appearance.TextColorDown := clSilver;
      end;
    tsCustom:
      begin
      end;
...
begin
  ...
  begin
    ...
    Canvas.Brush.Style := bsClear;
    Canvas.Font.Color := SelectionAppearance.TextColorDisabled;
...
begin
  ...
  begin
    ...
    scale := CalcDPIScale(FSelectorPanel.Canvas.Handle, true);

    FSelectorPanel.MinButtonWidth := Round(18 * scale);
    FSelectorPanel.MinButtonHeight := Round(18 * scale);
    FSelectorPanel.FButtonHeight := Round(20 * scale);
...
begin
  ...
  begin
    ...
    begin
      ...
      bmp.Height := Min(Picture.Height, Self.Height - 2);

      {$IFDEF DELPHI_UNICODE}
      ClearBitmap32(bmp,clblack,0);
      {$ENDIF}

      if ((bmp.Width <> Picture.Width) or (bmp.Height <> Picture.Height)) then
      begin
...
procedure TAdvCustomOfficeColorSelector.GetToolPicture(var pic: TGDIPPicture);
begin
  inherited;
...
procedure TAdvCustomOfficeColorSelector.DrawCustom(ACanvas: TCanvas; R: TRect);
var
  bw,bh,l,t: integer;
  FScale: single;
  FDropW: integer;
begin
  ...
  begin
    ...
    FScale := CalcDPIScale(ACanvas, GetParentForm(Self));

    if DropDownButton then
      FDropW := Round(FScale * DropDownSectWidth);

    if  Assigned(Picture) and not Picture.Empty then
    begin
      bw := Min(Picture.Width, Width - 3);
      bh := Min(Picture.Height, Height - 2);
    end;

    if  Assigned(Images) and (ImageIndex >= 0) then
    begin
      bw := Min(Images.Width, Width - 3);
      bh := Min(Images.Height, Height - 2);
    end;

    ACanvas.Brush.Color := SelectedColor;
    ACanvas.Pen.Color := SelectedColor;

    if (bw > 0) and (bh > 0) then
    begin
      l := (Width - FDropW - bw) div 2;
      t := (Height - bh) div 2;
      ACanvas.Rectangle(l, t + bh - Round(4 * FScale), l + bw, t + bh);
    end
    else
    begin
      FScale := CalcDPIScale(ACanvas, GetParentForm(Self));
      ...
      ACanvas.Rectangle(l, t, l + bw, t + bh);
    end;
  end;
end;
...
begin
  ...
  btnsize := Round(12 * CalcDPIScale(FSelectorPanel.Canvas.Handle, true));
  btnoffs := Round(3 * CalcDPIScale(FSelectorPanel.Canvas.Handle, true));
  ...
  begin
    ...
    begin
      ...
      FSelectorPanel.Canvas.Rectangle(R.Left + btnoffs, R.Top + btnoffs, R.Left + btnoffs + btnsize, R.Top + btnoffs + btnsize);
    end;
  end
  else
  begin
    ...
    begin
      FSelectorPanel.Canvas.Pen.Color := clGray;
      FSelectorPanel.Canvas.Brush.Color := FSelectorPanel.Items.Items[Index].BackGroundColor;
      FSelectorPanel.Canvas.Rectangle(R.Left + btnoffs + 1, R.Top + btnoffs + 1, R.Left + btnoffs + 1 + btnsize, R.Top + btnoffs + 1 + btnsize);
    end;
    FSelectorPanel.Canvas.Brush.Style := bsClear;
    FSelectorPanel.Canvas.Font.Color := CaptionAppearance.TextColor;
...
begin
  ...
  begin
    ...
    bmp.Canvas.Font.Color := CaptionAppearance.TextColor;
...
procedure TAdvCustomOfficeScrollSelector.SetComponentStyle(AStyle: TTMSStyle);
begin
  FStyle := AStyle;
