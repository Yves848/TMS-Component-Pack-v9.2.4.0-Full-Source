...
type
  TNarrowDownOption = (ndCaseSensitive, ndRemoveAccented);

  TNarrowDownOptions = set of TNarrowDownOption;
...
type
  ...
  protected
    ...
    procedure DrawButtonText(Canvas: TCanvas; const Caption: string;
      TextBounds: TRect; State: TButtonState; BiDiFlags: LongInt; Shadow: boolean);
    function DrawButton(Canvas: TCanvas; const Client: TRect;
      ...
      BiDiFlags: LongInt; Shadow: boolean): TRect;
  ...
  TSearchType = (stSearch, stNarrowDown);
  ...
  private
    ...
    FSearchFromFocusCell: boolean;
    FSearchType: TSearchType;
    FSearchErrorTextColor: TColor;
    ...
    procedure SetSearchColor(const Value: TColor);
    procedure SetSearchTextColor(const Value: TColor);
  protected
    procedure Changed;
  ...
  published
    ...
    property SearchColor: TColor read FSearchColor write SetSearchColor default clWindow;
    property SearchTextColor: TColor read FSearchTextColor write SetSearchTextColor default clWindowText;
    property SearchErrorColor: TColor read FSearchErrorColor write FSearchErrorColor default clRed;
    property SearchErrorTextColor: TColor read FSearchErrorTextColor write FSearchErrorTextColor default clWhite;
    property SearchDirection: TSearchDirection read FSearchDirection write FSearchDirection default sdTopBottom;
    property SearchFromFocusCell: boolean read FSearchFromFocusCell write FSearchFromFocusCell default false;
    property SearchType: TSearchType read FSearchType write SetSearchType default stSearch;
  ...
  private
    ...
    FToggleNodeOnDblClick: Boolean;
    FSelectOnRightClickNewSelectionOnly: Boolean;
  ...
  published
    ...
    property SelectOnRightClickNewSelectionOnly: Boolean read FSelectOnRightClickNewSelectionOnly write FSelectOnRightClickNewSelectionOnly default False;
    ...
    property ToggleNodeOnDblClick: Boolean read FToggleNodeOnDblClick write FToggleNodeOnDblClick default False;
...
uses
  ActnList, ComObj, AdvUtil, AsgHTMLE, Forms, Math
{$IFDEF DELPHIXE2_LVL}
  , VCL.Themes
{$ENDIF}
  ;
...
begin
  ...
  begin
    Brush.Style := bsClear;
    if (State = bsDisabled) and Shadow then
    begin
...
begin
  ...
  if ShowCaption then
    DrawButtonText(Canvas, Caption, Result, State, BiDiFlags, Shadow);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        LStyle.DrawElement(Canvas.Handle, LDetails, paintrect);

        LStyle.GetElementColor(LDetails, ecTextColor, LColor);
  ...
  if FDown and (GroupIndex <> 0) then
    DrawButton(Canvas, PaintRect, Offset, Caption, FLayout, FMargin,
      FSpacing, bsDown, FTransparent, DrawTextBiDiModeFlags(0),shadow)
  else
    DrawButton(Canvas, PaintRect, Offset, Caption, FLayout, FMargin,
      FSpacing, FState, FTransparent, DrawTextBiDiModeFlags(0),shadow);
end;
...
begin
  ...
  begin
    ...
    tlBottom: BT := ClientRect.Bottom - sz;
    end;
    if FAlignment = taRightJustify then
      BL := ClientRect.Right - sz - 1
    else
      BL := 0;

    r := Rect(BL, BT, BL + sz, BT + sz);
    ...
    begin
      ...
      try
        ...
        cbChecked:
          begin
            if not Enabled then
              LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedDisabled)
            else
            if FDown then
              LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedPressed)
            else
              if FHot then
                LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedHot)
              else
        ...
        cbUnchecked:
          begin
            ...
            if FDown then
              LDetails := LStyle.GetElementDetails(tbCheckBoxUnCheckedPressed)
            else
              if FHot then
                LDetails := LStyle.GetElementDetails(tbCheckBoxUnCheckedHot)
              else
        ...
        cbGrayed:
          begin
            if not Enabled then
              LDetails := LStyle.GetElementDetails(tbCheckBoxMixedDisabled)
            else
            if FDown then
              LDetails := LStyle.GetElementDetails(tbCheckBoxMixedPressed)
            else
              if FHot then
                LDetails := LStyle.GetElementDetails(tbCheckBoxMixedHot)
              else
                LDetails := LStyle.GetElementDetails(tbCheckBoxMixedNormal);
          end;
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          tlBottom: r.Top := ClientRect.Bottom - sz - 3;
          end;
          r.Bottom := r.Top + sz - 1;
          flg := 0;
    ...
    r.Top := r.Top + 4;
    r.Left := r.Left + Round(10 * FDPIScale);
...
begin
  ...
  begin
    ...
    FSearchErrorTextColor := (Source as TSearchFooter).SearchErrorTextColor;
    ...
    FSearchFromFocusCell := (Source as TSearchFooter).SearchFromFocusCell;
    FSearchType := (Source as TSearchFooter).SearchType;
    FSearchColor := (Source as TSearchFooter).SearchColor;
    FSearchTextColor := (Source as TSearchFooter).SearchTextColor;
  end;
end;
...
begin
  ...
  FSearchFromFocusCell := false;
  FSearchColumn := -1;
  FSearchType := stSearch;
  ...
  FSearchErrorTextColor := clWhite;
  FSearchColor := clWindow;
  FSearchTextColor := clWindowText;
...
procedure TSearchFooter.FontChanged(Sender: TObject);
begin
  Changed;
end;
...
procedure TSearchFooter.SetSearchColor(const Value: TColor);
begin
  if (FSearchColor <> Value) then
  begin
    FSearchColor := Value;
    Changed;
  end;
...
procedure TSearchFooter.SetSearchTextColor(const Value: TColor);
begin
  if (FSearchTextColor <> Value) then
  begin
    FSearchTextColor := Value;
    Changed;
  end;
end;

procedure TSearchFooter.SetSearchType(const Value: TSearchType);
begin
  if (FSearchType <> Value) then
  begin
    FSearchType := Value;
    Changed;
  end;
...
begin
  ...
  begin
    ...
    FToggleNodeOnDblClick := (Source as TMouseActions).ToggleNodeOnDblClick;
    ...
    FSelectOnRightClickNewSelectionOnly := (Source as TMouseActions).SelectOnRightClickNewSelectionOnly;
  end;
end;
