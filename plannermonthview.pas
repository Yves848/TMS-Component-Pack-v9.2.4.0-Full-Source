...
const
  ...
  BLD_VER = 4; // Build nr.
  DATE_VER = 'Feb, 2019'; // Month version
  ...
  // 3.1.1.0 : New : OnItemAnchorEnter, OnItemAnchorExit, OnItemAnchorClick events added
  // 3.1.1.1 : Fixed : Issue with item anchor handling for items without caption
  // 3.1.1.2 : Fixed : Performance improvement when loading data from large datasets
  // 3.1.1.3 : Fixed : Issue with Dbl Click and multiselect
  // 3.1.1.4 : Fixed : Rare issue with triggering OnItemMove/OnItemSize
...
var
  ...
  Allow: Boolean;
  Anchor, StrippedHTMLString, FocusAnchor, CID, CV, CT: string;
...
begin
  ...
  begin
    ...
    if IsHtml(APlannerItem, APlannerItem.NotesText, True) then
    begin
      R := ItemRectAtRow(APlannerItem, DateToXY(ad).Y);
      ...
      if HTMLDrawEx(Canvas, APlannerItem.NotesText, R,
        ...
        , cor, CID, CV, CT, nil,
        FPlanner.PictureContainer, Handle
      ) then
      ...
      begin
        ...
        begin
          FLastAnchor := Anchor;
          Self.Cursor := crHandPoint;
          if Assigned(OnItemAnchorEnter) then
            OnItemAnchorEnter(FPlanner, APlannerItem, Anchor);
        end;
        Exit;
      end
      else if FLastAnchor <> '' then
      begin
        FLastAnchor := '';
        if Assigned(OnItemAnchorExit) then
          OnItemAnchorExit(FPlanner, APlannerItem, Anchor);
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if Assigned(FOnItemAnchorClick) then
            FOnItemAnchorClick(Self, APlannerItem, APlannerItem.URL);
          Exit;
        end;
      end;
      ...
      begin
        ...
        tr.Left := tr.Left + FPlanner.TrackWidth;
        tr.Right := tr.Right - 4;

        Canvas.Font := APlannerItem.Font;
        if HTMLDrawEx(Canvas, APlannerItem.NotesText, tr,
          FPlanner.PlannerImages, X, Y, -1, -1, 1, True, False, False,
          True, True, False, APlannerItem.WordWrap
          , False
          , 1.0, FPlanner.URLColor, clNone, clNone, clGray, Anchor,
          StrippedHTMLString, FocusAnchor, XSize, YSize, ml, hl, hr
          , cor, CID, CV, CT, nil, FPlanner.PictureContainer, Handle) then
          begin
            FLastAnchor := '';
            if Assigned(FOnItemAnchorClick) then
              FOnItemAnchorClick(Self, APlannerItem, Anchor);
            Exit;
          end;
       end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FMouseOnItemStart then
      begin
        Sized := Items.Selected.ItemStartTime + dfDate <> FMouseDownStartDate;
        if not Items.HasMonthPlannerItem(Items.Selected.ItemStartTime + dfDate, Items.Selected.ItemEndTime) then
        begin
          OldStartTime := FMouseDownStartDate;
          OldEndTime := FMouseDownEndDate;
      ...
      begin
        Sized := Items.Selected.ItemEndTime + dfDate <> FMouseDownEndDate;
  ...
  begin
    ...
    begin
      ...
      Allow := not Items.HasMonthPlannerItem(Items.Selected.ItemStartTime + dfDate, Items.Selected.ItemEndTime + dfDate);
      if Allow or (Items.Selected.ItemEndTime + dfDate <> FMouseDownEndDate) then
      begin
        if Allow then
        ...
        begin
          Allow := true;
          if (Items.Selected.ItemEndTime <> FMouseDownEndDate) then
          TMonthPlannerItem(Items.Selected).MoveMonthPlannerItem(Items.Selected.ItemStartTime, Items.Selected.ItemEndTime, true, Allow);
        end;
