...
uses
  ...
  Variants, Types, ImgList, PictureContainer, AdvStyleIF
...
const
  ...
  // v2.2.3.0 : New : Public property AdvCardList.SortedCards[index]: TAdvCard added
  // v2.2.3.1 : Fixed : Conflict between checkbox click and card rearrange
  // v2.2.3.2 : Fixed : Issue with click in area between cards
  // v2.2.3.3 : Fixed : Issue with CardRearrange in combination with TFormSize component
  // v2.2.4.0 : New : CardRearrangeType property added
  // v2.2.4.1 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // v2.3.0.0 : New : High DPI support added
  // v2.4.0.0 : New : VCL Styles support added
  // v2.4.0.1 : Fixed : Issue with column resizing in specific circumstances
  //          : Fixed : Issue with checkbox click & dbl click
...
type
  ...
  public
    ...
    function GetCardCaptionHeight: integer;
    function GetItemValueWidth: integer;
    function GetItemLabelRealWidth: integer;
    function GetitemSpacing: integer;
    function GetVertMargins: integer;
    function GetHorzMargins: integer;
  published
  ...
  TCardRearrangeType = (crFull, crCaption);
  ...
  private
    ...
    FCardRearrangeType: TCardRearrangeType;
    ...
    function GetSortedCard(Index: integer): TAdvCard;
    function GetCardState(const pCard: TAdvCard): TCardState;
  protected
    FCardTemplate: TAdvCardTemplate;
    ...
    property CardRearrangeType: TCardRearrangeType read FCardRearrangeType write FCardRearrangeType default crFull;
    property DraggingCard: TAdvCard read FDraggingCard write FDraggingCard;

    function GetCardVertSpacing: integer;
    function GetCardHorzSpacing: integer;
  ...
  public
    ...
    property SortedCards[Index: integer]: TAdvCard read GetSortedCard;
...
begin
  if Assigned(FCardTemplate) and Assigned(FCardTemplate.FCardList) then
    FCardTemplate.FCardList.CardTemplateItemChanged(Item);
end;

procedure TAdvCardTemplateItems.ItemHideCondChanged(Item: TAdvCardTemplateItem);
begin
  if Assigned(FCardTemplate) and Assigned(FCardTemplate.FCardList) then
    FCardTemplate.FCardList.CardTemplateItemHideCondChanged(Item);
end;
...
function TAdvCardTemplate.GetCardCaptionHeight: integer;
begin
  Result := Round(FCardCaptionHeight * FCardList.FDPIScale);
end;

function TAdvCardTemplate.GetCardWidth: integer;
begin
  Result := Round(FCardWidth * FCardList.FDPIScale);
end;

function TAdvCardTemplate.GetHorzMargins: integer;
begin
  Result := Round(FHorMargins * FCardList.FDPIScale);
end;

function TAdvCardTemplate.GetItemLabelRealWidth: integer;
begin
  Result := Round(FItemLabelWidth * FCardList.FDPIScale);
end;

function TAdvCardTemplate.GetitemSpacing: integer;
begin
  Result := Round(FItemSpacing * FCardList.FDPIScale);
end;

function TAdvCardTemplate.GetItemValueWidth: integer;
begin
  Result := Round(FItemValueWidth * FCardList.FDPIScale);
end;
...
function TAdvCardTemplate.GetVertMargins: integer;
begin
  Result := Round(FVertMargins * FCardList.FDPIScale);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (x + sz.cx > R.Right - CardList.GetCardHorzSpacing - 5) then
      begin
        x := R.Left;
        y := y + sz.cy + sp;
      end
...
var
  ...
  begin
    ...
    begin
      ...
      Font.Height := Round(Font.Height * FCardList.FDPIScale);
      S := TItem.Caption;
      Result := TextHeight('gh');
    end;
  end;
    ...
    case TItem.DataType of
      idtImage:
        ...
        begin
          ...
          begin
            m := self.FCardList.CardTemplate.GetItemValueWidth;
      ...
      begin
        ...
        begin
          ...
          FCardList.Canvas.Font.Height := Round(TItem.ValueFont.Height * FCardList.FDPIScale);
          ...
          begin
            HTMLDrawEx(FCardList.Canvas, Item.AsString, Rect, FCardList.Images, -1,-1,-1,-1,2,false,true,false,false,false,false,true,FCardList.FFormScaled,1.0,clBlue,clNone,clNone,clGray,sa,sv,fa,xsize,ysize,hl,ml,hr,nil,Item.PictureContainer,2);
...
begin
  ...
  begin
    ...
    Card.FCaptionClientRect := Bounds(0, 0,
      FCardList.CardTemplate.GetCardWidth, FCardList.CardTemplate.GetCardCaptionHeight);
    Card.FCaptionCardRect := Card.FCaptionClientRect;
    SumHeight := FCardList.CardTemplate.GetCardCaptionHeight;
    // Items
    Inc(SumHeight, FCardList.CardTemplate.GetVertMargins);
    ...
    begin
      ...
      itLabeledItem:
        begin
          LabelWidth := FCardList.CardTemplate.GetItemLabelRealWidth;
          ValueWidth := FCardList.CardTemplate.GetItemValueWidth;
      ...
      itItem, itHTMLText:
        begin
          LabelWidth := 0;
          ValueIndent := 0;
          ValueWidth := FCardList.CardTemplate.GetCardWidth - FCardList.CardTemplate.HorMargins * 2;
        end;
      itLabel:
        begin
          LabelWidth := FCardList.CardTemplate.GetCardWidth - FCardList.CardTemplate.HorMargins * 2;
          ValueWidth := 0;
          ValueIndent := 0;
        end;
      itProgressBar:
      ...
      if ItemN <> 0 then
        Inc(SumHeight, FCardList.CardTemplate.GetItemSpacing);

      // label
      Item.FLabelClientRect := Bounds(0, 0, LabelWidth, ResHeight);
      Item.FLabelCardRect := Bounds(FCardList.CardTemplate.GetHorzMargins,
        SumHeight, LabelWidth, ResHeight);
      // value
      Item.FValueClientRect := Bounds(0, 0, ValueWidth, ResHeight);
      Item.FValueCardRect := Bounds(FCardList.CardTemplate.GetHorzMargins +
        LabelWidth + ValueIndent, SumHeight, ValueWidth, ResHeight);
      Inc(SumHeight, ResHeight);
    end; {for ItemN}
    Inc(SumHeight, FCardList.CardTemplate.GetVertMargins);
    Card.FClientRect := Bounds(0, 0, FCardList.CardTemplate.GetCardWidth, SumHeight);
    ...
    case FEdControlType of
      ...
      ctMaskEdit:
        ...
        begin
          ...
          Font.Height := Round(Font.Height * FDPIScale);
...
begin
  EditRect.Left := Round(EditRect.Left / FDPIScale);
  EditRect.Top := Round(EditRect.Top / FDPIScale);
  EditRect.Right := Round(EditRect.Right / FDPIScale);
  EditRect.Bottom := Round(EditRect.Bottom / FDPIScale);
...
begin
  ...
  begin
    ...
    ScrollBar.Height := Round(16 * FDPIScale);
  end
  else // ScrollBarType = sbtVertical
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          t := Cards.Items[j].FListRect.Top - GetCardVertSpacing;
    ...
    ScrollBar.Width := Round(16 * FDPIScale);
  end;
...
procedure TCustomAdvCardList.InitVCLStyle(init: boolean);
var
{$IFDEF DELPHIXE2_LVL}
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgFixedCellNormal), ecBorderColor, clr);
    CardNormalAppearance.CaptionBorderColor := clr;

    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgFixedCellNormal), ecFillColor, clr);
    CardNormalAppearance.CaptionColor.Color := clr;

    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgFixedCellNormal), ecFillColor, clr);
    CardNormalAppearance.Color.Color := clr;
    CardNormalAppearance.Color.ColorTo := clr;

    Details := StyleServices.GetElementDetails(thHeaderItemNormal);
    if StyleServices.GetElementColor(Details, ecTextColor, clr) then
    begin
      {$IFDEF DELPHIXE6_LVL}
      if seFont in StyleElements then
      begin
      {$ENDIF}
      CardNormalAppearance.CaptionFont.Color := clr;
      Font.Color := clr;
      {$IFDEF DELPHIXE6_LVL}
      end;
      {$ENDIF}
    ...
    if StyleServices.GetElementColor(Details, ecTextColor, clr) then
    begin
      CardNormalAppearance.ItemLabelFont.Color := clr;
      CardNormalAppearance.ItemEditFont.Color := clr;
    end;
  end;
{$ENDIF DELPHIXE2_LVL}
end;
...
{$IFDEF DELPHIXE2_LVL}
{ The CM_STYLECHANGED message traps style changes made at runtime by the end-user. }
procedure TCustomAdvCardList.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  CardTemplateChanged;
end;
...
begin
  ...
  begin
    ...
    if Value > FColumns then
      Value := FColumns;
    ShiftListRects((FLeftCol - Value) * GetColumnWidth);
    FLeftCol := Value;
    if (FColumns - FLeftCol + 1) * GetColumnWidth <= Width then
      FViewedColumns := FColumns - FLeftCol + 1
    else
      FViewedColumns := Width div GetColumnWidth;
    UpdateScrollBar;
    Invalidate;
  end;
end;
...
function TCustomAdvCardList.GetSortedCard(Index: integer): TAdvCard;
begin
  Result := nil;
  if Index < Length(FSortedCards) then
    Result := Cards[FSortedCards[index]];
end;

function TCustomAdvCardList.GetCardState(const pCard: TAdvCard): TCardState;
begin
  if pCard <> nil then
  begin
    if pCard.FEditing and CardEditingAppearance.Enabled then
      Result := csEditing
    else if pCard.FSelected and CardSelectedAppearance.Enabled then
      Result := csSelected
    else if pCard.FMouseOver and CardHoverAppearance.Enabled then
      Result := csHover
    else
      Result := csNormal;
  end
  else
    Result := csNormal; { a fallback default }
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      res := HTMLDrawEx(Canvas, Card.ItemList[ItemN].AsString, R, Images, pt.X, pt.Y,-1,-1,2,true,false,false,false,false,false,true,FFormScaled,1.0,clBlue,clNone,clNone,clGray,sa,sv,fa,xsize,ysize,hl,ml,hr,nil,Item.PictureContainer,2);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FromY := AfterCard.FListRect.Bottom + GetCardVertSpacing;
      end
      else
      begin
        FromX := FCanvasLeft + GetCardHorzSpacing;
        FromY := GetCardVertSpacing;
      end;

      if ScrollBar.Visible then
        n := ScrollBar.Height + 2
      else
        n := 0;

      if (FromY + (Card.FClientRect.Bottom - Card.FClientRect.Top) >= Height - n) and (AfterCard <> nil) then
      // wrap
      begin
        Card.FListRect := Bounds(FromX + CardTemplate.GetCardWidth + GetCardHorzSpacing * 2,
          GetCardVertSpacing, CardTemplate.GetCardWidth, Card.FHeight);
        Result := clrNextColumn;
      end else
      // continue the column
      begin
        Card.FListRect := Bounds(FromX, FromY, CardTemplate.GetCardWidth, Card.FHeight);
        Result := clrFollowed;
      end;
      ...
      begin
        ...
        Item := Card.ItemList[ItemN];
        if (not TItem.Visible) or Item.FHided then
          Continue;
    ...
    begin
      ...
      for n := 0 to High(FSortedCards) do
        ...
        begin
          ...
          inc(FRows);
          Inc(FCanvasRight, GetColumnWidth);
      ...
      if FColumns <= (Width div GetColumnWidth) then
      begin
        FLeftCol := 1;
      ...
      begin
        ShiftListRects(-(Cards[OldLeftTopCard].FListRect.Left - GetCardHorzSpacing));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FromX := AfterCard.FListRect.Left + CardTemplate.GetCardWidth + GetCardHorzSpacing * 2;
        if Assigned(uppCard) then
          FromY := uppCard.FListRect.Top + uppCard.FHeight + GetCardVertSpacing
        else
      ...
      Card.FColumn := c;
      if (FromX + (Card.FClientRect.Right - Card.FClientRect.Left) + GetCardHorzSpacing >= Width - n) and
        (AfterCard <> nil) then
      // wrap
      begin
        if Assigned(uppCard) then
        begin
          Card.FListRect := Bounds(FCardHorSpacing, uppCard.FListRect.Top + uppCard.FHeight + GetCardVertSpacing, CardTemplate.GetCardWidth, Card.FHeight);
    ...
    begin
      ...
      SetMouseOverCard(-1);
      if ScrollBar.Visible then      
        FViewedColumns := (Width - ScrollBar.Width) div GetColumnWidth
      else
        FViewedColumns := Width div GetColumnWidth;
...
function TCustomAdvCardList.GetColumnWidth: integer;
begin
  Result := Round(FColumnWidth * FDPIScale);
end;
...
begin
  ...
  begin
    ...
    begin
      if FCanvasLeft + Value > 0 then
        Value := -FCanvasLeft;
      if FCanvasRight + Value < 0 then
        Value := -FCanvasRight;
...
begin
  ...
  begin
    ...
    begin
      ...
      // just in case
      if DoDblClick and (CardTemplate.Items[ItemN].DataType = idtBoolean) then
        DoDblClick := False;
...
begin
  ...
  begin
    ...
    // First expression required because Card Header would have value -1.
    if (ItemN > 0) and (CardTemplate.Items[ItemN].DataType = idtBoolean) then
    begin
      FDblClick := false;
      Click; // DblClick() is called by TControl.WMLButtonDblClk - redirect back to Click() for boolean types only
      exit;
    end;
...
begin
  ...
  if Card.FAppearance.ReplaceLabelFont then
  begin
    Canvas.Font.Assign(Card.FAppearance.ItemLabelFont);
    Canvas.Font.Height := Round(Canvas.Font.Height * FDPIScale);
  end
  else
  begin
    Canvas.Font.Assign(TItem.FCaptionFont);
    Canvas.Font.Height := Round(Canvas.Font.Height * FDPIScale);
  end;
  ...
  begin
    ...
    StyleServices.DrawText(Canvas.Handle, lDetails, S, R, TTextFormatFlags(DrawFlags));
    {$ENDIF}
  end
  else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (x + sz.cx < R.Right - GetCardHorzSpacing - 5) then
          x := x + sz.cx + sp
        else
        begin
          x := R.Left;
    ...
    if (TItem.ItemType = itHTMLText) then
    begin
      HTMLDrawEx(Canvas, Item.AsString, R, Images, -1,-1,-1,-1,2,false,false,false,false,false,false,true,FFormScaled,1.0,clBlue,clNone,clNone,clGray,sa,sv,fa,xsize,ysize,hl,ml,hr,nil,Item.PictureContainer,2);
    end
    else
    ...
    begin
      ...
      begin
        ...
        Canvas.Font.Height := Round(Card.FAppearance.FItemEditFont.Height * FDPIScale);
      end
      else
      begin
...
var
  ...
  {$IFDEF DELPHIXE2_LVL}
  lThemeHeader: TThemedHeader;
  lThemeGrid: TThemedGrid;
  lCardState: TCardState;
  lDetails: TThemedElementDetails;
  {$ENDIF}
...
begin
  ...
  { background - excluding the border }
  if FUseVCLStyles then
  begin
    {$IFDEF DELPHIXE2_LVL}
    lDetails := StyleServices.GetElementDetails(tgCellNormal);
  ...
  { Draw caption header rectangle }
  if FUseVCLStyles then
  begin
    {$IFDEF DELPHIXE2_LVL}
    lCardState := GetCardState(Card);
    case lCardState of
      csNormal:   lThemeHeader := thHeaderItemNormal;
      csHover:    lThemeHeader := thHeaderItemHot;
      csSelected: lThemeHeader := thHeaderItemPressed;
      csEditing:  lThemeHeader := thHeaderItemPressed;
      else
        lThemeHeader := thHeaderItemNormal;
    end;
    lDetails := StyleServices.GetElementDetails(lThemeHeader);
    StyleServices.DrawElement(Canvas.Handle, lDetails, R);
    {$ENDIF}
  end
  else
  ...
  begin
    // center vertically...
    delta := CardTemplate.GetCardCaptionHeight - Images.Height;
  ...
  begin
    ...
    lCardState := GetCardState(Card);
    case lCardState of
      csNormal:   lThemeGrid := tgFixedCellNormal;
      csHover:    lThemeGrid := tgFixedCellHot;
      csSelected: lThemeGrid := tgFixedCellPressed;
      csEditing:  lThemeGrid := tgFixedCellPressed;
      else
        lThemeGrid := tgFixedCellNormal;
    end;
    lDetails := StyleServices.GetElementDetails(lThemeGrid);
    StyleServices.DrawText(Canvas.Handle, lDetails, S, R, TTextFormatFlags(DrawFlags));
    {$ENDIF}
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        x := FCanvasLeft + GetColumnWidth * n;
      ...
      if not Editing then
        ...
        begin
          ...
          begin
            if (ScrollBarType = sbtHorizontal) then
            begin
              x := Cards[FSelectedIndex].FListRect.Right + GetColumnWidth div 3;
      ...
      if not Editing then
        ...
        begin
          ...
          begin
            if (ScrollBarType = sbtHorizontal) then
            begin
              x := Cards[FSelectedIndex].FListRect.Left - GetColumnWidth div 3;
              y := Cards[FSelectedIndex].FListRect.Top + (Cards[FSelectedIndex].FHeight div 2);
              Card := CardAtXY(x, y, False);
              if Assigned(Card) then
                i := GetSortedIndex(Card) - GetSortedIndex(Cards[FSelectedIndex]);
...
function TCustomAdvCardList.UseVCLStyles: boolean;
begin
  result := FUseVCLStyles;
end;

{$IFNDEF DELPHIXE10_LVL}
procedure TCustomAdvCardList.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TCustomAdvCardList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
function TCustomAdvCardList.GetCardHorzSpacing: integer;
begin
  Result  := Round(FCardHorSpacing * FDPIScale);
end;

function TCustomAdvCardList.GetCardVertSpacing: integer;
begin
  Result  := Round(FCardVertSpacing * FDPIScale);
end;
...
begin
  case Layout of
    ...
    tlBottom: Result.Top := ValueListRect.Bottom - Round(FCheckBoxSize * FDPIScale);
    tlCenter: Result.Top := ValueListRect.Top + (ValueListRect.Bottom - ValueListRect.Top - Round(FCheckBoxSize * FDPIScale)) div 2;
  end;
  Result.Bottom := Result.Top + Round(FCheckBoxSize* FDPIScale);

  case Align of
    taLeftJustify: Result.Left := ValueListRect.Left;
    taRightJustify: Result.Left := ValueListRect.Right - Round(FCheckBoxSize * FDPIScale);
    taCenter: Result.Left := ValueListRect.Left + (ValueListRect.Right - ValueListRect.Left - Round(FCheckBoxSize * FDPIScale)) div 2;
  end;
  Result.Right := Result.Left + Round(FCheckBoxSize * FDPIScale);
end;
...
begin
  ...
  // column sizing only takes affect with left mouse button
  if (Button = mbLeft) and FCursorOverGridLine then
  begin
    FOldColumnWidth := GetColumnWidth;
...
begin
  ...
  begin
    ...
    if (ItemN >= 0) and CardRearrange then
    begin
      TItem := CardTemplate.Items[ItemN];

      Item := Card.ItemList[ItemN];

      if TItem.DataType = idtBoolean then
      begin
        Pt := Point(X,Y);
        if PtInRect(GetCheckBoxRect(Card.ItemList[ItemN].FValueListRect, TItem.ValueAlignment, TItem.ValueLayout), Pt) then
        begin
          inherited;
          Exit;
        end;
      ...
      R := Item.FUnitedListRect;
      res := HTMLDrawEx(Canvas, Card.ItemList[ItemN].AsString, R, Images, X, Y,-1,-1,2,true,false,false,false,false,false,true,FFormScaled,1.0,clBlue,clNone,clNone,clGray,sa,sv,fa,xsize,ysize,hl,ml,hr,nil,Item.PictureContainer,2);
  ...
  if CardRearrangeType = crFull then
    ItemN := -1;

  if CardRearrange and not FCursorOverGridLine and Assigned(SelectedCard) and (Card = SelectedCard) and not (csDesigning in ComponentState) and not Sorted and (ItemN = -1) then
  begin
    Frm := GetParentForm(Self);
...
begin
  ...
  begin
    ...
    Pt := ClientToScreen(Point(FResizingColumn * GetColumnWidth, Y));
    SetCursorPos(Pt.x, Pt.y);
    FGridLineDragging := False;
    ColumnSized;
  end;
...
begin
  ...
  if FGridLineDragging then
  begin
    ColumnWidth := FOldColumnWidth - (FDownXY.X - X);
    Exit;
  end;
...
//      else
  ...
  begin
    ...
    begin
      if (X >= (GetColumnWidth * n) - (FGridLineWidth div 2)) and
        (X <= (GetColumnWidth * n) + (FGridLineWidth div 2)) then
      begin
...
begin
  FColumnWidth := FCardTemplate.FCardWidth + FCardHorSpacing * 2;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        InflateRect(R, FACL.GetCardHorzSpacing, FACL.GetCardVertSpacing);
        P := FACL.ClientToScreen(Point(R.Left, R.Top));
        i := (R.Bottom - R.Top) div 2;
        if (pt.Y < P.Y + i) then
        begin
          FACL.HideAllArrows(FACL.ArwU);
          P := FACL.ClientToScreen(Point(R.Left - 4 + (R.Right - R.Left) div 2, R.Top + FACL.GetCardVertSpacing));
        ...
        begin
          ...
          P := FACL.ClientToScreen(Point(R.Left - 4 + (R.Right - R.Left) div 2, R.Bottom - FACL.GetCardVertSpacing - 10));
          FACL.ArwD.Left := P.X;
          FACL.ArwD.Top := P.Y;
        end;
      ...
      begin
        ...
        begin
          ...
          P := FACL.ClientToScreen(Point(R.Left + FACL.GetCardHorzSpacing, R.Top - 4 + (R.Bottom - R.Top) div 2));
        ...
        begin
          ...
          P := FACL.ClientToScreen(Point(R.Right - 10 - FACL.GetCardHorzSpacing, R.Top - 4 + (R.Bottom - R.Top) div 2));
          FACL.ArwL.Left := P.X;
          FACL.ArwL.Top := P.Y;
        end;
