...
const
  ...
  // v3.4.5.1 : Fixed : issue with TDBMultiMonthSource with year-crossing
  // v3.4.5.2 : Improved : TDBTimeLineSource ReadDBItem filtering on date
  // v3.4.5.3 : Fixed : Issue with adding events in afternoon for the TDBHalfDayPeriodSource
...
//                cdte := FEndDate;
              ...
              while ((smo < emo) or (sye < eye)) and (cdts < FEndDate) do
              begin
...
//  D.Locate(KeyField,APlanner.Items.Selected.DBKey,[]);
  ...
  if (int(dts) <= int(FEndDate)) and (int(dte) >= int(FStartDate)) then
  begin
