...
const
  ...
  // 2.0.0.0 : New : VCL Styles support added
...
{$NODEFINE TOwnerDrawState}
 ...
 {---------------------------------------------------------}
  ...
  {--------------------------------}
  TListSectionCollection = class(TOwnedCollection)
  private
  ...
  private
    ...
    {$IFDEF DELPHIXE2_LVL}
    FStyledSectionFontColor: TColor;
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, s, r, SubItemImages, 0, 0, -1, -1, 1, False, True, False, True, True, True, FWordWrap, FFormScaled,
      1.0, clBlue, clNone, clNone, clGray, Anchor, stripped, fa, xsize, ysize, ml, hl, hr, FImageCache, FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
    Result := YSize + 2;
    Exit;
  end;
...
procedure TSectionListBox.InitVCLStyle(Init: Boolean);
{$IFDEF DELPHIXE2_LVL}
var
...
{$IFDEF DELPHIXE2_LVL}
  ...
  FUseVCLStyles := (LStyle.Name <> 'Windows') and LStyle.Enabled;

  if FUseVCLStyles then
  begin
    LDetails := LStyle.GetElementDetails(tgFixedCellNormal);
//    FStyledSectionBorderColor := LStyle.GetStyleColor(scBorder);
//    FStyledSectionClientColor := LStyle.GetStyleColor(scCategoryButtons);
    if {$IFDEF DELPHIXE6_LVL}(seFont in StyleElements) and {$ENDIF} LStyle.GetElementColor(LDetails, ecTextColor, clr) then
      FStyledSectionFontColor := clr
    else
      FStyledSectionFontColor := LStyle.GetStyleFontColor(sfCategoryPanelGroupHeaderNormal);
//                               LStyle.GetStyleFontColor(sfCatgeoryButtonsCategoryNormal)
    FStyledBorderColor := LStyle.GetStyleColor(scBorder);
    FStyledClientColor := LStyle.GetStyleColor(scEdit);
    FStyledFontColor := LStyle.GetStyleFontColor(sfListItemTextNormal);
  end;
{$ENDIF}
end;
...
begin
  ...
  if (fTabPositions.Count > 0) and (fTabPositions.Count < MAX_TABS) then
    ...
    begin
      d[i] := Round(FDPIScale * FTabPositions.Items[i].TabPosition);
    end;
  ...
  begin
    ...
    if (loword(dwExtent) > max) and (not GetItemSection(i).EndEllipsis) then
      max := loword(dwExtent);
  end;

  if (max < self.width) then
    max := 0;
  SendMessage(self.handle, lb_SETHORIZONTALEXTENT, max, 0)
end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TSectionListBox.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
{$IFNDEF DELPHIXE10_LVL}
procedure TSectionListBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TSectionListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
procedure TSectionListBox.DrawItem(Index: Integer; ARect: TRect; State: TOwnerDrawState);
const
...
var
  ...
  begin
    ...
    begin
      ...
      if FUseVCLStyles then
      begin
        {$IFDEF DELPHIXE6_LVL} if (seFont in StyleElements) then {$ENDIF}
          Canvas.Font.Color := FStyledSectionFontColor;
      end;
      {$ENDIF}
    end
    else
    begin
      ...
      begin
        Canvas.Font.Assign(ListSection.Font);
        Canvas.Font.Height := Round(FDPIScale * ListSection.Font.Height);
      end
      else
      begin
        Canvas.Font.Assign(Font);
      ...
      begin
        {$IFDEF DELPHIXE6_LVL} if (seFont in StyleElements) then {$ENDIF}
          Canvas.Font.Color := FStyledFontColor;
        {$IFDEF DELPHIXE6_LVL} if (seClient in StyleElements) then {$ENDIF}
          Canvas.Brush.Color := FStyledClientColor;
      end;
      {$ENDIF}
    end;
    ...
    {$IFDEF DELPHIXE2_LVL}
      if Section and FUseVCLStyles then
      begin
        LDetails := LStyle.GetElementDetails(tgFixedCellNormal);
        ...
        try
          ...
          if LStyle.DrawElement(bmp.Canvas.Handle, LDetails, Rect(0, 0, bmp.Width, bmp.Height)) then
            Canvas.CopyRect(R, bmp.Canvas, Rect(0, 0, bmp.Width, bmp.Height));
        finally
          FreeAndNil(bmp);
        end;
      end
      else
      if (not Section) and FUseVCLStyles and (odSelected in State) then
      begin
    ...
    if Section and not fix then
      r.Left := r.Left + Round(FDPIScale * FSectionIndent)
    else
      r.Left := r.Left + Round(FDPIScale * FSubItemIndent);
                                     {change from subitemheight}
    if Section then
    begin
      if SectionHeight > 0 then
        h := Round(FDPIScale * SectionHeight)
      else
        h := Round(FDPIScale * 14);
    end
    else
      h := Round(FDPIScale * ListSection.ItemHeight);

    if (ListSection.ControlType = scCheckbox) and not Section then
    begin
      DrawRect := r;
      DrawRect.Right := DrawRect.Left + Round(FDPIScale * 14);

      case val of
        vtaTop: DrawRect.Top := DrawRect.Top + 1;
        vtaCenter: DrawRect.Top := DrawRect.Top + ((DrawRect.Bottom - DrawRect.Top - Round(FDPIScale * 12)) shr 1);
        vtaBottom: DrawRect.Top := DrawRect.Bottom - Round(FDPIScale * 14);
      end;

      DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * 14);
    ...
    if (ListSection.ControlType = scRadioButton) and not Section then
    begin
      DrawRect := r;
      DrawRect.Right := DrawRect.Left + Round(FDPIScale * 10);
      case val of
        vtaTop: DrawRect.Top := DrawRect.Top + Round(FDPIScale * 2);
        vtaCenter: DrawRect.Top := DrawRect.Top + ((DrawRect.Bottom - DrawRect.Top - Round(FDPIScale * 10)) shr 1);
        vtaBottom: DrawRect.Top := DrawRect.Bottom - Round(FDPIScale * 10);
      end;
      DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * 10);
    ...
    begin
      ...
      begin
        ...
        r.Left := r.Left + SectionImages.Width + Round(FDPIScale * fImageSpacing);
      end
      else
      begin
        case val of
        ...
        r.Left := r.Left + subitemimages.Width + Round(FDPIScale * fImageSpacing);
      end;
    end;
    ...
    begin
      ...
      if pos('</', s) > 0 then
        HTMLDrawEx(Canvas, s, r, subitemimages, 0, 0, -1, -1, 1, False, False, False, (odselected in state), True, True, FWordWrap, FFormScaled,
          1.0, clBlue, clNone, clNone, clGray, anchor, stripped, fa, xsize, ysize, ml, hl, hr, FImageCache, FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale)
      else
      begin
        ...
        if not FFullFocus and not Section then
        begin
          if odSelected in State then
            Canvas.Brush.Color := FSelectionColor;
        end;
        ...
        if (pos(#9, s) = 0) and not Section then
        begin
        ...
        begin
          ...
          begin
            ...
            if (not Section and vertlines) or (fTabPosMove and (ListSection.State = lssExpanded)) then
              for i := 0 to fTabPositions.Count - 1 do
              begin
    ...
    if Section {$IFDEF DELPHIXE2_LVL} and (not FUseVCLStyles) {$ENDIF} then
    begin
    ...
    if Section and ListSection.fSortShow and (ListSection.State = lssExpanded) then
    begin
    ...
    if Section and not fix then
    begin
      er.Left := r.Left + Round(FDPIScale * 4);
      er.Right := er.Left + Round(FDPIScale * 8);
      er.Top := r.Top + ((h - Round(FDPIScale * 8)) shr 1);
      er.Bottom := er.Top + Round(FDPIScale * 8);
      ...
      case fNodeType of
        lnFlat:
          begin
            {$IFDEF DELPHIXE2_LVL}
            if FUseVCLStyles then
              Canvas.Pen.Color := FStyledSectionFontColor
            else
            {$ENDIF}
            if dis then
              Canvas.Pen.Color := clGray
            ...
            if not dis then
              if st then
              begin
                Canvas.moveto(er.Left + Round(FDPIScale), er.Top - 1 + (er.Bottom - (er.Top - 1)) div 2);
                Canvas.lineto(er.Right - 1 - Round(FDPIScale), er.Top - 1 + (er.Bottom - (er.Top - 1)) div 2);
                Canvas.moveto(er.Left - 1 + (er.Right - (er.Left - 1)) div 2, er.Top + Round(FDPIScale));
                Canvas.lineto(er.Left - 1 + (er.Right - (er.Left - 1)) div 2, er.Bottom - 1 - Round(FDPIScale));
              end
              else
              begin
                Canvas.moveto(er.Left + Round(FDPIScale), er.Top - 1 + (er.Bottom - (er.Top - 1)) div 2);
    ...
    if (odFocused in State) and not (fix and Section) and (fWallpaper.Empty) and
      not (Section and (fSectionFocus = sf3D)) then
    begin
      er := ARect;
      ...
      begin
        case val of
          vtaTop: er.Top := ARect.Top;
          vtaCenter: er.Top := ARect.Top + ((ARect.Bottom - ARect.Top - hiword(tl)) shr 1);
          vtaBottom: er.Top := ARect.Bottom - hiword(tl);
        end;
        case val of
          vtaTop: er.Bottom := ARect.Top + hiword(tl);
          vtaCenter: er.Bottom := er.Top + hiword(tl);
          vtaBottom: er.Bottom := ARect.Bottom;
        end;
        er.Left := t - 1;
        er.Right := t + loword(tl) + 1;
      end;

      if not Section or (Section and (SectionFocus <> sfNone)) then
...
begin
  ...
  begin
    ...
    OnDrawItem(Self, ListSection, imidx, index - imofs - 1, Canvas, ARect, state);
    Exit;
  end;
  ...
  DrawListText(Canvas, ARect.Left, ARect.Top, DText, sct, st, dis, brcol, txtcol);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if HTMLDrawEx(Canvas, s, r, subitemimages, message.xpos, message.ypos, -1, -1, 1, True, False, False, True, True, True, FWordWrap, FFormScaled,
            1.0, clBlue, clNone, clNone, clGray, anchor, stripped, fa, xsize, ysize, ml, hl, hr, FImageCache, FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale) then
          begin
            Cursor := crHandPoint;
...
begin
  ...
  FMemo.Height := Max(Canvas.TextHeight('gh'), (r.Bottom - r.Top));
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.Font.Height := Round(FDPIScale * ls.Font.Height);
    end;
    ...
    if SendMessage(Handle, LB_GETITEMHEIGHT, Index - 1, 0) <> Round(FDPIScale * ln) then
      SendMessage(Handle, LB_SETITEMHEIGHT, Index - 1, Round(FDPIScale * ln));
  end
  else
  begin
    idx := ls.Index;
    ...
    if SendMessage(Handle, LB_GETITEMHEIGHT, Index - 1, 0) <> Round(FDPIScale * h) then
    begin
      SendMessage(Handle, LB_SETITEMHEIGHT, Index - 1, Round(FDPIScale * h))
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ln := LinesInText(s, ls.AutoSize, ls.Itemheight);
      SendMessage(Handle, LB_SETITEMHEIGHT, i - 1, Round(FDPIScale * ln));
    end
    else
    begin
      idx := ls.Index;
      h := GetCustomSectionHeight(idx);
      SendMessage(Handle, LB_SETITEMHEIGHT, i - 1, Round(FDPIScale * h));
    end;
  end;
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FUseVCLStyles then
      begin
        clr := FStyledClientColor;
        if not ((clr <> clNone) {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements) {$ENDIF}) then
          clr := Self.Color;
      end
      else
      {$ENDIF}
...
constructor TListSection.Create(ACollection: TCollection);
begin
...
begin
  if (Source is TListSection) then
  begin
    FCaption := (Source as TListSection).Caption;
    FSubItems.Assign((Source as TListSection).SubItems);
    FAlignment := (Source as TListSection).Alignment;
    FColor := (Source as TListSection).Color;
    FControlType := (Source as TListSection).ControlType;
    FEndEllipsis := (Source as TListSection).EndEllipsis;
    FFixed := (Source as TListSection).Fixed;
    FRadioIndex := (Source as TListSection).RadioIndex;
    FFontUsage := (Source as TListSection).FontUsage;
    FFont.Assign((Source as TListSection).Font);
    FImageIndex := (Source as TListSection).ImageIndex;
    FLines := (Source as TListSection).Lines;
    FState := (Source as TListSection).State;
  end;
end;
...
begin
  (FSubItems as TSectionListStrings).FListSection := nil;
...
begin
  Result := ((Collection as TListSectionCollection).Owner as TSectionListBox)
end;
...
begin
  ...
  with (Owner as TSectionListBox) do
  begin
    BeginUpdate;
    ...
    begin
      ...
      begin
        ...
        SendMessage(Handle, LB_SETITEMHEIGHT, i + j + 1, Round(FDPIScale * ln));
      end;
    end;
    FExpandContractBusy := False;
    EndUpdate;
...
begin
  ...
  begin
    ...
    SendMessage(ListOwner.Handle, LB_SETITEMHEIGHT, Result, Round(ListOwner.FDPIScale * ListOwner.SectionHeight));
...
begin
  ...
  if not ListOwner.fDestroyed then
  begin
    ListOwner.Items.Delete(i);
...
begin
  ...
  if SendMessage(ListOwner.Handle, lb_getitemdata, i, 0) = 1 then
    SendMessage(ListOwner.Handle, lb_setitemheight, i, Round(ListOwner.FDPIScale * ListOwner.SectionHeight))
  else
  begin
    ln := ListOwner.LinesInText(ListOwner.Items[i], ASection.AutoSize, ASection.itemheight);
    SendMessage(ListOwner.Handle, lb_setitemheight, i, Round(ListOwner.FDPIScale * ln));
  end;
...
begin
  ...
  SendMessage(ListOwner.Handle, lb_Setitemheight, i + ASection.SubItems.Count + 1, Round(ListOwner.FDPIScale * ln));
...
begin
  ...
  ln := ListOwner.LinesInText(s, ASection.AutoSize, ASection.itemheight);
  SendMessage(ListOwner.Handle, LB_SETITEMHEIGHT, index + 1, Round(ListOwner.FDPIScale * ln));
  ListOwner.UpdateHorzScrollBar;
end;
...
begin
  ...
  begin
    ...
    SendMessage(Handle, LB_SETITEMHEIGHT, i, Round(ListOwner.FDPIScale * SectionHeight));
