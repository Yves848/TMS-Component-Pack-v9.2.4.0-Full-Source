...
const
  ...
  // v1.3.0.3 : New : Property Align exposed
  // v1.3.0.4 : Fixed : Issue with form DoubleBuffered = true
  // v1.3.1.0 : Improved : Support for HighDPI
...
type
  ...
  protected
    ...
    function AutoCorrectRadius(AValue: integer): integer;
...
begin
  ...
  //Check if radius of button isn't too big.
  DPIScale := GetDPIScale(Self, Canvas);
  w := Round(Width / DPIScale);
  h := Round(Height / DPIScale);
  FCornerRadius := Min(FCornerRadius, Min(w, h) div 2);
...
begin
  ...
  calcCornerRadius := Round(FDPIScale * FCornerRadius);
  ...
  DrawButtonContent(ACanvas, R, Caption, '', ShowCaption, Font, nil, -1, True, Layout, False, Enabled, DrawFocused,
    dpRight, Pic, AntiAlias, True, nil, False, False, True, False{AutoSize}, True{Rounded}, calcCornerRadius, 2, StringTrimmingNone, ButtonShape, ButtonDirection);
end;
...
begin
  ...
  case (ButtonShape) of
    btsRectangle: DrawRoundRectangle(graphic, R, calcCornerRadius, OuterBorderColor);
    btsCurvedRectangle:
    begin
  ...
  case (ButtonShape) of
    ...
    else path := CreateRoundRectangle(R, calcCornerRadius);
  end;
  ...
  begin
    case (ButtonShape) of
      ...
      begin
        path := CreateRoundRectangle(R, calcCornerRadius);
        Rgn := TGPRegion.Create(path);
        graphic.SetClip(Rgn);
        path.Free;
      end;
  ...
  begin
    ...
    case (ButtonShape) of
      ...
      else path := CreateTopRoundRectangle(R2, calcCornerRadius);
    end;
  ...
  case (ButtonShape) of
    btsRectangle: DrawRoundRectangle(graphic, R, calcCornerRadius, InnerBorderColor);
    btsCurvedRectangle:
    begin
      path := CreateConcaveRectangle(R, ButtonDirection);
...
begin
  ...
  FCornerRadius := AutoCorrectRadius(FCornerRadius);
...
function TAdvCustomGlassButton.AutoCorrectRadius(AValue: integer): integer;
begin
  if (csDesigning in ComponentState) or not (csLoading in ComponentState) then
  begin
    Result := Min(AValue, Min(Width, Height) div 2);
  end
  else
    Result := AValue;
...
begin
  ...
  begin
    FCornerRadius := AutoCorrectRadius(Value);
    Invalidate;
  end;
end;
