...
const
  ...
  // 2.4.5.2 : Fixed : Issue with Down picture handling
  // 2.4.5.3 : Improved : Transparent drawing of icons on caption buttons
  // 2.4.5.4 : Fixed : Issue with style changing and disabled style
  // 2.4.6.0 : New : Public property DoubleClicks: boolean added
  // 2.4.7.0 : New : Appearance.TextColorDisabled property added
  // 2.4.7.1 : Improved : High DPI handling for Notes
  // 2.4.7.2 : Fixed : Issue with painting down state
  // 2.4.7.3 : Fixed : Issue with high DPI and form.Scaled = false
  // 2.4.7.4 : Fixed : Issue with Windows high-contrast themes
  // 2.4.7.5 : Fixed : Issue with GroupIndex > 0 and state handling from key press
  // 2.4.7.6 : Fixed : Return key handling for grouped buttons
  // 2.4.7.7 : Fixed : Issue with actions and button check style
  // 2.4.7.8 : Fixed : Regression with actions and button check style
  // 2.4.7.9 : Fixed : Issue with Style = bsCheck and Down state handling
  // 2.4.7.10: Fixed : Issues with spacing for bottom & bottom aligned layouts
  // 2.4.7.11: Fixed : Issue to let TAdvGlowButton work with ParentFont = true
  // 2.4.7.12: Improved : High-DPI handling for dropdown button
  // 2.4.8.0 : Improved : Per monitor high DPI handling
  // 2.4.8.1 : Fixed : Issue with fsStrikeOut font style
  // 2.4.8.2 : Fixed : Office 2016 black style font color issue
  // 2.5.0.0 : New : VCL Styles support
  // 2.5.0.1 : Fixed : Issue with custom font color
  // 2.5.0.2 : Fixed : Issue with hover for checkbox style button
  // 2.5.0.3 : Fixed : Issue with VCL styles and RDP
  // 2.5.0.4 : Improved : Dropdown arrow painted with font color
...
type
  ...
  published
    ...
    property TextColorDisabled: TColor read FTextColorDisabled write FTextColorDisabled default clGray;
  end;
  ...
  private
    ...
    FDoubleClicks: boolean;
    FGotDownKey: boolean;
    FCheckBeforeAction: boolean;
    ...
    procedure SetDoubleClicks(const Value: boolean);
  protected
    FHot: Boolean;
    ...
    procedure DrawCustom(ACanvas: TCanvas; R: TRect); virtual;
  ...
  public
    ...
    property DoubleClicks: boolean read FDoubleClicks write SetDoubleClicks;
...
procedure DrawArrow(AControl: TControl; ACanvas: TCanvas; ArP: TPoint; ArClr, ArShad: TColor; Direction: TDropDownDirection);
var
  i: integer;
  FArrSize, s: integer;
begin
  FArrSize := Round(GetDPIScale(AControl, ACanvas) * 3);
  ArP.X := ArP.X + 1 - FArrSize div 2;
  ArP.Y := ArP.Y - FArrSize div 2;

  ACanvas.Pen.Color := ArClr;
  ...
  ddDown:
    begin
      ...
      begin
        ACanvas.MoveTo(ArP.X + I, ArP.Y + I);
        ACanvas.LineTo(ArP.X + (FArrSize * 2) - I - 1, ArP.Y + I);
      end;
    end;
  ddRight:
    begin
      ...
      begin
        ACanvas.MoveTo(ArP.X + I, ArP.Y + I);
        ACanvas.LineTo(ArP.X + I, ArP.Y + (FArrSize * 2) - I - 1);
      end;
  ...
  ddUp:
    begin
      ...
      begin
        s := FArrSize - 1 - I;
        ACanvas.MoveTo(ArP.X + s, ArP.Y + I);
        ACanvas.LineTo(ArP.X + (FArrSize * 2) - s - 1, ArP.Y + I);
      end;
    end;
  end;
end;

procedure DrawGDIPArrow(AControl: TControl; ACanvas: TCanvas; ArP: TPoint; Clr: TColor);
var
...
function DrawVistaButton(Owner: TControl; Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC, PCF: TColor;
   ...
   Trimming: TStringTrimming; Notes: TStringList; NotesFont: TFont;Checked, Down, TranspHover, GDIPArrow: boolean; MargH,MargV: integer; NoDefBorder: boolean; AlwaysAccel: boolean; Frm: TCustomForm): TSize;
var
  graphics : TGPGraphics;
  w,h,h2,h2d: Integer;
...
begin
  ...
  FScale := CalcDPIScale(Canvas, frm);

  FDropDownSectWidth := Round(DropDownSectWidth * FScale);
...
//    Layout := blGlyphLeftAdjusted;
  ...
  if (fsUnderline in NotesFont.Style) then
    nfs := nfs + 4;
  if (fsStrikeOut in NotesFont.Style) then
    nfs := nfs + 8;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Canvas.Font.Height := Round(Canvas.Font.Height * GetDPIScale(Owner, Canvas));
    ...
    begin
      ...
      if Notes.Text <> '' then
      begin
        nfont := TGPFont.Create(nfontFamily, Round(NotesFont.Size * GetDPIScale(Owner, Canvas)), nfs, UnitPoint);
        ...
        case Layout of
          ...
          begin
            ...
            begin
              ...
              x2 := w - 2 - ImgW;
              if DropDownButton then
                ImgX := -2 + Round(r.Right - FDropDownSectWidth - szRect.right - imgw) div 2
              else
          ...
          begin
            ...
            begin
              ...
              ImgY := round(y2 - sizerect.Height);
              ImgY := Max(0, ImgY div 2);
              ImgY := round(sizerect.Height + 5) + ImgY + Spacing;
            ...
            begin
              ...
              ImgY := round(y2 - sizerect.Height);
              ImgY := Max(0, ImgY div 2);
              ImgY := round(sizerect.Height + 2) + ImgY + Spacing;
              if ImgY > (h - ImgH) then ImgY := h - ImgH - 2;
            end;
          end;
          blGlyphBottomAdjusted:
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            nfont := TGPFont.Create(nfontFamily, CalcFontSize(Canvas, NotesFont, GetDPIScale(Owner, Canvas)), nfs, UnitPoint);
  ...
  begin
    ...
    begin
      ...
      if (DropDir = ddDown) then
        AP.X := DwR.Left + ((DwR.Right - DwR.Left - Round(3 * FScale)) div 2)
      else
        AP.X := DwR.Left + ((DwR.Right - DwR.Left - Round(5 * FScale)) div 2);
      ...
      begin
        if not Enabled then
          DrawGDIPArrow(Owner, Canvas, AP, clGray)
        else
          DrawGDIPArrow(Owner, Canvas, AP, AFont.Color);
      end
      else
      begin
        if not Enabled then
          DrawArrow(Owner, Canvas, AP, clGray, clWhite, DropDir)
        else
          DrawArrow(Owner, Canvas, AP, AFont.Color, clWhite, DropDir);
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if Assigned(OnClick) and (GroupIndex = 0) then
      OnClick(Self);
  end;
end;
...
begin
  ...
  if (Key in [VK_SPACE, VK_RETURN]) and FGotDownKey then
  begin
    if (FGroupIndex = 0) then
    begin
      FDown := False;
    ...
    begin
      if (State = absDown) or (State = absExclusive) then
      begin
        FDown := false;
        State := absUp;
      end
...
var
  ...
  FDropDownSectWidth: Integer;
begin
  FCheckBeforeAction := FDownChecked;
  ...
  FDropDownSectWidth := Round(DropDownSectWidth * CalcDPIScale(Canvas));
...
//      if not PtInRect(ClientRect, pt) then
  ...
  begin
//    if (Style = bsCheck) and not Assigned(Action) then
//    begin
...
begin
  ...
  if (Style = bsCheck) and PtInRect(ClientRect, Point(Msg.XPos,Msg.YPos)) then
  begin
    if not (Assigned(Action) and (FDownChecked <> FCheckBeforeAction)) then
      SetDown(not FDownChecked);
    FStepPush := 0;
  end;
...
procedure TAdvCustomGlowButton.DrawCustom(ACanvas: TCanvas; R: TRect);
begin
//
end;
...
begin
  ...
  //            PenColor := BlendColor(BorderColorDown, BorderColorChecked, FStepPush);
      ...
      if Appearance.TextColor <> clNone then
      begin
        AFont.Color := Appearance.FTextColor;
      end;
      ...
      if not Enabled then
        AFont.Color := Appearance.TextColorDisabled;
      ...
      begin
        ...
        sz := DrawVistaButton(Self, Canvas, drrect, GradColor, GradColorTo, GradColorMirror,GradColorMirrorTo,
          ...
          FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspHover and Transparent, InternalIsOnGlassQAT, MarginHorz, MarginVert, Appearance.NoBorderDefault, AlwaysShowAccel, GetParentForm(Self));
      ...
      sz := DrawVistaButton(Self, Canvas, drrect, GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo,
        ...
        False, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspHover and Transparent, InternalIsOnGlassQAT, MarginHorz,MarginVert,Appearance.NoBorderDefault, AlwaysShowAccel, GetParentForm(Self));
...
procedure TAdvCustomGlowButton.SetDoubleClicks(const Value: boolean);
begin
  FDoubleClicks := Value;
  if FDoubleClicks then
    ControlStyle := ControlStyle - [csDoubleClicks]
  else
    ControlStyle := ControlStyle + [csDoubleClicks];
end;
...
begin
  with Message do
    ...
    begin
      ...
      begin
        if (CharCode = VK_RETURN) and (GroupIndex > 0) then
        begin
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvCustomGlowButton.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  Appearance.TextColorDisabled := ATones.Disabled.TextColor;
  Appearance.TextColorDown := ATones.Selected.TextColor;
...
begin
  ...
  Appearance.ColorDisabled := BrightnessColor(clWhite,-5,-5,-5);
  Appearance.ColorDisabledTo := BrightnessColor(clWhite, -5,-5,-5);
  Appearance.ColorMirrorDisabled := BrightnessColor(clSilver, -5,-5,-5);
  Appearance.ColorMirrorDisabledTo := BrightnessColor(clWhite, -5,-5,-5);
...
begin
  ...
  with Appearance do
    Result := DrawVistaButton(Self, Canvas,ClientRect,FColor, FColorTo, FColorMirror, FColorMirrorTo,
        ...
        FOverlappedText, FWordWrap, True, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspHover and Transparent, InternalIsOnGlassQAT,MarginHorz,MarginVert,NoBorderDefault, AlwaysShowAccel, GetParentForm(Self));
...
begin
  ...
  begin
    ...
    TextColorDisabled := (Source as TGlowButtonAppearance).TextColorDisabled;
