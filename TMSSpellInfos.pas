...
uses
  Windows, Classes, Math, StrUtils, SysUtils, Types, Generics.Collections;
...
begin
  ...
  if a <> nil then
    ...
    begin
      // assume the AFF file has rules for both uppercase & lowercase
      Result := ReplaceStr(Result, a[i].FirstString, a[i].SecondString);
...
begin
  Value := Trim(Value);
  s := Unescape(Trim(Copy(Value, 1, Pos(' ', Value) - 1)));
  FirstString := UTF8ToWideString(s);
