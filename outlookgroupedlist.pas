...
type
  ...
  protected
    ...
    property FormScaled: boolean read FFormScaled;
...
begin
  ...
  case Message.CharCode of
    Ord('A'):
      if (ssCtrl in Shift) and (soMultiSelect in SelectionOptions) then
      begin
        // select all
        AddAllToSelection;
        Invalidate;
...
begin
  HTMLDrawEx(Canvas, s, fr, {FImages}nil, 0, 0, -1, 0, 0, True, False, False, False, False,
                       False, False, FFormScaled, 1.0, clBlue, clNone,clNone, clNone, anchorval,stripval,focusanchor,
...
begin
  ...
  begin
    ...
    begin
      DrawTextFlags := DT_LEFT or DT_BOTTOM or DT_SINGLELINE or DT_NOCLIP or DT_END_ELLIPSIS or DT_NOPREFIX;
      {$IFDEF DELPHIXE2_LVL}
      if StyleServices.Enabled and (StyleServices.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL}and (seFont in StyleElements){$ENDIF} then
      begin
        if IsItemSelected(Item) then
          TargetCanvas.Pen.Color := aol.GroupSelectionTextColor
        else
          TargetCanvas.Pen.Color := aol.GroupFont.Color
      end;
      {$ENDIF}
      //draw the text of the item
      DrawText(TargetCanvas.Handle, PChar(Caption), -1, rc, DrawTextFlags);
    end;
    ...
    begin
      DrawTextFlags := DT_RIGHT or DT_BOTTOM or DT_SINGLELINE or DT_NOCLIP or DT_NOPREFIX;
      {$IFDEF DELPHIXE2_LVL}
      if StyleServices.Enabled and (StyleServices.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL}and (seFont in StyleElements){$ENDIF} then
      begin
        if IsItemSelected(Item) then
      ...
      {$ENDIF}
      DrawText(TargetCanvas.Handle, PChar(ChildCount), -1, rc, DrawTextFlags);
    end;
  end
  else
  begin
...
initialization
  {$IFDEF DELPHIXE2_LVL}
  TCustomStyleEngine.RegisterStyleHook(TOutlookGroupedList, TScrollBoxStyleHook);
  {$ENDIF}

finalization
  {$IFDEF DELPHIXE2_LVL}
  TCustomStyleEngine.UnRegisterStyleHook(TOutlookGroupedList, TScrollBoxStyleHook);
  {$ENDIF}
