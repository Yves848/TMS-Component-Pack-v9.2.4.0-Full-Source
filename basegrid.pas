...
begin
  ...
  if ((ActiveRowShow) and (ARow = Row) and
     ((ACol <> Col) or (not FShowSel) or (goRowSelect in Options)) and (ACol >= FixedCols)) or (not internal and (gdSelected in AState)) then
  begin
...
//  if not (gdFixed in AState) or Ctl3D then
  begin
    // Draw grid borders
    if not ((MCol = 0) and not Ctl3D and (ARow >= FixedRows) and HasNodes) then
    ...
    begin
      ...
      begin
        if (Row <= BC.Y + CellSpanY) and (Row < RowCount - 1) and
            (BC.Y < RowCount - 1) and (BC.Y + CellSpanY + 1 < RowCount) then
          Row := BC.Y + CellSpanY + 1;
      end;
    end;
  VK_NEXT:
    begin
