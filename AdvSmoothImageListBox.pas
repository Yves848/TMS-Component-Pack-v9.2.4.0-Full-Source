...
const
  ...
  // v2.5.0.5 : Fixed : Issue with setting Transparent property
  // v2.5.0.6 : Fixed : Issue with transparency on double buffered forms
  // v2.5.0.7 : Fixed : Issue with resetting to default cursor property
  // v2.5.0.8 : Fixed : Issue with width/height of scrollbar under high DPI
  // v2.5.0.9 : Improved : ZoomItem method
  // v2.5.0.10: Fixed : Issue with textrendering on toplayer items
  // v2.5.1.0 : New : TAdvSmoothImageListBoxItem.ClearImage method added
  // v2.5.2.0 : New : Property to turn off mouse wheel scrolling
  // v2.6.0.0 : New : VCL Styles support
  // v2.6.1.0 : New : Per monitor support for high DPI
...
type
  ...
  protected
    ...
    procedure DoZoom(AItemIndex: Integer = -1); virtual;
    //--- DB Ver support
  ...
  public
    ...
    procedure ZoomItem(AItemIndex: Integer = -1); virtual;
  ...
  published
    ...
    property MouseWheelScrolling: Boolean read FMouseWheelScrolling write FMouseWheelScrolling default True;
    property OfficeHint: TAdvHintInfo read FOfficeHint write SetOfficeHint;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              nh := Round(InsideRect.Bottom - InsideRect.Top - FDPIScale * Header.GetHeight - FDPIScale * Footer.GetHeight);
            end
            else if Columns = 0 then
            begin
              nw := InsideRect.Right - InsideRect.Left;
              nh := Round(InsideRect.Bottom - InsideRect.Top - FDPIScale * Header.GetHeight - FDPIScale * Footer.GetHeight - FDPIScale * GetScrollBarHeight);
            end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothImageListBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothImageListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
  FHeader.FDPIScale := FDPIScale;
  FFooter.FDPIScale := FDPIScale;
...
procedure TAdvSmoothImageListBox.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    InitPreview;
    SetComponentStyle(tsOffice2016White);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      f := TGPFont.Create(ff, Round(FDPIScale * Font.Size), fs, UnitPoint);
            ...
            begin
              ...
              begin
                ...
                begin
                  if (FHoveredItemIndex = Index) and Hover then
                  begin
                    R.X := R.X - (Round(FDPIScale * FOwner.FItemAppearance.FHoverSize) / 2);
                    R.Y := R.Y - (Round(FDPIScale * FOwner.FItemAppearance.FHoverSize) / 2);
                    R.Width := R.Width + Round(FDPIScale * FOwner.FItemAppearance.FHoverSize);
                    R.Height := R.Height + Round(FDPIScale * FOwner.FItemAppearance.FHoverSize);
                  end;
    ...
    begin
      ...
      begin
        ...
        HTMLDrawGDIP(g, FFont, str, htmlr, FImages, 0, 0, -1, -1, FShadowOffset, False, true, false, false,
          False, False, true, 1.0, FURLColor, clNone, clNone, FShadowColor, a, s, k, XSize, YSize, l, m, hr, nil, FContainer, 2, FDPIScale);
...
begin
  ...
  begin
    ...
    if (GetFullSize > 0) and (Self.Height > 0) then
      h := ((Self.Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset) / (GetFullSize + Height + GetShadowOffset)) * Self.Height;

    pos := 0;

    if (GetPosition > 0) and (GetFullSize > 0) then
      pos := (Self.Height - Round(FDPIScale * Header.GetHeight) - h - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset) / GetFullSize * GetPosition;

    pos := pos + Round(FDPIScale * Header.GetHeight);
  ...
  begin
    ...
    p.R := MakeRect(pos, InsideRect.Bottom - sc.size - 2 - Round(FDPIScale * Header.GetHeight)  - GetShadowOffset, h, sc.size);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      f := TGPFont.Create(ff, Round(FDPIScale * FFont.Size), fs, UnitPoint);
...
begin
  ...
  case FScrollBar.Kind of
    sbHorizontal: scrl := Round(FDPIScale * GetScrollBarHeight);
    sbVertical: scrl := Round(FDPIScale * GetScrollBarWidth);
  end;
  
  if Rows = 0 then
  begin
    Result := MakeRect(InsideRect.Left, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + hb, InsideRect.Right - InsideRect.Left - GetShadowOffset - scrl,
      InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - fb - hb - GetShadowOffset - Round(FDPIScale * Header.GetHeight));
  end
  else
  begin
    Result := MakeRect(InsideRect.Left, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + hb, InsideRect.Right - InsideRect.Left - GetShadowOffset,
      InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - fb - hb - GetShadowOffset - Round(FDPIScale * Header.GetHeight) - scrl);
  end;
end;
...
begin
  ...
  begin
    ...
    Result := Max(0, Result - Height + Round(FDPIScale * Footer.GetHeight) + GetShadowOffset);
  end
  else if Columns = 0 then
  begin
    if FDisplayList.Count > 0 then
      Result := Round(FDisplayList.GetItem(FDisplayList.Count - 1).RectangleOrig.X + ItemAppearance.GetColumnWidth +
        {FCurrentScPos + }Round(FDPIScale * ItemAppearance.ItemHorizontalSpacing));
...
begin
  ...
  begin
    if FTextVisible then
      Result := Round(FDPIScale * FTextHeight);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        x := Round(FDPIScale * Left);
        y := Round(FDPIScale * Top);
      end;
...
begin
  ...
  begin
    if FTextVisible then
      Result := Round(FDPIScale * FTextWidth);
  end;
end;
...
begin
  ...
  begin
    if FImageVisible then
      Result := Round(FDPIScale * FImageHeight) + Round(FDPIScale * FImageMargin.Top) + Round(FDPIScale * FImageMargin.Bottom);
  end;
end;
...
begin
  ...
  begin
    if FImageVisible then
      Result := FImageWidth + Round(FDPIScale * FImageMargin.Left) + Round(FDPIScale * FImageMargin.Right);
  end;
end;
...
begin
  ...
  begin
    if FTextVisible then
      Result := Round(FDPIScale * FTextHeight) + Round(FDPIScale * FTextMargin.Top) + Round(FDPIScale * FTextMargin.Bottom);
  end;
end;
...
begin
  ...
  begin
    if FTextVisible then
      Result := Round(FDPIScale * FTextWidth) + Round(FDPIScale * FTextMargin.Left) + Round(FDPIScale * FTextMargin.Right);
  end;
end;
...
begin
  ...
  spc := Round(FDPIScale * ItemAppearance.Splitter.Spacing);
  ssz := Round(FDPIScale * ItemAppearance.Splitter.Size);
  hspc := Round(FDPIScale * ItemAppearance.ItemHorizontalSpacing);
  vspc := Round(FDPIScale * ItemAppearance.ItemVerticalSpacing);
  ...
  if Rows = 0 then
  begin
    starty := bw + vspc + Round(FDPIScale * Header.GetHeight);
    startx := bw + hspc;
  end
  else if Columns = 0 then
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          r := MakeRect(x - (ssz / 2) - (hspc + spc) / 2, bw + Round(FDPIScale * Header.GetHeight), ssz,
            InsideRect.Bottom - shadoff - bw - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - scrl);
      ...
      begin
        FVisibleCol := Columns;
        FVisibleRow := Round((dr.Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight)) / (h + vspc));
      end;
    end;
  end;
...
begin
  inherited;
  if not MouseWheelScrolling then
    Exit;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            r := Bounds(Round(Rectangle.X), Round(Rectangle.Y + GetPosition), Round(Rectangle.Width), Round(Rectangle.Height));
            if r.Top - GetPosition > Height - Round(FDPIScale * Footer.GetHeight) - (r.Bottom-r.Top) then
            begin
              FScPosTo := r.Bottom - (Height - Round(FDPIScale * Footer.GetHeight));
              FScPosTo := FScPosTo + ItemAppearance.ItemVerticalSpacing;
            end
            else if r.Top < GetPosition + Round(FDPIScale * Header.GetHeight) then
            begin
              FScPosTo := r.Top - Round(FDPIScale * Header.GetHeight);
              FScPosTo := FScPosTo - Round(FDPIScale * ItemAppearance.ItemVerticalSpacing);
            end;
          end
          else if Columns = 0 then
          begin
...
begin
  ...
  begin
    ...
    begin
      FScrollBar.Height := 0;
      FScrollBar.Top := - Round(FDPIScale * Fscrollbar.Width);
    end;
    if (Columns = 0) then
    begin
      FScrollBar.Width := 0;
      FScrollBar.Left := - Round(FDPIScale * Fscrollbar.Height);
    end;
  ...
  begin
    FScrollBar.Kind := sbVertical;
    FScrollBar.Left := InsideRect.Right - Round(FDPIScale * GetScrollBarWidth) - GetOuterBorderWidth;
    FScrollBar.Top := InsideRect.Top + Round(FDPIScale * Header.GetHeight);
    FScrollBar.Height := InsideRect.Bottom - InsideRect.Top - GetShadowOffset - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight);
    FScrollBar.Width := GetSystemMetrics(SM_CYVSCROLL);
    FScrollBar.LargeChange := Round(FDPIScale * ItemAppearance.ItemHeight);
  end
  else if Columns = 0 then
  begin
    FScrollBar.Kind := sbHorizontal;
    FScrollBar.Width := InsideRect.Right - InsideRect.Left - GetShadowOffset - 1;
    FScrollBar.Height := GetSystemMetrics(SM_CYHSCROLL);
    FScrollBar.Left := InsideRect.Left + 1;
    FScrollBar.Top := InsideRect.Bottom - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset - Round(FDPIScale * GetScrollBarHeight);
    FScrollBar.LargeChange := Round(FDPIScale * ItemAppearance.ItemWidth);
  end;
...
procedure TAdvSmoothImageListBox.ZoomItem(AItemIndex: Integer);
begin
  DoZoom(AItemIndex);
end;
...
procedure TAdvSmoothImageListBoxItem.ClearImage;
begin
  LoadImageFromStream(nil);
  FImage.Assign(nil);
end;
...
begin
  with FOwner do
  begin
    result := Round(FDPIScale * ItemAppearance.ItemWidth);
    ...
    begin
      w := InsideRect.Right - InsideRect.Left - GetShadowOffset - scrl - Round(FDPIScale * ItemAppearance.ItemHorizontalSpacing);
      if Columns > 0 then
      begin
        cw := Round((w - (Columns * Round(FDPIScale * ItemAppearance.ItemHorizontalSpacing))) / Columns);
        if cw < Round(FDPIScale * ItemAppearance.ItemWidth) then
          result := cw
      end;
    end;
  end;
...
begin
  with FOwner do
  begin
    result := Round(FDPIScale * ItemAppearance.ItemHeight);
    ...
    if Self.AutoSize then
    begin
      h := InsideRect.Bottom - InsideRect.Top - GetShadowOffset - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight) - scrl - Round(FDPIScale * ItemAppearance.ItemVerticalSpacing);
      if Rows > 0 then
      begin
        rh := Round((h - (Rows * Round(FDPIScale * ItemAppearance.ItemVerticalSpacing))) / Rows);
        if rh < Round(FDPIScale * ItemAppearance.ItemHeight) then
          result := rh
      end;
    end;
  end;
...
begin
  ...
  begin
    rc := InsideRect;
    Result := MakeRect(rc.Left, rc.Top, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Self.Height));
  end;
end;
...
begin
  ...
  begin
    rc := InsideRect;
    Result := MakeRect(rc.Left, rc.Bottom - Round(FDPIScale * Self.Height) - GetShadowOffset, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Self.Height));
  end;
end;
...
begin
  ...
  case Shape of
    ...
    begin
      fl.RoundingType := rtBoth;
      fl.Rounding := Round(FOwner.FDPIScale * Size / 2);
    end;
    nsRoundRect:
    begin
      fl.RoundingType := rtBoth;
      fl.Rounding := Round(FOwner.FDPIScale * Size / 6);
    end;
  end;
...
begin
  Result := MakeRect(R.X + Button * Round(FOwner.FDPIScale * (Size + 3)), R.Y, Round(FOwner.FDPIScale * Size), Round(FOwner.FDPIScale * Size));
end;
...
var
  ...
  x, y: single;
begin
  w := Round(FOwner.FDPIScale * (Size + 3)) * 4;
  h := Round(FOwner.FDPIScale * Size);
  ...
  begin
    x := Round(FOwner.FDPIScale * FLeft);
    y := Round(FOwner.FDPIScale * FTop);
  end;
