...
const
  ...
  // v1.5.1.1 : Improved : Mouse wheel handling
  // v1.5.1.2 : Fixed : Issue with erratic triggering of OnChange
  // v1.5.1.3 : Fixed : Issue with mouse wheel handling on Windows 7
  // v1.5.2.0 : Improved : Label positioning in high DPI
  // v1.5.3.0 : New : Exposed StyleElements property
  // v1.5.3.1 : Fixed : Issue with triggering OnChange event during lookup
  // v1.6.0.0 : New : Property EnableWheel added
  // v1.6.0.1 : Fixed : Issue with EnableWheel in specific circumstances
  // v1.6.1.0 : Improved : Performance when loading many items
  // v1.7.0.0 : New : Support for VCL styles added
  //          : New : Property GridLineColor added
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$IFDEF DELPHIXE2_LVL}
        lDetails := StyleServices.GetElementDetails(ttItemDisabled);
...
begin
  ...
  FWheelmsg := RegisterWindowMessage(MSH_SCROLL_LINES);
...
procedure TColumnComboBox.SetGridLineColor(const Value: TColor);
begin
...
procedure TColumnComboBox.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
const
  lColorStates: array[Boolean] of TStyleColor = (scComboBoxDisabled, scComboBox);
var
  i: integer;
  clr: TColor;
{$ENDIF xe2}
begin
  ...
  begin
    ...
    {$ENDIF xe6}
      ...
      if Assigned(FLabel) and (LabelCaption <> '') then
        FLabel.Color := StyleServices.GetSystemColor(clBtnFace);
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF xe6}
...
{$IFDEF DELPHIXE2_LVL}
procedure TColumnComboBox.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
begin
  ...
  Result := (verinfo.dwMajorVersion = 10);
end;
...
procedure TColumnComboBox.WndProc(var Message: TMessage);
var
  oldidx: integer;
begin
  if (Message.Msg = WM_MOUSEWHEEL) and not FEnableWheel then
  begin
    Message.Result := 0;
    Exit;
  end;

  if (Message.Msg = WM_MOUSEWHEEL) and IsWin10 and FEnableWheel then
  begin
    oldidx := ItemIndex;
