...
const
  ...
  // 2.1.1.0 : New : Support for High DPI added
  // 2.1.2.0 : Improved : High DPI support
  // 2.1.2.1 : Fixed : Issue  with design-time color initialization
  // 2.1.2.2 : Fixed : Font rotation issue on old Delphi versions
  // 2.2.0.0 : New : Per monitor high DPI support
  //         : New : VCL styles support
  // 2.2.0.1 : Improved : Splitter grip painting in VCL Styles
  // 2.2.1.0 : New : Metro style support added
  //         : Fixed : Issue with fixed caption height when used with small images
  //         : Fixed : Bottom  to  top rendering issue with VCL Styles
  // 2.2.1.1 : Fixed : Issue with high-dpi with sections and controls in sections
...
type
  ...
  {$ENDIF}
  TAdvNavBar = class(TCustomControl, ITMSStyle, ITMSTones)
  private
    FDPIScale: single;
    FTMSStyle: TTMSStyle;
  ...
  protected
    ...
    function GetFixedBtnWidth: integer;
...
begin
  ...
  begin
    ...
    begin
      if (P.Y > h) and (P.Y < h + AdvNavBar.GetSectionHeight) then
      begin
        if (i > 1) then
        begin
          Msg.Result := 1;
...
begin
  ...
  begin
    if FSizing then
    begin
      d := Round((Y - FSizeY)/ FAdvNavBar.FDPIScale);
      if FSizeHeight + d > AdvNavBar.GetSectionHeight then
        TAdvNavBarPanelSection(Sections.Items[FSizeSection - 1]).Height := FSizeHeight + d
      else
      if TAdvNavBarPanelSection(Sections.Items[FSizeSection - 1]).Height <> AdvNavBar.GetSectionHeight + 1 then
        TAdvNavBarPanelSection(Sections.Items[FSizeSection - 1]).Height := AdvNavBar.GetSectionHeight + 1;
...
begin
  ...
  begin
    for i := 1 to Sections.Count do
    begin
      if (Y > h) and (Y < h + AdvNavBar.GetSectionHeight) then
      begin
        if (i > 1) then
        begin
          FSizeSection := i - 1;
          FSizeHeight := Round(TAdvNavBarPanelSection(Sections.Items[FSizeSection - 1]).Height);
          FSizing := true;
          FSizeY := Y;
        end;
        Exit;
      end;
      h := h + Round(TAdvNavBarPanelSection(Sections.Items[i - 1]).Height * FAdvNavBar.FDPIScale);
    end;
  end;
...
begin
  ...
  for i := 1 to Sections.Count do
  begin
    if FAdvNavBar.FUseVCLStyles then
    begin
      {$IFDEF DELPHIXE2_LVL}
      lDetails := StyleServices.GetElementDetails(thHeaderItemHot);
      StyleServices.DrawElement(Canvas.Handle, lDetails, Rect(0, h, width, h + AdvNavBar.GetSectionHeight));
      {$ENDIF}
    end
    else
    begin
      DrawGradient(Canvas, AdvNavBar.SectionColor, AdvNavBar.SectionColorTo, 16, Rect(0, h, width, h + AdvNavBar.GetSectionHeight), False);
    end;

    Canvas.Pen.Color := AdvNavBar.BorderColor;
    Canvas.MoveTo(0, h + AdvNavBar.GetSectionHeight);
    Canvas.LineTo(Width, h + AdvNavBar.GetSectionHeight);
    ...
    Canvas.Font.Height := Round(Canvas.Font.Height * AdvNavBar.FDPIScale);
    ...
    {$IFDEF DELPHIXE2_LVL}
    case CaptionAlignment of
      taRightJustify: tf := [tfRight, tfSingleLine, tfTop];
      taCenter: tf := [tfLeft , tfSingleLine, tfTop];
    else
      tf := [tfLeft , tfSingleLine, tfTop];
    end;
    {$ENDIF}
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      StyleServices.DrawText(Canvas.Handle, lDetails, TAdvNavBarPanelSection(Sections.Items[i - 1]).Caption, r, tf, AdvNavBar.SectionFont.Color);
      {$ENDIF}
    end
    else
    begin
...
begin
  ...
  FPreCollapsedWidth := Round(Width / FDPIScale);
...
begin
  ...
  begin
    ...
    begin
      ...
      if FDefaultTabPosition = tpBottom then
        Result := Rect(0, Height - ((Index + 1) * GetCaptionTabHeight + GetFixedCaptionTabHeight) - 1, Width, (Height - ((index + 1 {FixedTab}) * GetCaptionTabHeight + GetFixedCaptionTabHeight)) + GetCaptionTabHeight)
      else
        Result := Rect(0, ((Index) * GetCaptionTabHeight + GetFixedCaptionTabHeight), Width, (((index {FixedTab}) * GetCaptionTabHeight + GetFixedCaptionTabHeight)) + GetCaptionTabHeight + 1);
    end;
  end;
end;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvNavBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvNavBar.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  case CaptionAlignment of
    taRightJustify:
      DRWSTYLE := DT_RIGHT or DT_SINGLELINE or DT_END_ELLIPSIS;
    taCenter:
      DRWSTYLE := DT_CENTER or DT_SINGLELINE or DT_END_ELLIPSIS;
    else
  ...
  {$IFDEF DELPHIXE2_LVL}
    ...
    else if Index = FHoverTabIndex then
    begin
    ...
    StyleServices.DrawElement(Canvas.Handle, lDetails, TabR);

    { Render active tab panel caption at the top of the control }
    if (Index = FActiveTabIndex) then
    begin
      ...
      begin
        StyleServices.DrawText(
          Canvas.Handle,
          StyleServices.GetElementDetails(thHeaderItemHot),
          TAdvNavBarPanel(FAdvNavBarPanel[Index]).AdjustedCaption,
          DR,
          TTextFormatFlags(DRWSTYLE{ or DT_VCENTER})
        );
      end;
    ...
    Canvas.Brush.Style := bsClear;
    Canvas.Font.Color := clrText;

    { calculate new text rectangle }
    DR := TabR;
    if Assigned(FImages) then
    begin
      DR.Left := DR.Left + FImages.Width + (Margins.Left + Margins.Right);
      DR.Width := DR.Width - (FImages.Width + (Margins.Left + Margins.Right));
    end;
    StyleServices.DrawText(
        Canvas.Handle,
        lDetails,
    ...
    exit;  { VCL Styles has nothing further to do. }
  {$ENDIF}
  end
  else
  begin
    ...
    begin
      ...
      Canvas.LineTo(Width, R.Bottom - 1);
...
begin
  ...
  try
    ...
    begin
      ...
      {$IFDEF DELPHIXE_LVL}
      tf.Height := Round(tf.Height * FDPIScale);
      {$ENDIF}
...
begin
  ...
  begin
    ...
    // We don't render the "normal" state. It should blend with the background.
  ...
  if Assigned(FImages) then
  begin
    if (FDPIScale > 1) or ((FImages.Width = 16) and (FImages.Height = 16)) then
    begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      StyleServices.DrawText(Canvas.Handle, lDetails, bCaption, DR, [tfLeft, tfEndEllipsis, tfVerticalCenter]);
      {$ENDIF}
    end
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.TextOut(R.Left + 2 + Round(4 * FDPIScale), R.Top + 8, '<');
    end
    else
    begin
      DrawGradient(Canvas, DefaultTabColor, DefaultTabColorTo, 16, R, false);
...
begin
  ...
  R.Left := Width - Round(20 * FDPIScale);
  R.Right := Width - 4;
  R.Top := R.Top + 4;
  R.Bottom := R.Bottom - 4;
  ...
  begin
    ...
    // we don't want to render the "normal" state. Text should blend with background.
    {$ENDIF}
  end
  else
  begin
  ...
  Canvas.Font.Style := [fsBold];
  Canvas.Font.Color := CaptionFont.Color;
  Canvas.Font.Size := Round(8 * FDPIScale);
  ...
  begin
    ...
    TxtRect := R;
    TxtRect.Width := TxtRect.Width - 2;
    TxtRect.Top := TxtRect.Top + 2;
    StyleServices.DrawText(Canvas.Handle, lDetails, S, TxtRect, TTextFormatFlags(DT_RIGHT or DT_SINGLELINE or DT_VCENTER));

    TxtRect.Width := TxtRect.Width - 4;
    StyleServices.DrawText(Canvas.Handle, lDetails, S, TxtRect, TTextFormatFlags(DT_RIGHT or DT_SINGLELINE or DT_VCENTER));
    {$ENDIF}
  end
  else
  begin
    ...
    if not IsOffice2010 then
    begin
      Canvas.TextOut(R.Right - Round(10 * FDPIScale), R.Top + 2, s);
      Canvas.TextOut(R.Right - Round(14 * FDPIScale), R.Top + 2, s);
    end
    else
    begin
      Canvas.TextOut(R.Right - Round(12 * FDPIScale), R.Top + 2, s);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      // Render tab/button background only if hover or down
      lDetails := StyleServices.GetElementDetails(thHeaderItemNormal);
    ...
    begin
      if FUSeVCLStyles then
        Canvas.Pen.Color := SectionFont.Color
      else
    ...
    begin
      ...
      begin
        ...
        begin
          {$IFDEF DELPHIXE2_LVL}
          DR := Rect(R.Right - GetIndicatorWidth + 6, R.Top, R.Right, R.Bottom);
          StyleServices.DrawText(Canvas.Handle, lDetails, '>', DR, [tfLeft]);
          DR := Rect(R.Right - GetIndicatorWidth + 9, R.Top, R.Right, R.Bottom);
          StyleServices.DrawText(Canvas.Handle, lDetails, '>', DR, [tfLeft]);
          {$ENDIF}
        end
        else
...
procedure TAdvNavBar.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
//  clrPanelBG: TColor;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    StyleServices.GetElementColor(StyleServices.GetElementDetails(thHeaderItemHot), ecFillColor, clr);
    SectionColor := clr;
    SectionColorTo := clr;

    StyleServices.GetElementColor(StyleServices.GetElementDetails(thHeaderItemHot), ecTextColor, clr);
    SectionFont.Color := clr;
    ...
    {$ENDIF}
      ...
      CaptionFont.Color := clrFont;
(*

//      PanelCaption.ActiveFont.Color := StyleServices.GetSystemColor(clHighlight);
  //    PanelCaption.ActiveFont.Color := StyleServices.GetSystemColor(clHighlightText);
...
*)
    ...
    {$ENDIF}
      ...
      for i := 0 to self.PanelCount-1 do
        Panels[i].Color := clr;
(*
...
//        Panels[i].SelectColor := StyleServices.GetSystemColor(clHighlight);
//        Panels[i].SelectBorderColor := StyleServices.GetSystemColor(clHighlight);
      end;
*)
    {$IFDEF DELPHIXE6_LVL}
    end;
...
//    GroupLineColor  := StyleServices.GetStyleColor(scGenericGradientEnd);

    //StyleServices.GetElementColor(StyleServices.GetElementDetails(tgFixedCellNormal), ecTextColor, clr);
//    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgCellNormal), ecTextColor, clr);
//    GroupFont.Color      := clr;
//    GroupCountFont.Color := clr;
//
  end;
{$ENDIF DELPHIXE2_LVL}
end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvNavBar.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
function TAdvNavBar.GetSectionHeight: Integer;
begin
  Result := Round(SectionHeight * FDPIScale);
end;
...
begin
  if CaptionVisible then
    Result := Round(CaptionHeight * FDPIScale)
  else
    Result := 0;
end;
...
begin
  Result := Round(CaptionTabHeight * FDPIScale);
end;
...
begin
  ...
  {$IFDEF DELPHIXE2_LVL}
    ...
    R.Top := R.Top + 2;
    R.Bottom := R.Bottom - 1;
    lDetails := StyleServices.GetElementDetails(trGripperVert);
    StyleServices.DrawElement(Canvas.Handle, lDetails, R);
  {$ENDIF}
  end
  else
...
begin
  ...
  begin
    ...
    if BottomIconAlign = biLeft then
    begin
      lf := FFixedBtnMargin + abs({(FAdvNavBarPanel.Count -1- FDisplayCaptionTabsCount)} FixedBtnMaxIndex - index) * GetFixedBtnWidth;
    end
    else // BottomIconAlign = biRight
    begin
      lf := R.Right - FFixedBtnMargin - GetFixedBtnWidth;
      lf := lf - abs({(FAdvNavBarPanel.Count -1- FDisplayCaptionTabsCount)} FixedBtnMaxIndex - index) * GetFixedBtnWidth;
    end;
    Result := Rect(lf, R.Top, lf + GetFixedBtnWidth, R.Bottom);
  end;
...
function TAdvNavBar.GetFixedBtnWidth: integer;
begin
  if Assigned(SmallImages) then
    Result := SmallImages.Width + 8
  else
  if Assigned(Images) then
    Result := Images.Width + 8
  else
    Result := 24
end;
...
begin
  ...
  begin
//    if Assigned(Images) then
//      Result := Round(Images.Height * FDPIScale)
//    else
    Result := GetCaptionTabHeight
  end
  else
    Result := 0;
end;
...
begin
  ...
  if FPopupIndicator then
    R.Right := R.Right - GetIndicatorWidth - 1;
  wd := R.Right - R.Left;
  Result := (wd - FFixedBtnMargin * 2) div GetFixedBtnWidth;
end;
...
//    MoveSplitterInTabs(-1);
  ...
  if (Width <= Round(FCollapsedWidth * FDPIScale)) and AllowCollaps then
    SetCollapsed(True);

  if (Width <= Round(CollapsedWidth * FDPIScale)) then
...
begin
  if ShowSplitter then
    Result := Round(DefSplitterHeight * FDPIScale)
  else
    Result := 0;
end;
...
begin
  ...
  if (FAdvNavBarPanel.Count > 0) then
  begin
    cw := MinClientHeight + GetCaptionHeight + (SplitterPosition + 1) * GetFixedBtnWidth + SplitterHeight;

    if (AHeight < cw) then
    begin
      if AHeight - MinClientHeight - GetCaptionHeight - GetFixedBtnWidth - SplitterHeight > 0 then
      begin
        nsh := AHeight - MinClientHeight - GetCaptionHeight - GetFixedBtnWidth - SplitterHeight;
        ns := trunc(nsh / GetFixedBtnWidth);
...
begin
  ...
  begin
    ...
    begin
      Inc(ARect.Top,Round(Panel.Sections[i].FHeight * Panel.FAdvNavBar.FDPIScale )) ;
    end;

    Inc(ARect.Top, Panel.AdvNavBar.GetSectionHeight + 1); //One more for this section....

    if Index <> Panel.Sections.Count - 1 then
    begin //if NOT the last panel, set bottom, otherwise use ClientRect bottom
      ARect.Bottom := ARect.Top + Round(FHeight * Panel.FAdvNavBar.FDPIScale) - Panel.AdvNavBar.GetSectionHeight - 1;
    end;
...
begin
  ...
  try
    if Value then
    begin
      if FPreCollapsedWidth > Round(FCollapsedWidth * FDPIScale) then
      begin
        FPreCollapsedWidth := Round(Width / FDPIScale);
      end;

      Width := Round(FCollapsedWidth * FDPIScale);
    ...
    begin
      ...
      begin
        Width := Round(FPreCollapsedWidth* FDPIScale);
      end;
...
begin
  ...
  begin //Already collapsed
    if (NewWidth > 0) and (NewWidth <= Round(FCollapsedWidth * FDPIScale)) then
    begin
       Result := Width = 0; //*WAS* Sqz'ed all the way down, ok to show as collapsed and visible
       if Result then
         NewWidth := Round(FCollapsedWidth * FDPIScale);
    end
    else
      if NewWidth > Round(FCollapsedWidth * FDPIScale) then
      begin
...
procedure TAdvNavBar.SetColorTones(ATones: TColorTones);
begin
  CaptionColor := ATones.Selected.BrushColor;
  CaptionColorTo := clNone;

  CaptionFont.Color := ATones.Selected.TextColor;
  Font.Color := ATones.Background.TextColor;

  ActiveColor := ATones.Selected.BrushColor;
  ActiveColorTo := ATones.Selected.BrushColor;

  ActiveMirrorColor := clNone;
  ActiveMirrorColorTo := clNone;
  ActiveTextColor := ATones.Selected.TextColor;

  DefaultTabColor := ATones.Background.BrushColor;
  DefaultTabColorTo := clNone;

  DefaultTabMirrorColor := clNone;
  DefaultTabMirrorColorTo := clNone;

  DownTabColor := ATones.Selected.BrushColor;
  DownTabColorTo := ATones.Hover.BrushColor;
  DownTextColor := ATones.Selected.TextColor;
  DownTabMirrorColor := clNone;
  DownTabMirrorColorTo := clNone;

  HoverTextColor := ATones.Hover.TextColor;
  HoverTabColor := ATones.Hover.BrushColor;
  HoverTabColorTo := ATones.Hover.BrushColor;
  HoverTabMirrorColor := clNone;
  HoverTabMirrorColorTo := clNone;

  SplitterColor := ATones.Foreground.BrushColor;
  SplitterColorTo := ATones.Foreground.BrushColor;

  BorderColor := ATones.Background.BorderColor;
  SectionColor := ATones.Foreground.BrushColor;
  SectionColorTo :=  ATones.Foreground.BrushColor;
