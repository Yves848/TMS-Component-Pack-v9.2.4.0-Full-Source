{**************************************************************************}
{ TAdvSmoothDatePickerDE DESIGN TIME EDITOR                                }
...
uses
  Classes, AdvSmoothDatePicker, DesignIntf, DesignEditors;

type
  TAdvSmoothDatePickerEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothDatePickerEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothDatePicker).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothDatePicker) then
         (Component as TAdvSmoothDatePicker).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothDatePickerEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothDatePickerEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
