{**************************************************************************}
{ TAdvSmoothWin8MarqueeDE DESIGN TIME EDITOR                                   }
...
uses
  Classes, AdvSmoothWin8Marquee, DesignIntf, DesignEditors;

type
  TAdvSmoothWin8MarqueeEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothWin8MarqueeEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothWin8Marquee).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothWin8Marquee) then
         (Component as TAdvSmoothWin8Marquee).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothWin8MarqueeEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothWin8MarqueeEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
