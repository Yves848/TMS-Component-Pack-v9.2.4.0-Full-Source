...
const
  ...
  // v1.9.0.3 : Fixed : Issue with transparency on double buffered forms
  // v1.9.1.0 : Improved : Support for High DPI
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothProgressBar.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothProgressBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  Appearance.Font.Height := MulDiv(Appearance.Font.Height, M, D);
  Appearance.ProgressFont.Height := MulDiv(Appearance.ProgressFont.Height, M, D);
end;
