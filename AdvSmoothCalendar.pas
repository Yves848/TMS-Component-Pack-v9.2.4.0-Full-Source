...
const
  ...
  // v2.3.0.4 : Fixed : Issue with transparency on double buffered forms
  // v2.3.0.5 : Fixed : Issue with offset weeknumbers when applying metro style
  // v2.3.0.6 : Fixed : Warnings in 64bit code
  // v2.3.0.7 : Fixed : Week number drawing in metro style
  // v2.3.1.0 : New : OnDateDblClick event added
  // v2.3.2.0 : Improved : Support for High DPI (text size, header/footer width, placement and updown button)
  // v2.4.0.0 : New : VCL Styles support
...
type
  ...
  TAdvSmoothCalendarDateEvent = procedure(Sender: TObject; Date: TDateTime) of object;
  ...
  private
    ...
    FOnDateDblClick: TAdvSmoothCalendarDateEvent;
  ...
  protected
    ...
    procedure DoChangeMonth(Sender: TObject; Month: integer); virtual;
    procedure DoChangeYear(Sender: TObject; Year: integer); virtual;
    procedure DoChangeYearRange(Sender: TObject; YearFrom, YearTo: integer); virtual;
    procedure DoDateDblClick(ADate: TDateTime); virtual;
    procedure FillChanged(Sender: TObject);
  ...
  published
    ...
    property OnDateDblClick: TAdvSmoothCalendarDateEvent read FOnDateDblClick write FOnDateDblClick;
...
procedure TAdvSmoothCalendar.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
procedure TAdvSmoothCalendar.DoDateDblClick(ADate: TDateTime);
begin
  if Assigned(OnDateDblClick) then
    OnDateDblClick(Self, ADate);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      f := TGPFont.Create(ff, cf.Size * DPIScale, fs, UnitPoint);
    ...
    begin
      ...
      status.Font.Size := Round(status.Font.Size * DPIScale);
      date := GetDate(i, dim);
      wd := GetDayOfWeekColumn(date);
      str := '';
      x := xpos + xoff + (wd * cw);
      ...
      begin
        ...
        x := x - (status.GetWidth / 3 * 2) + (OffsetX * DPIScale);
        y := y - (status.GetHeight / 2) + (OffsetY * DPIScale);
...
begin
  ...
  begin
    ...
    layr := GetDateValuesRect;
    layr.Y := Header.GetHeight * DPIScale;
    wwd := GetCellWidth;
    dpw := DaysPerWeek;
    ...
    sf := TGPStringFormat.Create;
    f := TGPFont.Create(ff, FDayOfWeekFont.Size * DPIScale, fs, UnitPoint);
    g.SetTextRenderingHint(TextRenderingHintClearTypeGridFit);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Fill.BorderColor = clNone) or (Fill.BorderWidth = 0) then
        r := Rect(InsideRect.Left, InsideRect.Bottom - Round(Height * DPIScale) - FOwner.GetShadowOffset, InsideRect.Right - FOwner.GetShadowOffset, InsideRect.Bottom - FOwner.GetShadowOffset)
      else
        r := Rect(InsideRect.Left, InsideRect.Bottom - Round(Height * DPIScale) - FOwner.GetShadowOffset, InsideRect.Right - FOwner.GetShadowOffset - 1, InsideRect.Bottom - FOwner.GetShadowOffset - 1);
      ...
      begin
        ...
        sf := TGPStringFormat.Create;
        f := TGPFont.Create(ff, FFont.Size * DPIScale, fs, UnitPoint);
        g.SetTextRenderingHint(TextRenderingHintClearTypeGridFit);
...
begin
  ...
  begin
    ...
    begin
      ...
      DPIScale := GetDPIScale(Self, Canvas);
       r := Rect(InsideRect.Left, InsideRect.Top , InsideRect.Right - FOwner.GetShadowOffset - bw, InsideRect.Top + Round(Height * DPIScale) - bw);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        g.DrawString(s, Length(s), f, fillr, sf, b);
        dweek := dweek + 7;
        Inc(K);
      end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  c := Round(Header.GetHeight * DPIScale / 2);
  s := Round(Header.ArrowSize * DPIScale);
  if left then
  begin
    Result.X := s;
    Result.Width := s;
...
begin
  ...
  h := Round(Header.GetHeight * DPIScale);
  f := Round(Footer.GetHeight * DPIScale);;
...
begin
  ...
  begin
    ...
    begin
      ...
      r := Rect(InsideRect.Left, InsideRect.Bottom - Round(Height * DPIScale), InsideRect.Right - 1, InsideRect.Bottom);
...
begin
  ...
  begin
    ...
    begin
      ...
      r := Rect(InsideRect.Left, InsideRect.Top , InsideRect.Right, InsideRect.Top + Round(Height * DPIScale));
...
begin
  ...
  begin
    ...
    layr := MakeRect(InsideRect.Left + wnoff, InsideRect.Top + (Header.GetHeight * DPIScale), InsideRect.Right - wnoff, InsideRect.Bottom - InsideRect.Top - Header.GetHeight * DPIScale);
    dpw := DaysPerWeek;
...
procedure TAdvSmoothCalendar.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FUpDown.Height := round(header.Height * DPIScale) - 2;
            FUpDown.Width := Round(FUpDown.Height * 2/3);
            FUpDown.Top := Round(((header.Height * DPIScale) - FUpDown.Height) / 2);
            FUpDown.Parent := Self;
...
procedure TAdvSmoothCalendar.WMLDblClk(var Msg: TWMLButtonDblClk);
var
  dt: TDateTime;
begin
  inherited;

  dt := XYToDate(Msg.XPos, Msg.YPos);
  if dt <> -1 then
    DoDateDblClick(dt);
end;
