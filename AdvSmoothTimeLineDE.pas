{**************************************************************************}
{ TAdvSmoothTimeLine DESIGN TIME EDITOR                                    }
...
uses
  Classes, AdvSmoothTimeLine,
  DesignIntf, DesignEditors;

type
  TAdvSmoothTimeLineEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothTimeLineEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothTimeLine).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothTimeLine) then
         (Component as TAdvSmoothTimeLine).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothTimeLineEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothTimeLineEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
