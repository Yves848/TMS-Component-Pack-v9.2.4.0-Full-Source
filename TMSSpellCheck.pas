...
uses
  ...
  {$IFDEF FMXLIB}
  ,FMX.Forms, FMX.Controls, FMX.ExtCtrls, FMX.Types, FMX.TMSTypes, FMX.TMSBaseControl, System.IOUtils
  {$ENDIF}
  {$IFNDEF FMXLIB}
...
const
  ...
  BLD_VER = 2; // Build nr.
  DATE_VER = 'Jun, 2019'; // Month version
  ...
  // v1.0.4.1 : Fixed : Issue with correct dialog & correct panel handling
  // v1.0.4.2 : Fixed : Issue with Add button handling for correct dialog
  // v1.0.5.0 : Improved : Added support to use network folder names in path to spell check DB
  // v1.0.5.1 : Fixed : Issue with correct in multiline richeditor text
  // v1.0.6.0 : New : UseSychronize property added
  //          : New : UseProgressUI property added
  // v1.0.6.1 : Fixed : Issue with Ignore vs Ignore All
  // v1.0.6.2 : Fixed : Issue with hamming distance calculation
...
type
  ...
  public
    ...
    property UseSynchronize: Boolean read FUseSynchronize write FUseSynchronize default true;
    property UseProgressUI: Boolean read  FUseProgressUI write FUseProgressUI default true;
  end;
...
begin
  ...
  begin
    s := IntToBinByte(PByteArray(@Word2[I])[0]);
    for C := 1 to 8 do
      r2[((I - 1) * 16) + C] := s[C];
    s := IntToBinByte(PByteArray(@Word2[I])[1]);
    for C := 1 to 8 do
      r2[(((I - 1) * 16) + 8) + C] := s[C];
  end;
...
begin 
  if (EventType in FParent.AsyncEventsList) or not (FParent.UseSynchronize) then
    Execute
  else
