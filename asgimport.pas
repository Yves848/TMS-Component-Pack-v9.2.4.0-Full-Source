...
type
  ...
  TCSVEncoding = (encANSI, encUTF7, encUTF8, encUnicode);
  ...
  published
    ...
    {$IFDEF DELPHI_XE10}
    property Encoding: TCSVEncoding read FEncoding write FEncoding default encANSI;
    {$ENDIF}
    property Grid: TAdvStringGrid read FGrid write FGrid;
...
{$R *.DFM}
 ...
 {$IFDEF DELPHI_XE7}
function GetEncoding(ACSVEncoding:TCSVEncoding): TEncoding;
begin
  Result := TEncoding.ANSI;
  case ACSVEncoding of
    encUTF7: Result := TEncoding.UTF7;
    encUTF8: Result := TEncoding.UTF8;
    encUnicode: Result := TEncoding.Unicode;
  end;
end;
{$ENDIF}
  ...
  with FForm do
    try
      ...
      begin
        ...
        {$IFDEF DELPHI_XE7}
        GridPreview.LoadFromCSV(FileName, GetEncoding(Encoding), FRowsToDisplay);
        {$ELSE}
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHI_XE7}
    Grid.LoadFromCSV(FileName, GetEncoding(FEncoding), LoadRowCount);
    {$ELSE}
...
begin
  ...
  GridPreview.ColCount := GridPreview.FixedCols + 1;
  {$IFDEF DELPHI_XE7}
  GridPreview.LoadFromCSV(FileName, GetEncoding(FEncoding), FDisplayRowCount);
  {$ELSE}
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHI_XE7}
    GridPreview.LoadFromCSV(FileName, GetEncoding(FEncoding), DisplayRowCount);
    {$ELSE}
