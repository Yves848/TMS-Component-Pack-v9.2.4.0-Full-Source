    ...
    case Appearance.Layout of
      ...
      begin
        if Appearance.DescriptionLayout <> dlBottom then
          ir := MakeRect(K * iw, r.Y, iw, r.Height - Round(Appearance.ProgressSize * DPIScale))
        else
          ir := MakeRect(K * iw, r.Y, iw, r.Height - Round(Appearance.ProgressSize * DPIScale) - Round(Appearance.DescriptionSize * DPIScale));
      end
    else
      if Appearance.DescriptionLayout <> dlBottom then
        ir := MakeRect(K * iw, r.Y, iw, r.Height)
      else
        ir := MakeRect(K * iw, r.Y, iw, r.Height - Round(Appearance.DescriptionSize * DPIScale));
    end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothStepControl.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothStepControl.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  begin
    DPIScale := GetDPIScale(Self, Canvas);
    cSpc := Round(DPIScale * 4);

    rpr := MakeRect(r.X, r.Y + r.Height - Round(DPIScale * Appearance.ProgressSize), r.Width, Round(DPIScale * Appearance.ProgressSize));
    rpr := MakeRect(rpr.X + cSpc, rpr.Y + 2 *cSpc, rpr.Width - 2 * cSpc, rpr.Height - 3 * cSpc);

    pth := GDIPFill.CreateRoundRectangle(rpr, cSpc, rtBoth, False);
    g.SetClip(pth);
    Appearance.ProgressBackGround.Fill(g, rpr);
    g.ResetClip;

    p := TGPPen.Create(MakeColor(Appearance.ProgressBackGround.BorderOpacity, Appearance.ProgressBackGround.BorderColor), Round(DPIScale * Appearance.ProgressBackGround.BorderWidth));
    g.DrawPath(p, pth);
    p.Free;

    pth.Free;

    rwidth := (rpr.Width - 2 * cSpc) / StepActions.Count;
    if ActiveStep = StepActions.Count then
      pr := rpr.Width - 2 * cSpc
    else
    begin
      pr := ((rpr.Width - 2 * cSpc) / StepActions.Count) * ActiveStep;
      pr := pr + rwidth / 2;
    end;

    if pr > 0 then
    begin
      rpr := MakeRect(rpr.X + cSpc, rpr.Y + cSpc, pr, rpr.Height - 2 * cSpc);
      pth := GDIPFill.CreateRoundRectangle(rpr, cSpc, rtBoth, False);
      g.SetClip(pth);
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  cSpc := Round(DPIScale * 20);
  for I := GetStopStep downto GetStartStep do
    if RectanglesInterSect(MakeRect(r.X - cSpc, r.Y, r.Width + 2 * cSpc, r.Height), StepActions[i].fitemrect) then
...
begin
  ...
  if FOwner.Appearance.ShapeStyle <> sasNone then
    s := Round(DPIScale * FOwner.Appearance.ShapeSize);

  spc := Round(DPIScale * 10);
  textspc := Round(DPIScale * 3);
  border := Round(DPIScale * FOwner.Appearance.ShapeBorderWidth);
  outline := Round(DPIScale * 25);
  trSpc := Round(DPIScale * 10);
  trSz := Round(DPIScale * 10) + trSpc;
  lspcv := Round(DPIScale * FOwner.Appearance.LayoutSpacingVertical);
  lspch := Round(DPIScale * FOwner.Appearance.LayoutSpacingHorizontal);
...
//  app.BackGroundFill.FocusColor := app.BackGroundFill.BorderColor;
  ...
  ft := g.MakeFont(f, Round(DPIScale * f.Size));
  ...
  begin
    ...
    begin
      ...
      begin
        pth.AddLine(rtext.X + rtext.Width - 1 - outline / 2 - Round(DPIScale * FOwner.Appearance.SeparatorWidth), rtext.Y + 1, rtext.X + rtext.Width -1 - outline /2, rtext.Y + 1);
        ...
        pth.AddLine(rtext.X + rtext.Width -1 - outline /2, rtext.Y + rtext.height - 1, rtext.X + rtext.Width - outline / 2 - Round(DPIScale * FOwner.Appearance.SeparatorWidth) - 1, rtext.Y + rtext.height - 1);
        pth.AddLine(rtext.X + rtext.Width -1 - outline /2 - Round(DPIScale * FOwner.Appearance.SeparatorWidth), rtext.Y + rtext.Height - 1, rtext.X + rtext.Width -1 + outline / 2 - Round(DPIScale * FOwner.Appearance.SeparatorWidth), rtext.Y + rtext.Height / 2);
        pth.CloseFigure;
      end;
      ...
      g.SetSmoothingMode(SmoothingModeAntiAlias);
      FOwner.Appearance.SeparatorFill.Fill(g, MakeRect(rText.X + rText.Width - (outline / 2) - Round(DPIScale * FOwner.Appearance.SeparatorWidth), rtext.Y, outline + Round(DPIScale * FOwner.Appearance.SeparatorWidth), rtext.Height));
  ...
  begin
    pth := GDIPFill.CreateRoundRectangle(MakeRect(r.X, r.Y, r.Width, r.Height), Round(DPIScale * FOwner.Appearance.LayoutRounding), rtBoth, False);
...
begin
  ...
  begin
    if str <> '' then
    begin
      DPIScale := GetDPIScale(FOwner.FOwner.FOwner, FOwner.FOwner.FOwner.Canvas);
      ...
      HTMLDrawGDIP(g, ft, str,htmlr,FOwner.FOwner.FOwner.FImageList, fx,fy,-1,-1,FShadowOffset,DoAnchor,false,false,false,
        False,False,true,1.0,FURLColor,clNone,clNone,FShadowColor,a,s,k,XSize,YSize,l,m,hr,nil,FOwner.FOwner.FOwner.FPictureContainer,2, DPIScale);
