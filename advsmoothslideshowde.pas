{**************************************************************************}
{ TAdvSmoothSlideShowDE DESIGN TIME EDITOR                                 }
...
uses
  Classes, AdvSmoothSlideShow, DesignIntf, DesignEditors;

type
  TAdvSmoothSlideShowEditor = class(TDefaultEditor)
  protected
...
procedure TAdvSmoothSlideShowEditor.EditProperty(const PropertyEditor: IProperty; var Continue: Boolean);
var
  PropName: string;
begin
...
procedure TAdvSmoothSlideShowEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothSlideShow).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothSlideShow) then
         (Component as TAdvSmoothSlideShow).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothSlideShowEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothSlideShowEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
