...
const
  ...
  // 1.4.1.0 : New : Property LineSpacing added
  // 1.4.2.0 : New : Support for LINE-HEIGHT attribute for <P> tag
  // 1.4.2.1 : Fixed : Issue with AutoSizingType adHeight
  // 1.4.2.2 : Fixed : Issue with shade color for system colors
  // 1.5.0.0 : New : VCL Styles support added
  // 1.5.1.0 : New : Per monitor support for high DPI
...
procedure THTMLPopupWindow.CreateWnd;
begin
  inherited;
  ...
  begin
    FDPIScale := GetDPIScale(FOwner, GetParentForm(FOwner).Canvas);
  end
  else
...
begin
  ...
  HTMLDrawEx(Canvas, Caption, rd, FImages, 0, 0, -1, FHoverLink, 1, False, False, False, False, True, FHover, True, True,
             1.0, clBlue, clNone, clNone, clGray, Anchor, Stripped, FocusAnchor, XSize, YSize,
             HyperLinks, MouseLink, hr, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
...
begin
  ...
  if FShadeEnable then
    PaintShading(ColorToRGB(FShadeStartColor),ColorToRGB(FShadeEndColor),FShadeSteps,FShadeDirection = sdHorizontal)
  else
  begin
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLPopup.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}

procedure THTMLPopup.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    { If shading is enabled the developer should know what text color would
      me most appropriate, so lets not override it then. }
    if not ShadeEnable then
    begin
...
begin
  ...
  FHTMLPopupWindow.Width := Round(FDPIScale * FWidth);
  FHTMLPopupWindow.Height := Round(FDPIScale * FHeight);
  FHTMLPopupWindow.Left := Round(FDPIScale * FLeft);
  FHTMLPopupWindow.Top := Round(FDPIScale * FTop);
  ...
  FHTMLPopupWindow.ShadeSteps := Round(FDPIScale * FShadeSteps);
  FHTMLPopupWindow.MarginX := Round(FDPIScale * MarginX);
  FHTMLPopupWindow.MarginY := Round(FDPIScale * MarginY);
  FHTMLPopupWindow.Font.Assign(FFont);
  FHTMLPopupWindow.Font.Height := Round(FDPIScale * FFont.Height);
  FHTMLPopupWindow.LineSpacing := Round(FDPIScale * FLineSpacing);

  FHTMLPopupWindow.AutoHide := FAutoHide and (FHideType = htMouseLeave);
  FHTMLPopupWindow.BorderSize := Round(FDPIScale * FBorderSize);
...
begin
  ...
  if HTMLDrawEx(Canvas, Caption, rd, FImages, pt.X, pt.Y, -1, -1, 1, True, False, False, False, False, False, True, True,
             1.0, clBlue, clNone, clNone, clGray, a, s, fa, XSize, YSize,
             hl, ml, hr, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale) then
  begin
    Anchor := a;
  end;
...
begin
  ...
  CreatePopup;
  FTop := Round(r.Bottom / FDPIScale);
  FLeft := Round ((r.Right - FHTMLPopupWindow.Width - Round(FDPIscale * 20)) / FDPIScale);
  Show;
  ...
  begin
    ...
    if FHTMLPopupWindow.Top - Round(FDPIScale * 20) < r.Bottom - FHTMLPopupWindow.Height - FRollUpSpace then
      FHTMLPopupWindow.Top := r.Bottom - FHTMLPopupWindow.Height - FRollUpSpace
    else
      FHTMLPopupWindow.Top := FHTMLPopupWindow.Top - Round(FDPIScale * 20);
...
procedure THTMLPopup.SetLineSpacing(const Value: integer);
begin
  FLineSpacing := Value;
end;
