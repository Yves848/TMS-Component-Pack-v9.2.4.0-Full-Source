...
const
  ...
  // v1.1.0.1 : Fixed : Issue with transparency on double buffered forms
  // v1.1.1.0 : Improved: Support for High DPI (Rounding and Fontsize)
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothCircularProgress.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothCircularProgress.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;

  if not AutoRounding then
  begin
    OuterCircle.FFill.Rounding := MulDiv(OuterCircle.FFill.Rounding, M, D);
    InnerCircle.FFill.Rounding := MulDiv(InnerCircle.FFill.Rounding, M, D);
    ProgressCircle.FFill.Rounding := MulDiv(ProgressCircle.FFill.Rounding, M, D);
    ProgressCircle.FBackGroundFill.Rounding := MulDiv(ProgressCircle.FBackGroundFill.Rounding, M, D);
  end;

  Digits.Font.OnChange := nil;

  Digits.Font.Height := MulDiv(Digits.Font.Height, M, D);

  Digits.Font.OnChange := Digits.FontChanged;

  ProgressCircle.StepFont.Height := MulDiv(ProgressCircle.StepFont.Height, M, D);
end;
...
begin
  ...
  calcFont.Size := Digits.Font.Size;
  calcFont.Height := FLoadFontHeight;
  ...
  begin
    ...
    pth.AddString(ADigit, Length(ADigit), f, GetFontStyle(Digits.Font.Style), calcFont.Size * DPIScale, rs, sf);
    g.FillPath(b, pth);
    b.Free;
  end;

  pth.Reset;
  pth.AddString(ADigit, Length(ADigit), f, GetFontStyle(Digits.Font.Style), calcFont.Size * DPIScale, r, sf);
...
procedure TAdvSmoothCircularProgress.Loaded;
begin
  inherited;
  FLoadFontHeight := Digits.Font.Height;
end;
...
begin
  (FOwner as TAdvSmoothCircularProgress).FLoadFontHeight := FFont.Height;
