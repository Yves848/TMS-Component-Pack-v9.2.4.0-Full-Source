...
uses
  Windows, Classes, Graphics, Forms, Controls, Messages, ImgList, Math, Types, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.7.2.0 : Improved : High DPI handling
  // 1.7.3.0 : New : Support for multimonitor High DPI handling
  // 1.7.3.1 : Fixed : Issue with DPI scale initialization
...
type
  ...
  private
    ...
    procedure DrawCloseButton(Canvas: TCanvas; ARect: TRect; Active: Boolean);
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomTabSet.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
end;
{$ELSE}
procedure TAdvCustomTabSet.ChangeScale(M, D: Integer);
begin
  inherited;
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if ClientHeight < FTabHeight + 5 then
      MemBitmap.Height := Round((FTabHeight + 5) * FDPIScale)
    else
    ...
    begin
      LastTabPos := LastTabPos - GetCloseButtonWidth;     { tabs draw until this position }
      //Scroller.Left := Scroller.Left - CloseButtonWidth;
      Scroller.Left := Width - Scroller.Width - 2- GetCloseButtonWidth;
    end
    else
      Scroller.Left := Width - Scroller.Width - 2;
    Scroller.Top:= 3;
  ...
  begin
    ...
    if ClientWidth  < FTabHeight + 5 then
      MemBitmap.Width  := Round((FTabHeight + 5) * FDPIScale)
    ...
    if FCloseButtonAt = cbTabSet then
    begin
      LastTabPos := LastTabPos - GetCloseButtonHeight - 2;
      Scroller.Top := Height - Scroller.Height - 2 - GetCloseButtonHeight;
    end
    else
  ...
  begin
    ...
    { set up the canvas }
    if TabPosition in [pTop, pBottom] then
      R := Rect(TabPos.StartPos, 0, TabPos.StartPos + TabPos.Size, Round(FTabHeight * FDPIScale))
    else // if TabPosition in [pLeft, pRight] then
      R := Rect(0,TabPos.StartPos, Round(FTabHeight * FDPIScale), TabPos.StartPos + TabPos.Size);

    { restore font for drawing the text }
    MemBitmap.Canvas.Font.Assign(Canvas.Font);

    //Selecting paiting color and gradient direction to start paiting
    //HorizontalGradient:= FGradientDirection = gdHorizontal;
    HorizontalGradient := FAdvTabs.Items[RealItemIndex].TabGradientDirection <> gdVertical;
    ...
    begin
      ...
      begin
        ...
        HorizontalGradient := FAdvTabs.Items[RealItemIndex].HoverGradientDirection <> gdVertical;
        ShowHoverTab := true;
        HoverTabRect := R;
      end
      else
...
//    RealItemIndex := integer(FDuplicateTabs.Objects[Tab + FirstIndex]);
      ...
      begin
        ...
        begin
          if (ClosePosition = cpLeft) then
            TextR.Left:= TextR.Left + GetCloseButtonWidth + {2}3
          else
            TextR.Right := TextR.Right - GetCloseButtonWidth - 3;
        end
        else
        begin
          if TabPosition = pLeft then
          begin
            if (ClosePosition = cpLeft) then
              TextR.Bottom := TextR.Bottom - GetCloseButtonHeight -3{+ 3}
            else
              TextR.Top := TextR.Top + GetCloseButtonHeight + 3
          end
          else
          begin
            if (ClosePosition = cpLeft) then
              ...
              TextR.Bottom := TextR.Bottom - GetCloseButtonHeight - 3;
          end;
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        MemBitmap.Canvas.Font.Height := Round(MemBitmap.Canvas.Font.Height * FDPIScale);
      end
      else
...
begin
  ...
  try
    ...
    Working.Height := Round(FTabHeight * FDPIScale);
    MaskColor := clOlive;
    ...
    begin
      ...
      begin
        ...
        Brush.Style := bsSolid;
        FillRect(Rect(0, 0, EdgeWidth, Round(FTabHeight * FDPIScale)));
      end;
      case EdgePart of
    ...
    begin
      ...
      begin
        if FCloseButtonAt = cbTabs then
          ...
          Dec(Result, Scroller.Width + 4 + FTabOverlap + GetCloseButtonWidth);
      end
      else
      begin
        if FCloseButtonAt = cbTabs then
          ...
          Dec(Result, Scroller.Height + 4 + FTabOverlap + GetCloseButtonHeight);
      end;
    end;
  end;
...
function TAdvCustomTabSet.GetCloseButtonHeight: integer;
begin
  Result := Round(FDPIScale * CloseButtonHeight);
end;

function TAdvCustomTabSet.GetCloseButtonWidth: integer;
begin
  Result := Round(FDPIScale * CloseButtonWidth);
end;
...
begin
  ...
  begin
    ...
    Font.Assign(ActiveFont);
    Font.Height := Round(FDPIScale * Font.Height);
...
//------------------------------------------------------------------------------
procedure TAdvCustomTabSet.DrawCloseButton(Canvas: TCanvas; ARect: TRect; Active: Boolean);
var
  a,pw: integer;
  P: TPoint;
  btnR: TRect;
  ...
  begin
    ...
    begin
      ...
      begin
        Brush.Color := clWhite;
        Pen.Color := clGray;
        btnR := Rect(ARect.Left + 2, ARect.Bottom - GetCloseButtonHeight, ARect.Left + GetCloseButtonWidth, ARect.Bottom - 2);
...
//------------------------------------------------------------------------------
procedure TAdvCustomTabSet.DrawHoverCloseButton(ARect: TRect);
var
  p: TPoint;
  pw: integer;
  btnR: TRect;
begin
  ...
  begin
    case TabPosition of
      ...
      begin
        if (ClosePosition = cpLeft) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + FLowerSelected{2} {Active};
          ARect.Bottom := ARect.Bottom - FTabMargin.TopMargin;
          if not FTabBackGroundSelected.Empty then
            ARect.Bottom := ARect.Bottom + TabOverlap;
        end
      ...
      begin
        if (ClosePosition = cpLeft) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin -FLowerSelected+2;
          ARect.Top := ARect.Top + FTabMargin.TopMargin;
        end
        else
        begin
      ...
      begin
        ...
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + {2}FLowerSelected;
        end
        else
        begin
          ARect.Left := ARect.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
  ...
  begin
    ...
    begin
      ...
      begin
        ARect.Left := ARect.Left + FTabMargin.LeftMargin + FLowerSelected;
        ARect.Bottom := ARect.Top + FTabMargin.RightMargin + GetCloseButtonHeight;
      end;
      if not FTabBackGroundSelected.Empty then
        ARect.Bottom := ARect.Bottom + TabOverlap;
    ...
    begin
      ...
      begin
        ...
        begin
          ARect.Left := ARect.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + FLowerSelected;
        end;
      {end
      else // Bottom
      begin
...
//------------------------------------------------------------------------------
procedure TAdvCustomTabSet.DrawDownCloseButton(ARect: TRect);
var
  p: TPoint;
  btnR: TRect;
  pw: integer;
 ...
 // Rect.Top := Rect.Top + FTabMargin.TopMargin + 2;
  ...
  begin
    ...
    begin
      ...
      if not FTabBackGroundSelected.Empty then ARect.Bottom := ARect.Bottom + TabOverlap;
...
begin
  ...
  begin
    ...
    begin
      ...
      r.Left := r.Left + FTabMargin.LeftMargin;
      r := Rect(r.Left + 2, r.Top, r.Left + GetCloseButtonWidth, r.Top + GetCloseButtonHeight);
    end;
    if PtInRect(r, Point(X, Y)) then
    begin
      Result := true;
...
begin
  ...
  if TabPosition in [pTop, pBottom] then
    aRect:= Rect(Width - GetCloseButtonWidth-1,3,Width-2, GetCloseButtonHeight+3 )
  else
    aRect:= Rect(3, Height - GetCloseButtonHeight, GetCloseButtonWidth+2, Height);
...
begin
  if TabPosition in [pTop, pBottom] then
    aRect:= Rect(Width - GetCloseButtonWidth,1,Width, GetCloseButtonHeight + 2)
  else
    aRect:= Rect(4, Height - GetCloseButtonHeight+2, GetCloseButtonWidth+1, Height-1);
...
begin
  if TabPosition in [pTop, pBottom] then
    aRect:= Rect(Width - GetCloseButtonWidth-1,3,Width-3, GetCloseButtonHeight+2 )
  else
    aRect:= Rect(3, Height - GetCloseButtonHeight, GetCloseButtonWidth+1, Height-1);
