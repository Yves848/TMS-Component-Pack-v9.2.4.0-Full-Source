...
const
  ...
  // v1.0.5.0 : Improved : Handling of queries with ORDER BY clause
  // v1.0.5.1 : Fixed : Issue with non-equal operator
  // v1.0.6.0 : New : SQLDateSeparator property added
  // v1.0.7.0 : New : public property FilterRowCount & KeepLastFilter added
  // v1.0.7.1 : Improved : Test added for SQL property type on dataset
  // v1.0.8.0 : New : SQLDateFormat property added to control format of dates in SQL WHERE condition
...
type
  ...
  public
    ...
    procedure FilterOperationsToPanel; virtual;
    property FilterRowCount: integer read GetFilterRowCount;
    property KeepLastFilter: boolean read FKeepLastFilter write FKeepLastFilter;
  ...
  published
    property SQLDateSeparator: string read FSQLDateSeparator write FSQLDateSeparator;
    property SQLDateFormat: string read FSQLDateFormat write FSQLDateFormat;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (UPos('WHERE', FQueryString, P) > 0) and Assigned(SQLObj) then
        begin
...
procedure TAdvDBFilterPanel.InitFilter;
begin
  AddClear(Self);
end;

procedure TAdvDBFilterPanel.FilterOperationsToPanel;
var
  I: integer;
  //grp: TAdvFilterGroupBox;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // ensure ORDER BY is as last statement
          if UPos('ORDER BY', FQueryString, VP) > 0 then
          begin
            OrderBy := ' ' + Copy(FQueryString, VP, Length(FQueryString));
            FQueryString := Copy(FQueryString, 0, VP - 1);
          end;

          if Con <> '' then
            FQueryString := FQueryString + ' WHERE ' + Con + OrderBy;
...
function TAdvDBFilterPanel.GetFilterRowCount: integer;
begin
  Result := CustomControl.ControlCount;
end;
...
begin
  ...
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).SQLDateSeparator := FSQLDateSeparator;
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).SQLDateFormat := FSQLDateFormat;
...
procedure TFilterOperations.LoadFromFile(const AFilename: string);
var
  s: TStringStream;
begin
...
procedure TFilterOperations.LoadFromStream(AStream: TStream);
var
  sl: TStringList;
  i: integer;
begin
...
procedure TFilterOperations.SaveToFile(const AFilename: string);
var
  s: TStringStream;
begin
  s := TStringStream.Create;
...
procedure TFilterOperations.SaveToStream(AStream: TStream);
var
  sl: TStringList;
  i: integer;
begin
...
{ TFilterOperationItem }

function TFilterOperationItem.GetAsString: string;
var
  opr: integer;
begin
  opr := integer(Operation);
  Result := Action + '|' + BoolToStr(CaseSensitive) + '|' + Column + '|' + IntToStr(opr) + '|' + Symbol + '|"' + Value+'"';
end;

procedure TFilterOperationItem.SetAsString(const Value: string);
var
  sl: TStringList;
  i: integer;
  s: string;
begin
  ...
  begin
    Action := sl.Strings[0];
    CaseSensitive := StrToBool(sl.Strings[1]);
    Column := sl.Strings[2];
    TryStrToInt(sl.Strings[3], i);
    Operation := TFilterOperation(i);
    Symbol := sl.Strings[4];
    s := sl.Strings[5];
    FValue := Copy(s, 2, Length(s) - 2);
  end
  else
    raise Exception.Create('Could not parse filter');
end;
