...
const
  ...
  // 1.6.2.2 : Improved : High DPI handling with Form.Scaled = false
  // 1.6.2.3 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // 1.6.3.0 : New : Exposed Tag property in TAdvOfficeStatusPanel
  // 1.6.4.0 : Improvements for per monitor high DPI
  // 1.7.0.0 : New : VCL Styles support
...
begin
  ...
  begin
    ...
    Tag := TAdvOfficeStatusPanel(Source).Tag;
  end
  else
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, SimpleText, dr, fImages, 0, 0, -1, -1, 1, true, false, false, false, (FTimerCount > 5), URLStyle = usNone,
      false, FFormScaled, 1.0, URLColor, clNone, clNone, clGray, anchor, stripped, focusanchor, xsize, ysize,
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        R.Right := R.Left + Round(Panels[I].Width * LScale);
        PanelPos := PanelPos + Round(Panels[I].Width * LScale);
      end;
    end;
...
begin
  ...
  begin
    if i <> FStretchPanel then
      Result := Result + Round(Panels[i].Width * LScale);
  end;
end;
...
begin
  ...
  if HTMLDrawEx(Canvas, AText, r, FImages, x, y, -1, -1, 1, true, false, false, true, true, false, true, FFormScaled,
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvOfficeStatusBar.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
{$ENDIF}
  ...
  if Assigned(frm) and (frm is TForm) and not (frm as TForm).Scaled then
  begin
    UseSystemFont := false;
    Font := (frm as TForm).Font;
  end;
