...
type
  ...
  private
    FAdvMemoStylers: TAdvCustomMemoStylerManager;
  ...
  public
    constructor Create(Styler: TAdvCustomMemoStylerManager);
  ...
  published
    property DefaultStylerIndex: integer read FDefaultStylerIndex write SetDefaultStylerIndex default -1;
...
constructor TAdvMemoStylersCollection.Create(Styler: TAdvCustomMemoStylerManager);
begin
...
begin
  ...
  TAdvMemoStylerManager(Self.Owner).FItemsCount := TAdvMemoStylerManager(Self.Owner).FItemsCount + 1;
end;
...
var
  i: Integer;
begin
  for i := 0 to TAdvMemoStylerManager(FItems.FAdvMemoStylers).FItems.Count - 1 do
    LoadStylerFromFile(TAdvMemoStylerManager(FItems.FAdvMemoStylers).FItems[I].FStyler,FileName);
end;
...
begin
  ...
  for I := 0 to TAdvMemoStylerManager(FItems.FAdvMemoStylers).FItems.Count - 1 do
  begin
    SaveStylerToFile(TAdvMemoStylerManager(FItems.FAdvMemoStylers).FItems[I].FStyler,FileName);
  end;
end;
...
procedure TAdvMemoStylerManager.SetDefaultStylerIndex(const Value: integer);
begin
  if (csLoading in ComponentState) or ((Value >= -1) and (Value <= Items.Count - 1)) then
    FDefaultStylerIndex := Value;
end;
...
begin
  ...
  begin
     ...
     begin
        SExt := FItems.Items[i].Styler.Extensions + ';';
        if pos(Ext, SExt) > 0 then
        begin
...
function TAdvMemoStylerManager.GetItemsCount: integer;
begin
  Result := FItems.Count;
end;
