...
type
  ...
  procedure DrawBitmapTransp(Canvas:TCanvas;bmp:TBitmap;bkcolor: TColor;r: TRect; scale: single);
  procedure DrawBitmapResourceTransp(Canvas: TCanvas; bkColor: TColor; r: TRect; ResName:string; scale: single);
  ...
  function IsType(s: string; ADecimalSeparator, AThousandSeparator: Char): TAutoType; overload;
  ...
  function GetUnquotedStr(S: string): string;
  ...
  procedure DrawProgressLin(Canvas: TCanvas; r: TRect; Color1,TColor1,Color2,TColor2: TColor; pd: double; Mx,My,Min,Max:Integer; Scale: double; Fmt: string; BorderColor: TColor; print: boolean);
  ...
  procedure DrawBitmapAlpha(ACanvas: TCanvas; ALeft, ATop: Integer; ABitmap: TBitmap);
...
uses
  AdvXPVS, Messages, Stdctrls, Forms, Math;
...
var
  dest_sz,i: integer;
  dest: array[0..2047] of widechar;
begin
  dest_sz := Min(2047, foldstring(MAP_COMPOSITE,PChar(s),Length(s), @dest, sizeof(dest) ));
...
begin
  ...
  begin
    ...
    if (s[i] in Charset) and not odd(q) and not odd(r) then
    begin
      spos := i;
      Result := s[i];
      Break;
...
begin
  ...
  begin
    ...
    if (InArray(s[i])) and not odd(q) and not odd(r) then
    begin
      spos := i;
      Result := s[i];
      Break;
...
begin
  Result := IsNumChar(ch) or (ch = FormatSettings.ThousandSeparator) or (ch = FormatSettings.DecimalSeparator) or (ch = 'e') or (ch = 'E');
end;

function CheckFloatNumNoExp(ch: char): boolean;
begin
  Result := IsNumChar(ch) or (ch = FormatSettings.ThousandSeparator) or (ch = FormatSettings.DecimalSeparator) or (ch = '-') or (ch = #8) or (ch = #9) or (ch = #27) or (ch = #13);
end;
...
begin
  Result := IsType(s, FormatSettings.DecimalSeparator, FormatSettings.ThousandSeparator);
end;

function IsType(s: string; ADecimalSeparator, AThousandSeparator: Char): TAutoType;
var
  i: Integer;
...
begin
  ...
  isS := True;
  isE := False;
  isD := Pos(FormatSettings.DateSeparator,s) > 0;
  isT := Pos(FormatSettings.TimeSeparator,s) > 0;
  ...
  begin
    ...
    if not (CheckNum(s[i]) or (s[i] = '-') or (s[i] = AThousandSeparator) or (s[i] = ADecimalSeparator) ) then
      isF := False;

    if not (CheckNum(s[i]) or (s[i] = FormatSettings.DateSeparator)) then
      isD := False;

    if not (CheckNum(s[i]) or (s[i] = FormatSettings.TimeSeparator)) then
      isT := False;

    if not (CheckSignedFloatNum(s[i]) and (s[i] <> AThousandSeparator)) then
      isS := False;

    if (s[i] = AThousandSeparator) and (i - th < 4) and (th <> -1) then
      isF := False;

    if (s[i] = AThousandSeparator) then
      th := i;
    ...
    if (s[i] = FormatSettings.TimeSeparator) then
    begin
      inc(ts);
...
begin
  ...
  if VarPos(FormatSettings.DateSeparator,s,vp)>0 then
  begin
    su := Copy(s,1,vp - 1);
...
begin
  ...
  try
    ...
    DrawBitmapTransp(Canvas, bmp, bkColor, r, Scale);
  finally
    bmp.Free;
  end;
end;
...
begin
  ...
  TmpBmp.Height := Round(bmp.Height * scale);
  TmpBmp.Width := Round(bmp.Width * scale);
  tgtrect.left :=0;
  tgtrect.top :=0;
  tgtrect.right := Round(bmp.Width * scale);
  tgtrect.bottom := Round(bmp.Height * scale);
  r.bottom := r.top + Round(bmp.Height * scale);
  r.Right := r.Left + Round(bmp.Width * scale);
  TmpBmp.Canvas.Brush.Color := bkcolor;
  srcColor := bmp.canvas.pixels[0,0];
  if scale = 1 then
    TmpBmp.Canvas.BrushCopy(tgtrect,bmp,tgtrect,srcColor)
  else
    TmpBmp.Canvas.StretchDraw(tgtrect, bmp);
...
var
  ...
  pt: Array of TPoint;
begin
  ARect.Left := ARect.Right - Round(16 * DPIScale);
...
begin
  {$IFDEF DELPHIXE_LVL}
  if (ABitmap.PixelFormat = pf32bit) And (ABitmap.AlphaFormat = afPremultiplied) then
  begin
    ...
    AlphaBlend(ACanvas.Handle, ALeft, ATop, ABitmap.Width, ABitmap.Height, ABitmap.Canvas.Handle,
      0, 0, ABitmap.Width, ABitmap.Height, LBlendFunc);
  end
  else
  {$ENDIF}
...
begin
  Result := false;
  if VarPos(FormatSettings.DateSeparator, s,vp) > 1 then
  begin
    delete(s,1,vp);
    Result := Pos(FormatSettings.DateSeparator,s) > 1;
  end;
end;
