...
const
  ...
  // 1.2.0.3 : Fixed : Issue with use of Appearance.Color, Appearance.ColorTo
  // 1.2.1.0 : Improved : Support for High DPI
...
begin
  ...
  Y := X;
  W := Round((ShapeWidth * FDPIScale) - Appearance.Pen.Width + 1);
  H := Round((ShapeHeight * FDPIScale) - Appearance.Pen.Width + 1);
...
begin
  ...
  begin
    ...
    FDPIScale := GetDPIScale(Self, Canvas);
    txtXCalc := Round(FDPIScale * TextOffsetX);
    txtYCalc := Round(FDPIScale * TextOffsetY);

    TextR := Rect(R.Left + txtXCalc, R.Top + txtYCalc, R.Right, R.Bottom);
    ...
    HTMLDrawEx(Canvas, Text, TextR, FImages, 0, 0, -1, -1, 1, false, false, false, false, false, false,
      true, true, 1.0, Appearance.URLColor, clNone, clNone, clGray, anchor, stripped, focusanchor, xsize, ysize,
      hyperlinks, mouselink, re, nil , FContainer, 0, BidiMode, clNone, clNone, [], FDPIScale);
...
begin
  ...
  R := Rect(R.Left + txtXCalc, R.Top + txtYCalc, R.Right, R.Bottom);

  InflateRect(R, -2, -2);

  if HTMLDrawEx(Canvas, AText, R, FImages, x, y, -1, -1, 1, true, false, false, true, true, false, true, true,
