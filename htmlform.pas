...
const
  ...
  // v1.4.0.0 : Improved : High DPI handling
  // v1.4.0.1 : Fixed : Issue with <IND> tag in high DPI mode
  // v1.4.0.2 : Fixed : Issue with control mask parsing
  ...
  try
    ...
    begin
      ...
      HTMLDrawEx(ACanvas,s,r,FImages,pt.x,pt.y,fhl,FHoverHyperlink,FShadowOffset,True,True,False,False,FBlinking,FHover,not FEllipsis,FCtrlDown,'',1.0,
        FURLColor,FHoverColor,FHoverFontColor,FShadowColor,Anchor,Stripped,FocusAnchor,AnchorHint,x,y,hl,ml,HR,CtrlRect,CID,CV,CT,FImageCache,FContainer,Handle,BidiMode);
    ...
    begin
      ...
      HTMLDrawEx(ACanvas,s,r,nil,0,0,fhl,FHoverHyperlink,FShadowOffset,False,False,false,False,FBlinking,FHover,not FEllipsis,FCtrlDown,'',1.0,
        clWhite,clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,AnchorHint,x,y,hl,ml,HR,CtrlRect,CID,CV,CT,FImageCache,FContainer,Handle,BidiMode);
      ...
      HTMLDrawEx(ACanvas,s,r,FImages,0,0,fhl,FHoverHyperlink,FShadowOffset,False,False,false,False,FBlinking,FHover,not FEllipsis,FCtrlDown,'',1.0,
        clGray,clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,AnchorHint,x,y,hl,ml,HR,CtrlRect,CID,CV,CT,FImageCache,FContainer,Handle,BidiMode);
     end
    else
...
begin
  ...
  begin
    HTMLDrawEx(Canvas,s,r,FImages,x,y,-1,FHoverHyperlink,FShadowOffset,True,False,False,False,FBlinking,FHover,not FEllipsis,FCtrlDown,'',1.0,
        FURLColor,FHoverColor,FHoverFontColor,FShadowColor,Anchor,Stripped,FocusAnchor,AnchorHint,xsize,ysize,hl,FHoverHyperlink,HoverRect,CtrlRect,CID,CV,CT,FImageCache,FContainer,Handle,BidiMode);
  ...
  if HTMLDrawEx(Canvas,s,r,FImages,x,y,-1,-1,FShadowOffset,True,False,False,False,false,FHover,not FEllipsis,FCtrlDown,'',1.0,
    clWhite,clNone,clNone,clNone,Anchor,Stripped,FocusAnchor, AnchorHint,xsize,ysize,hl,FHoverHyperlink,HoverRect,CtrlRect,CID,CV,CT,FImageCache,FContainer,Handle,BidiMode) then
  begin
    Result := Anchor;
  end
  else
...
begin
  ...
  begin
    ...
    if (CT = 'MASK') then
    begin
      m := GetControlProp(s,CID);
      m := StringReplace(m, '&gt;','>', [rfReplaceAll]);
      m := StringReplace(m, '&lt;','>', [rfReplaceAll]);
      m := UnFixMarkup(m);
      FEditControl.EditMask := m;
    end
    else
    ...
    FEditControl.Height := CR.Bottom - CR.Top - Round(6 * scale);
  ...
  begin
    ...
    FComboControl.Top := CR.Top + Round(6 * scale);
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLForm.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
procedure THTMLForm.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF}
