...
const
  ...
  // v1.5.0.1 : Fixed : Issue with transparency on double buffered forms
...
begin
  ...
  f := TGPFont.Create(ff, Font.Size * DPIScale, fs, UnitPoint);
  ...
  begin
    ...
    case Direction of
      ...
      begin
        ...
        if ShowTickMarks and altickleft then
          g.DrawLine(pleft, numr.X, pos, numr.X + Round(DPIScale * TickMarkSize), pos);

        if ShowTickMarks and altickright then
          g.DrawLine(pright, numr.X + numr.Width - Round(DPIScale * TickMarkSize), pos, numr.X + numr.Width, pos);
      end;
      dHorizontal:
      begin
        pt := MakePoint(numr.X + numr.Width - pos + (DPIScale * ThumbSize / 2) - (sizer.Width / 2), (numr.Height - sizer.Height) / 2);
        //draw tickmarks
        if ShowTickMarks and altickleft then
          g.DrawLine(pleft, pos, numr.Y , pos, numr.Y + Round(DPIScale * TickMarkSize));
        if ShowTickMarks and altickright then
          g.DrawLine(pright, pos, numr.Y + numr.Height - Round(DPIScale * TickMarkSize), pos, numr.Y + numr.Height);
      end;
    end;
    ...
    begin
      ...
      case Direction of
        ...
        begin
          pty := pt.Y;
          y := InsideRect.Bottom - GetPosition - (DPIScale * ThumbSize / 2) - (sizer.Height / 2);
          if (pty < y) then
        ...
        begin
          pty := pt.X;
          y := GetPosition + (DPIScale * ThumbSize / 2) - (sizer.Width / 2);
          if (pty > y) then
            b := TGPSolidBrush.Create(MakeColor(255, FFont.Color))
          else
            b := TGPSolidBrush.Create(MakeColor(255, FProgressFont.Color));
...
begin
  ...
  DPIScale := GetDPIScale(Self, Canvas);
  calcIndent := Round(DPIScale * 7);
  toplr := MakeRect(InsideRect.Left, InsideRect.Top, InsideRect.Right, InsideRect.Bottom);
  olr := MakeRect(InsideRect.Left + calcIndent, InsideRect.Top + calcIndent, InsideRect.Right - calcIndent * 2, InsideRect.Bottom - calcIndent * 2);
  path := CreateRoundRectangle(olr, calcIndent, rtBoth);
  ...
  path := CreateRoundRectangle(Makerect(olr.X, olr.Y, olr.Width, olr.Height), Round(DPIScale * 10), rtBoth);
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  calcLine := Round(DPIScale * 30);
  case Direction of
    dVertical:
    begin
      o := MakeRect(InsideRect.Left, InsideRect.Bottom - GetPosition - (DPIScale * ThumbSize / 2), InsideRect.Right, GetPosition + (DPIScale * ThumbSize / 2));
      FProgressFill.Fill(g, o);
      if IsGlowAnimation then
      begin
        rgn := TGPRegion.Create(o);
        g.SetClip(rgn);
        lr := MakeRect(o.X, InsideRect.Bottom - FGlowPos - calcLine, o.Width, calcLine);
        rr := MakeRect(o.X, InsideRect.Bottom - FGlowPos, o.Width, calcLine);
    ...
    dHorizontal:
    begin
      o := MakeRect(InsideRect.Left, InsideRect.Top, GetPosition + (DPIScale * ThumbSize / 2), InsideRect.Bottom);
      FProgressFill.Fill(g, o);
      if IsGlowAnimation then
      begin
        rgn := TGPRegion.Create(o);
        g.SetClip(rgn);
        lr := MakeRect(FGlowPos - calcLine, o.Y, calcLine, o.Height);
        rr := MakeRect(FGlowPos, o.Y, calcLine, o.Height);
...
begin
  ...
  DPIScale := GetDPIScale(Self, Canvas);
  FThumbFill.BeginUpdate;
  FThumbFill.Rounding := Round(FThumbFill.Rounding * DPIScale);
  case Direction of
    dVertical:
    begin
      slr := GetSliderRect;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  case Direction of
    dVertical: Result := MakeRect(InsideRect.Left, InsideRect.Top + (DPIScale * ThumbSize / 2), InsideRect.Right, InsideRect.Bottom - DPIScale * ThumbSize);
    dHorizontal: Result := MakeRect(InsideRect.Left + (DPIScale * ThumbSize / 2), InsideRect.Top, InsideRect.Right - DPIScale * ThumbSize, InsideRect.Bottom);
  end;
end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  case Direction of
    dVertical: Result := MakeRect(InsideRect.Left + 7, GetNumRect.Y + GetNumRect.Height - GetPosition - (DPIScale * ThumbSize / 2), InsideRect.Right - 14, DPIScale * ThumbSize);
    dHorizontal: Result := MakeRect(GetNumRect.X + GetPosition - (DPIScale * ThumbSize / 2), InsideRect.Top + 7, DPIScale * ThumbSize, InsideRect.Bottom - 14);
  end;
end;
...
procedure TAdvSmoothTrackBar.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
...
procedure TAdvSmoothTrackBar.CMStyleChanged(var Message: TMessage);
begin
  inherited;
