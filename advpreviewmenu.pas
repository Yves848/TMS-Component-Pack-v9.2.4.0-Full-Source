...
{$ENDIF}
  ...
  // v6.0.2.0 : Improved : High DPI handling
  // v6.0.2.1 : Improved : Further High DPI improvements
  // v6.0.2.2 : Improved : Further High DPI improvements for standalone AdvPreviewMenu use
...
{ TItemActionLink }
  ...
  protected
    ...
    function GetSubMenuItemHeightDPI: Integer;
  ...
  private
    ...
    FDPIScale: single;
    FMenuSpacingDPI: Integer;
    FLineSpace: Integer;
    FLineSpaceDPI: Integer;
    FScrollHeightDPI: Integer;
  ...
  protected
    ...
    property MenuSpacingDPI: integer read FMenuSpacingDPI write FMenuSpacingDPI;
    property LineSpaceDPI: Integer read FLineSpaceDPI write FLineSpaceDPI;
    property ScrollHeightDPI: Integer read FScrollHeightDPI write FScrollHeightDPI;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    function GetVersionNr: integer;
    procedure SetDPIScale(FDPI: Integer);
...
//------------------------------------------------------------------------------
function DrawVistaText(Canvas: TCanvas; Alignment: TAlignment; VAlign: TVAlign; DTSTYLE: DWORD; r: TRect; Caption:string; WideCaption: widestring; AFont: TFont; iDPIScale: single; Enabled: Boolean; RealDraw: Boolean; AntiAlias: TAntiAlias): TRect;
var
...
begin
  ...
  begin
    ...
    font := TGPFont.Create(fontFamily, Round(AFont.Size * iDPIScale) , fs, UnitPoint);
...
procedure DrawRightArrow(Canvas: TCanvas; X, Y, S: Integer; iDPIScale: Single; Clr: TColor);
var
  i: Integer;
begin
  if not Assigned(Canvas) then
...
procedure TAdvPreviewMenu.SetDPIScale(fDPI: Integer);
begin
  FDPIScale := fDPI/96;
  FMenuSpacingDPI := Round(FMenuSpacing * DPIScale);
  FLineSpaceDPI := Round(FLineSpace * DPIScale);
  FScrollHeightDPI := Round(SCROLL_HEIGHT * DPIScale);
end;
...
begin
  Result := Round((ButtonHeight  + 2  * 2) * DPIScale) + 2;
end;
...
procedure TPreviewMenuWindow.WMDPICHANGED(var Message: TMessage);             
begin                                                                           
  Message.Result := 0; // dont process this message - we calculate by our own   
end;                                                                            
...
begin
  ...
  FItemDown := -1;
  FSubMenuItem := -1;
  FSubMenuItemHeightDPI := MINITEM_HEIGHT;
  FSubMenuItemHot := -1;
  FSubMenuItemDown := -1;
...
begin
  ...
  begin
    if PtInRect(Bounds(0, 1, Round(55 * AdvPreviewMenu.DPIScale), AdvPreviewMenu.GetTopFrameHeight - 2), Point(X, Y)) then
    begin
      HideMenuWindow;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    if (d > 50)
    {$IFDEF DELPHIXE_LVL}
      or (ssTouch in Shift) or (ssPen in Shift)
    {$ENDIF}
    then
    begin
      SelectSubMenuItem(i);
...
//    R2.Bottom := R2.Bottom - 1;
    ...
    if IsWin7 then
    begin
      DrawWin7Button(Canvas, MakeRect(0, 1, Round(55 * AdvPreviewMenu.DPIScale), FAdvPreviewMenu.GetTopFrameHeight - 3), FAdvPreviewMenu.FMenuButtonBorderColor , FAdvPreviewMenu.FMenuButtonInnerBorderColor,
      ...
      if IsGlass then
      begin
        DrawGDIPText(Canvas, nil, taCenter, Bounds(0, 1, Round(55 * AdvPreviewMenu.DPIScale), FAdvPreviewMenu.GetTopFrameHeight - 3), FAdvPreviewMenu.FMenuText, FAdvPreviewMenu.FMenuText, FAdvPreviewMenu.FMenuFont, true, true, aaClearType);
      end;
...
begin
  ...
  begin
    ...
    begin
      //------ Draw Image
      R2 := Rect(R.Left + AdvPreviewMenu.MenuSpacingDPI, R.Top, R.Right - AdvPreviewMenu.MenuSpacingDPI, R.Bottom);
      ...
      begin
        ...
        DrawVistaText(Canvas, taLeftjustify, tvaCenter, DT_SINGLELINE or DT_VCENTER or DT_LEFT, R2, AdvPreviewMenu.MenuItems.Items[Index].Caption, '', Canvas.Font, AdvPreviewMenu.DPIScale, True, True, AdvPreviewMenu.AntiAlias);
        (*
      ...
      begin
        Canvas.Font.Assign(MenuItemAppearance.Font);
        Canvas.Font.Color := TxtClr;
        DrawVistaText(Canvas, taLeftjustify, tvaCenter, DT_SINGLELINE or DT_VCENTER or DT_LEFT, R2, '', AdvPreviewMenu.MenuItems.Items[Index].WideCaption, Canvas.Font, AdvPreviewMenu.DPIScale, True, True, AdvPreviewMenu.AntiAlias);
        (*  
    ...
    if (AdvPreviewMenu.MenuItems.Items[Index].SubItems.Count > 0) then
      DrawRightArrow(Canvas, R.Right - MENUARROW_WIDTH + ((MENUARROW_WIDTH - 6) div 2), R.Top + (R.Bottom - R.Top - 6) div 2, 6, AdvPreviewMenu.DPIScale, Canvas.Font.Color);
...
begin
  ...
  begin
    ...
    SubMenuItemSpacing := Round(FAdvPreviewMenu.MenuItems.Items[FSubMenuItem].SubmenuItemSpacing * AdvPreviewMenu.DPIScale);
  ...
  begin
    ...
    SubMenuItemSpacing := Round(FAdvPreviewMenu.SubMenuItemSpacing * AdvPreviewMenu.DPIScale);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          VAlign := tvaTop;
          R2.Top := R2.Top + 2 * AdvPreviewMenu.LineSpaceDPI;
        end;
        if (SubItems.Items[AIndex].Title <> '') then
        begin
          DrawVistaText(ACanvas, taLeftjustify, VAlign, DTSTYLE, R2, SubItems.Items[AIndex].Title, '', ACanvas.Font, AdvPreviewMenu.DPIScale, True, True, FAdvPreviewMenu.AntiAlias);
          (*
          DrawText(ACanvas.Handle, PChar(SubItems.Items[AIndex].Title), -1, R2, DTSTYLE);
          *)
          R3 := Rect(0,0,1000,100);
          ACanvas.Font.Size := Round(ACanvas.Font.Size * AdvPreviewMenu.DPIScale);
          ...
          R2.Top := R2.Top + R3.Bottom + AdvPreviewMenu.LineSpaceDPI;
        end
        else
        begin
          DrawVistaText(ACanvas, taLeftjustify, VAlign, DTSTYLE, R2, '', SubItems.Items[AIndex].WideTitle, ACanvas.Font, AdvPreviewMenu.DPIScale, True, True, FAdvPreviewMenu.AntiAlias);
          (*
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrawVistaText(ACanvas, taLeftjustify, tvaTop, DT_SINGLELINE or DT_TOP or DT_LEFT, R2, SubItems.Items[AIndex].Notes[i], '', ACanvas.Font, AdvPreviewMenu.DPIScale, True, True, FAdvPreviewMenu.AntiAlias);
            (*
      ...
      begin
        ACanvas.Font.Assign(MenuItemAppearance.SubItemFont);
        ACanvas.Font.Color := TxtClr;
        DrawVistaText(ACanvas, taLeftjustify, tvaTop, {DT_WORDBREAK or }DT_TOP or DT_LEFT, R2, '', SubItems.Items[AIndex].WideNotes, ACanvas.Font, AdvPreviewMenu.DPIScale, True, True, FAdvPreviewMenu.AntiAlias);
        (*
    ...
    if Assigned(SubItems.Items[AIndex].SubMenu) then
      DrawRightArrow(ACanvas, R.Right - MENUARROW_WIDTH + 4, R.Top + (R.Bottom - R.Top - 6) div 2, 6, AdvPreviewMenu.DPIScale, ACanvas.Font.Color);
...
begin
  ...
  if (AdvPreviewMenu.SubMenuWidth > 0) then
    rw := Round(AdvPreviewMenu.SubMenuWidth * AdvPreviewMenu.DPIScale);
  ...
  begin
    ...
    begin
      ...
      AdvGlowBtn.Width := Round(AdvPreviewMenu.Buttons.Items[i].Width * AdvPreviewMenu.DPIScale);
      AdvGlowBtn.Height := Round(AdvPreviewMenu.ButtonHeight * AdvPreviewMenu.DPIScale);
      ...
      AdvGlowBtn.Font.Size := Round(AdvGlowBtn.Font.Size + AdvPreviewMenu.DPIScale);
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.Font.Size := Round(Canvas.Font.Size * AdvPreviewMenu.DPIScale);
      R := Rect(0,0,1000,100);
  ...
  w := w + AdvPreviewMenu.MenuSpacingDPI * 2 + 2 + Round(AdvPreviewMenu.DPIScale * 15);
  h := h + AdvPreviewMenu.MenuSpacingDPI;
  AWidth := w;
  AHeight := h;
end;
...
function TAdvPreviewMenuPanel.GetSubMenuItemHeightDPI: Integer;
begin
  Result := FSubMenuItemHeightDPI;
end;
...
begin
  ...
  Result.Bottom := Result.Top + Round(FSubMenuCaptionHeight * AdvPreviewMenu.DPIScale);
end;
...
begin
  ...
  Result.Top := Result.Top + Round(FSubMenuCaptionHeight * AdvPreviewMenu.DPIScale) + Abs(Index - FTopSubMenuItem) * FSubMenuItemHeightDPI;
  if FSubMenuScroller.Visible and (FSubMenuScroller.CanGoBack)then
    Result.Top := Result.Top + AdvPreviewMenu.ScrollHeightDPI;
  Result.Bottom := Result.Top + FSubMenuItemHeightDPI;
...
begin
  ...
  FSubMenuItemHeightDPI := Max(MINITEM_HEIGHT, h);
end;
...
begin
  ...
  begin
    ...
    SubMenuItemSpacing := Round(FAdvPreviewMenu.MenuItems.Items[Index].SubmenuItemSpacing * AdvPreviewMenu.DPIScale);
    ...
    SubMenuItemHeight := Round(FAdvPreviewMenu.MenuItems.Items[Index].SubMenuItemHeight * AdvPreviewMenu.DPIScale);
  end
  ...
  begin
    ...
    SubMenuCaption := FAdvPreviewMenu.SubMenuCaption;
    WideSubMenuCaption := FAdvPreviewMenu.WideSubMenuCaption;
    SubMenuItemHeight := Round(FAdvPreviewMenu.SubMenuItemHeight* AdvPreviewMenu.DPIScale);
  end
  else
  begin
    Exit;
  ...
  begin
    ...
    begin
      ...
      tw := R.Right + j + 20;
      th := R.Bottom + AdvPreviewMenu.LineSpaceDPI;
    end
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          R := DrawVistaText(Canvas, taLeftjustify, tvaTop, DT_CALCRECT or DT_LEFT or DT_SINGlELINE, R, SubItems.Items[i].Notes[n], '', Canvas.Font, AdvPreviewMenu.DPIScale, True, False, FAdvPreviewMenu.AntiAlias);
          tw := Max(tw, R.Right + j + 4);
          th := th + AdvPreviewMenu.LineSpaceDPI + R.Bottom;
        end;
      end;
    end
    ...
    begin
      ...
      if (FAdvPreviewMenu.SubMenuWidth > 0) then
        R.Right := Round(AdvPreviewMenu.SubMenuWidth * AdvPreviewMenu.DPIScale);
      ...
      tw := Max(tw, R.Right + j);
      th := th + (AdvPreviewMenu.LineSpaceDPI * Max(1, R.Bottom div Canvas.TextHeight('gh')))+ 4 + R.Bottom;
    end;
...
begin
  ...
  if MenuWidth + P.X + 52 > Screen.MonitorFromPoint(P).BoundsRect.Right then
  begin
...
begin
  ...
  if (AdvPreviewMenu.MenuItems.Items[Index].ShortCutSubItemsHint <> '') and (AdvPreviewMenu.MenuItems.Items[Index].ShortCutSubItemHintShowing) then
  begin
    Canvas.Font.Assign(Self.Font);
    R := GetItemRect(Index);
    GetShortCutHintSize(Canvas, FAdvPreviewMenu.MenuItems.Items[Index].ShortCutSubItemsHint, h, w);
    pt.X := R.Right - w - 3;
    pt.Y := R.Bottom - AdvPreviewMenu.MenuSpacingDPI - h;
    DrawShortCutHint(Canvas, AdvPreviewMenu.MenuItems.Items[Index].ShortCutSubItemsHint, pt, True);
  end;
end;
...
begin
  ...
  begin
    ...
    pt.X := R.Left + 4 + AdvPreviewMenu.MenuSpacingDPI * 2; //(R.Left + (R.Right - R.left) div 3) - w;
    pt.Y := R.Bottom - AdvPreviewMenu.MenuSpacingDPI - h + 2;
...
begin
  ...
  begin
    ...
    GetShortCutHintSize(Canvas, shCut, h, w);
    pt.X := R.Left + AdvPreviewMenu.MenuSpacingDPI * 2;
    pt.Y := R.Bottom - AdvPreviewMenu.MenuSpacingDPI - h;
    DrawShortCutHint(Canvas, shCut, pt, True);
  end;
end;
...
begin
  ...
  begin
    Result := GetRightFrameRect;    
    Result := Rect(Result.Left + 3, Result.Bottom - AdvPreviewMenu.ScrollHeightDPI - 2, Result.Right - 2, Result.Bottom - 2);
    if (FSubMenuItem >= 0) then
    begin
...
begin
  ...
  begin
    ...
    begin
      Result := GetRightFrameRect;
      Result := Rect(Result.Left + 3, Result.Top + 1, Result.Right - 2, Result.Top + AdvPreviewMenu.ScrollHeightDPI + 1);
      if (FSubMenuItem >= 0) then
        Result.Right := Result.Right - SHADOW_WIDTH;
    end
    else
    begin
      Result := Rect(Result.Left + 2, Result.Bottom, Result.Right - 1, Result.Bottom + AdvPreviewMenu.ScrollHeightDPI + 1);
    end;
  end;
end;
...
var
  i: Integer;
begin
  FSubMenuVisibleItemCount := (GetRightFrameRect.Bottom - GetRightFrameRect.Top - FSubMenuCaptionHeight - SHADOW_WIDTH - 3) div FSubMenuItemHeightDPI; // available SubMenuItem space
  i := VisibleSubItemCount(FSubMenuItem);
  if (i > FSubMenuVisibleItemCount) then
  begin
    //--- recalculate it with bottom scroll button
    FSubMenuVisibleItemCount := (GetRightFrameRect.Bottom - GetRightFrameRect.Top - FSubMenuCaptionHeight - SHADOW_WIDTH - 3 - AdvPreviewMenu.ScrollHeightDPI) div FSubMenuItemHeightDPI; // available SubMenuItem space
...
begin
  ...
  begin
    bmp := TBitmap.Create;
    try
      bmp.Height := Max(Self.Height, (SubItems.Count + 3) * FSubMenuItemHeightDPI);
      ...
      begin
        ...
        begin
          R.Top := R.Top + FSubMenuItemHeightDPI;
          R.Bottom := R.Bottom + FSubMenuItemHeightDPI;
        end;
      end;

      h := (Value - FTopSubMenuItem) * FSubMenuItemHeightDPI;
      i := j;
      Canvas.CopyMode := cmSrcCopy;
      while (i < h) do
      begin
  ...
  begin
    ...
    try
      ...
      R := R1;
      R := Rect(R.Left, R.Top + FTopSubMenuItem * FSubMenuItemHeightDPI, R.Right, R.Bottom + FTopSubMenuItem * FSubMenuItemHeightDPI);

      h := (FTopSubMenuItem - Value) * FSubMenuItemHeightDPI;
      i := j;
      Canvas.CopyMode := cmSrcCopy;
      while (i < h) do
      begin
