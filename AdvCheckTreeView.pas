...
const
  ...
  // v1.0.1.2 : Fixed : Issue with chCheckChilds
  // v1.0.1.3 : Fixed : Issue with node expand/collaps and event handler OnNodeCheckedChanging
  // v1.0.1.4 : Fixed : Issue with OnNodeCheckedChanging when images are used
...
begin
  ...
  if (chCheckChilds in FCheckHierarchy) then
  begin
...
begin
  ...
  begin
    b := GetChecked(tn1);
    R := tn1.DisplayRect(true);
    if Assigned(Images) then
      R.Left := R.Left - Images.Width;
  end;

  if (Message.XPos > R.Left) or (Message.XPos < R.Left - 16)  then
  begin
    inherited;
    Exit;
  end;
