{**************************************************************************}
{ TAdvSmoothProgressBar DESIGN TIME EDITOR                                 }
...
uses
  Classes, AdvSmoothFillPreview, AdvSmoothProgressBar, AdvSmoothFillEditor, GDIPFill,
  DesignIntf, DesignEditors;

type
  TAdvSmoothProgressBarEditor = class(TDefaultEditor)
  protected
...
procedure TAdvSmoothProgressBarEditor.EditProperty(const PropertyEditor: IProperty; var Continue: Boolean);
var
  PropName: string;
begin
...
procedure TAdvSmoothProgressBarEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothProgressBar).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothProgressBar) then
         (Component as TAdvSmoothProgressBar).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothProgressBarEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothProgressBarEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
