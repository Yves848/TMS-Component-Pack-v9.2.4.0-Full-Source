...
const
  ...
  // v1.3.0.3 : Fixed : Issue with setting defaultbutton after calling GetWidth or GetHeight
  // v1.3.1.0 : New : Rounding property on button collection
...
begin
  ...
  begin
    ...
    f := TGPFont.Create(ff, Round(FDPIScale * ButtonFont.Size), fs, UnitPoint);
    ...
    begin
      ...
      begin
        ...
        if ((Buttons[I].PictureLocation = blPictureLeft) or (Buttons[I].PictureLocation = blPictureRight)) then
          sri.Width := sri.Width + Buttons[I].Picture.Width + Round(FDPIScale * Buttons[I].Spacing)
        else
          sri.Width := Buttons[I].Picture.Width;
      end;

      sri.Width := Max(75, ((sri.Width + Round(FDPIScale * 10)) + Round(FDPIScale * ButtonSpacing)));
      ...
      begin
        ...
        if ((Buttons[I].PictureLocation = blPictureTop) or (Buttons[I].PictureLocation = blPictureBottom)) then
          sri.Height := sri.Height + Buttons[I].Picture.Height + Round(FDPIScale * Buttons[I].Spacing)
        else
          sri.Height := Buttons[I].Picture.Height;
      end;

      sri.Height := Max(25, ((sri.Height + Round(FDPIScale * 10)) + Round(FDPIScale * ButtonSpacing)));
  ...
  begin
    ...
    sf := TGPStringFormat.Create;
    f := TGPFont.Create(ff, Round(FDPIScale * CaptionFont.Size), fs, UnitPoint);

    g.MeasureString(Caption, Length(Caption), f, maxr, sf, sri);
    FMaxDialog.maxcaptionheight := CaptionHeight;
    FMaxDialog.maxcaptionwidth := sri.Width + Round(FDPIScale * 20);
  ...
  begin
    ...
    begin
      ...
      ca.Font.Height := Round(FDPIScale * FFont.Height);
      ...
      HTMLDrawGDIP(g, FFont, Text,Bounds(Round(maxr.X), Round(maxr.Y), Round(maxr.Width), Round(maxr.Height)), FImages, 0, 0, -1, -1, FShadowOffset,
        False, true, false, false, False, False, true, 1.0, FURLColor, clNone, clNone, FShadowColor, a, s, k, XSize, YSize, l, m, hr, nil, FContainer, 2, FDPIScale);
      {$ENDIF}

      FMaxDialog.maxhtmlheight := YSize + Round(FDPIScale * 20) + Top;
      FMaxDialog.maxhtmlwidth := XSize + Round(FDPIScale * 20) + Left;
  ...
  begin
    FMaxDialog.maxprogressheight := ProgressHeight + Round(FDPIScale * 25);
    FMaxDialog.maxprogresswidth := ProgressWidth + Round(FDPIScale * 20);
  ...
  FMaxDialog.totalmaxheight := FMaxDialog.totalmaxheight + Round(FDPIScale * Margin);
  FMaxDialog.totalmaxwidth := FMaxDialog.totalmaxwidth + Round(FDPIScale * (Margin * 2));
...
begin
  ...
  begin
    ...
    b.Font := ButtonFont;
    b.Font.Height := Round(FDPIScale * ButtonFont.Height);
    b.Rounding := btn.Rounding;
    c := btn.Color;
...
begin
  ...
  boundsr := MakeRect(totalr.X + Round(FDPIScale * Margin), totalr.Y + Round(FDPIScale * Margin), totalr.Width - Round(FDPIScale * Margin * 2), totalr.Height - Round(FDPIScale * Margin * 2));
...
begin
  ...
  result.X := result.X + Round(FDPIScale * 5);
  result.Width := result.Width - Round(FDPIScale * 5);
end;
...
begin
  ...
  if Assigned(Dialog.Owner) and (Dialog.Owner is TControl) then
    FDPIScale := GetDPIScale(TControl(Dialog.Owner), Canvas)
  else
    FDPIScale := GetDPIScale(Self, Canvas);

  if Assigned(Dialog) then
  begin
    Dialog.FDPIScale := FDPIScale;
    Dialog.CalculateMaximum;
    Width := Round(Dialog.FMaxDialog.totalmaxwidth);
    Height := Round(Dialog.FMaxDialog.totalmaxheight + FDPIScale * Dialog.Margin * 2);
    UpdateButtons;
    UpdateWindow;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      captionr.X := captionr.X - Round(FDPIScale * 5);
      captionr.Width := captionr.Width + Round(FDPIScale * 5);
      htmlr := GetHTMLRect(R);
...
begin
  ...
  begin
    ...
    Canvas.Font.Height := Round(FDPIScale * Dialog.ButtonFont.Height);
  ...
  begin
    ...
    Canvas.Font.Height := Round(FDPIScale * Dialog.HTMLText.Font.Height);
  ...
  begin
    ...
    begin
      ...
      case ButtonLayout of
        blhorizontal:
        begin
          smaxw :=  (FMaxDialog.maxbuttonwidth * Buttons.Count) + Round(FDPIScale * ButtonSpacing);
          smaxh :=  FMaxDialog.maxbuttonheight;
        end;
        blVertical:
        begin
          smaxw :=  FMaxDialog.maxbuttonwidth + Round(FDPIScale * ButtonSpacing);
      ...
      if smaxw > buttonr.Width then
        Self.Width := Round(smaxw + FDPIScale * Margin * 2);

      if smaxh > buttonr.Height then
        Self.Height := Round(GetCaptionRect(ClientRect).Height + GetHTMLRect(ClientRect).Height + GetProgressBarRect(ClientRect).Height + smaxh + Round(FDPIScale * (Margin * 2 + ButtonSpacing)));

      bwidth := FMaxDialog.maxbuttonwidth - Round(FDPIScale * ButtonSpacing);
      bheight := FMaxDialog.maxbuttonheight - Round(FDPIScale * ButtonSpacing);
      ...
      case ButtonLayout of
        ...
        begin
          ...
          for I := 0 to Buttons.Count - 1 do
            Buttons[I].FBtnr := MakeRect(buttonr.X + (I * bwidth) + ((I + 1) * Round(FDPIScale * ButtonSpacing)),
        ...
        begin
          for I := 0 to Buttons.Count - 1 do
            Buttons[I].FBtnr := MakeRect(buttonr.X + (buttonr.Width - bwidth) / 2,
              buttonr.Y + (I * bheight) + ((I + 1) * Round(FDPIScale * ButtonSpacing)), bwidth, bheight);
        end;
      end;
    end;
  end;
...
begin
  inherited;
  if Assigned(Dialog) and not Dialog.FBlockFRMSet then
  begin
...
begin
  ...
  begin
    ...
    FRounding := (Source as TAdvSmoothMessageDialogButton).Rounding;
    Changed;
  end;
end;
...
procedure TAdvSmoothMessageDialogButton.SetRounding(const Value: integer);
begin
