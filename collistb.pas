...
const
  ...
  //          : New : GridLineColor property added
  // v1.3.1.0 : New : Per monitor support for high DPI
...
{$IFDEF DELPHIXE2_LVL}
  VCL.Themes,
{$ENDIF}
  ExtCtrls, ShellApi, Commctrl, ImgList, AdvStyleIF;
...
begin
  ...
  begin
    ...
    Canvas.Font.Height := Round(FDPIScale * FColumns.Items[col - 1].Font.Height);
    ...
    { Enabled/Disabled state for text color }
    {$IFDEF DELPHIXE2_LVL}
    if FUseVCLStyles then
      Canvas.Font.Color := StyleServices.GetStyleFontColor(lFontColorStates[Enabled])
    else
    {$ENDIF}
    begin
      if not Enabled then
    ...
    dr := r;
    dr.right := dr.left + Round(FDPIScale * FColumns.Items[col - 1].Width);
  ...
  // paint remaining section after last column
  ...
  begin
    ...
    begin
      imgidx := imgidx + Round(FDPIScale * FColumns.Items[col - 1].Width);
...
begin
  ...
  begin
    ...
    try
      ...
      res := Round(FDPIScale * (Canvas.TextHeight('gh') + 4)); {some overlap on fonts}
...
procedure TColumnListBox.SetGridLineColor(const Value: TColor);
begin
...
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TColumnListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
...
{$IFDEF DELPHIXE2_LVL}
procedure TColumnListBox.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
procedure TColumnListBox.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
const
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    {$ENDIF xe6}
      ...
      for i := 0 to Columns.Count-1 do
        Columns[i].Font.Color := Font.Color;
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF xe6}
  end;
