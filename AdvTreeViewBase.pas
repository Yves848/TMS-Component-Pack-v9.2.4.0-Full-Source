...
const
  ...
  {$HINTS OFF}
  TMSTreeViewPlatformsMobile = {$IF COMPILERVERSION > 32}pidiOSSimulator32 or pidiOSSimulator64{$ELSE}pidiOSSimulator{$IFEND} or {$IFDEF DELPHIXE8_LVL}pidiOSDevice32 or pidiOSDevice64{$ELSE}pidiOSDevice{$ENDIF} or {$IF COMPILERVERSION > 32}pidAndroid32Arm or pidAndroid64Arm{$ELSE}pidAndroid{$IFEND};
  {$HINTS ON}
  TAdvTreeViewColorBlack = claBlack;
  ...
  {$ELSE}
  TAdvTreeViewColorBlack = clBlack;
...
type
  ...
  TAdvTreeViewBaseControlBeforeDrawEvent = procedure(Sender: TObject; ACanvas: TCanvas; ARect: TRectF; var ADefaultDraw: Boolean) of object;
  TAdvTreeViewBaseControlAfterDrawEvent = procedure(Sender: TObject; ACanvas: TCanvas; ARect: TRectF) of object;

  {$IFDEF FMXLIB}
  TAdvTreeViewBaseControl = class(TAdvBaseControl);
  {$ENDIF}
  {$IFDEF CMNLIB}
  TAdvTreeViewBaseControl = class(TCustomControl, IAdvProductInfo)
  private
  ...
  private
    FOnAfterDraw: TAdvTreeViewBaseControlAfterDrawEvent;
    FOnBeforeDraw: TAdvTreeViewBaseControlBeforeDrawEvent;
    FDesignTime: Boolean;
  ...
  published
    property OnBeforeDraw: TAdvTreeViewBaseControlBeforeDrawEvent read FOnBeforeDraw write FOnBeforeDraw;
    property OnAfterDraw: TAdvTreeViewBaseControlAfterDrawEvent read FOnAfterDraw write FOnAfterDraw;
  end;
...
procedure TAdvTreeViewBase.Paint;
var
  df: Boolean;
begin
  df := True;
  if Assigned(OnBeforeDraw) then
    OnBeforeDraw(Self, Canvas, LocalRect, df);

  if df then
  begin
    inherited Paint;
    if Assigned(OnAfterDraw) then
      OnAfterDraw(Self, Canvas, LocalRect);
  end;
end;
...
{$IFDEF CMNLIB}
function TAdvTreeViewBaseControl.GetVersion: string;
begin
  Result := '';
end;

function TAdvTreeViewBaseControl.GetDocURL: string;
begin
  Result := '';
end;

function TAdvTreeViewBaseControl.GetTipsURL: string;
begin
  Result := '';
end;
