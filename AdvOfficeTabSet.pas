...
const
  ...
  // 3.6.2.0 : Improved : Handling of tab edit & tab reorder simultaneously
  // 3.6.2.1 : Fixed : Issue with tab edit and AllowTabUndock = true
  // 3.6.2.2 : Improved : High-DPI handling with a fixed tab width is used
  // 3.6.2.3 : Improved : Drawing of larger badge text
  // 3.6.2.4 : Fixed : Issue with showing OfficeHints on tabs
  // 3.6.2.5 : Fixed : Auto hiding of scroller buttons in left/right tab position mode
  // 3.6.2.6 : Fixed : Issue with high DPI & reparenting
  // 3.6.3.0 : New : Support for multimonitor High DPI handling
  //         : Fixed : Issue with High DPI in Delphi 10.2 Tokyo
  // 3.6.3.1 : Improved : High DPI handling when styler is assigned
  // 3.6.3.2 : Fixed : Small tab button surrounding rectangle drawing issue
...
type
  ...
  TTabItemAllowEvent = procedure(Sender: TObject; ATab: TOfficeTabCollectionItem; var Allow: boolean) of object;
  ...
  private
    ...
    FOnTabUnDocking: TTabItemAllowEvent;
    FDidScale: boolean;
    procedure WMEraseBkGnd(var Message: TWMEraseBkGnd); message WM_ERASEBKGND;
  ...
  protected
    ...
    property OnTabUnDocking: TTabItemAllowEvent read FOnTabUnDocking write FOnTabUnDocking;
...
function TTabSetTabSettings.RealWidth: integer;
begin
  Result := Round(FWidth * DPIScale);
end;
...
begin
  ...
  begin
    ...
    FDPIScale := CalculateDPIScale(Self);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        gpgraphics.FillRectangle(gpbrush, badgexy.X, badgexy.Y, d + 4, 14);
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvCustomOfficeTabSet.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    ...
    if FReorderStart and TabRearrange and ((Abs(FReorderPt.X - X) > 3) or (Abs(FReorderPt.Y - Y) > 3)) and (Tab >= 0) then
    begin
      FEditStart := false;
      FReorderStart := false;
      BeginDrag(true);
    ...
    begin
      ...
      if Assigned(OnTabUnDocking) then
        OnTabUndocking(Self, AdvOfficeTabs[Tab], Allow);
      if Allow then
      begin
        P := ClientToScreen(P);
...
begin
  ...
  begin
    ...
    IsActive := (Tab = ActiveTabIndex);
    if (Button = mbLeft) and (Tab >= 0) and AdvOfficeTabs[Tab].Enabled and IsActive and FEditStart then
    begin
      r := GetTabRect(Tab);
      FEditIndex := Tab;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomOfficeTabSet.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  frm: TCustomForm;
  i: integer;
{$ELSE}
procedure TAdvCustomOfficeTabSet.ChangeScale(M, D: Integer);
var
  i: integer;
{$ENDIF}
        ...
        begin
          ...
          begin
            ...
            begin
              Canvas.Font.Assign(TabAppearance.Font);
              if Assigned(AdvOfficeTabSetStyler) then
                Canvas.Font.Height := Round(FDPIScale * TabAppearance.Font.Height);
            end;
...
var
  ...
  begin
    ...
    begin
      aw := r.Bottom - r.Top;
      aw := Height - aw - 2 - 2 * Abs(TabSettings.Spacing);
    end;
    ...
    begin
      ...
      begin
        ...
        if TabPosition in [tpTop, tpBottom] then
          ...
          tw := tw + (r.Bottom - r.Top) + TabSettings.Spacing;
