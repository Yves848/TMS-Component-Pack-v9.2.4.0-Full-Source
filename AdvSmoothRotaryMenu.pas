...
const
  ...
  // v1.1.1.1 : Fixed : Issue with Canvas handle for high DPI in StartAnimation.
...
begin
  ...
  begin
    ...
    cW := Round(FDPIScale * Width);
    cH := Round(FDPIScale * Height);
...
begin
  ...
  f := g.MakeFont(AItem.Font, Round(AItem.Font.Size * FDPIScale));
...
begin
  if Owner is TAdvSmoothRotaryMenuDialog then
  begin
    frm := (Owner as TAdvSmoothRotaryMenuDialog).Form;
    if Assigned(frm) then
      FDPIScale := GetDPIScale(frm, nil);
  end
  else
...
begin
  ...
  begin
    ...
    begin
      FDPIScale := GetDPIScale(Control, nil);
      cW := Round(FDPIScale * Width);
      cH := Round(FDPIScale * Height);
      ...
      case Position of
        rmpCenterLeft, rmpTopLeft, rmpBottomLeft: X := X - cW;
        rmpCenterCenter, rmpTopCenter, rmpBottomCenter: X := X - (cW - Control.Width) div 2;
        rmpCenterRight, rmpTopRight, rmpBottomright: X := X + Control.Width;
      end;

      case Position of
        rmpTopLeft, rmpTopCenter, rmpTopRight: Y := Y - cH;
        rmpCenterCenter, rmpCenterRight, rmpCenterLeft: Y := Y - (cH - Control.Height) div 2;
...
begin
  ...
  begin
    FAnimation := Value;
    FStopAngle := FSaveStopAngle;
    FStopAngleTo := FSaveStopAngle;
    Changed;
  end;
end;
...
begin
  ...
  begin
      FSaveW := Round(FDPIScale * FW);
      FSaveH := Round(FDPIScale * FH);
  end;
  ...
  if amSize in AnimationMode then
  begin
    X := Round(FDPIScale * Width) div 2;
    Y := Round(FDPIScale * Height) div 2;
    W := 0;
    H := 0;
  end;
