...
procedure TLUPersist.Assign(Source: TPersistent);
begin
  if (Source is TLUPersist) then
  begin
    FEnable := (Source as TLUPersist).Enable;
    FLocation := (Source as TLUPersist).Location;
    FKey := (Source as TLUPersist).Key;
    FSection := (Source as TLUPersist).Section;
    FCount := (Source as TLUPersist).Count;
    FMaxCount := (Source as TLUPersist).MaxCount;
  end;
end;

constructor TLUPersist.Create;
begin
  inherited;
  FKey := 'LUEdit';
  FSection := 'Values';
