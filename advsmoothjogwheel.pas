...
const
  ...
  // v1.1.2.6 : Fixed : Issue with transparency on double buffered forms
  // v1.1.3.0 : Improved : Support for High DPI
...
begin
  ...
  case FDirection of
    wdHorizontal:
    begin
      r := MakeRect(Position, sp, cGSi, Height - sp * 2);
      m := LinearGradientModeVertical;
    end;
    wdVertical:
    begin
      r := MakeRect(sp, Position, Width - sp * 2, cGSi);
      m := LinearGradientModeHorizontal;
    end;
  end;
  ...
  path := CreateRoundRectangle(r, cGSi div 3, rtBoth, false);
  g.FillPath(b, path);
  b.free;
  path.free;
  ...
  case FDirection of
    wdHorizontal:
    begin
      r := MakeRect(Position + 0.5, 4 + 0.5, cGSi - 1, Height - 9);
      m := LinearGradientModeVertical;
    end;
    wdVertical:
    begin
      r := MakeRect(4 + 0.5, Position + 0.5, Width - 9, cGSi - 1);
      m := LinearGradientModeHorizontal;
    end;
  end;
  ...
  path := CreateRoundRectangle(r, (cGSi - 1) div 3, rtBoth, false);
  g.FillPath(b, path);
  b.free;
  path.free;
end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  cGSi := Round(DPIScale * GripSize);
  cGSp := Round(DPIScale * GripSpacing);
  ...
  cnt := 0;
  case FDirection of
    wdHorizontal: cnt := Round(Width / ((cGSp / 2) + cGSi)) + 5;
    wdVertical: cnt := Round(Height / ((cGSp / 2) + cGSi)) + 5;
  end;

  i := (GetPosition div cGSp) - (cnt div 2);
  j := 0;
  while j <= cnt do
  begin
    case FDirection of
      wdHorizontal: pos := -GetPosition + (i * cGSp) + (Width - cGSi) / 2;
      wdVertical: pos := -GetPosition + (i * cGSp) + (Height - cGSi) / 2;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DPIScale := GetDPIScale(Self, Canvas);
          cS := Round(DPIScale * Size);
          cW := Round(DPIScale * Width);

          case FDirection of
            wdHorizontal:
            begin
              case Position of
                wpCenter: r := MakeRect(pos - (cW / 2) + (Self.Width / 2), (Height - cS) / 2, cW, cS);
                wpTop: r := MakeRect(pos - (cW / 2) + (Self.Width / 2), 0, cW, cS);
                wpBottom: r := MakeRect(pos - (cW / 2) + (Self.Width / 2), Height - cS, cW, cS);
              end;            
            end;
            wdVertical:
            begin
              case Position of
                wpCenter: r := MakeRect((Self.Width - cS) / 2, pos - (cW / 2) + (Self.Height / 2), cS, cW);
                wpTop: r := MakeRect(0, pos - (cW / 2) + (Self.Height / 2), cS, cW);
                wpBottom: r := MakeRect(Self.Width - cS, pos - (cW / 2) + (Self.Height / 2), cS, cW);
              end;                        
            end;
          end;
