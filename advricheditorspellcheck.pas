...
type
  ...
  private
    ...
    FContextMenuIgnoreText: string;
    FContextMenuIgnore: boolean;
  ...
  published
    property ContextMenuIgnore: boolean read FContextMenuIgnore write FContextMenuIgnore default false;
    property ContextMenuIgnoreText: string read FContextMenuIgnoreText write FContextMenuIgnoreText;
...
begin
  ...
  FContextMenuIgnoreText := '&Ignore';
  FContextMenuIgnore := false;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ofs := 1;
          if FContextMenuIgnore then
          begin
            InsertMenu(popup, $FFFFFFFF, MF_BYPOSITION, 1, pchar(FContextMenuIgnoreText));
            ofs := 2;
          end;
          ...
          if (i = 1) and FContextMenuIgnore then
          begin
...
procedure TAdvRichEditorProofingToolBar.Loaded;
begin
  inherited;
  UpdateHints;
end;
...
procedure TAdvRichEditorProofingToolBar.UpdateHints;
var
  agb: TAdvCustomGlowButton;
begin
  agb := GetButton(integer(btCheckDocument));
  agb.Hint := Hints.CheckDocumentTitle;
  agb.OfficeHint.Title := Hints.CheckDocumentTitle;
  agb.OfficeHint.Notes.Text := Hints.CheckDocumentContent;

  agb := GetButton(integer(btClearSpellCheck));
  agb.Hint := Hints.ClearSpellCheckTitle;
  agb.OfficeHint.Title := Hints.ClearSpellCheckTitle;
  agb.OfficeHint.Notes.Text := Hints.ClearSpellCheckContent;

  agb := GetButton(integer(btConfigureSpellCheck));
  agb.Hint := Hints.ConfigSpellCheckTitle;
  agb.OfficeHint.Title := Hints.ConfigSpellCheckTitle;
  agb.OfficeHint.Notes.Text := Hints.ConfigSpellCheckContent;

  agb := GetButton(integer(btSelectLanguage));
  agb.Hint := Hints.SelectLanguageTitle;
  agb.OfficeHint.Title := Hints.SelectLanguageTitle;
  agb.OfficeHint.Notes.Text := Hints.SelectLanguageContent;
end;
