...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'July, 2019'; // Month version
  ...
  // 1.8.4.0 : Improved : High DPI handling
  // 1.8.4.1 : Improved : Automatic client alignment of esPopup custom inplace editor controls
  // 1.8.5.0 : New : Name property added to TInspectorItem
  //         : New : InspectorPanel.Items.Item[Name] added to access items by name
  // 1.8.6.0 : New : Support for mousewheel scroll added
  // 1.8.6.1 : Improved : Color property drawing
  // 1.8.7.0 : New : Search for item by name at InspectorBar level
  // 1.8.8.0 : Improved : High DPI support
  // 1.9.0.0 : New : VCL Styles support added
  // 1.9.1.0 : New : Per monitor support for high DPI
...
type
  ...
  private
    ...
    function GetItemByName(AName: string): TInspectorItem;
  protected
  ...
  public
    ...
    property Item[AName: string]: TInspectorItem read GetItemByName;
  published
  ...
  protected
    ...
    function ScaledCaptionHeight: integer;
...
function TInspectorItems.GetItemByName(AName: string): TInspectorItem;
var
  i: integer;
begin
  Result := nil;
...
begin
  Result := InspectorBar.BarHeight - ((InspectorBar.getNumbervisiblePanelDown(InspectorBar.GetRealTopPanel) + 1) * InspectorBar.ScaledCaptionHeight);
end;
...
function TInspectorPanel.ScaledItemHeight: integer;
begin
  Result := Round(InspectorBar.FDPIScale * ItemHeight);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if dx > 0 then
      begin
        Self.TopItem := Self.TopItem + (dx div ScaledItemHeight) + 1;
      end;

      y := (InspectorBar.GetNumbervisiblePanelUp(InspectorBar.GetRealTopPanel) + 1) * InspectorBar.ScaledCaptionHeight;
...
begin
  ...
  begin
    ...
    FName := (Source as TInspectorItem).Name;
  end;
end;
...
begin
  ...
  begin
    {$IFDEF DELPHIXE2_LVL}
    if Hot then
      lDetails := StyleServices.GetElementDetails(TThemedHeader.thHeaderItemHot)
    else
      lDetails := StyleServices.GetElementDetails(TThemedHeader.thHeaderItemNormal);
  ...
  begin
    ...
    begin
      ...
      begin
        bkgw := PanelCaption.Background.Width;
        bkgh := PanelCaption.Background.Height;
        bkgw := Round(bkgw * FDPIScale);
        bkgh := Round(bkgh * FDPIScale) + 1;
        ...
        begin
          ...
          Canvas.StretchDraw(Rect(r.Left + Ind, r.Top, r.Left + Ind + bkgw, r.Top + bkgh), PanelCaption.Background);

          Canvas.CopyRect(Rect(0, r.Top, Ind, R.Top + bkgh), PanelCaption.Background.Canvas,
        ...
        begin
          ind := bkgw;
          Canvas.StretchDraw(Rect(r.Left, r.Top, r.Left + bkgw, r.Top + bkgh), PanelCaption.Background);

          Canvas.CopyRect(Rect(ind, r.Top, R.Right, R.Top + bkgh), PanelCaption.Background.Canvas,
      ...
      begin
        bkgw := PanelCaption.ActiveBackground.Width;
        bkgh := PanelCaption.ActiveBackground.Height;
        bkgw := Trunc(bkgw * FDPIScale);
        bkgh := Trunc(bkgh * FDPIScale) + 1;
        ...
        begin
          ...
          Canvas.StretchDraw(Rect(r.Left + Ind, r.Top, r.Left + Ind + bkgw, r.Top + bkgh), PanelCaption.ActiveBackground);

          Canvas.CopyRect(Rect(0, r.Top, Ind, R.Top + bkgh), PanelCaption.ActiveBackground.Canvas,
        ...
        begin
          ...
          Canvas.StretchDraw(Rect(r.Left, r.Top, r.Left + bkgw, r.Top + bkgh), PanelCaption.ActiveBackground);

          Canvas.CopyRect(Rect(ind, r.Top, R.Right, R.Top + bkgh), PanelCaption.ActiveBackground.Canvas,
    ...
    begin
      ...
      Canvas.StretchDraw(R,FShadedHeader);
    end;
  end;
  ...
  begin
    case ocGraphic of
      ...
      ocgCross:
        begin
          Canvas.StretchDraw(Rect(pt.X + 4, pt.Y + 4, pt.X + 4 + Round(8 * FDPIScale), pt.Y + 4 + Round(8 * FDPIScale)), FNodeCloseGlyph);
        end;
      ocgTriangle:
        begin
  ...
  begin
    case ocGraphic of
      ...
      ocgCross:
        begin
          Canvas.StretchDraw(Rect(pt.X + 4, pt.Y + 4, pt.X + 4 + Round(8 * FDPIScale), pt.Y + 4 + Round(8 * FDPIScale)), FNodeOpenGlyph);
        end;
      ocgTriangle:
        begin
          Canvas.Brush.Color := clBlack;
  ...
  {$IFDEF DELPHIXE2_LVL}
  case PanelCaption.VAlignment of
  vaCenter: tf := [tfVerticalCenter];
  vaTop: tf := [tfTop];
  vaBottom: tf := [tfBottom];
  end;

  case PanelCaption.Alignment of
  taLeftJustify: tf := tf + [tfLeft];
  taCenter: tf := tf + [tfCenter];
  taRightJustify: tf := tf + [tfRight];
  end;
  {$ENDIF}
  ...
  OffsetRect(R, Round(PanelCaption.Indent * FDPIScale), 0);
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      StyleServices.DrawText(Canvas.Handle, lDetails, Panel.Caption, R, tf);
      {$ENDIF}
    end
    else
...
begin
  ...
  begin
    ...
    if i <= realtoppanel then
      j := (getNumbervisiblePanelUp(i)) * ScaledCaptionHeight
    else
      j := BarHeight - ((getNumbervisiblePanelDown(i) + 1) * ScaledCaptionHeight);

    if (y >= j) and (y <= j + ScaledCaptionHeight) then
    begin
...
//------ Buttons for scroll  Click-------------
  j := (GetNumbervisiblePanelup(realtoppanel) + 1) * ScaledCaptionHeight; //Click Up
  i := BarHeight - ((GetNumbervisiblePanelDown(realtoppanel)) * ScaledCaptionHeight); //Click Down
        ...
        begin
          ...
          begin
            ...
            if FPanels[realtoppanel].VariableItemHeight then
              h := FPanels[realtoppanel].Items[m].Height
            else
              h := FPanels[realtoppanel].ScaledItemHeight;
        ...
        begin
          ...
          begin
            ...
            if FPanels[realtoppanel].VariableItemHeight then
              ...
              Inc(tempHeight, FPanels[realtoppanel].ScaledItemHeight);
    ...
    begin
      ...
      begin
        ...
        if FPanels.Items[i].VariableItemHeight then
          ...
          tempHeight := FPanels.Items[i].ScaledItemHeight;
...
// compute absolute coordinates from local coordinates
    ...
    begin
  //for open panel
      Inc(j, ScaledCaptionHeight); //Panel caption
  ...
  //Items
      ...
      if i = FLastOpen then
      begin
        j := BarHeight - (FPanels.Count - FLastOpen - 1) * ScaledCaptionHeight;
      end;
    end
    else
    begin
...
begin
  ...
  begin
    ...
    begin
      DrawCaption(FPanels.Items[i - 1], Rect(0, indexV, Width, indexV + ScaledCaptionHeight));
      IndexV := IndexV + ScaledCaptionHeight;
    end;
    if i > realtoppanel then
      Break;
    inc(i);
  ...
  R := Rect(3, realtoppanel * ScaledCaptionHeight + 3, Width - 3, (realtoppanel + 1) * ScaledCaptionHeight - 3);
  ...
  begin
    ...
    dec(indexv, 1);
    DrawCaption(FPanels.Items[i - 1], Rect(0, BarHeight - (j) * ScaledCaptionHeight, Width, BarHeight - ((j - 1) * ScaledCaptionHeight)));
  end;

  //focused panel background
  indexv := GetNumbervisiblePanelUp(FPanels.Count) + 1;
  h := BarHeight - (indexv) * ScaledCaptionHeight;
  Top := (getNumbervisiblePanelUp(Realtoppanel)) * ScaledCaptionHeight;

  R := Rect(PanelIndent, Top + ScaledCaptionHeight, Width, Top + ScaledCaptionHeight + h);
  ...
  RealTop := Top + ScaledCaptionHeight;
  RealHeight := BarHeight - RealTop - (getNumbervisiblePanelDown(realtoppanel) * ScaledCaptionHeight);
  ...
  for i := TopPanel.FTopItem + 1 to TopPanel.Items.Count do
    ...
    begin
      if TopPanel.VariableItemHeight then
        ...
        tempTotalHeight := tempTotalHeight + TopPanel.ScaledItemHeight;
  ...
  begin
    if TopPanel.VariableItemHeight then
      ...
      tempHeight := TopPanel.ScaledItemHeight;
...
// j holds the current drawing position
  ...
  //draw the panel caption for closed panels
      DrawCaption(FPanels.Items[i - 1],
        Rect(0, j, RealWidth, j + ScaledCaptionHeight));
      Inc(j, ScaledCaptionHeight);
    end
    else
    begin
  //draw the panel caption for opened panels
      ...
      for q := 0 to FPanels.Items[i - 1].Items.Count - 1 do
        if FPanels.Items[i - 1].Items[q].Visible then
          if FPanels.Items[i - 1].VariableItemHeight then
            ...
            Inc(h, FPanels.Items[i - 1].ScaledItemHeight);
    ...
    // if it is the last panel and it is shorter than TInspectorbar
        RealTop := BarHeight - (FPanels.Count - FLastOpen - 1) * ScaledCaptionHeight - h - Top;
        if RealTop < 0 then
          RealTop := 0;
      end
      else
...
begin
  ...
  FShadedHeader.Height := Round(FDPIScale * FInspectorCaption.FHeight);
...
begin
  ...
  begin
    if FShowUP or FShowDown then
      r.Right := r.Right - Round(19 * FDPIScale); //18 width of scroll button
  end;
  ...
  Canvas.Font.Height := Round(FDPIScale * Canvas.Font.Height);
  ...
  case Panel.Style of
    psLargeIcon:
      begin
        ...
        Canvas.Font.Assign(Panel.Font);
        Canvas.Font.Height := Round(FDPIScale * Panel.Font.Height);
    ...
    psSmallIcon:
      begin
        ...
        begin
          ...
          IcoRect.Right := IcoRect.Left + 16;
          IcoRect.Bottom := IcoRect.Top + 16;
          R.Left := R.Left + Round(16 * FDPIScale) + 4;
        end;

        if Assigned(FImages) and (Item.ImageIndex > -1) and not Item.AutoIcon then
        begin
          if (FImages.Height < Panel.ScaledItemHeight) then
            VInd := (Panel.ScaledItemHeight - FImages.Height) div 2
          else
            VInd := 0;
    ...
    psProperties:
      begin
        ...
        Canvas.Font.Height := Round(FDPIScale * Panel.CaptionFont.Height);
        ...
        begin
          ...
          if th > Round(16 * FDPIScale) then
            DrwRect := Rect(R.Left + 2, R.Top + (th - Round(16 * FDPIScale)) div 2, R.Left + Round(20 * FDPIScale), R.Top + (th - Round(14 * FDPIScale)) div 2 + Round(16 * FDPIScale))
          else
            DrwRect := Rect(R.Left + 2, R.Top + 2, R.Left + Round(20 * FDPIScale), R.Top + Round(16 * FDPIScale));

          Canvas.Pen.Color := clBlack;
          Canvas.Brush.Color := Item.ColorValue;
          Canvas.Rectangle(DrwRect.Left, DrwRect.Top, DrwRect.Right, DrwRect.Bottom);

          R.Left := R.Left + Round(22 * FDPIScale);
          if th <= Round(16 * FDPIScale) then
             R.Top := R.Top + 2;
        ...
        begin
          ...
          begin
          {$IFDEF DELPHIXE2_LVL}
            DrwRect := Rect(R.Right - Round(15 * FDPIScale), R.Top , R.Right, Min(R.Top + Round(17 * FDPIScale), R.Bottom - 1));
            lDetails := StyleServices.GetElementDetails(tcDropDownButtonNormal);
            StyleServices.DrawElement(Canvas.Handle, lDetails, DrwRect);
          {$ENDIF}
          end
          else
          begin
            ...
            begin
              ...
              DrawFrameControl(Canvas.Handle, Rect(R.Right - Round(16 * FDPIScale), R.Top, R.Right, R.Top + Round(18 * FDPIScale)), DFC_SCROLL, DFCS_SCROLLCOMBOBOX or DFCS_FLAT);
          end; 
          R.Right := R.Right - Round(16 * FDPIScale);
        end;
        ...
        begin
          ...
          begin
            ...
            if Item.ReadOnly then
              lButtonTheme := tbPushButtonDisabled
            else if FMouseDown then
              lButtonTheme := tbPushButtonPressed
            else if Hot then
              lButtonTheme := tbPushButtonHot
            else
              lButtonTheme := tbPushButtonNormal;
            lDetails := StyleServices.GetElementDetails(lButtonTheme);
            StyleServices.DrawElement(Canvas.Handle, lDetails, DrwRect);
            {$ENDIF}
          end
          else
          ...
          begin
            {$IFDEF DELPHIXE2_LVL}
            StyleServices.DrawText(Canvas.Handle, lDetails, ValueText, DrwRect, TTextFormatFlags(DT_CENTER or DT_SINGLELINE or DT_VCENTER));
            Canvas.Font.Color := Panel.CaptionFont.Color; // restore font color for later rendering
            {$ENDIF}
          end
          else
        ...
        begin
          ...
          begin
            ...
            DrwRect := Rect(R.Right - Round(18 * FDPIScale), R.Top - 1, R.Right, R.Bottom - 2);
            ...
            CloseThemeData(HTheme);
            DrwRect := Rect(R.Right - Round(12 * FDPIScale), R.Top + 2, R.Right, R.Bottom);
          end
          else
          begin
            DrawFrameControl(Canvas.Handle, Rect(R.Right - Round(14 * FDPIScale), R.Top, R.Right, R.Top + Round(18 * FDPIScale)), DFC_BUTTON, DFCS_BUTTONPUSH or DFCS_FLAT);
            DrwRect := Rect(R.Right - Round(10 * FDPIScale), R.Top + 2, R.Right, R.Bottom);
          end;
        ...
        begin
          ...
          begin
          {$IFDEF DELPHIXE2_LVL}
            DrwRect := Rect(R.Right - Round(17 * FDPIScale), R.Top-2, R.Right, R.Top + H);
            lDetails := StyleServices.GetElementDetails(tsUpNormal);
            StyleServices.DrawElement(Canvas.Handle, lDetails, DrwRect);

            DrwRect := Rect(R.Right - Round(17 * FDPIScale), R.Top + H, R.Right, R.Top + 2 * H + 1);
          ...
          begin
            ...
            begin
              ...
              DrwRect := Rect(R.Right - Round(17 * FDPIScale), R.Top, R.Right, R.Top + H + 2);
              ...
              DrwRect := Rect(R.Right - Round(17 * FDPIScale), R.Top + H + 2, R.Right, R.Top + 2 + 2 * H + 1);
            ...
            begin
              DrawFrameControl(Canvas.Handle, Rect(R.Right - Round(16 * FDPIScale), R.Top, R.Right, R.Top + H), DFC_SCROLL, DFCS_SCROLLUP or DFCS_FLAT);
              DrawFrameControl(Canvas.Handle, Rect(R.Right - Round(16 * FDPIScale), R.Top + H, R.Right, R.Top + 2 * H), DFC_SCROLL, DFCS_SCROLLDOWN or DFCS_FLAT);
            end;
          end;  // if FUseVCLStyles else ...
        ...
        begin
          ...
          begin
            {$IFDEF DELPHIXE2_LVL}
            DrwRect := Rect(R.Left, R.Top, R.Left + Round(14 * FDPIScale), R.Top + Round(14 * FDPIScale));
          ...
          begin
            ...
            begin
              if Item.BoolValue then
                DrawFrameControl(Canvas.Handle, Rect(R.Left, R.Top, R.Left + Round(14 * FDPIScale), R.Top + Round(14 * FDPIScale)), DFC_BUTTON, DFCS_BUTTONCHECK or DFCS_FLAT or DFCS_MONO or DFCS_CHECKED)
              else
                DrawFrameControl(Canvas.Handle, Rect(R.Left, R.Top, R.Left + Round(14 * FDPIScale), R.Top + Round(14 * FDPIScale)), DFC_BUTTON, DFCS_BUTTONCHECK or DFCS_FLAT or DFCS_MONO);
            end;
          end; // FUseVCLStyles
          R.Left := R.Left + Round(16 * FDPIScale);
        end;
        ...
        begin
          ...
          Canvas.Font.Height := Round(FDPIScale * Panel.ModifiedFont.Height);
        end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                StyleServices.DrawText(Canvas.Handle, lDetails, ValueText, R, TTextFormatFlags(DTSTYLE));
                {$ENDIF}
              end
              else
...
{$IFDEF DELPHIXE2_LVL}
  lDetails: TThemedElementDetails;
  lElement: TThemedScrollBar;
{$ENDIF}
begin
  y := (GetNumbervisiblePanelUp(getRealTopPanel) + 1) * ScaledCaptionHeight;
  if y < 0 then
    Exit;
  r.Bottom := y + 18;
  r.Right := self.Width;
  ...
  {$IFDEF DELPHIXE2_LVL}
    ...
    if FMouseDown and Hot then
      lElement := tsArrowBtnUpPressed
    else
    begin
      if Hot then
        lElement := tsArrowBtnUpHot
      else
        lElement := tsArrowBtnUpNormal
    end;
    lDetails := StyleServices.GetElementDetails(lElement);
    StyleServices.DrawElement(Canvas.Handle, lDetails, r);
  {$ENDIF}
  end
  else
...
{$ENDIF}
begin
  y := BarHeight - (GetNumbervisiblePanelDown(GetRealtoppanel)) * ScaledCaptionHeight;
  r.Bottom := y;
  r.Right := self.Width;
  r.Left := r.Right - 18;
  r.Top := y - 18;
  ...
  {$IFDEF DELPHIXE2_LVL}
    ...
    if FMouseDown and Hot then
      lElement := tsArrowBtnDownPressed
    else
    begin
      if Hot then
        lElement := tsArrowBtnDownHot
      else
        lElement := tsArrowBtnDownNormal
    end;
    lDetails := StyleServices.GetElementDetails(lElement);
    StyleServices.DrawElement(Canvas.Handle, lDetails, r);
  {$ENDIF}
...
function TInspectorBar.ScaledCaptionHeight: integer;
begin
  Result := Round(FDPIScale * PanelCaption.Height);
end;
...
begin
  ...
  begin
        ...
        for j := 0 to FPanels[i].Items.Count - 1 do
          if FPanels[i].Items[j].Visible then
            if FPanels[i].VariableItemHeight then
              ...
              Inc(h, FPanels[i].ScaledItemHeight);
    end;
    ...
    begin
      ...
      FScrollBar.Width := GetSystemMetrics(SM_CXVSCROLL);
    ...
    for i := Panel.TopItem to Item.Index - 1 do
      if Panel.Items[i].Visible then
        if Panel.VariableItemHeight then
          ...
          Inc(tempHeight, Panel.ScaledItemHeight);

    i := (((GetNumbervisiblePanelUp(getRealTopPanel)) + 1) * ScaledCaptionHeight) + tempHeight;

    if Panel.VariableItemHeight then
      j := i + item.Height
    else
      j := i + Panel.ScaledItemHeight;
    ...
    begin
        ...
        begin
          ...
          for m := 1 to Item.Index do
            if FPanels.Items[i - 1].Items[m - 1].Visible then
              if FPanels.Items[i - 1].VariableItemHeight then
                ...
                Inc(j, FPanels.Items[i - 1].ScaledItemHeight);
          ...
          begin
            if FPanels.Items[i - 1].VariableItemHeight then
              ...
              tempHeight := FPanels.Items[i - 1].ScaledItemHeight;
      ...
      //for open panel
          ...
          if i = FLastOpen + 1 then
            j := Height - (FPanels.Count - FLastOpen - 1) * ScaledCaptionHeight;
        end
        else
      //for close panel
          Inc(j, ScaledCaptionHeight);
  ...
  case InspectorItem.PropertyType of
    ...
    ptIntSpin:
      begin
        ...
        FInspectorSpin.Font.Height := Round(FDPIScale * FInspectorSpin.Font.Height);
    ...
    ptInteger, ptFloat, ptText, ptPassword:
      begin
        ...
        FInspectorEdit.Font.Height := Round(FDPIScale * FInspectorEdit.Font.Height);
        ...
        EditRect := Rect(2, 1, FInspectorEdit.Width, Round(FDPIscale * FInspectorEdit.Height));
    ...
    ptTextButton, ptPropButton:
      begin
        ...
        FInspectorEditBtn.Font.Height := Round(FDPIScale * FInspectorEditBtn.Font.Height);
    ...
    ptFont:
      begin
        ...
        if InspectorItem.Modified then
          FInspectorEditBtn.Font.Color := InspectorItem.InspectorPanel.ModifiedFont.Color;
    ...
    ptValues, ptValuesList:
      begin
        ...
        FInspectorCombo.Width := EditRect.Right - EditRect.Left + 2;
        ...
        FInspectorCombo.ItemHeight := EditRect.Bottom - EditRect.Top - 6;
    ...
    ptColor, ptFixedColor:
      begin
        ...
        FInspectorColorCombo.Font.Height := Round(FDPIScale * FInspectorColorCombo.Font.Height);
    ...
    ptDate, ptTime:
      begin
        ...
        if InspectorItem.Modified then
          ...
          FInspectorDateTimePicker.Font.Height := Round(FDPIScale * FInspectorDateTimePicker.Font.Height);
...
begin
  ...
  begin
    ...
    if i <= realTopPanel then
      ...
      j := Height - ((GetNumbervisiblePanelDown(i) + 1) * ScaledCaptionHeight);
  ...
  j := (getNumbervisiblePanelUp(realTopPanel) + 1) * ScaledCaptionHeight; //Click Up
  i := Height - ((getNumbervisiblePanelDown(realTopPanel)) * ScaledCaptionHeight); //Click Down
  ...
  begin
    ...
    begin
      ...
      if tempPanel.VariableItemHeight then
        ...
        Inc(tempheight, tempPanel.ScaledItemHeight);
      if tempheight > y then
        Break;
    end;
    if k < tempPanel.Items.Count then
...
begin
  ...
  Result := (y < (ys + 1) * ScaledCaptionHeight)
    or (y > Height - (yj) * ScaledCaptionHeight);
end;
...
begin
  ...
  //Items
    ...
    else inc(j, ScaledCaptionHeight); //for close panel
  end;
end;
        ...
        for k := 0 to FPanels[i].Items.Count - 1 do
          if FPanels[i].Items[k].Visible then
            if FPanels[i].VariableItemHeight then
              ...
              Inc(tempHeight, FPanels[i].ScaledItemHeight);
  ...
  //Items
        ...
        begin
          ...
          if FPanels[i].VariableItemHeight then
            ...
            tempHeight := Panels[i].ScaledItemHeight;
...
begin
  ...
  begin
    RealTopPanel := getRealTopPanel;
    if RealTopPanel = -1 then exit;
    j := (getNumbervisiblePanelUp(RealTopPanel) + 1) * ScaledCaptionHeight;
    i := Height - (getNumbervisiblePanelDown(RealTopPanel)) * ScaledCaptionHeight;
...
function TInspectorBar.GetItemByName(AName: string): TInspectorItem;
var
  i: integer;
  it: TInspectorItem;
begin
...
begin
  ...
  begin
    RealTopPanel := GetRealTopPAnel;
    if Panel.Index <= RealTopPanel then
      j := (GetNumbervisiblePanelUp(Panel.Index)) * ScaledCaptionHeight
    else
      j := Height - ((GetNumbervisiblePanelDown(Panel.Index) + 1) * ScaledCaptionHeight);
    Result := Rect(0, j, Width, j + ScaledCaptionHeight);
  end
  else
  begin
    if FScrollBar.Visible then j := -FscrollBar.Position
...
{$IFNDEF DELPHIXE10_LVL}
procedure TInspectorBar.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TInspectorBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Panels[i - 1].Control.Top := (getNumbervisiblePanelUp(RealTopPanel) + 1) * ScaledCaptionHeight;
          Panels[i - 1].Control.Height := BarHeight - ((getNumbervisiblePanelDown(0) + 1) * ScaledCaptionHeight); //All panles =>(down(0)+1)
          ...
          for k := 1 to Panels[i - 1].Items.Count do
            if Panels[i - 1].Items[k - 1].Visible then
              if Panels[i - 1].VariableItemHeight then
                ...
                Inc(tempHeight, Panels[i - 1].ScaledItemHeight);
...
begin
  ...
  if not State then
  begin
    Canvas.StretchDraw(Rect(X + 4, Y + 4, X + 4 + Round(8 * FDPIScale), Y + 4 + Round(8 * FDPIScale)), FNodeCloseGlyph);
   // Canvas.Draw(X + 4, Y + 4, FNodeCloseGlyph);
  end
  else
  begin
    Canvas.StretchDraw(Rect(X + 4, Y + 4, X + 4 + Round(8 * FDPIScale), Y + 4 + Round(8 * FDPIScale)), FNodeOpenGlyph);
...
function TInspectorBar.DoMouseWheelDown(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
  if (TopPanel >= 0) and (TopPanel < Panels.Count) then
  begin
    Panels[TopPanel].TopItem :=  Panels[TopPanel].TopItem + 1;
    Invalidate;
  end;
...
function TInspectorBar.DoMouseWheelUp(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
  if (TopPanel >= 0) and (TopPanel < Panels.Count) then
  begin
...
procedure TInspectorBar.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
  clrPanelBG: TColor;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    {$ENDIF}
      ...
      StyleServices.GetElementColor(lDetails, ecTextColor, clrFont);
      Font.Color := clrFont;

      PanelCaption.ActiveFont.Color := StyleServices.GetSystemColor(clHighlight);

      StyleServices.GetElementColor(StyleServices.GetElementDetails(tgCellNormal), ecTextColor, clr);
      PanelCaption.Font.Color := clr;
      for i := 0 to Panels.Count-1 do
      begin
        Panels[i].Font.Color := clr; // when value is a button, and text color of that button
        Panels[i].EditFontColor := clr; // text color in Value column
        Panels[i].CaptionFont.Color := clr; // text color in Name column
      end;
      HelpPanel.Font.Color := clr;
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF}
    ...
    {$ENDIF}
      ...
      Color := clr; // background of whole component
      HelpPanel.Color := clr; // background of help panel
      clrPanelBG := clr;

      StyleServices.GetElementColor(StyleServices.GetElementDetails(tgFixedCellNormal), ecFillColor, clr);
      PanelCaption.Color := clr; // panel header background color

      StyleServices.GetElementColor(StyleServices.GetElementDetails(tgCellNormal), ecBorderColor, clr);
      clrPanelGrid := clr;
      for i := 0 to Panels.Count-1 do
      begin
        Panels[i].Color := clrPanelBG; // background color of Name & Value columns. Excludes panel header though.
        Panels[i].EditColor := clrPanelBG;

        if Panels[i].CaptionColor <> clNone then
          Panels[i].CaptionColor := clrPanelBG;

        Panels[i].GridLineColor := clrPanelGrid;
        if Panels[i].EditBorderColor <> clNone then
          Panels[i].EditBorderColor := clrPanelGrid;
      end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TInspectorBar.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if mode = imSinglePanelActive then
      begin
        h := BarHeight - ((getNumbervisiblePanelDown(getRealTopPanel)) * ScaledCaptionHeight);
