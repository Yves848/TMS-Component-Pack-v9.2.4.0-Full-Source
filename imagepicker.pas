...
const
  ...
  // 1.1.3.0 : New : Per monitor support for high DPI
...
begin
  ...
  dr.Left := dr.Left + Round(FDPIScale * 2);
  ...
  begin
      FImages.Draw(Canvas, dr.Left, dr.top + ((dr.Bottom - dr.Top) - Images.Width ) div 2, imgidx);
  end;
...
begin
  FEditHeight := Value;
  SendMessage(Handle, CB_SETITEMHEIGHT, -1, Round(FDPIScale * Value));
  SendMessage(Handle, CB_SETITEMHEIGHT, 0, Round(FDPIScale * Value));
end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TImagePicker.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TImagePicker.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
  SetEditHeight(FEditHeight);
end;
...
begin
  ...
  CBN_DROPDOWN:
    begin
      MoveWindow(self.Handle, self.Left, self.Top, width, Round(FDPIScale * (EditHeight + FDropheight)),True);
      DropDown;
      FDropped := True;
