...
uses
  ...
  ActnList, SysUtils, CommCtrl, AdvGDIP, Types, AeroRes
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes, pngimage
  {$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      if FindResource(HInstance,PChar(BitBtnResNames[Kind]),RT_RCDATA) <> 0 then
      begin
        Stream := TResourceStream.Create(HInstance, BitBtnResNames[Kind], RT_RCDATA);
        pic := TPNGImage.Create;
        BitBtnGlyphs[Kind] := TBitmap.Create;
        try
          BitBtnGlyphs[Kind].PixelFormat := pf32bit;
          BitBtnGlyphs[Kind].Transparent := true;
          pic.LoadFromStream(Stream);
          BitBtnGlyphs[Kind].Width := pic.Width;
          BitBtnGlyphs[Kind].Height := pic.Height;
          BitBtnGlyphs[Kind].Canvas.Draw(0,0,pic);
        finally
          pic.Free;
          Stream.Free;
        end;
