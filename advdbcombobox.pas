...
uses
  AdvCombo, Windows, Classes, StdCtrls, SysUtils, Graphics, DB, DBCtrls,
  Messages, Controls, ImgList;
...
const
  ...
  // 1.0.4.0 : Improved : Control can be used without datasource assigned as non DB control
  // 1.0.4.1 : Improved : Internal stringlist handling
  // 1.0.4.2 : Fixed : Issue with updating items
  // 1.0.5.0 : Improved : Behavior on high DPI

type
  TAdvDBComboBoxStrings = class(TCustomComboBoxStrings)
  private
    FLastDeleted: string;
    FLastDeletedIndex: integer;
    function GetStoredStrings(Index: Integer): String;
    procedure PutStoredStrings(Index: Integer; const Value: String);
    procedure InsertStoredStrings(Index: Integer; const Value: String);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        x := (Rect.Right - Rect.Left - Images.Width) div 2;
        y := (Rect.Bottom - Rect.Top - Images.Height) div 2;

        if e = 0 then
          Images.Draw(Canvas, Rect.Left + 2 + x, Rect.Top + 1 + y, i);
      end
      else
...
begin
  ...
  begin
    FLastDeleted := FStoredStrings[Index];
    FLastDeletedIndex := Index;
...
begin
  ...
  begin
    if FLastDeletedIndex <> -1 then
      FStoredStrings.Insert(Index,FLastDeleted)
    else
      FStoredStrings.Insert(Index,'');
