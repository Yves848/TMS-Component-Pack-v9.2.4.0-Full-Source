...
const
  ...
  // v2.9.2.11: Fixed : Issue with setting database cursor
  // v2.9.2.12: Fixed : Issue with slow updating when applying style in combination with many items
  // v2.9.2.13: Fixed : Issue with transparency on double buffered forms
  // v2.9.2.14: Fixed : Access violation clicking on item in certain circumstances
  // v2.9.2.15: Fixed : Issue with calculation of caption under specific font settings
  // v2.9.2.16: Fixed : Issue with focusing a disabled listbox
  // v2.9.2.17: Fixed : Issue with leading spaces for lookup bar initialization
  // v2.9.2.18: New : Select parameter added for the ScrollToItem() method
  // v2.9.2.19: Improved : Item font persistence
  // v2.9.2.20: Fixed : Issue with focused item index when dragging & dropping an item
  // v2.9.3.0 : New : Property to turn off mouse wheel scrolling
  // v3.0.0.0 : New : VCL Styles support
  // v3.0.1.0 : New : StyleElements property added
  // v3.0.1.1 : Improved : Delphi XE6 and newer handling of StyleElements
...
type
  ...
  private
    ...
    function IsInfoFontStored: Boolean;
    function IsNotesFontStored: Boolean;
    function IsNotesSelectedFontStored: Boolean;
    function IsCaptionSelectedFontStored: Boolean;
    function IsInfoSelectedFontStored: Boolean;
  protected
    FSelected: Boolean;
    procedure Changed;
  ...
  published
    ...
    property CaptionFont: TFont read FCaptionFont write SetCaptionFont stored IsCaptionFontStored;
    property CaptionSelectedFont: TFont read FCaptionSelectedFont write SetCaptionSelectedFont stored IsCaptionSelectedFontStored;
    property InfoFont: TFont read FInfoFont write SetInfoFont stored IsInfoFontStored;
    property InfoSelectedFont: TFont read FInfoSelectedFont write SetInfoSelectedFont stored IsInfoSelectedFontStored;
    property NotesFont: TFont read FNotesFont write SetNotesFont stored IsNotesFontStored;
    property NotesSelectedFont: TFont read FNotesSelectedFont write SetNotesSelectedFont stored IsNotesSelectedFontStored;
  ...
  public
    ...
    procedure ScrollToItem(ItemIndex: integer; Select: boolean = true);
...
begin
  ...
  if item <> -1 then
  begin
    if Assigned(FDisplayList.GetItem(item).DisplayItem) then
    begin
...
procedure TAdvSmoothListBox.ScrollToItem(ItemIndex: integer; Select: boolean = true);
var
  i, sh: integer;
begin
  if (ItemIndex >= 0) and (ItemIndex <= Items.Count - 1) then
...
begin
  if Assigned(Filter.Edit) and Filter.Edit.HandleAllocated then
    Filter.Edit.Clear;
end;

{$IFDEF DELPHIXE2_LVL}

procedure TAdvSmoothListBox.CMStyleChanged(var Message: TMessage);
begin
  inherited;
  {$IFDEF DELPHIXE6_LVL}
  if (seClient in StyleElements) then
...
function TAdvSmoothListBoxItem.IsCaptionFontStored: Boolean;
begin
  Result := (CaptionFont.Name <> 'Tahoma') or (CaptionFont.Height <> -11) or (CaptionFont.Style <> []) or (ColorToRGB(CaptionFont.Color) <> clBlack);
end;

function TAdvSmoothListBoxItem.IsCaptionSelectedFontStored: Boolean;
begin
  Result := (CaptionSelectedFont.Name <> 'Tahoma') or (CaptionSelectedFont.Height <> -11) or (CaptionSelectedFont.Style <> []) or (ColorToRGB(CaptionSelectedFont.Color) <> clBlack);
end;
...
function TAdvSmoothListBoxItem.IsInfoFontStored: Boolean;
begin
  Result := (InfoFont.Name <> 'Tahoma') or (InfoFont.Height <> -11) or (InfoFont.Style <> []) or (ColorToRGB(InfoFont.Color) <> clBlack);
end;

function TAdvSmoothListBoxItem.IsInfoSelectedFontStored: Boolean;
begin
  Result := (InfoSelectedFont.Name <> 'Tahoma') or (InfoSelectedFont.Height <> -11) or (InfoSelectedFont.Style <> []) or (ColorToRGB(InfoSelectedFont.Color) <> clBlack);
end;

function TAdvSmoothListBoxItem.IsNotesFontStored: Boolean;
begin
  Result := (NotesFont.Name <> 'Tahoma') or (NotesFont.Height <> -11) or (NotesFont.Style <> []) or (ColorToRGB(NotesFont.Color) <> clBlack);
end;

function TAdvSmoothListBoxItem.IsNotesSelectedFontStored: Boolean;
begin
  Result := (NotesSelectedFont.Name <> 'Tahoma') or (NotesSelectedFont.Height <> -11) or (NotesSelectedFont.Style <> []) or (ColorToRGB(NotesSelectedFont.Color) <> clBlack);
end;
