...
const
  ...
  // 1.3.3.8 : Fixed : Issue with hover and transparent
  // 1.4.0.0 : New : VCL Styles support
...
type
  ...
  private
    ...
    procedure DoPaint(ACanvas: TCanvas; bkg: Boolean);
  ...
  {$IFDEF DELPHIXE2_LVL}
  TParamLabelStyleHook = class(TStaticTextStyleHook)
  strict protected
...
{$WARNINGS OFF}
procedure TParamLabel.DoPaint(ACanvas: TCanvas; bkg: boolean);
var
  r,mr,cr: TRect;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      FHC  := StyleServices.GetSystemColor(clHighlight);
      FHFC := StyleServices.GetSystemColor(clHighlightText);
      {$ENDIF xe2}
    end
    else
    begin
...
procedure TParamLabel.CreateWnd;
begin
  inherited CreateWnd;
  if not (csDesigning in ComponentState) then
  begin
...
procedure TParamLabel.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF xe2}
begin
  ...
  begin
    ...
    {$ENDIF xe6}
      ...
      ParamColor := StyleServices.GetSystemColor(clHighlight);
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF xe6}
  end;
...
procedure TParamLabel.InvalidateCtrl;
var
  r: TRect;
begin
  Invalidate;
...
{$IFDEF DELPHIXE2_LVL}
procedure TParamLabel.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
{ TParamLabelStyleHook }
procedure TParamLabelStyleHook.Paint(Canvas: TCanvas);
var
 c: TParamLabel;
begin
  inherited Paint(Canvas);
  c := TParamLabel(Control);
  { now delegate painting to the original component's DoPaint() }
  c.DoPaint(Canvas,true);
end;

initialization
  TCustomStyleEngine.RegisterStyleHook(TParamLabel, TParamLabelStyleHook);

finalization
  TCustomStyleEngine.UnRegisterStyleHook(TParamLabel, TParamLabelStyleHook);
{$ENDIF}
