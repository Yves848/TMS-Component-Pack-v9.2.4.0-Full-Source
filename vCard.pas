...
const
  ...
  // v1.1.0.8 : Improved : Handling of encoding during VCard import
  // v1.1.1.0 : Improved : Handling of encoded text
...
function QuotedPrintableDecode(todecode: string; isquoted: boolean = true): string;
...
begin
  ...
  try
    ...
    sl.LoadFromStream(Stream, TEncoding.Unicode);
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if vp > 0 then
        begin
          vContact.LastName := QuotedPrintableDecode(Copy(s, 1, vp - 1), isquot);
          Delete(s, 1, vp);

          vp := Pos(';', s);
          if vp > 0 then
          begin
            vContact.FirstName := QuotedPrintableDecode(Copy(s, 1, vp - 1), isquot);
            Delete(s, 1, vp);
            vp := Pos(';', s);
            if vp > 0 then
            begin
              vContact.MiddleName := QuotedPrintableDecode(Copy(s, 1, vp - 1), isquot);
            end;
          end;
        end;
      end;
      ...
      begin
        ...
        isquot := pos('=QUOTED', s) > 0;

        if Pos(':',s) > 0 then
          Delete(s, 2, Pos(':',s) - 2);
      end;

      if (Pos('FN:', s) = 1) and Assigned(vContact) then
      begin
        vContact.FullName := QuotedPrintableDecode(GetPropVal(s), isquot);
        LastProp := '';
      end;
      ...
      begin
        ...
        begin
          ...
          str := QuotedPrintableDecode(str, isquot);
...
begin
  ...
  begin
    ...
    begin
      if todecode[i] <> '=' then
        Result := Result + todecode[i]
      else
      begin
        if i + 2 <= Length(todecode) then
        begin
          j := HexToInt(todecode[i + 1] + todecode[i + 2]);
          Result := Result + chr(j);
          inc(i, 2);
        end;
      end;
...
begin
  ...
  begin
    Result := Result + 'ADR;CHARSET=Windows-1252;ENCODING=QUOTED-PRINTABLE';
    ...
    Result := Result + ':' + vContact.Addresses[i].FPOBox + ';' +
      vContact.Addresses[i].Number + ';' + QuotedPrintableEncode(vContact.Addresses[i].Street) + ';' +
      vContact.Addresses[i].City + ';' + QuotedPrintableEncode(vContact.Addresses[i].Region) + ';' +
      vContact.Addresses[i].PostCode + ';' + QuotedPrintableEncode(vContact.Addresses[i].Country) + lf;
