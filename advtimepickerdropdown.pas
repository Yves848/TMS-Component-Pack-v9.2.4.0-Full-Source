...
const
  ...
  // v1.2.1.1 : Fixed : Design-time issue with changed validation routine
...
begin
  ...
  FInternalCall := true;
  InitEditMask;
  SetTextDirect('00' + FormatSettings.TimeSeparator + '00' + FormatSettings.TimeSeparator + '00');
  FInternalCall := false;
...
procedure TAdvTimePickerDropDown.DoEnter;
begin
  inherited;
end;
...
begin
  ...
  VK_DOWN:
    begin
      ...
      if (ss < pos(FormatSettings.TimeSeparator, s1)) then
      begin
        i := h + 1;
        if (i >= 24) then
          i := 0;
      ...
      begin
        ss := ss - pos(FormatSettings.TimeSeparator, s1);
        Delete(s1, 1, pos(FormatSettings.TimeSeparator, s1));
        if (ss < pos(FormatSettings.TimeSeparator,s1)) or not ShowSeconds then
        begin
          i := m + 1;
          if (i >= 60) then
            i := 0;
...
function TAdvTimePickerDropDown.Validate(const Value: string;
...
begin
  if FShowSeconds then
    EditMask := '!90' + FormatSettings.TimeSeparator + '00' + FormatSettings.TimeSeparator + '00;1; '
  else
    EditMask := '!90' + FormatSettings.TimeSeparator + '00;1; ';
end;
...
begin
  ...
  begin
    ...
    st := Trim(Copy(Txt, 1, Pos(FormatSettings.TimeSeparator, Txt)-1));
    if not IsValidNum(st) then
      Exit;
    if (st <> '') then
      h := StrToInt(st)
    ...
    st := Trim(copy(Txt, Pos(FormatSettings.TimeSeparator, Txt)+1, Length(Txt)));
    if ShowSeconds then
      st := Trim(copy(st, 1, Pos(FormatSettings.TimeSeparator, st)-1));
    if not IsValidNum(st) then
      Exit;
    if (st <> '') then
      m := StrToInt(st)
    ...
    begin
      st := Trim(Copy(Txt, Pos(FormatSettings.TimeSeparator, Txt) + 1, Length(Txt)));
      st := Trim(Copy(st, Pos(FormatSettings.TimeSeparator, st) + 1, Length(st)));
...
begin
  if ShowSeconds then
    Result := GetTwoDigitString(h) + FormatSettings.TimeSeparator + GetTwoDigitString(m) + FormatSettings.TimeSeparator + GetTwoDigitString(s)
  else
    Result := GetTwoDigitString(h) + FormatSettings.TimeSeparator + GetTwoDigitString(m);
end;
