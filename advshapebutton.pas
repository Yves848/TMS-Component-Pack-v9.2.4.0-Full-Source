...
const
  ...
  // 6.1.1.0 : Improved : Application menu button can now also display a glyph when in Office 2013, Office 2016 modes
  // 6.2.0.0 : New : Layout settings plPictureLeft, plPictureRight added
  //         : New : Properties Default / Cancel added
  // 6.2.1.0 : New : Support for multimonitor High DPI handling
  // 6.2.1.1 : Improved : Positioning of menu frame
  // 6.2.1.2 : Fixed : Issue with frame height for specific styles
  // 6.2.1.3 : Improved : Menu frame positioning for Office2016 style apps
  // 6.2.1.4 : Fixed : Issue with hot picture handling on mouse over
  // 6.2.1.5 : Fixed : High DPI handling for TAdvShapeButton size
...
type
  ...
  TShapeButtonLayout = (plPictureCenter, plPictureOnTop, plPictureAtBottom, plPictureLeft, plPictureRight);
  ...
  public
    ...
    destructor Destroy; override;
    property ITones: boolean read FITones write FITones;
  end;
...
procedure TAdvCustomShapeButton.CreateWnd;
begin
  inherited;
  FActive := FDefault;
end;
...
procedure TAdvCustomShapeButton.CMDialogKey(var Message: TCMDialogKey);
begin
  with Message do
    if
...
begin
  ...
  if (Down <> FInitialDown) then
    Down := FInitialDown;
  FTBSize := Round(Width/ADVToolBarDPI_Scale);
end;
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHI_UNICODE}
    if (Parent is TCustomForm) and (Appearance.Shape <> bsRectangle) then
    {$ENDIF}
...
begin
  ...
  dpi_scale := CalculateDPIScale(Self);

  if not Transparent then
  begin
    clr := AAppearance.Color;

    if ((FMouseDownInControl and FMouseInControl) or (Down) or FMenuBeingShown) then
  ...
  begin
    ...
    x := (Width - Round(Pic.Width * dpi_scale)) div 2;
    y := (Height - Round(Pic.Height * dpi_scale)) div 2;
    ...
    case Layout of
      ...
      begin
        x := 3;
        TextR := R;
        TextR.Left := TextR.Left + Round(Pic.Width * dpi_scale) + 3 + TextOffset;
      end;
      plPictureRight:
      begin
        x := Width - 3 - Round(Pic.Width * dpi_scale);
        TextR := R;
        TextR.Right := TextR.Right - 3 - Round(Pic.Width * dpi_scale) - TextOffset;
      end;
      plPictureCenter:
      begin
        ;  // do nothing
    ...
    if not FIsAeroVista then
    begin
      if AAppearance.ShowPicture {or (pic.Height <= 24)} then
      begin
        ACanvas.StretchDraw(Rect(x,y,x+round(Pic.Width*dpi_scale),y+round(pic.Height*dpi_scale)),Pic);
      end;
    end
    else
    begin
...
procedure TAdvCustomShapeButton.Keypress(var Key: char);
begin
  inherited;
...
//    Result := (Parent as TAdvToolBarPager).Parent.Height + 2 - (Height + c + Top {GetSystemMetrics(SM_CYCAPTION)} + szf * 2 + bw * 2);
  ...
  begin
    if GetParentForm(Self).BorderStyle <> bsSizeable then
      Result := (Parent as TAdvToolBarPager).Parent.Height - (GetMyTop + Height) - 1
    else
      Result := (Parent as TAdvToolBarPager).Parent.ClientHeight - (GetMyTop + Height) {-1 - szf};
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FMenuFrame.ITones := FITones or FOffice2013;
...
begin
  with Message do
    if Sender is TAdvShapeButton then
      FActive := Sender = Self
    else
      FActive := FDefault;
