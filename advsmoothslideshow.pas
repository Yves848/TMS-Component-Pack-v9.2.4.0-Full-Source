...
const
  ...
  // v1.4.2.1 : Fixed : Issue retrieving correct image size when loading gif images
  // v1.4.3.0 : Improved : Support for High DPI
...
begin
  ...
  begin
    ...
    ft := g.MakeFont(DescriptionFont, Round(DescriptionFont.Size * DPIScale));
...
begin
  ...
  spc := Round(DPIScale * PlaceHolderSpacing);
  spcthumb := Round(DPIScale * ThumbNailSpacing);
  spcholder := 0;
  if ThumbNails then
  begin
    case ThumbNailPosition of
      tpRight, tpLeft:
        spcholder := Round(DPIScale * ThumbNailHolderMargin.Left) + Round(DPIScale * ThumbNailHolderMargin.Right) +
          spcthumb * 2 + Round(DPIScale * ThumbNailWidth);
      tpBottom, tpTop:
        spcholder := Round(DPIScale * ThumbNailHolderMargin.Top) + Round(DPIScale * ThumbNailHolderMargin.Bottom) +
          spcthumb * 2 + Round(DPIScale * ThumbNailHeight);
    end;
  end;

  ML := Round(DPIScale * Margin.Left);
  MR := Round(DPIScale * Margin.Right);
  MT := Round(DPIScale * Margin.Top);
  MB := Round(DPIScale * Margin.Bottom);

  if not ThumbNailsFloating then
  begin
    case ThumbNailPosition of
      tpLeft:
        R := MakeRect(ML + spc + spcholder, MT + spc,
          Width - MR - ML - spcholder - spc * 2,
          Height - MB - MT - spc * 2);
      tpTop:
        R := MakeRect(ML + spc, MT + spcholder + spc,
          Width - MR - ML - spc * 2,
          Height - MB - MT - spcholder - spc * 2);
      tpRight:
        R := MakeRect(ML + spc, MT + spc,
          Width - MR - spcholder - ML - spc * 2,
          Height - MB - MT - spc * 2);
      tpBottom:
        R := MakeRect(ML + spc, MT + spc,
          Width - MR - ML - spc * 2,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FThumbItemRect := MakeRect(tx, ty, Round(DPIScale * ThumbNailWidth), Round(DPIScale * ThumbNailHeight));
        case ThumbNailPosition of
          tpLeft, tpRight:
            ty := ty + Round(DPIScale * ThumbNailHeight) + spcthumb;
          tpBottom, tpTop:
            tx := tx + Round(DPIScale * ThumbNailWidth) + spcthumb;
        end;

        if Assigned(FThumbnail) and ThumbNails then
        begin
          FThumbRect := MakeRect(FThumbItemRect.X + 3 +
              (FThumbItemRect.Width - Round(DPIScale * FThumbnail.Width) - 3) / 2,
            FThumbItemRect.Y + 3 +
              (FThumbItemRect.Height - Round(DPIScale * FThumbnail.Height) - 3) / 2,
            Round(DPIScale * FThumbnail.Width) - 3, Round(DPIScale * FThumbnail.Height) - 3);
        end
        else
  ...
  case ThumbNailPosition of
    tpRight, tpLeft:
      MaxScrollPos := max((spcthumb * (cnt + 1)) +
          (Round(DPIScale * ThumbNailHeight * cnt)) - rthumb.Height, 0);
    tpBottom, tpTop:
      MaxScrollPos := max((spcthumb * (cnt + 1)) +
          (Round(DPIScale * ThumbNailWidth * cnt)) - rthumb.Width, 0);
  end;
end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  R := GetThumbNailRect;
  result := MakeRect(R.X - Round(DPIScale * ThumbNailHandles.Size) / 2, R.Y +
      (R.Height - Round(DPIScale * ThumbNailHandles.Size)) / 2, Round(DPIScale * ThumbNailHandles.Size),
    Round(DPIScale * ThumbNailHandles.Size));
end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  R := GetThumbNailRect;
  result := MakeRect(R.X + (R.Width - Round(DPIScale * ThumbNailHandles.Size) / 2) - 1, R.Y +
      (R.Height - Round(DPIScale * ThumbNailHandles.Size)) / 2, Round(DPIScale * ThumbNailHandles.Size),
    Round(DPIScale * ThumbNailHandles.Size));
end;
...
begin
  DPIScale := GetDPIScale(Self, Canvas);
  R := GetThumbNailRect;
  result := MakeRect(R.X + (R.Width - Round(DPIScale * ThumbNailHandles.Size)) / 2,
    R.Y + R.Height - 1 - Round(DPIScale * ThumbNailHandles.Size) / 2, Round(DPIScale * ThumbNailHandles.Size),
    Round(DPIScale * ThumbNailHandles.Size));
end;
...
begin
  ...
  R := GetThumbNailRect;
  result := MakeRect(R.X + (R.Width - Round(DPIScale * ThumbNailHandles.Size)) / 2,
    R.Y - Round(DPIScale * ThumbNailHandles.Size) / 2, Round(DPIScale * ThumbNailHandles.Size),
    Round(DPIScale * ThumbNailHandles.Size));
end;
...
var
  ...
  DPIScale: single;
  ML, MR, MB, MT: integer;
  tnhML, tnhMT, tnhMB, tnhMR: integer;
  tnH,tnW: integer;
begin
  ...
  MT := Round(DPIScale * Margin.Top);
  MB := Round(DPIScale * Margin.Bottom);
  tnhML := Round(DPIScale * ThumbNailHolderMargin.Left);
  tnhMR := Round(DPIScale * ThumbNailHolderMargin.Right);
  tnhMT := Round(DPIScale * ThumbNailHolderMargin.Top);
  tnhMB := Round(DPIScale * ThumbNailHolderMargin.Bottom);
  tnH := Round(DPIScale * ThumbNailHeight);
  tnW := Round(DPIScale * ThumbNailWidth);

  case ThumbNailPosition of
    tpLeft:
      result := MakeRect(ML + tnhML, MT + tnhMT, spcthumb * 2 + tnW, Height - tnhMB - tnhMT - MT - MB);
    tpTop:
      result := MakeRect(ML + tnhML, MT + tnhMT, Width - tnhMR - tnhML - ML - MR, spcthumb * 2 + tnH);
    tpRight:
      result := MakeRect(Width - MR - tnhMR - spcthumb * 2 - tnW, MT + tnhMT, spcthumb * 2 + tnW, Height - tnhMB - tnhMT - MT - MB);
    tpBottom:
      result := MakeRect(ML + tnhML, Height - MB - tnhMB - spcthumb * 2 - tnH, Width - tnhMR - tnhML - ML - MR, spcthumb * 2 + tnH);
  end;
end;
