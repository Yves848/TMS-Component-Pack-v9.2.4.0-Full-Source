{**************************************************************************}
{ TAdvSmoothSpinnerDE DESIGN TIME EDITOR                                   }
...
uses
  Classes, AdvSmoothSpinner, DesignIntf, DesignEditors;

type
  TAdvSmoothSpinnerEditor = class(TDefaultEditor)
  protected
...
procedure TAdvSmoothSpinnerEditor.EditProperty(const PropertyEditor: IProperty; var Continue: Boolean);
var
  PropName: string;
begin
...
procedure TAdvSmoothSpinnerEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothSpinner).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothSpinner) then
         (Component as TAdvSmoothSpinner).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothSpinnerEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothSpinnerEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
