...
const
  ...
  // 1.3.1.0 : Improved : Support for HighDPI
...
procedure DrawGauge(Canvas: TCanvas; R : TRect; Settings : TGaugeSettings; DPIScale: single);
...
var
  ...
  txt: string;
  IPW: integer;
  spaceScaled, sizeScaled: integer;
  
begin
  spaceScaled := Round(2 * DPIScale);
  sizeScaled := Round(7 * DPIScale);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Settings.Orientation = goHorizontal) then
          begin
            RectL.Left := R.Left + spaceScaled;
            RectL.Right := RectL.Left + sizeScaled;
            RectL.Top := R.Top + spaceScaled;
            RectL.Bottom := R.Bottom - spaceScaled;
          end
          else
          begin
            RectL.Left := R.Left + spaceScaled;
            RectL.Right := R.Right - spaceScaled;
            RectL.Bottom := R.Bottom - spaceScaled;
            RectL.Top := RectL.Bottom - sizeScaled;
          end;
          ...
          NumberOfBlock := (Round((Settings.Position * WidthBar) / 100) div (spaceScaled+sizeScaled)) - 1;
          EmptyWidth := Round((Settings.Position * WidthBar) / 100) mod (spaceScaled+sizeScaled);

          for i := 0 to NumberOfBlock - 1 do
          begin
            if (Settings.Orientation = goHorizontal) then
            begin
              RectL.Left := RectL.Right + spaceScaled;
              RectL.Right := RectL.Left + sizeScaled;
            end
            else
            begin
              RectL.Bottom := RectL.Top - spaceScaled;
              RectL.Top := RectL.Bottom - sizeScaled;
            end;
          ...
          if (EmptyWidth >= spaceScaled) then
          begin
            if (Settings.Orientation = goHorizontal) then
            begin
              RectL.Left := RectL.Right + spaceScaled;
              RectL.Right := RectL.Left + (EmptyWidth - spaceScaled);
            end
            else
            begin
              RectL.Bottom := RectL.Top - spaceScaled;
              RectL.Top := Max( R.Top + spaceScaled , RectL.Bottom - (EmptyWidth - spaceScaled));
            end;
      ...
      begin
        ...
        begin
          RectL.Left := R.Left + spaceScaled;
          RectL.Top := R.Top + spaceScaled;
          RectL.Right := RectL.Left + WidthPart - spaceScaled;
          RectL.Bottom := R.Bottom - spaceScaled;
        end
        else
        begin
          RectL.Left := r.Left + spaceScaled;
          RectL.Bottom := R.Bottom - spaceScaled;
          RectL.Top := RectL.Bottom - WidthPart + spaceScaled;
          RectL.Right := r.Right - spaceScaled;
        end;
...
begin
  ...
  Settings.Font.Height := Round (Settings.Font.Height * FDPIScale);
  ...
  try
    bmp.Width := Math.Max(0, r.Right - r.Left);
    bmp.Height := Math.Max(0, r.Bottom - r.Top);

    DrawGauge(bmp.Canvas,R,Settings, FDPIScale);
