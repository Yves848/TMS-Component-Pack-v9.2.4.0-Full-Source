...
begin
  ...
  begin
    ...
    begin
      FDragNode := FDownNode;
      FDragMode := Assigned(FDownNode) and (Interaction.DragDropMode <> tdmNone) and not FDblClicked and not XYToNodeExpand(FDownNode, X, Y) and not Assigned(XYToNodeCheck(FDownNode, X, Y).ANode);
      FReorderMode := Assigned(FDownNode) and Interaction.Reorder and not FDragMode and not FDblClicked and not XYToNodeExpand(FDownNode, X, Y) and not Assigned(XYToNodeCheck(FDownNode, X, Y).ANode);
      if FReorderMode then
      begin
        CreateDragBitmap;
        {$IFDEF FMXLIB}
...
begin
  ...
  if Assigned(c) then
  begin
    if not XYToNodeExpand(c, pf.X, pf.Y) and not Assigned(XYToNodeCheck(c, X, Y).ANode) then
    begin
      FDownNode := c;
      if Assigned(FDownNode) then
      begin
...
begin
  ...
  FTopFontColor := TAdvTreeViewColorBlack;
  FBottomFontColor := TAdvTreeViewColorBlack;
