...
begin
  ...
  begin
    Result := Round(DEFAULT_CAPTIONHEIGHT * GetDPIScale(Self, Canvas));
  end;
end;
...
begin
  ...
  LScale := GetDPIScale(Self, Canvas);

  if HasMinimize or HasMaximize then
  begin
    Result := Result + 2 * Round(DEFAULT_BTNSIZE *LScale);
  end;

  if (biSystemMenu in BorderIcons) then
    Result := Result + Round(DEFAULT_BTNSIZE * LScale);
end;
  ...
  case SysIcon of
    ...
    begin
      ...
      begin
        ...
        Result.Left := Result.Right - Round(DEFAULT_BTNSIZE * LScale);
      end;
    end;
    siMax:
    begin
    ...
    begin
      ...
      if (Appearance.CaptionStyle = csMetro) then
        Result.Top := Result.Top + 2 + (Result.Bottom - Result.Top - Round(DEFAULT_BTNSIZE * LScale)) div 2
      else
        Result.Top := Result.Top + (Result.Bottom - Result.Top - Round(DEFAULT_BTNSIZE * LScale)) div 2;

      Result.Right := Result.Right - 6;
      Result.Left := Result.Right - Round(DEFAULT_BTNSIZE * LScale);
      Result.Bottom := Result.Top + Round(DEFAULT_BTNSIZE * LScale);
    end;
  end;
end;
  ...
  try
    ...
    if Assigned(FImages) and (FImageIndex >= 0) then
      ...
      if not Application.Icon.Empty and Appearance.ShowAppIcon then
        offsetX := Round(24 * LScale);

    bmp.Canvas.Font.Assign(Appearance.CaptionFont);
    bmp.Canvas.Font.Height := Round(bmp.Canvas.Font.Height * LScale);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DrawIconEx(bmp.Canvas.Handle, 4, 4, Application.Icon.Handle, Round(16 * LScale), Round(16 * LScale), 0,0, DI_NORMAL);
        end;
      end;
...
var
  ...
  begin
    ...
    begin
      Pen.Color := Clr;
                     {/}
      MoveTo(R.Left, R.Top + LSize - 1);
      LineTo(R.Left + LSize, R.Top);
      MoveTo(R.Left + 1, R.Top + LSize - 1);
      LineTo(R.Left + LSize - 1, R.Top);
      MoveTo(R.Left + 2, R.Top + LSize - 1);
      LineTo(R.Left + LSize + 1, R.Top);
                     {\}
      MoveTo(R.Left, R.Top + 1);
      LineTo(R.Left + LSize, R.Top + LSize);
      MoveTo(R.Left + 1, R.Top + 1);
      LineTo(R.Left + LSize - 1, R.Top + LSize);
  ...
  begin
    ...
    begin
      ...
      LineTo(R.Left + LSize + 1 +x, R.Top+y);
      MoveTo(R.Left+x, R.Top+1+y);
      LineTo(R.Left + LSize + 1 +x, R.Top+1+y);
      MoveTo(R.Left+x, R.Top + 2+y);
      LineTo(R.Left + LSize + 1 +x, R.Top + 2+y);
      MoveTo(R.Left+x, R.Top + 2+y);
      LineTo(R.Left+x, R.Top + LSize+y);
      LineTo(R.Left + LSize+x, R.Top + LSize+y);
      LineTo(R.Left + LSize+x, R.Top + 2+y);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      LineTo(R.Left + LSize, R.Top);
      MoveTo(R.Left + 2, R.Top + 1);
      LineTo(R.Left + LSize, R.Top + 1);
      MoveTo(R.Left + LSize, R.Top);
      LineTo(R.Left + LSize, R.Top + 6);

      MoveTo(R.Left, R.Top + 3);
      LineTo(R.Left + LSize -1, R.Top + 3);
      MoveTo(R.Left, R.Top + LSize div 2);
      LineTo(R.Left + 7, R.Top + LSize div 2);
      MoveTo(R.Left, R.Top + LSize div 2);
      LineTo(R.Left, R.Top + Lsize);
      LineTo(R.Left + 6, R.Top + LSize);
      LineTo(R.Left + 6, R.Top + LSize div 2);
    end;
...
begin
  ...
  begin
    ...
    R.Left := R.Left + (R.Right - R.Left - LSize) div 2;
    DrawMinimize(R, Clr);
  end;
  ...
  begin
    ...
    R.Top := R.Top + (R.Bottom - R.Top - LSize) div 2;
    R.Left := R.Left + (R.Right - R.Left - LSize) div 2;
    if (WindowState = wsMaximized) then
      DrawMaximize(R, Clr)
    else
