...
const
  ...
  // 1.6.4.0  : New : Support for multimonitor High DPI handling
  // 1.6.4.1  : Improved : Handling for high DPI with Form.Scaled = false
...
type
 ...
 protected
  ...
  {$IFDEF DELPHIXE6_LVL}
  THTMLButtonStyleHook = class(TButtonStyleHook)
  protected
    procedure DrawButton(ACanvas: TCanvas; AMouseInControl: Boolean); override;
  end;

  THTMLCheckBoxStyleHook = class(TCheckBoxStyleHook)
  protected
    procedure Paint(Canvas: TCanvas); override;
  end;

  THTMLRadioButtonStyleHook = class(TRadioButtonStyleHook)
  protected
    procedure Paint(Canvas: TCanvas); override;
  end;
  {$ENDIF}
...
begin
  inherited;
  FScaleFactor := CalculateDPIScale(Self);
...
begin
  ...
  if HTMLDrawEx(Canvas,s,r,FImages,x,y,-1,-1,FShadowOffset,True,False,False,False,False,False,not FEllipsis,FFormScaled,1.0,FURLColor,
                clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode) then
    Result := Anchor;
end;
...
procedure THTMLButton.CreateWnd;
var
  frm: TCustomForm;
begin
  inherited CreateWnd;
...
begin
  ...
  HTMLDrawEx(FCanvas,s,R,FImages,0,0,-1,-1,FShadowOffset,False,True,False,False,False,False,not FEllipsis,FFormScaled,1.0,clBlue,
                clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode);
  ...
  begin
    ...
    HTMLDrawEx(FCanvas,s,R,FImages,0,0,-1,-1,FShadowOffset,False,False,False,False,False,False,not FEllipsis,FFormScaled,1.0,clWhite,
               clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode);

    FCanvas.Font.Color := clGray;
    Offsetrect(r,-1,-1);

    HTMLDrawEx(FCanvas,s,R,FImages,0,0,-1,-1,FShadowOffset,False,False,False,False,False,False,not FEllipsis,FFormScaled,1.0,clGray,
               clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode);

  end
  else
    HTMLDrawEx(FCanvas,s,R,FImages,0,0,-1,-1,FShadowOffset,False,False,False,False,False,False,not FEllipsis,FFormScaled,1.0,clBlue,
              clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode);
...
{ THTMLButtonStyleHook }
{$IFDEF DELPHIXE6_LVL}
procedure THTMLButtonStyleHook.DrawButton(ACanvas: TCanvas; AMouseInControl: Boolean);
var
  c: THTMLButton;
  R, hr: TRect;
  Anchor, Stripped, FocusAnchor: string;
  ...
  clr: TColor;
begin
  c := THTMLButton(Control);

  { Don't call inherited otherwise it renders text via DrawControlText() which
    doesn't support HTML syntax. }
//  inherited DrawButton(ACanvas, AMouseInControl);

  { Find the correct text color based on various states of the button. }
  ...
  begin
    ...
    else if (c.Focused or c.Default) then
  ...
  { Let VCL Styles draw the button }
  StyleServices.DrawElement(ACanvas.Handle, lDetails, R);

  { Set the correct default text color }
  ...
  FImageCache := THTMLPictureCache.Create;
  try
    HTMLDrawEx(ACanvas, Text , R, c.Images, 0,0,-1,-1, c.ShadowOffset,False,False,False,
        False,False,False,not c.Ellipsis, TForm(c.Owner).Scaled,1.0,clBlue,
        clNone,clNone,c.ShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,
        HyperLinks,MouseLink,hr,FImageCache, c.PictureContainer,0, c.BidiMode);
  finally
    FImageCache.Free;
  end;
end;

procedure THTMLCheckBoxStyleHook.Paint(Canvas: TCanvas);
var
  c: THTMLCheckBox;
  lDetails:  TThemedElementDetails;
  R, hr: TRect;
...
//  inherited DrawButton(ACanvas, AMouseInControl);
  ...
  Canvas.Font.Color := StyleServices.GetStyleFontColor(sfCheckBoxTextNormal);

  { Render the HTML text }
  FImageCache := THTMLPictureCache.Create;
  try
    HTMLDrawEx(Canvas, Text , R, c.Images, 0,0,-1,-1, c.ShadowOffset,False,True,False,
        False,False,False,not c.Ellipsis, TForm(c.Owner).Scaled,1.0,clBlue,
        clNone,clNone,c.ShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,
        HyperLinks,MouseLink,hr,FImageCache, c.PictureContainer,0, c.BidiMode);

    R.Top := R.Top + (R.Bottom  - R.Top  - YSize) div 2;

    HTMLDrawEx(Canvas, Text , R, c.Images, 0,0,-1,-1, c.ShadowOffset,False,False,False,
        False,False,False,not c.Ellipsis, TForm(c.Owner).Scaled,1.0,clBlue,
        clNone,clNone,c.ShadowColor,Anchor,Stripped,FocusAnchor,XSize,YSize,
        HyperLinks,MouseLink,hr,FImageCache, c.PictureContainer,0, c.BidiMode);
  finally
...
procedure THTMLRadioButtonStyleHook.Paint(Canvas: TCanvas);
var
  c: THTMLRadioButton;
  lDetails:  TThemedElementDetails;
  R, hr: TRect;
...
{ [Un]RegisterStyleHook here because class contructor/destructor concept is not
  supported by all Delphi versions. }
{$IFDEF DELPHIXE6_LVL}
initialization
  TCustomStyleEngine.RegisterStyleHook(THTMLButton, THTMLButtonStyleHook);
  TCustomStyleEngine.RegisterStyleHook(THTMLCheckBox, THTMLCheckBoxStyleHook);
  TCustomStyleEngine.RegisterStyleHook(THTMLRadioButton, THTMLRadioButtonStyleHook);

finalization
  TCustomStyleEngine.UnRegisterStyleHook(THTMLButton, THTMLButtonStyleHook);
  TCustomStyleEngine.UnRegisterStyleHook(THTMLCheckBox, THTMLCheckBoxStyleHook);
  TCustomStyleEngine.UnRegisterStyleHook(THTMLRadioButton, THTMLRadioButtonStyleHook);
