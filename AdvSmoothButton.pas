...
const
  ...
  // v2.1.1.6 : Fixed : Issue with arrow keys not changing focus
  // v2.1.1.7 : Improved : property TextRendering to change the rendering of the text
  // v2.1.1.8 : Fixed : Issue with font family fallback to Arial if font is not found
  // v2.1.1.9 : Fixed : Issue with setting cursor at runtime
  // v2.1.2.0 : Improved : Support for High DPI
  // v2.1.3.0 : New : Added Drag and Drop properties

type
  TAdvSmoothButton = class;

  TAdvSmoothButtonTextRenderingHint = (btAntiAlias, btAntiAliasGridFit, btClearType);
  ...
  private
    ...
    FInitPause: Integer;
    FTextRendering: TAdvSmoothButtonTextRenderingHint;
    ...
    procedure SetTextRendering(const Value: TAdvSmoothButtonTextRenderingHint);
  protected
  ...
  published
    ...
    property Visible;
    property Enabled;
    property TextRendering: TAdvSmoothButtonTextRenderingHint read FTextRendering write SetTextRendering default btClearType;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothButton.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;

  FAppearance.Rounding := MulDiv(FAppearance.Rounding, M, D);
  Status.Appearance.Font.Height := MulDiv(Status.FAppearance.Font.Height, M, D);
...
//  if FTransparent and not FMouseEnter then
  ...
  begin
    ...
    case TextRendering of
      btAntiAlias: g.SetTextRenderingHint(TextRenderingHintAntiAlias);
      btAntiAliasGridFit: g.SetTextRenderingHint(TextRenderingHintAntiAliasGridFit);
      btClearType: g.SetTextRenderingHint(TextRenderingHintClearTypeGridFit);
    end;
    ...
    FAppearance.Draw(g, Caption, 0, 0, Width, Height, vs, hs, c, FColorDown, BevelColor, fc,
      Shadow, FDown and FMouseInside, Bevel, false, false, rtBoth, Picture, 0, 0, true, picidx, picname, g.GetTextRenderingHint);
    ...
    begin
      ...
      begin
        ...
        y := Status.OffsetTop;
        Appearance.Draw(g, x, y, 0, 0, true,Status.Caption);
      end;
    end;
...
procedure TAdvSmoothButton.SetTextRendering(
  const Value: TAdvSmoothButtonTextRenderingHint);
begin
