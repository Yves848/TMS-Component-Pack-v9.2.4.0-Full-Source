...
const
  ...
  // v1.4.2.0 : New : Support for LINE-HEIGHT attribute for <P> tag
  // v1.4.2.1 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // v1.4.2.2 : Improved : HTML engine more robust against malformed unicode character tags
...
type
  ...
  private
    ...
    procedure DoPaint(bkg: Boolean; const ACanvas: TCanvas = nil);
...
{$IFDEF DELPHIXE2_LVL}
  THTMLStaticTextStyleHook = class(TStaticTextStyleHook)
  strict protected
...
procedure THTMLStaticText.DoPaint(bkg: Boolean; const ACanvas: TCanvas);
var
  R,CR,HR: TRect;
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, s, r, FImages, pt.x, pt.y, fhl, FHoverHyperlink, FShadowOffset, True, True, False, False, FBlinking, FHover, not FEllipsis, FFormScaled, 1.0,
      FURLColor, FHoverColor, FHoverFontColor, FShadowColor, Anchor, Stripped, FocusAnchor, x, y, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);

    if (y >= Height + FVOffset ) then
    begin
      HR := CR;
      HR.Top := CR.Bottom - Round(FDPIScale * 10);
      HR.Left := HR.Right - Round(FDPIScale * 10);
    ...
    begin
      HR := CR;
      HR.Bottom := CR.Top + Round(FDPIScale * 10);
      HR.Left := HR.Right - Round(FDPIScale * 10);
  ...
  begin
    ...
    HTMLDrawEx(Canvas, s, r, FImages, pt.x, pt.y, fhl, FHoverHyperlink, FShadowOffset, True, False, False, False, FBlinking, FHover, not FEllipsis, FFormScaled, 1.0,
      FURLColor, FHoverColor, FHoverFontColor, FShadowColor, Anchor, Stripped, FocusAnchor, x, y, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
  ...
  begin
    ...
    HTMLDrawEx(Canvas, s, r, nil, 0, 0, fhl, FHoverHyperlink, FShadowOffset, False, False, false, False, FBlinking, FHover, not FEllipsis, FFormScaled, 1.0,
      clWhite, clNone, clNone, FShadowColor, Anchor, Stripped, FocusAnchor, x, y, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
    Canvas.Font.Color := clGray;

    OffsetRect(r,-1,-1);
    HTMLDrawEx(Canvas, s, r, FImages, 0, 0, fhl, FHoverHyperlink, FShadowOffset, False, False, false, False, FBlinking, FHover, not FEllipsis, FFormScaled, 1.0,
      clGray, clNone, clNone, FShadowColor, Anchor, Stripped, FocusAnchor, x, y, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
   end
  else
   HTMLDrawEx(Canvas, s, R, FImages, pt.x, pt.y, fhl, FHoverHyperlink, FShadowOffset, False, False, false, False, FBlinking, FHover, not FEllipsis, FFormScaled, 1.0,
     FURLColor, FHoverColor, FHoverFontColor, FShadowColor, Anchor, Stripped, FocusAnchor, x, y, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
...
{ NOTE: Only called when VCL Theming is not enabled }
...
procedure THTMLStaticText.CreateWnd;
var
  frm: TCustomForm;
begin
  inherited;
  ...
  FDPIScale := GetDPIScale(frm, frm.Canvas);
...
procedure THTMLStaticText.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
...
begin
  ...
  if (FVAlignment in [tvaCenter,tvaBottom]) then
  begin
    HTMLDrawEx(Canvas, s, r, FImages, pt.x, pt.y, -1, -1, FShadowOffset, True, False, False, False, False, FHover, not FEllipsis, FFormScaled, 1.0,
      FURLColor, FHoverColor, FHoverFontColor, FShadowColor, Anchor, Stripped, FocusAnchor, xsize, ysize, hl, ml, HR, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
  ...
  if HTMLDrawEx(Canvas, s, r, FImages, x, y, -1, -1, FShadowOffset, True, False, False, False, False, FHover, not FEllipsis, FFormScaled, 1.0,
    clWhite, clNone, clNone, clNone, Anchor, Stripped, FocusAnchor, xsize, ysize, hl, FHoverHyperlink, HoverRect, FImageCache, FContainer, FLineSpacing, BiDiMode, clHighlight, clHighlightText, [], FDPIScale) then
  begin
    Result := Anchor;
  end
  else
...
begin
  ...
  begin
    ...
    if FUpScroll then
    begin
      if (X > HR.Right - Round(FDPIScale * 10)) and (Y < HR.Top + Round(FDPIScale * 10)) then
         VOffset := VOffset - 4;
    end;

    if FDownScroll then
    begin
      if (X > HR.Right - Round(FDPIScale * 10)) and (Y > HR.Bottom - Round(FDPIScale * 10)) then
         VOffset := VOffset + 4;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FUpScroll then
      begin
        if (pt.X > HR.Right - Round(FDPIScale * 10)) and (pt.Y < HR.Top + Round(FDPIScale * 10)) then
           VOffset := VOffset - 4;
      end;

      if FDownScroll then
      begin
        if (pt.X > HR.Right - Round(FDPIScale * 10)) and (pt.Y > HR.Bottom - Round(FDPIScale * 10)) then
           VOffset := VOffset + 4;
      end;
    end;
  end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure THTMLStaticText.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure THTMLStaticText.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
var
  Frm: TCustomForm;
begin
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLStaticText.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
...
{$IFDEF DELPHIXE2_LVL}
{ THTMLStaticTextStyleHook }
procedure THTMLStaticTextStyleHook.Paint(Canvas: TCanvas);
var
 c: THTMLStaticText;
begin
  inherited Paint(Canvas);
  c := THTMLStaticText(Control);
  { now delegate painting to the original component's DoPaint() }
  c.DoPaint(true, Canvas);
end;

initialization
  TCustomStyleEngine.RegisterStyleHook(THTMLStaticText, THTMLStaticTextStyleHook);

finalization
  TCustomStyleEngine.UnRegisterStyleHook(THTMLStaticText, THTMLStaticTextStyleHook);
{$ENDIF}
