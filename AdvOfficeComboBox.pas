...
const
  ...
  // v1.3.0.2 : Fixed : Issue at design-time in classic undocked mode
  // v1.3.0.3 : Improved : Drop up top position limited to screen top
  // v1.3.1.0 : Improved : Per monitor high DPI support
  // v1.3.1.1 : Fixed : Design-time issue
  // v1.3.2.0 : Improved : Office 2016 Black style colors
  // v1.3.2.1 : Fixed : DC leak
...
begin
  ...
  try
    Canvas.Font.Assign(Font);
    MinHeight := Canvas.TextHeight('gh');
  finally
    ReleaseDC(Handle, Canvas.Handle);
    Canvas.Free;
  end;
...
begin
  ...
  begin
    FDropDownList.Height := Max(20, (FItems.Count * GetListItemHeight) + 4);
  end
  else
  begin
    FDropDownList.Height := Max(20, (FDropDownCount * GetListItemHeight) + 4);
  end;
  ...
  begin
    ...
    // ensure top of list to be visible
    if FDropDownList.Top < 0 then
      FDropDownList.Top := 0;
  end;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomOfficeComboBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvCustomOfficeComboBox.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;

  FButton.Width := Round(12 * GetDPIScale(Self, nil));
end;
...
begin
  Result := Round(FItemHeight * GetDPIScale(Self, nil));
end;
