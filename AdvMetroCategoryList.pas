...
const
  ...
  // 1.1.0.0 : New : OnCategoryCheck, OnItemCheck events added
  // 1.1.0.1 : Fixed : Issue with cursor config
  // 1.1.0.2 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // 1.1.1.0 : Improved : Support for High DPI
...
type
  ...
  TCategoryCheckEvent = procedure(Sender: TObject; Category: TCategory; var Allow: boolean) of object;
  TItemCheckEvent = procedure(Sender: TObject; Item: TCategoryItem; var Allow: boolean) of object;
  ...
  private
    ...
    function GetCheckBoxRect(PaintRect: TRect; CheckBoxImage: TGDIPPicture; ADPIScale: single = 1): TRect;
    function GetTextRect(PaintRect: TRect; ADPIScale: single = 1): TRect;
  ...
  private
    ...
    function GetCheckBoxRect(PaintRect: TRect; CheckBoxImage: TGDIPPicture; ADPIScale: single = 1): TRect;
    function GetCollapseRect(PaintRect: TRect; Image: TGDIPPicture; ADPIScale: single = 1): TRect;
    function GetTextRect(PaintRect: TRect; ADPIScale: single = 1): TRect;
  ...
  private
    ...
    FOnCategoryCheck: TCategoryCheckEvent;
    FOnItemCheck: TItemCheckEvent;
    FDefCursor: TCursor;
    FFormScaled: boolean;
  ...
  protected
    ...
    procedure DoCategoryCheck(Category: TCategory; var Allow: boolean); virtual;
    procedure DoItemCheck(Item: TCategoryItem; var Allow: boolean); virtual;
  ...
  published
    ...
    property OnCategoryCheck: TCategoryCheckEvent read FOnCategoryCheck write FOnCategoryCheck;
    property OnClick;
    ...
    property OnGesture;
    property OnItemCheck: TItemCheckEvent read FOnItemCheck write FOnItemCheck;
...
function TCategory.GetCheckBoxRect(PaintRect: TRect; CheckBoxImage: TGDIPPicture; ADPIScale : single = 1): TRect;
var
  Height: Integer;
begin
  ...
  Result.Right := Result.Left + Round(ADPIScale * CheckBoxImage.Width);
  Result.Top := PaintRect.Top + (Height - Round(ADPIScale * CheckBoxImage.Height)) div 2;
  Result.Bottom := Result.Top + Round(ADPIScale * CheckBoxImage.Height);
end;

function TCategory.GetCollapseRect(PaintRect: TRect; Image: TGDIPPicture; ADPIScale: single = 1): TRect;
var
  Height: Integer;
begin
  Height := FEndY - FStartY;
  ...
  Result.Left := Result.Right - Round(ADPIScale * Image.Width);
  Result.Top := PaintRect.Top + (Height - Round(ADPIScale * Image.Height)) div 2;
  Result.Bottom := Result.Top + Round(ADPIScale * Image.Height);
end;
...
function TCategory.GetTextRect(PaintRect: TRect; ADPIScale: single = 1): TRect;
begin
  Result := PaintRect;
  ...
  if FShowCheckBox then
    Result.Left := Result.Left + Round(ADPIScale * 21);
...
begin
  ...
  Canvas.Font.Height := Round(GetDPIScale(Self, Canvas) * FCategoryFont.Height);
  res := HTMLDrawEx(Canvas, Category.Text, TextRect, FImages, X, Y, -1, -1, 1, True, False, False, False, False, False, True, FFormScaled, 1.0,
...
begin
  ...
  Canvas.Font.Height := Round(GetDPIScale(Self, Canvas) * FItemFont.Height);
  res := HTMLDrawEx(Canvas, Item.Text, TextRect, FImages, X, Y, -1, -1, 1, True, False, False, False, False, False, True, FFormScaled, 1.0,
             clBlue, clNone, clNone, clGray, a, s, focusanchor, xs, ys, HyperLinks, MouseLink, r, nil, FPictureContainer, 0);
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            case FCheckedState of
              ...
              begin
                ...
                Inc(TextOffset, Round (DPIScale * (FImageUnChecked.Width + 5)));
              end;
              cbChecked:
              begin
                FImageChecked.GetImageSizes;
                Inc(TextOffset, Round (DPIScale * (FImageChecked.Width + 5)));
              end;
              cbGrayed:
              begin
                FImageGrayed.GetImageSizes;
                Inc(TextOffset, Round (DPIScale * (FImageGrayed.Width + 5)));
              end;
            end;
          end;
          if FShowScroll then
          ...
          begin
            FImageLeft.GetImageSizes;
            Inc(TextOffset, Round (DPIScale * (FImageLeft.Width + 5)));
          end;
          if Assigned(FImageRight) then
          begin
            FImageRight.GetImageSizes;
            Inc(TextOffset, Round (DPIScale * (FImageRight.Width + 5)));
          end;
          tr := Bounds(0, 0, Width - TextOffset, $FFF);
          Canvas.Font := FCategoryFont;
          Canvas.Font.Height := Round (DPIScale * FCategoryFont.Height);

          HTMLDrawEx(Canvas, text, tr, FImages, -1, -1, -1, -1, -1, True, True, False, False, False, False, True, FFormScaled, 1.0,
        ...
        FEndY := FStartY + Max (FCategoryMinHeight, FTextHeight + Round (DPIScale * 10));

        if Assigned(FImageLeft) then
        begin
          FImageLeft.GetImageSizes;
          FEndY := Max(FEndY, FStartY + FImageLeft.Height + Round (DPIScale * 10));
        end;
        if Assigned(FImageRight) then
        begin
          FImageRight.GetImageSizes;
          FEndY := Max(FEndY, FStartY + FImageRight.Height + Round (DPIScale * 10));
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              tr := Bounds(0, 0, Width - TextOffset, $FFF);
              Canvas.Font := FItemFont;
              Canvas.Font.Height := Round (DPIScale * FItemFont.Height);
              HTMLDrawEx(Canvas, Text, tr, FImages, -1, -1, -1, -1, -1, True, True, False, False, False, False, True, FFormScaled, 1.0,
                    clBlue, clNone, clNone, clGray, a, s, fs, xs, ys, links, mlinks, hr, nil, FPictureContainer, 0);
            ...
            FEndY := FStartY + Max (FItemMinHeight, FTextHeight + Round(DPIScale * 10));
            MaxY := FEndY;
  ...
  if (Categories.Count = 0) or ReachedFirstDirty or FChangedScale then
  begin
    FMaxY := MaxY;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvMetroCategoryList.ChangeScale(M, D: Integer);
    {$ENDIF}
    {$IFDEF DELPHIXE10_LVL}
procedure TAdvMetroCategoryList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
    {$ENDIF}
begin
  inherited;
  FChangedScale := True;
...
procedure TAdvMetroCategoryList.CreateWnd;
var
  frm: TCustomForm;
begin
  inherited;
...
procedure TAdvMetroCategoryList.DoCategoryCheck(Category: TCategory;
  var Allow: boolean);
begin
  if Assigned(OnCategoryCheck) then
    OnCategoryCheck(Self, Category, Allow);
end;

procedure TAdvMetroCategoryList.DoItemCheck(Item: TCategoryItem;
  var Allow: boolean);
begin
  if Assigned(OnItemCheck) then
    OnItemCheck(Self, Item, Allow);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ReachedItem := True;
          text := AnchorTextAt(X, Y, Category, Category.GetTextRect(r, DPIScale));
          if text <> '' then
            Cursor := crHandPoint
          else
            Cursor := FDefCursor;
        ...
        begin
          ...
          for J := minJ to maxJ do
            begin
              ...
              begin
                ReachedItem := True;
                text := AnchorTextAt(X, Y, Item, Item.GetTextRect(r, DPIScale));
                if text <> '' then
                  Cursor := crHandPoint
                else
                  Cursor := FDefCursor;
...
var
  ...
  Category: TCategory;
  Item: TCategoryItem;
  ReachedItem, Allow, CatAllow: Boolean;
  text: string;
  DPIScale: single;
begin
  inherited;
    ...
    begin
      ...
      begin
        ...
        if Category.FShowCheckBox then
          case Category.FCheckedState of
            cbUnchecked: r := Category.GetCheckBoxRect(r, FImageUnChecked, DPIScale);
            cbChecked: r := Category.GetCheckBoxRect(r, FImageChecked, DPIScale);
            cbGrayed: r := Category.GetCheckBoxRect(r, FImageGrayed, DPIScale);
          end;
        ...
        begin
          CatAllow := true;
          DoCategoryCheck(Category, CatAllow);
          if CatAllow then
            Category.ChangeCheck(FCategoryCheckboxRange)
        end
        else
        begin
          ...
          else if PtInRect(Category.GetCollapseRect(r, FImageCollapse, DPIScale), Point(X, Y)) then
      ...
      begin
        ...
        begin
          ...
          begin
             case Category.FCheckedState of
              cbUnchecked: r := Category.GetCheckBoxRect(fr, FImageUnChecked, DPIScale);
              cbChecked: r := Category.GetCheckBoxRect(fr, FImageChecked, DPIScale);
              cbGrayed: r := Category.GetCheckBoxRect(fr, FImageGrayed, DPIScale);
             end;
          end;
          ...
          begin
            text := AnchorTextAt(X, Y, Category, Category.GetTextRect(fr, DPIScale));
            if text <> '' then
            begin
              if Assigned(FOnAnchorClick) then
                FOnAnchorClick(Category, text);
            end
            else if PtInRect(Category.GetCollapseRect(fr, FImageCollapse, DPIScale), Point(X, Y)) then
              CollapseExpand(Category)
            else
            begin
              if Assigned(FOnItemSelect) then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if Item.FChecked then
                  r := Item.GetCheckBoxRect(r, FImageChecked, DPIScale)
                else
                  r := Item.GetCheckBoxRect(r, FImageUnChecked, DPIScale);
              end;
              ...
              begin
                ...
                begin
                  if Assigned(FOnAnchorClick) then
                    FOnAnchorClick(Item, AnchorTextAt(X, Y, Item, Item.GetTextRect(r, DPIScale)))
                end
                else
                begin
                  if Assigned(FOnItemSelect) then
...
begin
  ...
  text := Category.Text;
  tr := Category.GetTextRect(r, DPIScale);
  Canvas.FillRect(r);
  ...
  begin
    ...
    if (Pic <> nil) then
    begin
      cbr := Category.GetCheckBoxRect(r, Pic, DPIScale);
  ...
  cer := Category.GetCollapseRect(r, FImageCollapse, DPIScale);
  if Category.FCollapse then
    Pic := FImageExpand
  else
    Pic := FImageCollapse;
  ...
  HTMLDrawEx(Canvas, text, tr, FImages, -1, -1, -1, -1, -1, False, False, False, False, False, False, True, FFormScaled, 1.0,
        clBlue, clNone, clNone, clGray, a, s, fs, xs, ys, links, mlinks, hr, nil, FPictureContainer, 0);
...
begin
  ...
  begin
    ...
    if not Allow then
      Exit;
    tr := Item.GetTextRect(r, DPIScale);
    text := Item.Text;
    ...
    begin
      if Item.FChecked then
        cbr := Item.GetCheckBoxRect(r, FImageChecked, DPIScale)
      else
        cbr := Item.GetCheckBoxRect(r, FImageUnChecked, DPIScale);
      if Item.FChecked then
...
function TCategoryItem.GetCheckBoxRect(PaintRect: TRect; CheckBoxImage: TGDIPPicture; ADPIScale: single = 1): TRect;
var
  Height: Integer;
begin
  Height := FEndY - FStartY;
...
function TCategoryItem.GetTextRect(PaintRect: TRect; ADPIScale: single = 1): TRect;
begin
  Result := PaintRect;
  InflateRect(Result, -5, -5);
