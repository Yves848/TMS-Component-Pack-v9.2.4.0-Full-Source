...
uses
  Windows, Classes, Graphics, Dialogs, SysUtils, Contnrs, Messages, Controls,
  AdvStyleIF, GDIPFill, AdvGDIP, AdvSmoothCalendar, Math, DateUtils
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.2.0.2 : Fixed : Issue with transparency on double buffered forms
  // v1.2.0.3 : Fixed : Issue with calendar border drawing
  // v1.3.0.0 : New : VCL Styles support
...
begin
  ...
  w := Round(rt.Width) div Columns;
  h := Round(rt.Height) div Rows;
...
procedure TAdvSmoothCalendarGroup.CMStyleChanged(var Message: TMessage);
begin
  inherited;
