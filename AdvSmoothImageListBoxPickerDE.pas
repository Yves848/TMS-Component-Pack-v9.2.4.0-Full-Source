{**************************************************************************}
{ TAdvSmoothImageListBoxPickerDE DESIGN TIME EDITOR                        }
...
unit AdvSmoothImageListBoxPickerDE;
...
uses
  Classes, AdvSmoothImageListBoxPicker, DesignIntf, DesignEditors;

type
  TAdvSmoothImageListBoxPickerEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothImageListBoxPickerEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothImageListBoxPicker).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothImageListBoxPicker) then
         (Component as TAdvSmoothImageListBoxPicker).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothImageListBoxPickerEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothImageListBoxPickerEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
