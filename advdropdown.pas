...
const
  ...
  // v1.4.3.0 : Improved : Painting of dropdown in Windows 10 style
  // v1.4.4.0 : New : Property HoverSelect added in TAdvMultiColumnDropDown
  // v1.4.5.0 : New : ReturnIsTab added in TAdvMultiColumnDropDown
  // v1.4.5.1 : Fixed : Issue with focus & click on dropdown button
  // v1.4.5.2 : Fixed : Rare issue with making dropdown visible
  // v1.4.5.3 : Improved : High DPI handling
  // v1.4.5.4 : Fixed : Issue when setting border color to clNone at runtime
  // v1.4.5.5 : Fixed : Issue with programmatic deactivate from dropdown control
  // v1.4.5.6 : Fixed : Issue with '-' char handling in TAdvCalculatorDropDown
  // v1.4.5.7 : Improved : Color settings initialization
  // v1.4.5.8 : Improved : OnKeyDown event also triggered when EditorEnabled = false for arrow keys, home, end, pgup, pgdown
  // v1.4.5.9 : Fixed : Issue with focused painting
  // v1.4.5.10: Improved : High DPI handling for attached label
  // v1.4.5.11: Fixed : Issue with row height handling & wordwrap in TAdvMultiColumnDropDown
  // v1.4.6.0 : New : OnDropDownCellClick event added in TAdvMultiColumnDropDown
  // v1.4.6.1 : Fixed : Keyboard handling issue in TAdvCalculatorDropDown
  // v1.4.6.2 : Fixed : Issue when streaming the TAdvDropDown to string and vice versa
  // v1.4.7.0 : New : LookupIncr property added in TAdvMultiColumnDropDown
...
type
  ...
  protected
    ...
    procedure OnDropDownSize; virtual;
...
begin
  ...
  begin
    ...
    FDropDownForm.AdvDropDown := Self;
    ...
    FDropDownForm.OnClose := FormClose;
    FDropDownForm.OnHide := OnFormHide;
    FDropDownForm.OnKeyPress := OnFormKeyPress;
    FDropDownForm.OnKeyDown := OnFormKeyDown;
    FDropDownForm.OnKeyUp := OnFormKeyUp;
    FDropDownForm.OnMouseWheelDown := OnFormMouseWheelDown;
    FDropDownForm.OnMouseWheelUp := OnFormMouseWheelUp;
    FDropDownForm.OnDestroy := OnFormDestroy;
    FDropDownForm.OnSizing := DropDownSizing;
    FDropDownForm.OnSize := DropDownSizing;
    FDropDownForm.Scaled := true;
...
begin
  ...
  *)
  ...
  begin
    ...
    SetWindowPos(FDropDownForm.Handle, HWND_TOPMOST, p.x, p.y, w, h, SWP_SHOWWINDOW);
...
begin
  ...
  begin
    ...
    if (Msg.CharCode in [VK_RETURN,VK_ESCAPE,VK_TAB,VK_UP,VK_DOWN,VK_LEFT,VK_RIGHT,VK_PRIOR,VK_NEXT,VK_HOME,VK_END]) then
      inherited;
    Exit;
  end;
...
begin
  ...
  FDropDownForm.IsDeactivating := false;
end;
...
{$HINTS OFF}
{$IF COMPILERVERSION > 30}
procedure TAdvCustomDropDown.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvCustomDropDown.ChangeScale(M, D: Integer);
{$IFEND}
{$HINTS ON}
var
  frm: TCustomForm;
...
var
  ...
  begin
    ...
    begin
      mdist := Length(s) - Pos(FormatSettings.decimalseparator, s);
      if (inspos >= Pos(FormatSettings.decimalseparator, s)) and (mdist >= FPrecision) then
        Result := true;
    end;
  end;
  ...
  begin
    ...
    while (Pos(FormatSettings.thousandseparator, s) > 0) do
    begin
      if abs(Pos(FormatSettings.thousandseparator, s) - inspos) < mdist then mdist := abs(Pos(FormatSettings.thousandseparator, s) - inspos);
      if (abs(Pos(FormatSettings.thousandseparator, s) - inspos) < 3) then
      begin
        Result := true;
        break;
      end;

      if inspos > Pos(FormatSettings.thousandseparator, s) then inspos := inspos - Pos(FormatSettings.thousandseparator, s);
      delete(s, 1, Pos(FormatSettings.thousandseparator, s));
    end;
...
// allow Ctrl-C, Ctrl-X, Ctrl-V
  ...
  begin
    msg.charcode := ord(FormatSettings.decimalseparator);
  end;
  ...
  and (msg.charcode <> vk_back) and (msg.charcode <> ord(FormatSettings.decimalseparator)) and not AllowMin(chr(msg.CharCode)) then Exit;
  ...
  case EditType of
    ...
    etMoney:
      begin
        if (chr(msg.charcode) = FormatSettings.decimalseparator) and ((Pos(FormatSettings.decimalseparator, self.Text) > 0) or (FPrecision = 0)) then
        begin
        ...
        if (msg.charcode in Money_Codes + Ctrl_Codes) or (chr(msg.charcode) = FormatSettings.decimalseparator) then
        begin
          if (chr(msg.charcode) = FormatSettings.thousandseparator) or (chr(msg.charcode) = FormatSettings.decimalseparator) then
          begin
            if scandistance(self.Text, SelStart) then
              Exit;
          end;

          if scanprecision(self.Text, SelStart) and (msg.charcode in [$30..$39, ord(FormatSettings.decimalseparator)]) and (SelLength = 0) then
            begin
              if (FPrecision > 0) and (SelStart - Length(fprefix) >= Pos(FormatSettings.decimalseparator, self.text))
          ...
          if (SelStart = 0) and (self.Text = '') and (msg.charcode = ord(FormatSettings.decimalseparator)) then
            begin
              inherited Text := '0' + FormatSettings.decimalseparator;
              SelStart := 2;
              SetModified(true);
              Exit;
            end;
          ...
          if (self.Text <> '') and (self.Text <> '-') and
            (chr(msg.charcode) <> FormatSettings.decimalseparator) then
          begin
    ...
    etFloat:
      begin
        if (msg.charcode = ord(',')) and (FormatSettings.DecimalSeparator <> ',') and (FormatSettings.ThousandSeparator <> ',') then
          Exit;
        if (msg.charcode = ord('.')) and (FormatSettings.DecimalSeparator <> '.') and (FormatSettings.ThousandSeparator <> '.') then
          Exit;
        ...
        begin
          if (chr(msg.charcode) = FormatSettings.decimalseparator) and
            (Pos(FormatSettings.decimalseparator, self.getseltext) = 0) and
            ((Pos(FormatSettings.decimalseparator, self.Text) > 0) or (FPrecision = 0)) then
          begin
          ...
          if ((msg.charcode = ord(',')) and (Pos(',', self.Text) > 0) and (Pos(',', self.getSelText) = 0)) and
            (chr(msg.charcode) <> FormatSettings.thousandseparator) then exit;
    ...
    case FEditType of
      ...
      etFloat, etMoney:
        begin
          ...
          begin
{$IFNDEF DELPHI_UNICODE}
            if not ((FPrecision = 0) and (Pos(FormatSettings.DecimalSeparator, StrPas(Content)) > 0)) then
{$ENDIF}
{$IFDEF DELPHI_UNICODE}
            if not ((FPrecision = 0) and (Pos(FormatSettings.DecimalSeparator, Clipboard.AsText) > 0)) then
{$ENDIF}
              begin
                if not (not Signed and (pos('-', newstr) > 0)) then
                begin
...
begin
  ...
  case FEditType of
    ...
    etNumeric, etFloat, etValidChars:
...
begin
  ...
  if (FPrecision > 0) and (Length(s) > FPrecision) then
    if s[Length(s) - FPrecision] = FormatSettings.Thousandseparator then
      s[Length(s) - FPrecision] := FormatSettings.Decimalseparator;
  try
    Result := StrToFloat(s);
  except
    Result := 0;
...
procedure TAdvCustomDropDown.OnDropDownSize;
begin
...
begin
  outputdebugstring('form keypress');
...
begin
  ...
  dropped :=  TAdvCustomDropDown(Owner.Owner).DroppedDown;
  ...
  begin
    ...
    begin
      DrawThemeBackground(htheme,Canvas.Handle,btn,CBXS_DISABLED,@ARect,nil)
    end
    else
    begin
      if ((FState in [bsDown, bsExclusive]) and not FUp) or dropped then
...
//  ControlStyle := ControlStyle - [csAcceptsControls, csSetCaption] + [csFramed, csOpaque];
  ...
  OnClick := (AOwner as TAdvCustomDropDown).DropDownButtonClick;
...
var
  Clr, ClrTo, MClr, MClrTo, BrClr, LTop, LBottom, EdgClr, RectClr: TColor;
  R, TR, IR: TRect;
  iw, ih: Integer;
  ts: TSize;
  DrawBkg: Boolean;
begin
  ...
  RectClr := Items[Index].Color;

  if RectClr = clNone then
    RectClr := Canvas.Brush.Color;
