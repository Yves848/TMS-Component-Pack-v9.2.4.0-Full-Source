...
const
  ...
  // v1.3.1.0 : New : Support for high DPI added
  // v1.3.1.1 : Fixed : Issue with high DPI for controls inside TAdvSmoothPopup
  // v1.3.1.2 : Fixed : Further improvement to high DPI handling
...
begin
  ...
  pbl := MakePoint(rt.X, rt.Y + rt.Height - GetFooterHeight);
  pbr := MakePoint(rt.X + rt.Width - mgr, rt.Y + rt.Height - GetFooterHeight);
  ...
  begin
    fnt := TFont.Create;
    fnt.Assign(HeaderFont);
    fnt.Height := Round(fnt.Height * DpiScale);
    ft := g.MakeFont(fnt);
    fnt.Free;
    sf := TGPStringFormat.Create;
  ...
  FFooterR := MakeRect(rt.X, rt.Y + rt.Height - GetFooterHeight, rt.Width, GetFooterHeight);

  FHeaderR := MakeRect(rt.X, rt.Y, rt.Width, GetHeaderHeight);
...
function TAdvSmoothPopup.GetFooterHeight: integer;
begin
  Result := Round(FooterHeight * DpiScale);
end;

function TAdvSmoothPopup.GetHeaderHeight: integer;
begin
  Result := Round(HeaderHeight * DpiScale);
end;

function TAdvSmoothPopup.GetHeight: Integer;
begin
  if AutoSize and Assigned(Control) then
    Result := Control.Height + GetMaxMarginSize + GetHeaderHeight + GetFooterHeight
  else
    Result := Round(Height * DpiScale) + GetMaxMarginSize;
end;
...
begin
  if AutoSize and Assigned(Control) then
    ...
    Result := Round(Width * DpiScale) + GetMaxMarginSize;
end;
...
begin
  ...
  begin
    ...
    ir := MakeRect(rt.X + 7, rt.Y + GetHeaderHeight, rt.Width - 13,
      rt.Height - GetFooterHeight - GetHeaderHeight);
    ...
    // apply possible high DPI scaling
//    frmC.Scaled := true;
//    frmC.ChangeScale(Round(DPIScale * 100), 100);
...
function TAdvSmoothPopup.DpiScale: single;
begin
//  Result := ScreenPPI/96;
  Result := 1;
end;

function TAdvSmoothPopup.ScreenPPI: integer;
var
  FHDC: HDC;
begin
  FHDC := GetDc(0);
  Result := GetDeviceCaps(FHDC, LOGPIXELSY);
  ReleaseDC(0, FHDC);
end;
...
begin
  ...
  fr := MakeRect(rt.X + 1, rt.Y + rt.Height - GetFooterHeight - 7, rt.Width - 1,
    (GetFooterHeight / 2) + 7);
  ...
  hr := MakeRect(rt.X + 1, rt.Y + 1, rt.Width - 1, GetHeaderHeight / 2);
