{**************************************************************************}
{ TAdvSmoothCircularProgressDE DESIGN TIME EDITOR                                   }
...
unit AdvSmoothCircularProgressDE;
...
uses
  Classes, AdvSmoothCircularProgress,
  DesignIntf, DesignEditors;

type
  TAdvSmoothCircularProgressEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothCircularProgressEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothCircularProgress).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothCircularProgress) then
         (Component as TAdvSmoothCircularProgress).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothCircularProgressEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothCircularProgressEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
