...
const
  ...
  // v1.5.1.2 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // v1.5.2.0 : New : Per monitor support for high DPI
...
begin
  Result := HTMLDrawEx(Canvas, s, fr, FImages, xpos, ypos, -1, HoverLink, ShadowOffset, checkhotspot, checkheight, print, selected, Blink,
                       ...
                       XSize, YSize, HyperLinks, MouseLink, HoverRect, FImageCache, FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
  FXSize := XSize; //Width;
  FYSize := YSize;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (y < Round(FDPIScale * FReflectionSize)) then
        begin
          op := Round( ((Round(FDPISCale * FReflectionSize) - y) / (FDPISCale * FReflectionSize) * FReflectionOpacityStart) +
                         y / (FDPISCale * ReflectionSize) * FReflectionOpacityEnd);
        end
        else
          op := 0;
