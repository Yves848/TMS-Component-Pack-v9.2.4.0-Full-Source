...
const
  ...
  // 1.4.0.0 : New : VCL Styles support added
    ...
    CDDS_ITEMPOSTPAINT:
      begin
        ...
        HTMLDrawEx(Canvas,tn.Text,TVcd.nmcd.rc,TImageList(HTMLImages),0,0,-1,-1,2,
          False,True,False,Selected and FShowSelection,False,FHover,Wordwrap,FMouseDown,False,1.0,FParamColor,
          FHoverColor,FHoverFontColor,clGray,a,s,f,xsize,ysize,hl,ml,hr,cr,CID,CV,CT,
          FImageCache,FContainer,Handle,0);

        r := TVcd.nmcd.rc;

        { Spaced after tv node text up to border of component. }
//        if  (TVcd.nmcd.uItemState and CDIS_FOCUS = CDIS_FOCUS) then
        begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Canvas.Font.Color := StyleServices.GetStyleFontColor(sfEditBoxTextSelected);
              {$ENDIF}
            end
            else
            begin
        ...
        if Selected and FShowSelection then
        begin
          if FUseVCLStyles {$IFDEF DELPHIXE6_LVL} and (seFont in StyleElements){$ENDIF} then
          begin
            {$IFDEF DELPHIXE2_LVL}
            ...
            Canvas.Pen.Color   := StyleServices.GetSystemColor(clHighlight);
            Canvas.Font.Color := StyleServices.GetStyleFontColor(sfEditBoxTextSelected);
            Urlcol             := StyleServices.GetSystemColor(clHighlightText);
            {$ENDIF}
          end
          else
          begin
        ...
        { Defines color when mouse hovers over params }
        if not FHover then
        begin
          FHC := clNone;
          FHFC := clNone;
...
procedure TParamTreeview.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
const
...
{$IFDEF DELPHIXE2_LVL}
procedure TParamTreeview.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
