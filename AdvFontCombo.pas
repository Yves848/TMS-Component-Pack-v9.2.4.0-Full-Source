...
const
  ...
  // 1.5.4.9 : Fixed : Issue with selection when recent selection display is disabled
  //         : Fixed : Issue with ItemIndex return when DisplayRecentSelection = true
  // 1.5.5.0 : Improved : High DPI support
  // 1.5.5.1 : Fixed : Issue at design-time in classic undocked mode
  // 1.5.5.2 : Fixed : Issue with last empty item when selecting
  // 1.5.5.3 : Improved : TDBAdvOfficeCombobox dataset editing state from OnChange event handler
  // 1.5.5.4 : Improved : Reset of text for non-matching entry only for csDropDownList style
  // 1.5.5.5 : Fixed : Issue with Home/End key handling
  // 1.5.6.0 : Improved : Office 2016 black style colors
  // 1.5.7.0 : Improved : Support for High DPI
  // 1.5.7.1 : Fixed : Design-time issue upon form loading
...
type
  ...
  protected
    ...
    procedure ChangeItemIndex; virtual;
...
begin
  ...
  DC := GetDC(Handle);
  FDPIScale := GetDeviceCaps(DC, LOGPIXELSY) / 96;
  ReleaseDC(Handle, DC);

  Loc.Bottom := ClientHeight + 1; {+1 is workaround for windows paint bug}
  Loc.Right := ClientWidth - GetButtonWidth - Round(5 * FDPIScale);

  if (BorderStyle = bsNone) then
  begin
    Loc.Top := 2;
    Loc.Left := Round(4 * FDPIScale);
  end
  else
  begin
    Loc.Top := 1;
...
begin
  R := ClientRect;
  Result := Rect(R.Right - GetButtonWidth - 1, R.Top + 1, R.Right - 1, R.Bottom - 2);
end;

function TAdvCustomComboBox.GetButtonWidth: integer;
begin
  Result := Round(FButton.Width * FDPIScale);
end;
...
begin
  ...
  P.X := BtnR.Left + ((BtnR.Right - BtnR.Left - Round(FDPIScale * 4)) div 2);
  P.Y := BtnR.Top + ((BtnR.Bottom - BtnR.Top - Round(FDPIScale * 2)) div 2);
...
begin
  ...
  if not (csDesigning in ComponentState) then
  begin
    FDPIScale := GetDPIScale(Self, FDropDownList.Canvas);
  end
  else
    FDPIScale := 1;
    ...
    case FAppearanceStyle of
      esOffice2003Blue:
        begin
          ...
          FButton.ArrowColor := clWindowText;
        end;
      esOffice2003Olive:
        begin
         { CaptionColor := $82C0AF;
...
begin
  ...
  if (Text <> '') and (FItems.IndexOf(Text) = -1) and (Style = csDropDownList) then
    Text := '';
end;
...
begin
  ...
  begin
    ...
    FDPIScale := GetDeviceCaps(FDropDownListBox.Canvas.Handle, LOGPIXELSY) / 96;
    Result := Round(Result * FDPIScale);
  end
  else
  begin
    DC := GetDC(Handle);
    ...
    Result := Round(FItemHeight * FDPIScale);
  end;
end;
...
procedure TAdvCustomComboBox.ChangeItemIndex;
begin
  //
end;
...
//  FInternalChange := true;
  ...
  case Key of
    ...
    VK_HOME: if FDropDownList.Visible then First;
    VK_END: if FDropDownList.Visible then Last;
    VK_ESCAPE:
      begin
        inherited Text := FOldText;
        if FDropDownList.Visible then
...
begin
  ...
  begin
    ...
    ItemIndex := FDropDownListBox.ItemIndex;
    if ItemIndex >= 0 then
      Text := FDropDownListBox.Items[ItemIndex];
    DoChange;
    DoSelect;
  end;
end;
