...
const
  ...
  // v1.4.0.1 : Fixed : Issue with transparency on double buffered forms
  // v1.4.1.0 : Improved : Support for High DPI (Text size, button height, text and image positioning)
...
begin
  ...
  begin
    ...
    begin
      ...
      FCurrentPos := DPIScale * (FCaptionRect.X + (FCaptionRect.Width / 2));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    y := Round(DPIScale * (ItemAppearance.Splitter.Height + 2));
...
begin
  ...
  begin
    DPIScale := GetDPIScale(Self, Canvas);
    g := TGPGraphics.Create(Canvas.Handle);
    p := TGPPen.Create(MakeColor(Appearance.LineOpacity, Appearance.LineColor), DPIScale * 0.1);

    bw := 0;
    if (Appearance.Fill.BorderColor <> clNone) then
      bw := Appearance.Fill.BorderWidth;

    g.DrawLine(p, bw, Round(DPIScale * (ItemAppearance.Splitter.Height + 2)) , Width - (2 * bw)  - GetShadowOffset, Round(DPIScale * (ItemAppearance.Splitter.Height + 2)));
    p.Free;
    g.Free;
  end;
end;
...
begin
  ...
  begin
    InitItemRects;
    DPIScale := GetDPIScale(Parent, Canvas);
    FCurrentPos := (Items[0].FCaptionRect.X + Items[0].FCaptionRect.Width / 2) * DPIScale;
    FPosTo := FCurrentPos;
    Changed;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      g.DrawString(FCaption, Length(FCaption), f, MakeRect(FCaptionRect.X, Round(FCaptionRect.Y * DPIScale), FCaptionRect.Width, Round(FCaptionRect.Height * DPIScale)), sf, b);
    ...
    ir := MakeRect(ClientRect.Left + 2, DPISCale * ItemAppearance.Splitter.Height + 4, ClientRect.Right - ClientRect.Left,
      ClientRect.Bottom - DPIScale * ItemAppearance.Splitter.Height - 4);
    ...
    begin
      ...
      picr := Bounds(Round(ClientRect.Left + FCurrentTextPos), Round(DPIScale * ItemAppearance.Splitter.Height + 4), Round(ir.Width), Round(ir.Height));

      if FPicturePosition <> ppCustom then
        DrawPicture(g, FCurrentTextOpc, Picture, FPicturePosition, pw, ph, picr, picr.Left, picr.Top)
      else
       DrawPicture(g, FCurrentTextOpc, Picture, FPicturePosition, pw, ph, picr, Round(FcurrentTextPos) + Round(DPIScale * PictureLeft), Round(DPIScale * PictureTop));
      ...
      begin
        ...
        begin
          ...
          sf := TGPStringFormat.Create;
          f := TGPFont.Create(ff, FNotesFont.Size * DPIScale, fs, UnitPoint);
        ...
        begin
          x := Round(DPIScale * NotesLeft);
          y := Round(DPIScale * NotesTop);
        end;
    ...
    begin
      ...
      begin
        ...
        rsplit := MakeRect(FItemRect.X + FItemRect.Width, FItemRect.Y + (FItemRect.Height / 2) - (DPIScale * Height / 2), 1, DPIScale * Height / 2);
        ...
        rsplit := MakeRect(FItemRect.X + FItemRect.Width, FItemRect.Y + (FItemRect.Height / 2), 1, Height * DPIScale / 2);
