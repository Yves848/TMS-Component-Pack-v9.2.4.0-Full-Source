...
function TDBAdvEdit.IsDB: boolean;
begin
  Result := true;
end;
...
begin
  if Assigned(DataSource) and (Message.CharCode in [32..255]) and (FDataLink.Field <> nil) and (Message.CharCode <> Ord('.'))
...
function TDBAdvMaskEdit.EditCanModify: Boolean;
begin
  if Assigned(DataSource) then
...
begin
  ...
  {$ENDIF}
    not FDataLink.Field.IsValidChar(Key) or (FDataLink.ReadOnly) or not EditCanModify then
  begin
    if not FDataLink.Field.IsValidChar(Key) then
      MessageBeep(0);
    Key := #0;
  end;
...
procedure TDBAdvMaskEdit.WMChar(var Message: TWMKeyDown);
begin
  inherited;
end;
