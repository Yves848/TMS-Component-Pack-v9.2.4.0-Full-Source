...
const
  ...
  // 1.6.0.2  : Improved : Resize sensitivity
  // 1.7.0.0  : New : VCL Styles support added
  // 1.7.1.0  : New : Per monitor support for high DPI
...
type
  ...
  published
    ...
    property OpenWidth: Integer read GetOpenWidth write SetOpenWidth default 150;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvToolPanelTab.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvToolPanelTab.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
var
  i: Integer;
begin
  ...
  begin
    if FPanels[i].Panel.Width > 0 then
      FPanels[i].Panel.Width := Round(FDPIScale * FPanels[i].FCalcOpenWidth);
    if FFirstChangeScale and (Position <> ppTop) then
    begin
      FPanels[i].Panel.Font.Height := MulDiv(FPanels[i].Panel.Font.Height,M,D);
    end;
  end;
  FFirstChangeScale := false;
end;
...
begin
  ...
  begin
    if (Msg.XPos <= Round(FDPIScale * FTabWidth)) and (GetAsyncKeyState(VK_LBUTTON) <> 0) and (GetPanelFromTab(Msg.XPos, Msg.YPos) >= 0) then
      Msg.Result := 1
    else
      inherited;
  end;
  ...
  begin
    if (Msg.XPos > Width - Round(FDPIScale * FTabWidth)) and (GetAsyncKeyState(VK_LBUTTON) <> 0) and (GetPanelFromTab(Msg.XPos, Msg.YPos) >= 0) then
      Msg.Result := 1
    else
      inherited;
  end;
  ...
  begin
    if (Msg.YPos <= Round(FDPIScale * FTabWidth)) and (GetAsyncKeyState(VK_LBUTTON) <> 0) and (GetPanelFromTab(Msg.XPos, Msg.YPos) >= 0) then
      Msg.Result := 1
    else
      inherited;
  end;
  ...
  begin
    if (Msg.YPos > Height - Round(FDPIScale * FTabWidth)) and (GetAsyncKeyState(VK_LBUTTON) <> 0) and (GetPanelFromTab(Msg.XPos, Msg.YPos) >= 0) then
      Msg.Result := 1
    else
      inherited;
  end;
...
begin
  ...
  r.Top := r.Top + Round(FDPIScale * FPaintOffset);
...
begin
  ...
  if Position = ppLeft then
    LeftPos := Round(FDPIScale * FTabWidth)
  else
    LeftPos := 0;
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Controls[j - 1].Width := Width - Round(FDPIScale * FTabWidth);
            Controls[j - 1].Top := 0;
          ...
          begin
            ...
            if (Position = ppTop) then
            begin
              Controls[j - 1].Top := Round(FDPIScale * FTabWidth);
            end
            else
            begin
              Controls[j - 1].Top := 0;
            end;
            Controls[j - 1].Height := self.Height -  Round(FDPIScale * FTabWidth);
            Controls[j - 1].Visible := True;
          end;
        end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          Controls[j - 1].Width := Width;
          Controls[j - 1].Height := Height - Round(FDPIScale * FTabWidth);
          if (Position = ppTop) then
            Controls[j - 1].Top := Round(FDPIScale * FTabWidth)
          else
            Controls[j - 1].Top := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        case Position of
          ppLeft: DrawGradient(bmp.Canvas, Color, ColorTo, 16, Rect(r.Left, r.Top, r.Left + Round(FDPIScale * FTabWidth), r.Bottom), True);
          ppRight: DrawGradient(bmp.Canvas, Color, ColorTo, 16, Rect(r.Right - Round(FDPIScale * FTabWidth), r.Top, r.Right, r.Bottom), True);
          ppTop: DrawGradient(bmp.Canvas, Color, ColorTo, 16, Rect(r.Left, r.Top, r.Right, r.Top + Round(FDPIScale * FTabWidth)), False);
          ppBottom: DrawGradient(bmp.Canvas, Color, ColorTo, 16, Rect(r.Left, r.Bottom - Round(FDPIScale * FTabWidth), r.Right, r.Bottom), False);
        end;
      end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if FTabColorTo = clNone then
                  Rectangle(r.Left, r.Top, r.Left + Round(FDPIScale * FTabWidth) - 1, r.Top + tw + 4)
                else
                begin
                  DrawGradient(bmp.Canvas, clr, clrto, 16, Rect(r.Left, r.Top, r.Left + Round(FDPIScale * FTabWidth) - 1, r.Top + tw + 4), True);
                  Brush.Style := bsClear;
                  Pen.Color := TabBorderColor;
                  Rectangle(r.Left, r.Top, r.Left + Round(FDPIScale * FTabWidth) - 1, r.Top + tw + 4)
                end;
              ...
              dx := r.Left + Round(FDPIScale * TabWidth) - ((Round(FDPIScale * FTabWidth) - th) div 2);
            end
            ...
            begin
              ...
              begin
                tbmp.Canvas.Draw(0, 0, FTabGlyph);
                BitmapStretch(tbmp, bmp.Canvas, r.right - Round(FDPIScale * FTabWidth), r.Top, tw + ih + 8);
              end
              else
              begin
                if FTabColorTo = clNone then
                  Rectangle(r.Right - Round(FDPIScale * FTabWidth) + 4, r.Top, r.Right, r.Top + tw + 4)
                else
                begin
                  DrawGradient(bmp.Canvas, clr, clrto, 16, Rect(r.Right - Round(FDPIScale * FTabWidth) + 4, r.Top, r.Right, r.Top + tw + 4), True);
                  Brush.Style := bsClear;
                  Pen.Color := TabBorderColor;
                  Rectangle(r.Right - Round(FDPIScale * FTabWidth) + 2, r.Top, r.Right, r.Top + tw + 4);
                end;
              end;
              dx := r.Right + 4 - (Round(FDPIScale * FTabWidth) - th) div 2;
            end
            ...
            begin
              ...
              begin
                if FTabColorTo = clNone then
                  Rectangle(r.Left, r.Top, r.Left + tw + 4, r.Top + Round(FDPIScale * FTabWidth) - 1)
                else
                begin
            ...
            begin
              ...
              begin
                if FTabColorTo = clNone then
                  Rectangle(r.Left, r.Bottom - Round(FDPIScale * FTabWidth) + 1, r.Left + tw + 4, r.Bottom - 1)
                else
                begin
              ...
              dx := r.Left + 4;
              dy := r.Bottom - Round(FDPIScale * FTabWidth) + 4;
            end;
            ...
            begin
              ...
              begin
                if Position = ppLeft then
                  Images.Draw(bmp.Canvas, r.Left + (Round(FDPIScale * FTabWidth) - Images.Width) div 2, r.Top + 6, TAdvToolPanel(Controls[i - 1]).ImageIndex)
                else {if Position = ppRight then}
                  Images.Draw(bmp.Canvas, r.Right + 1 - Images.Width - ((Round(FDPIScale * FTabWidth) - Images.Width) div 2), r.Top + 6, TAdvToolPanel(Controls[i - 1]).ImageIndex);
                dy := dy + ih + 4;
              end
              else {((Position = ppTop) or (Position = ppBottom))}
              begin
                if Position = ppTop then
                  Images.Draw(bmp.Canvas, r.Left + 6, r.Top + (Round(FDPIScale * FTabWidth) - Images.Width) div 2, TAdvToolPanel(Controls[i - 1]).ImageIndex)
                else {if Position = ppBottom then}
                  Images.Draw(bmp.Canvas, r.Left + 6, r.Bottom + 1 - Images.Width - ((Round(FDPIScale * FTabWidth) - Images.Width) div 2), TAdvToolPanel(Controls[i - 1]).ImageIndex);
                dx := dx + iw + 4;
              end;
            end;
            ...
            begin
              dx := dx + 2 - Round(FDPIScale * TabWidth) + (Round(FDPIScale * TabWidth) - th) div 2;
              dy := dy + tw - 4;
            end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  DrawGradient(bmp.Canvas, clr, clrto, 16, Rect(r.Right - Round(FDPIScale * FTabWidth) + 2, r.Top, r.Right, r.Top + tw + 4), True);
                  Brush.Style := bsClear;
                  Pen.Color := TabBorderColor;
                  Rectangle(r.Right - Round(FDPIScale * FTabWidth) + 2, r.Top, r.Right, r.Top + tw + 4);
                end;
            ...
            begin
              ...
              begin
                tbmp.Canvas.Draw(0, 0, FTabGlyph);
                BitmapStretchInWidth(tbmp, bmp.Canvas, r.Left, r.Bottom - Round(FDPIScale * FTabWidth), tw + iw + 8);
              end
              else
              begin
                if FTabColorTo = clNone then
            ...
            begin
              ...
              begin
                if Position = ppLeft then
                  Images.Draw(bmp.Canvas, r.Left + (Round(FDPIScale * FTabWidth) - Images.Width) div 2, r.Top + 6, Panels[i - 1].ImageIndex)
                else {if Position = ppRight then}
                  Images.Draw(bmp.Canvas, r.Right + 1 - Images.Width - ((Round(FDPIScale * FTabWidth) - Images.Width) div 2), r.Top + 6, Panels[i - 1].ImageIndex);
                dy := dy + ih + 4;
                ihu := ih + 4;
              end
              ...
              begin
                if Position = ppTop then
                  Images.Draw(bmp.Canvas, r.Left + 6, r.Top + (Round(FDPIScale * FTabWidth) - Images.Width) div 2, Panels[i - 1].ImageIndex)
                else {if Position = ppBottom then}
                  Images.Draw(bmp.Canvas, r.Left + 6, r.Bottom + 1 - Images.Width - ((Round(FDPIScale * FTabWidth) - Images.Width) div 2), Panels[i - 1].ImageIndex);
                dx := dx + iw + 4;
              end;
            end;
            ...
            begin
              ...
              if (FHoverPanel = i - 1) and (TabHoverColor <> clNone) then
              begin
                clr := StyleServices.GetStyleFontColor(sfEditBoxTextSelected)
              end
              else
              begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        OpenWidth := TAdvToolPanel(Controls[0]).FOpenWidth;
    ...
    FFirstChangeScale := true;

    if ((Position = ppLeft) or (Position = ppRight)) then
    begin
      Width := Round(FDPIScale * FTabWidth);
    end
    else {((Position = ppTop) or (Position = ppBottom))}
    begin
      Height := Round(FDPIScale * FTabWidth);
    end;
...
begin
  ...
  begin
    ...
    if (X > Width - Round(FDPIScale * FTabWidth)) and not FRollInOut and
  ...
  begin
    ...
    if (Y > Height - Round(FDPIScale * FTabWidth)) and not FRollInOut and
      not ((FLastMousePos.X = X) and (FLastMousePos.Y = Y)) then
    begin
      i := GetPanelFromTab(x, y);
      if (i <> FRollInCandidate) then
    ...
    ppRight:
      begin
        ...
        if (FPanelForm.Width > 0) and (i >= 0) and (i < Panels.Count) and (Panels[i].State = psOpened) then
          APanel.OpenWidth := Round(FPanelForm.Width / FDPIScale);
...
begin
  ...
  case Position of
    ppLeft:
      begin
        if delay = 0 then
        begin
          APanel.Left := Round(FDPIScale * FTabWidth);
        end
        else
          ...
          begin
            ...
            if APanel.Left + 15 < Round(FDPIScale * FTabWidth) then
              APanel.Left := APanel.Left + 15
            else
              APanel.Left := Round(FDPIScale * FTabWidth);
          end;
    ...
    ppRight:
      begin
        APanel.Align := alNone;
        APanel.Width := Round(FDPIScale * APanel.FOpenWidth);
        APanel.Anchors := [];
        ...
        while (FPanelForm.Width < Round(FDPIScale * APanel.FOpenWidth)) and (FPanelForm.Width < fw) do
        begin
          if delay = 0 then
          begin
            FPanelForm.Left := Round(FDPIScale * FPanelForm.Left) + FPanelForm.Width - Round(FDPIScale * APanel.FOpenWidth);
            FPanelForm.Width := Round(FDPIScale * APanel.OpenWidth);
            APanel.Left := 2;
  ...
  //          FPanelForm.Perform(WM_SetRedraw,0,0);
            ...
            if FPanelForm.Width + 15 < Round(FDPIScale * APanel.FOpenWidth) then
            begin
              FPanelForm.Left := FPanelForm.Left - 15;
              FPanelForm.Width := FPanelForm.Width + 15;
            end
            ...
            begin
              FPanelForm.Left := FPanelForm.Left - (Round(FDPIScale * APanel.FOpenWidth) - APanel.Width);
              FPanelForm.Width := Round(FDPIScale * APanel.FOpenWidth);
            end;
          ...
          // final panel positioning
            ...
            if delay = 0 then
            begin
              FPanelForm.Left := FPanelForm.Left - (Round(FDPIScale * APanel.OpenWidth) - APanel.Width) + 2;
              FPanelForm.Width := Round(FDPIScale * APanel.FOpenWidth);
            end;
          end;
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if APanel.Top + 15 < Round(FDPIScale * FTabWidth) then
              APanel.Top := APanel.Top + 15
            else
              APanel.Top := Round(FDPIScale * FTabWidth);
          end;
        end;
      end;
    ppBottom:
      begin
        APanel.Align := alNone;
        APanel.Height := Round(FDPIScale * APanel.OpenWidth);
        APanel.Anchors := [];
        ...
        if delay = 0 then
        begin
          FPanelForm.Height := Round(FDPIScale * APanel.OpenWidth);
          FPanelForm.Top := FPanelForm.Top - Round(FDPIScale * APanel.OpenWidth);
          APanel.Top := 2;
        ...
        begin
          ...
          while (FPanelForm.Height < Round(FDPIScale * APanel.OpenWidth)) and (FPanelForm.Height < fw) do
          begin
            t := GetTickCount;
            ...
            if FPanelForm.Height + 15 < Round(FDPIScale * APanel.OpenWidth) then
            begin
              FPanelForm.Top := FPanelForm.Top - 15;
              FPanelForm.Height := FPanelForm.Height + 15;
            end
            ...
            begin
              FPanelForm.Top := FPanelForm.Top - (Round(FDPIScale * APanel.OpenWidth) - APanel.Height) + 2;
              FPanelForm.Height := Round(FDPIScale * APanel.OpenWidth);
            end;
...
begin
  ...
  begin
    case Position of //TODO:OK v1.2
      ppLeft, ppRight:
        OutTab := (pt.X < Round(FDPIScale * FTabWidth)) and (GetPanelFromTab(0, pt.y) = -1);
      ppTop, ppBottom:
        OutTab := (pt.Y < Round(FDPIScale * FTabWidth)) and (GetPanelFromTab(pt.x, 0) = -1);
    end;
  end;
  ...
  begin
    ...
    begin
      if Position = ppLeft then
      begin
        Panels[FRollOutPanel].FOpenWidth := Round(Panels[FRollOutPanel].Panel.Width / FDPIScale);
      end;
...
begin
  ...
  begin
    ...
    begin
      if (Position = ppLeft) or (Position = ppRight) then
      begin
        Panels[j - 1].FOpenWidth := Panels[j - 1].Panel.Width;
      end
      else {if (Position = ppTop) or (Position = ppBottom)}
      begin
        Panels[j - 1].FOpenWidth := Panels[j - 1].Panel.Height;
      end;
      ...
      case Position of
        ppLeft:
          begin
            ...
            Panels[j - 1].Panel.Left := -Round(FDPIScale * Panels[j - 1].OpenWidth);
            Panels[j - 1].Panel.Width := Round(FDPIScale * Panels[j - 1].OpenWidth);
        ...
        ppTop:
          begin
            Panels[j - 1].Panel.Align := alNone;
            Panels[j - 1].Panel.Height := 0;
            Panels[j - 1].Panel.Height := Round(FDPIScale * Panels[j - 1].OpenWidth);
            Panels[j - 1].Panel.Top := -Round(FDPIScale * Panels[j - 1].OpenWidth);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              APanel.Left := APanel.Left + Round(FDPIScale * Panels[i - 1].OpenWidth);
            end;
          end;
          APanel.Align := alLeft;
          APanel.Width := Round(FDPIScale * APanel.OpenWidth);
          APanel.SendToBack;
          RemovePanel(nil);
          FRollOutPanel := -1
        end
      ...
      begin
        ...
        begin
          ...
          begin
            if Panels[i - 1].Panel.Locked then
              Panels[i - 1].Panel.Width := Round(FDPIScale * Panels[i - 1].Panel.OpenWidth);
          end;
        ...
        begin
          ...
          begin
            if Panels[i - 1].Panel.Locked and (Panels[i - 1].Panel <> APanel) then
            begin
              if APanel.Left > Round(FDPIScale * Panels[i - 1].Panel.Left) then
                Panels[i - 1].Panel.Left := Round(FDPIScale * Panels[i - 1].Panel.Left) + Round(FDPIScale * APanel.OpenWidth);
            end;
          end;
          ...
          APanel.Top := 0;
          APanel.Height := Height;
          APanel.Width := Round(FDPIScale * APanel.OpenWidth) - 2;
          APanel.Anchors := [akLeft, akTop, akRight, akBottom];

          pt := Point(self.Left + self.Width, self.Top);
          pt := Parent.ClientToScreen(pt);

          FPanelForm.Left := pt.X - self.TabWidth - Round(FDPIScale * APanel.OpenWidth);
      ...
      begin
        ...
        begin
          ...
          begin
            if Panels[i - 1].Panel.Locked and (Panels[i - 1].Panel <> APanel) then
            begin
              APanel.Top := APanel.Top + Round(FDPIScale * Panels[i - 1].OpenWidth);
            end;
          end;
    ...
    ppBottom:
      begin
        ...
        begin
          ...
          begin
            if Panels[i - 1].Panel.Locked then
              Panels[i - 1].Panel.Height := Round(FDPIScale * Panels[i - 1].Panel.OpenWidth);
          end;
        ...
        begin
          ...
          begin
            ...
            begin
              if APanel.Top > Panels[i - 1].Panel.Top then
                Panels[i - 1].Panel.Top := Panels[i - 1].Panel.Top + Round(FDPIScale * APanel.OpenWidth);
            end;
          end;
          ...
          APanel.Top := 2;
          APanel.width := width;
          APanel.Height := Round(FDPIScale * APanel.OpenWidth) - 2;
          APanel.Anchors := [akLeft, akTop, akRight, akBottom];
          FPanelForm.Left := pt.X;
          FPanelForm.Top := pt.Y - Round(FDPIScale * APanel.OpenWidth);
          FPanelForm.Width := Width;
          FPanelForm.Height := Round(FDPIScale * APanel.OpenWidth);
          FPanelForm.Visible := True;
          SetWindowPos(FPanelForm.Handle, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE or SWP_NOSIZE);
...
begin
  ...
  begin
    ...
    pt := Point(Left, Top);
    pt := Parent.ClientToScreen(pt);
    FPanelForm.Left := pt.X - Round(FDPIScale * FPanelForm.Width);
    FPanelForm.Top := pt.Y;
    FPanelForm.Height := Height;
...
procedure TAdvToolPanelTab.InitVCLStyle(init: boolean);
begin
  FUseVCLStyles := False;
  ...
  begin
    ...
    TabBorderColor  := StyleServices.GetSystemColor(clBtnShadow);
    TabColor        := StyleServices.GetSystemColor(clBtnFace);
    TabColorTo      := StyleServices.GetSystemColor(clBtnFace);
    TabHoverColor   := StyleServices.GetSystemColor(clHighlight);
    TabHoverColorTo := StyleServices.GetSystemColor(clHighlight);
    ...
    {$ENDIF xe6}
      ...
      ColorTo := StyleServices.GetSystemColor(clBtnFace);
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF xe6}
  end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvToolPanelTab.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(true);
  Invalidate;
end;
    ...
    begin
      ...
      begin
        ...
        if Position = ppLeft then
          ...
          Panels[i - 1].Panel.Left := Left - Round(FDPIScale * Panels[i - 1].Panel.OpenWidth)
        else if Position = ppTop then
          Panels[i - 1].Panel.Top := Height
        else if Position = ppBottom then
          Panels[i - 1].Panel.Top := Top - Round(FDPIScale * Panels[i - 1].Panel.OpenWidth);
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvToolPanel.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvToolPanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
...
begin
  ...
  if ShowClose then
    r2 := Rect(r.Right - Round(FDPIScale * 42), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 23), r.Top + Round(FDPIScale * 21))
  else
    r2 := Rect(r.Right - Round(FDPIScale * 21), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 5), r.Top + Round(FDPIScale * 21));

  r1 := Rect(r.Right - Round(FDPIScale * 21), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 5), r.Top + Round(FDPIScale * 21));
...
begin
  ...
  if CaptionButton then
    r := Rect(r.Left, r.Top, r.Right, r.Top + Round(FDPIScale * 21))
  else
    r := Rect(r.Right - Round(FDPIScale * 42), r.Top, r.Right, r.Top + Round(FDPIScale * 21));
...
begin
  if ((FTab.Position = ppLeft) or (FTab.Position = ppRight)) then
    FOpenWidth := Round(Width / FDPIScale)
  else {((FTab.Position = ppTop) or (FTab.Position = ppBottom))}
    FOpenWidth := Round(Height / FDPIScale);
...
// cancel drag when not started in caption
  if not ((pt.Y < Round(FDPIScale * FCaptionHeight)) and (pt.X < Width - 40)) then
  begin
    EndDrag(false);
    CancelDrag;
    Docking := False;
...
function TAdvToolPanel.GetOpenWidth: Integer;
begin
    Result := FOpenWidth;
end;
...
procedure TAdvToolPanel.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF xe2}
begin
  ...
  begin
    ...
    StyleServices.GetElementColor(StyleServices.GetElementDetails(thHeaderItemNormal), ecTextColor, clr);
    SectionLayout.CaptionFontColor := clr;
    SectionLayout.ItemFontColor := clr;
    FocusCaptionFontColor := clr;
    NoFocusCaptionFontColor := clr;

    SectionLayout.BorderColor      := StyleServices.GetSystemColor(clBtnShadow);
    SectionLayout.BackGroundColor  := clNone;

    NoFocusCaptionColor   := StyleServices.GetSystemColor(clHighlight);
    NoFocusCaptionColorTo := StyleServices.GetSystemColor(clHighlight);
    FocusCaptionColor     := StyleServices.GetSystemColor(clHighlight);
    FocusCaptionColorTo   := StyleServices.GetSystemColor(clHighlight);

    DownButtonColor    := StyleServices.GetSystemColor(clBtnShadow);
    DownButtonColorTo  := StyleServices.GetSystemColor(clBtnShadow);
    HoverButtonColor   := StyleServices.GetSystemColor(clBtnHighlight);
    HoverButtonColorTo := StyleServices.GetSystemColor(clBtnHighlight);
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvToolPanel.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(true);
  Invalidate;
end;
...
procedure TAdvToolPanel.Loaded;
begin
  inherited;
  if not (csDesigning in ComponentState) then
    InitVCLStyle(true);
...
begin
  ...
  if Floating or not FShowCaption then
    ...
    h := Round(FDPIScale * FCaptionHeight);

  if (csDesigning in ComponentState) and not FSizing then
  begin
    for i := 1 to Sections.Count do
    begin
      if (Y > h) and (Y < h + Round(FDPIScale * SectionHeight)) then
      begin
        if (i > 1) then
        begin
          FSizeSection := i - 1;
...
begin
  ...
  begin
    ...
    begin
      d := Y - FSizeY;
      if FSizeHeight + d > Round(FDPIScale * SectionHeight) then
        TAdvToolPanelSection(Sections.Items[FSizeSection - 1]).Height := Round(FDPIScale * FSizeHeight) + d
      else if Round(FDPIScale * TAdvToolPanelSection(Sections.Items[FSizeSection - 1]).Height) <> Round(FDPIScale * SectionHeight) + 1 then
        TAdvToolPanelSection(Sections.Items[FSizeSection - 1]).Height := Round(FDPIScale * SectionHeight) + 1;
      Exit;
    end;
  end;
  ...
  begin
    if Locked and Tab.AutoDock and AllowDocking and (Y < Round(FDPIScale * FCaptionHeight)) and not floating and (x > 0) and ((not FDockDots and (x < Width - 40)) or (FDockDots and (x < Round(FDPIScale * DOCKDOTS_WIDTH)))) then
    begin
  ...
  begin
    if Locked and Tab.AutoDock and AllowDocking and (Y < Round(FDPIScale * FCaptionHeight)) and not floating and ((not FDockDots and (x < Width - 40)) or (FDockDots and (x < Round(FDPIScale * DOCKDOTS_WIDTH)))) then
    begin
      DragKind := dkDock;
  ...
  if CaptionButton then
    ...
    r := Rect(r.Right - Round(42 * FDPIScale), r.Top, r.Right, r.Top + Round(FDPIScale * 21));
...
//*******
  ...
  begin
    ...
    begin
      ...
      r := ClientRect;
      r := Rect(r.Right - Round(FDPIScale * 20), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 5), r.Top + Round(FDPIScale * 18));
      ...
      if ShowClose then
        r := Rect(r.Right - Round(FDPIScale * 40), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 21), r.Top + Round(FDPIScale * 18))
      else
        r := Rect(r.Right - Round(FDPIScale * 20), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale * 5), r.Top + Round(FDPIScale * 18));
      ...
      r2.Right := r.Left;
      if FDockDots then
        r2.Left := r2.Left + Round(FDPIScale * DOCKDOTS_WIDTH);
      r2.Bottom := r2.Top + Round(FDPIScale * FCaptionHeight);
...
begin
  ...
  ct := (Round(FDPIScale * SectionHeight) - Round(FDPIScale * NODEBTN_SIZE)) div 2;
  for i := 1 to Sections.Count do
  begin
    if (i - 1 = SectionIndex) and (TAdvToolPanelSection(Sections.Items[i - 1]).Node) then
    begin
      Result := Rect(4 + Round(FDPIScale * FSectionLayout.Indent), h + ct{2 + 2}, 4 + Round(FDPIScale * NODEBTN_SIZE) + 1 + Round(FDPIScale * FSectionLayout.Indent), h + ct{2 + 2} + Round(FDPIScale * NODEBTN_SIZE) + 1);
      break;
    end;
    h := h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height);
  end;
end;
...
var
  ...
  FGColor: TColor;
  NormalShow: Boolean;
  lf, tp, i, h, a, j, fh, yo, xo, OldFontH: integer;
  tRgn, rgn1, rgn2: HRGN;
  {$IFDEF DELPHIXE2_LVL}
  lDetails: TThemedElementDetails;
  clr: TColor;
...
begin
  ...
  //------ BackGround Image
    if (not FUseVCLStyles) and Assigned(FBackGround) then
    begin
      R.Top := R.Top + Round(FDPIScale * CaptionHeight);
      R.Left := R.Left + 1;
...
//----- Section
    ...
    begin
      ...
      begin
        ...
        begin
          { draw the section header using VCL theming }
          CapR := Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h, width - 1, h + Round(FDPIScale * SectionHeight) + 1);
          {$IFDEF DELPHIXE2_LVL}
            lDetails := StyleServices.GetElementDetails(thHeaderItemNormal);
            StyleServices.DrawElement(Canvas.Handle, lDetails, CapR);
          {$ENDIF}
        end
        else
        begin
          ...
          begin
            ...
            case FSectionLayout.Corners of
              scRectangle:
                DrawGradient(Canvas, FSectionLayout.CaptionColor, FSectionLayout.CaptionColorTo, 48, Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h, width - 2, h + Round(FDPIScale * SectionHeight) + 1), FSectionLayout.CaptionGradientDir = gdHorizontal);
              scRoundLeft:
                begin
                  trgn := CreateRoundRectRgn(CapR.Left, CapR.Top, CapR.Right, CapR.Bottom - (CapR.Bottom - CapR.Top) div 2, Round(FDPIScale * 16), Round(FDPIScale * 16));
                  rgn1 := CreateRoundRectRgn(CapR.Left, CapR.Top, CapR.Right, CapR.Bottom, Round(FDPIScale * 16), Round(FDPIScale * 16));
                  ...
                  DrawGradient(Canvas, FSectionLayout.CaptionColor, FSectionLayout.CaptionColorTo, 48, Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h, width - 1, h + Round(FDPIScale * SectionHeight) + 1), FSectionLayout.CaptionGradientDir = gdHorizontal);
        ...
        begin
          ...
          MoveTo(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * SectionHeight));
          LineTo(Width - 2, h + Round(FDPIScale * SectionHeight));
        end;
        ...
        begin
          a := Round(FDPIScale * 4);
          ct := (Round(FDPIScale * SectionHeight) - Round(FDPIScale * NODEBTN_SIZE)) div 2;
          r3 := Rect(a + Round(FDPIScale * FSectionLayout.Indent), h + ct, a + Round(FDPIScale * NODEBTN_SIZE) + 1 + Round(FDPIScale * FSectionLayout.Indent), h + ct + Round(FDPIScale * NODEBTN_SIZE) + 1);
          if (FSectionLayout.BackGroundColor <> clNone) and (FSectionLayout.BackGroundColorTo <> clNone) then
            DrawGradient(Canvas, FSectionLayout.BackGroundColor, FSectionLayout.BackGroundColorTo, 16, r3, true);
          Brush.Style := bsClear;
          Pen.Width := 1;
          if FUseVCLStyles then
          begin
            pen.Color  := FSectionLayout.BorderColor;
            font.Color := FSectionLayout.BorderColor;
          end
          else
          begin
            pen.Color  := clGray;
          ...
          begin
            if not FUseVCLStyles then
              Pen.Color := clBlack;
            Canvas.MoveTo(r3.left + Round(FDPIScale * 2), r3.Top + Round(FDPIScale * 4));
            Canvas.LineTo(R3.Left + Round(FDPIScale * 7), r3.Top + Round(FDPIScale * 4));
          end
          else
          begin
            if not FUseVCLStyles then
            ...
            Canvas.MoveTo(r3.left + Round(FDPIScale * 2), r3.Top + Round(FDPIScale * 4));
            Canvas.LineTo(R3.Left + Round(FDPIScale * 7), r3.Top + Round(FDPIScale * 4));
            Canvas.MoveTo(r3.left + Round(FDPIScale * 4), r3.Top + Round(FDPIScale * 2));
            Canvas.LineTo(R3.Left + Round(FDPIScale * 4), r3.Top + Round(FDPIScale * 7));
          end;

          a := a + Round(FDPIScale * NODEBTN_SIZE);
        end;
                                                  // Draw Caption Image
        if Assigned(FSectionImages) and (TAdvToolPanelSection(Sections.Items[i - 1]).CaptionImageIndex > -1) then
        begin
          FSectionImages.Draw(Canvas, a + 2 + Round(FDPIScale * FSectionLayout.Indent), h + 2, TAdvToolPanelSection(Sections.Items[i - 1]).CaptionImageIndex);
          a := a + FSectionImages.Width;
        end; // Draw Caption Text
        Canvas.Brush.Style := bsClear;
        Font.Color := FSectionLayout.CaptionFontColor; // FNoFocusCaptionFontColor;
        ct := (Round(FDPIScale * SectionHeight) - Canvas.TextHeight('gh')) div 2;
        Canvas.TextOut(4 + a + Round(FDPIScale * FSectionLayout.Indent), h + ct{2}, TAdvToolPanelSection(Sections.Items[i - 1]).Caption);

        if TAdvToolPanelSection(Sections.Items[i - 1]).Expanded then
        begin
                                                   // Draw BackGround
          if FSectionLayout.BackGroundColor <> clNone then
            DrawGradient(Canvas, FSectionLayout.BackGroundColor, FSectionLayout.BackGroundColorTo, 128, Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * SectionHeight) + 1, width - 1, h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height) - Round(FDPIScale * FSectionLayout.Spacing)), FSectionLayout.BackGroundGradientDir = gdHorizontal);
                                                   // Draw Border
          ...
          begin
            ...
            MoveTo(Width - 2, h + Round(FDPIScale * SectionHeight) + 1);
            LineTo(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * SectionHeight) + 1);

            MoveTo(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * SectionHeight) + 1);
            LineTo(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height) - 1 - Round(FDPIScale * FSectionLayout.Spacing));

            MoveTo(1 + Round(FDPIScale * FSectionLayout.Indent), h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height) - 1 - Round(FDPIScale * FSectionLayout.Spacing));
            LineTo(Width - 2, h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height) - 1 - Round(FDPIScale * FSectionLayout.Spacing));
          end;
          ...
          for j := 0 to TAdvToolPanelSection(Sections.Items[i - 1]).SectionItems.Count - 1 do
          begin
            if (h + ct{4} + Round(FDPIScale * SectionHeight) + (j * fh)) >= (h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height) - Round(FDPIScale * FSectionLayout.Spacing)) then
              break;
            ...
            begin
              ...
              if Assigned(FSectionImages) and (a > 12) then
                MoveTo(a - FSectionImages.Width - 2 + Round(FDPIScale * FSectionLayout.Indent), h + cy{10} + Round(FDPIScale * SectionHeight) + (j * fh))
              else
                MoveTo(a + Round(FDPIScale * FSectionLayout.Indent), h + cy{10} + Round(FDPIScale * SectionHeight) + (j * fh));
              LineTo(Width - 2, h + cy{10} + Round(FDPIScale * SectionHeight) + (j * fh));
            end
            else
            begin
              ...
              begin
            //draw Image
                FSectionImages.Draw(Canvas, a - FSectionImages.Width - 2 + Round(FDPIScale * FSectionLayout.Indent), h + 4 + Round(FDPIScale * SectionHeight) + (j * fh), TAdvToolPanelSection(Sections.Items[i - 1]).SectionItems.Items[j].ImageIndex);
              end;
              ...
              begin
                ...
                if FUseVCLStyles then
                begin
                  lDetails := StyleServices.GetElementDetails(ttlTextLabelDisabled);
                  StyleServices.GetElementColor(lDetails, ecTextColor, clr);
                  Font.Color := clr;
                end
                else
              ...
              CapR := Rect(a + Round(FDPIScale * FSectionLayout.Indent), h + ct + Round(FDPIScale * SectionHeight) + (j * fh), Width - 2, h + ct + Round(FDPIScale * SectionHeight) + ((j+1) * fh));
...
//----- End Section
    ...
    r := ClientRect;
    r := Rect(r.Left, r.Top, r.Right - 1, r.Top + Round(FDPIScale * CaptionHeight));
    ...
    begin
      ...
      begin
        ...
        FillRect(r);
      if (not FUseVCLStyles) and FShowCaptionBorder then
      begin
        Brush.Style := bsClear;
        Pen.Color := clGray;
        Pen.Width := 1;
        RoundRect(r.Left, r.Top, r.Right, r.Bottom, Round(FDPIScale * 3), Round(FDPIScale * 3));
      end;
    end;
    ...
    begin
      fr := Rect(r.Right - Round(FDPIScale * 16) - Round(FDPIScale * 3), r.Top + Round(FDPIScale * 3), r.Right - Round(FDPIScale * 6), r.Top + Round(FDPIScale * 16));
      InflateRect(fr, 1, 1);
      ...
      begin
        ...
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) - 1, r.Top + Round(FDPIScale * 12));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) - 1 + Round(FDPIScale * 8) , r.Top + Round(FDPIScale * 4));
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width), r.Top + Round(FDPIScale * 12));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + Round(FDPIScale * 8) , r.Top + Round(FDPIScale * 4));

        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) -1 + Round(FDPIScale * 8) , r.Top + Round(FDPIScale * 12));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) - 1, r.Top + Round(FDPIScale * 4));
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + Round(FDPIScale * 8) , r.Top + Round(FDPIScale * 12));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width), r.Top + Round(FDPIScale * 4));
      end
      else
      begin
                 {/}
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) - 1, r.Top + 11);
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + 7, r.Top + 4);
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width), r.Top + 11);
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + 6, r.Top + 4);
                 {\}
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) - Round(FDPIScale), r.Top + Round(FDPIScale * 5));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + Round(FDPIScale * 7), r.Top + Round(FDPIScale * 12));
        MoveTo(r.Right - Round(FDPIScale * FCloseGlyph.Width), r.Top + Round(FDPIScale * 5));
        LineTo(r.Right - Round(FDPIScale * FCloseGlyph.Width) + Round(FDPIScale * 6), r.Top + Round(FDPIScale * 12));
      end;

      r.Right := r.Right - Round(FDPIScale * 16) - Round(FDPIScale * 5);
    end;
    ...
    begin
      fr := Rect(r.Right - Round(FDPIScale * 16) - Round(FDPIScale * 2), r.Top + Round(FDPIScale * 2), r.Right - Round(FDPIScale), r.Top + Round(FDPIScale * 16));
      InflateRect(fr, 1, 1);
      if PtInRect(fr, pt) and not (csDesigning in ComponentState) then
      begin
        if FButton3D then
      ...
      begin
        Pen.Color := FGColor;
                   {-}
        MoveTo(fr.left + Round(FDPIScale * 7), fr.Top + Round(FDPIScale * 2));
        LineTo(fr.left + Round(FDPIScale * 11), fr.Top + Round(FDPIScale * 2));
        MoveTo(fr.left + Round(FDPIScale * 7), fr.Top + Round(FDPIScale * 4));
        LineTo(fr.left + Round(FDPIScale * 11), fr.Top + Round(FDPIScale * 4));
                   {| |}
        MoveTo(fr.Left + Round(FDPIScale * 7), fr.Top + Round(FDPIScale * 2));
        LineTo(fr.Left + Round(FDPIScale * 7), fr.Top + Round(FDPIScale * 9));
        MoveTo(fr.Left + Round(FDPIScale * 11), fr.Top + Round(FDPIScale * 2));
        LineTo(fr.Left + Round(FDPIScale * 11), fr.Top + Round(FDPIScale * 9));
                   {__}
        MoveTo(fr.Left + Round(FDPIScale * 5), fr.Top + Round(FDPIScale * 9));
        LineTo(fr.Left + Round(FDPIScale * 14), fr.Top + Round(FDPIScale * 9));
                   {I}
        MoveTo(fr.Left + Round(FDPIScale * 9), fr.Top + Round(FDPIScale * 10));
        LineTo(fr.Left + Round(FDPIScale * 9), fr.Top + Round(FDPIScale * 14));
      end
      else if {not FUnlockedGlyph.Empty and}  ShowLock then
      begin
        Pen.Color := FGColor;
                   {  |}
        MoveTo(fr.right - Round(FDPIScale * 3), fr.Top + Round(FDPIScale * 4));
        LineTo(fr.right - Round(FDPIScale * 3), fr.Top + Round(FDPIScale * 8));
        MoveTo(fr.right - Round(FDPIScale * 5), fr.Top + Round(FDPIScale * 4));
        LineTo(fr.right - Round(FDPIScale * 5), fr.Top + Round(FDPIScale * 8));
                   {==}
        MoveTo(fr.right - Round(FDPIScale * 3), fr.Top + Round(FDPIScale * 4));
        LineTo(fr.right - Round(FDPIScale * 10), fr.Top + Round(FDPIScale * 4));
        MoveTo(fr.right - Round(FDPIScale * 3), fr.Top + Round(FDPIScale * 8));
        LineTo(fr.right - Round(FDPIScale * 10), fr.Top + Round(FDPIScale * 8));
                   {| }
        MoveTo(fr.right - Round(FDPIScale * 10), fr.Top + Round(FDPIScale * 2));
        LineTo(fr.right - Round(FDPIScale * 10), fr.Top + Round(FDPIScale * 11));
                   {I}
        MoveTo(fr.right - Round(FDPIScale * 11), fr.Top + Round(FDPIScale * 6));
        LineTo(fr.right - Round(FDPIScale * 15), fr.Top + Round(FDPIScale * 6));
      end;
    ...
    begin
      ...
      begin
        ...
        Canvas.Brush.Color := StyleServices.GetSystemColor(clBtnHighlight);
      end
      else
      {$ENDIF}
      begin
      ...
      r2 := Rect(r.Left, r.Top, r.Left + Round(FDPIScale * DOCKDOTS_WIDTH), r.Top + Round(FDPIScale * FCaptionHeight));

      R2.Top := R2.Top + Round(FDPIScale * 2);
      lf := r2.left + Round(FDPIScale) + ((Round(FDPIScale * DOCKDOTS_WIDTH) - 2 {Dots Width}) div 2);
      tp := r2.Top + (Round(FDPIScale * CaptionHeight) - Round(FDPIScale * 17)) div 2;

      R2.Left := lf + 1;
      r2.Top := tp;
      for i := 1 to 4 do
      begin
        Canvas.Rectangle(R2.Left, R2.Top, R2.Left + Round(FDPIScale * 2), R2.Top + Round(FDPIScale * 2));
        R2.Top := R2.top + Round(FDPIScale * 4);
      end;
      ...
      r.Left := r.Left + Round(FDPIScale * DOCKDOTS_WIDTH) - Round(FDPIScale);
    end;

    r := Rect(r.Left + Round(FDPIScale * 4), r.Top + Round(FDPIScale * 4), r.Right - Round(FDPIScale * 32) - Round(FDPIScale * 5), r.Top + Round(FDPIScale * CaptionHeight) - 2);

    if CaptionButton then
    begin
      R.Right := r.Right - Round(FDPIScale * 5);
      fr := Rect(r.Left - Round(FDPIScale), r.top - Round(FDPIScale * 2), r.Right, r.Bottom - Round(FDPIScale));
      ...
      begin
        ...
        begin
          ...
          begin
            lDetails := StyleServices.GetElementDetails(thHeaderItemNormal);
            StyleServices.DrawElement(self.Canvas.Handle, lDetails, fr);
          end
          else
          {$ENDIF}
          begin
      ...
      begin
        ...
        begin
          if FMouseDown then
            lDetails := StyleServices.GetElementDetails(tcDropDownButtonPressed)
          else
            lDetails := StyleServices.GetElementDetails(tcDropDownButtonHot);
        end
        else
          lDetails := StyleServices.GetElementDetails(tcDropDownButtonNormal);

        fr := Rect(r.Right - Round(FDPIScale * 20), r.top - Round(FDPIScale), r.Right - Round(FDPIScale * 3), r.Bottom - Round(FDPIScale));
        StyleServices.DrawElement(self.Canvas.Handle, lDetails, fr);
      end
      else
      {$ENDIF}
      begin
        ...
        if FActive then
          ...
          Pen.Color := FNoFocusCaptionFontColor;
           {-------}
        MoveTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 7), fr.Bottom - Round(FDPIScale * 10));
        LineTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 14), fr.Bottom - Round(FDPIScale * 10));
            {-----}
        MoveTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 8), fr.Bottom - Round(FDPIScale * 9));
        LineTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 13), fr.Bottom - Round(FDPIScale * 9));
             {---}
        MoveTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 9), fr.Bottom - Round(FDPIScale * 8));
        LineTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 12), fr.Bottom - Round(FDPIScale * 8));
              {-}
        MoveTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 10), fr.Bottom - Round(FDPIScale * 7));
        LineTo(fr.right - Round(FDPIScale * 17) + Round(FDPIScale * 11), fr.Bottom - Round(FDPIScale * 7));
        //-------
      end;
      r.Right := r.Right - Round(FDPIScale * 10);
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          R2 := Rect(a - m + Round(FDPIScale * FSectionLayout.Indent), h + 4 + Round(FDPIScale * SectionHeight) + (j * fh), a + Canvas.TextWidth(TAdvToolPanelSection(Sections.Items[i - 1]).SectionItems.Items[j].Caption) + Round(FDPIScale * FSectionLayout.Indent),
                     h + 4 + Round(FDPIScale * SectionHeight) + (j * fh) + fh);
          if PtInRect(R2, Point(X, Y)) then
          begin
            SectionIndex := i - 1;
            ItemRect := Rect(1 + Round(FDPIScale * FSectionLayout.Indent), R2.Top, Width - 1, R2.Bottom);
            Result := j;
            break;
          end;
        end;
...
begin
  ...
  begin
    ...
    begin
      if Actual then
        Result := Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h, width - 1, h + TAdvToolPanelSection(Sections.Items[i - 1]).ActualHeight)
      else
        Result := Rect(1 + Round(FDPIScale * FSectionLayout.Indent), h, width - 1, h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height));
      break;
    end;
    h := h + Round(FDPIScale * TAdvToolPanelSection(Sections.Items[i - 1]).Height);
  end;
...
begin
  ...
  if (Tab.Position = ppLeft) and (CanSize) then
    if (pt.X >= Width - 5) and (pt.X <= Width + 2) then
    begin
      Msg.Result := HTRIGHT;
    end;

  if (Tab.Position = ppRight) and (CanSize) and Locked then
    if (pt.X <= 5) and (pt.X >= -3) then
    begin
      Msg.Result := HTLEFT;
    end;
...
begin
  ...
  begin
    ...
    for i := 1 to Sections.Count do
    begin
      if (P.Y > h) and (P.Y < h + Round(FDPIScale * SectionHeight)) then
      begin
