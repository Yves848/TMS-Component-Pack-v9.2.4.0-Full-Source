...
const
  ...
  // 1.1.2.0 : New : Per monitor support for high DPI
...
begin
  if not FCalculatorLook.FButtonBkg.Empty then   //Because we don't want to scale images on high DPI
  begin
    FDPIScale := 1;
  end;

  FButtonWidth := Round(FDPISCale * FCalculatorLook.ButtonWidth);
  FButtonHeight := Round(FDPISCale * FCalculatorLook.ButtonHeight);

  n := 5;
  with FExtraButtons do
   begin
    if (fExtra1 <> '') or (fExtra2 <> '') or (fExtra3 <> '') or (fExtra4 <> '') then
      n := 6;
     ...
     begin
      ...
      if not FCalculatorLook.ButtonBkg.Empty then
        begin
         ...
         sp[18 + i].Spacing := - (FCalculatorLook.ButtonBkg.Width shr 1) - (Round(FDPIScale * FCalculatorLook.Font.Size) shr 1);
         sp[18 + i].Margin := 0;
        end;

      sp[18 + i].Font.Assign(FCalculatorLook.Font);
      sp[18 + i].Font.Height := Round(FDPIScale * FCalculatorLook.Font.Height);
  ...
  begin
    ...
    sp[i].Font.Height := Round(FDPIScale * FCalculatorLook.Font.Height);

    if not FCalculatorLook.ButtonBkg.Empty then
    begin
      sp[i].Glyph := FCalculatorLook.ButtonBkg;
      sp[i].Spacing := -(FCalculatorLook.ButtonBkg.Width shr 1) - (Round(FDPIScale * FCalculatorLook.Font.Size) shr 1);
      sp[i].Margin := 0;
    end;
...
begin
  ...
  FCalcForm.ButtonWidth := Round(FDPISCale * FCalculatorLook.ButtonWidth);
  FCalcForm.ButtonHeight := Round(FDPISCale * FCalculatorLook.ButtonHeight);
  ...
  if P.y + Round(FDPISCale * FCalcForm.Height) >= GetSystemMetrics(SM_CYSCREEN) then
    fDropDirection := True;

  if P.y - Round(FDPISCale * FCalcForm.Height) <= 0 then
    fDropDirection := False;

  if (FDropDirection=false) then
    FCalcForm.Top:=P.y + self.Height
  else
    FCalcForm.Top:=P.y - Round(FDPISCale * FCalcForm.Height);
...
{$IFNDEF DELPHIXE10_LVL}
procedure TMoneyEdit.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TMoneyEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(FButton, FButton.Canvas);
