...
const
  ...
  // v1.3.1.0 : Improved : Support for High DPI
  // v1.3.1.1 : Deprecated : TCalPanel
...
begin
  ...
  begin
    ...
    if daymode then
      polygon([Point(Round(FDPIScale * (xoffset + 10)), Round(FDPIScale * 1)), Point(Round(FDPIScale * (xoffset + 5)), Round(FDPIScale * 6)), Point(Round(FDPIScale * (xoffset + 10)), Round(FDPIScale * 11))])
    else
      polygon([Point(Round(FDPIScale * 10), Round(FDPIScale * 1)), Point(Round(FDPIScale * 5), Round(FDPIScale * 6)), Point(Round(FDPIScale * 10), Round(FDPIScale * 11))]);
...
begin
  ...
  begin
    ...
    polygon([Point(width - Round(FDPIScale * 10), Round(FDPIScale)), Point(width - Round(FDPIScale * 10), Round(FDPIScale * 11)), Point(width - Round(FDPIScale * 5), Round(FDPIScale * 6))]);
    brush.color := self.Color;
  end;
end;
...
begin
  ...
  Canvas.Font.Height := Round(fFont.Height * FDPIScale);
  ...
  begin
   ...
   {first day of the month}
    ...
    begin
      ...
      cyoffset := Round(FDPIScale * yoffset);
      cxoffset := Round(FDPIScale * xoffset);
      ...
      begin
        pen.color := ftextcolor;
          ...
          //lineto(18,height);
        moveto(cxoffset - 2, dy - 3 + cyoffset);
        lineto(cxoffset - 2, height);
        ...
        begin
          ...
          textout(4, i * dy + cyoffset, tmpstr);
        end;
      end;
     ...
     {draw days here}
      ...
      for i := 1 to 7 do
        textout(i * dx + cxoffset, cyoffset, ' ' + FNameofDays.GetDay(i + startday - 1));

     {draw line under days}
      pen.color := clblack;
      moveto(cxoffset + 0, dy - 5 + cyoffset);
      lineto(cxoffset + 7 * dx + 6, dy - 5 + cyoffset);
      pen.color := clwhite;
      moveto(cxoffset + 0, dy - 4 + cyoffset);
      lineto(cxoffset + 7 * dx + 6, dy - 4 + cyoffset);
      pen.color := clblack;
      ...
      for i := 1 to 7 do
        ...
        begin
          ...
          begin
            ...
            setbkmode(handle, TRANSPARENT);
            textout(cxoffset + i * dx, j * dy + cyoffset, inttostr(daysinmonth(pmo, pye) - (fd - i)));
          end
          else
          begin
            ...
            begin
              ...
              setbkmode(handle, TRANSPARENT);
              textout(cxoffset + i * dx, j * dy + cyoffset, inttostr(i + (j - 1) * 7 - fd - daysinmonth(mo, ye)));
            end
            else
            begin
              if assigned(fOnGetDateEvent) then
              ...
              begin
                ...
                textout(cxoffset + i * dx, j * dy + cyoffset, inttostr(i + (j - 1) * 7 - fd));
                ...
                begin
                  r.right := cxoffset + i * dx + 2;
                  r.top := j * dy + cyoffset - 2;
                  r.bottom := r.top + dy;
                  r.left := r.right - dx + 2;
                  pen.color := clBlack;
                  Drawfocusrect(r);
          ...
          begin
            pen.color := clgray;
            moveto(cxoffset + (i - 1) * dx + 4, (j + 1) * dy - 4 + cyoffset);
            lineto(cxoffset + (i - 1) * dx + 4, j * dy - 2 + cyoffset);
            lineto(cxoffset + i * dx + 2, j * dy - 2 + cyoffset);
            pen.color := clwhite;
            lineto(cxoffset + i * dx + 2, (j + 1) * dy - 4 + cyoffset);
            lineto(cxoffset + (i - 1) * dx + 4, (j + 1) * dy - 4 + cyoffset);
            pen.color := clblack;
          end;
  ...
  begin
    ...
    begin
      ...
      cyoffset := Round(FDPIScale * yoffset);
     {draw days here}
      textout((width - textwidth(selstr)) shr 1, cyoffset, selstr);

     {draw line under days}
      pen.color := clblack;
      moveto(0, dy - 5 + cyoffset);
      lineto(width, dy - 5 + cyoffset);
      pen.color := clWhite;
      moveto(0, dy - 4 + cyoffset);
      lineto(width, dy - 4 + cyoffset);
      pen.color := clblack;

      dx := width div 4;
      dy := (height - cyoffset) div 4;
      ...
      for i := 1 to 3 do
        ...
        begin
          ...
          begin
            ...
            pen.color := fselectcolor;
            font.color := finverscolor;
            textout((j - 1) * dx + 5, cyoffset - 10 + dy * i, SmallCaps(FNameofMonths.GetMonth(j + (i - 1) * 4)));
            brush.color := self.color;
            pen.color := ftextcolor;
            font.color := ftextcolor;
            setbkmode(handle, TRANSPARENT);
            ...
            textout((j - 1) * dx + 5, dy * i + cyoffset - 10, SmallCaps(FNameofMonths.Getmonth(j + (i - 1) * 4)));
        end;
    end;
  end;
end;
...
begin
  ...
  EventHint := '';
  if daymode and (y > dy + Round(FDPIScale * yoffset)) and fEventHints then
  begin
    dt := XYToDate(X, Y, false);
    if (dx > 0) and (dy > 0) then
    begin
      newpt.x := x div dx;
      newpt.y := ((y - Round(FDPIScale * yoffset)) - dy) div dy;
    end;
...
begin
 ...
 //if (x>5) and (x<15) and (y>0) and (y<15) then
  ...
  if (x + Round(FDPIScale * xoffset) > width - 15 + xofs) and (x + Round(FDPIScale * xoffset) < width + xofs) and (y > 0) and (y < 15) then
  begin
    if daymode then self.ChangeMonth(1) else self.ChangeYear(1);
    exit;
  end;
