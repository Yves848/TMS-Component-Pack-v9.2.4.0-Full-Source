...
type
  ...
  public
    ...
    procedure MoveNode(ANode: TAdvTreeViewNode; ADestinationNode: TAdvTreeViewNode; AIndex: Integer = -1); virtual;
    procedure CopyNode(ANode: TAdvTreeViewNode; ADestinationNode: TAdvTreeViewNode; AIndex: Integer = -1); virtual;
    ...
    function GetNodeText(ANode: TAdvTreeViewNode; AColumn: Integer): string; virtual;
    function GetVirtualNodeText(ANode: TAdvTreeViewVirtualNode; AColumn: Integer): string; virtual;
    ...
    function FindVirtualNodeByDBKey(ADBKey: String): TAdvTreeViewVirtualNode; virtual;
    function FindNodeByDBKey(ADBKey: String): TAdvTreeViewNode; virtual;
    function FindVirtualNodeByTextAndColumn(AText: string; AColumn: Integer; ARecurse: Boolean = False; ACaseSensitive: Boolean = True): TAdvTreeViewVirtualNode; virtual;
    function FindNodeByTextAndColumn(AText: string; AColumn: Integer; ARecurse: Boolean = False; ACaseSensitive: Boolean = True): TAdvTreeViewNode; virtual;
...
procedure TAdvTreeViewData.CopyNode(ANode: TAdvTreeViewNode;
...
function TAdvTreeViewData.GetNodeText(ANode: TAdvTreeViewNode;
...
function TAdvTreeViewData.GetVirtualNodeText(
  ANode: TAdvTreeViewVirtualNode; AColumn: Integer): string;
begin
  Result := '';
  if Assigned(ANode) then
  begin
...
function TAdvTreeViewData.FindNodeByDBKey(
  ADBKey: String): TAdvTreeViewNode;
var
  v: TAdvTreeViewVirtualNode;
begin
  Result := nil;
...
function TAdvTreeViewData.FindNodeByTextAndColumn(AText: string;
  AColumn: Integer; ARecurse, ACaseSensitive: Boolean): TAdvTreeViewNode;
var
  v: TAdvTreeViewVirtualNode;
begin
  Result := nil;
...
function TAdvTreeViewData.FindVirtualNodeByDBKey(
  ADBKey: String): TAdvTreeViewVirtualNode;
var
  n: TAdvTreeViewVirtualNode;
begin
  Result := nil;
...
function TAdvTreeViewData.FindVirtualNodeByTextAndColumn(AText: string;
  AColumn: Integer; ARecurse,
  ACaseSensitive: Boolean): TAdvTreeViewVirtualNode;
var
  n: TAdvTreeViewVirtualNode;
  t: String;
begin
  ...
  begin
    ...
    if TAdvUtils.MatchStrEx(AText, t, ACaseSensitive) then
    begin
      Result := n;
      Break;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := Result + TAdvUtils.SaveBitmapToHexStr(ExpandedIcons[I, False]) + ';';
      Result := Result + TAdvUtils.SaveBitmapToHexStr(ExpandedIcons[I, True]) + ';';
      Result := Result + TAdvUtils.SaveBitmapToHexStr(CollapsedIcons[I, False]) + ';';
      if I = Values.Count - 1 then
        Result := Result + TAdvUtils.SaveBitmapToHexStr(CollapsedIcons[I, True])
      else
        Result := Result + TAdvUtils.SaveBitmapToHexStr(CollapsedIcons[I, True]) + '|';
    end;
  end;
...
begin
  ...
  begin
    ...
    FDataPointer := (Source as TAdvTreeViewNode).DataPointer;
    ...
    FDBKey := (Source as TAdvTreeViewNode).DBKey;
  end;
end;
              ...
              try
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if y = 6 then
                        TAdvUtils.LoadBitmapFromHexStr(s[y], ExpandedIcons[L, False])
                      else if y = 7 then
                        TAdvUtils.LoadBitmapFromHexStr(s[y], ExpandedIcons[L, True])
                      else if y = 8 then
                        TAdvUtils.LoadBitmapFromHexStr(s[y], CollapsedIcons[L, False])
                      else
                        TAdvUtils.LoadBitmapFromHexStr(s[y], CollapsedIcons[L, True]);
                    end;
                  end;
                end;
              finally
...
function TAdvTreeViewVirtualNode.GetText(AColumn: Integer): String;
begin
  Result := '';
  if Assigned(FTreeView) then
