...
const
  ...
  // v1.4.1.0 : New : Support for high DPI added
  // v1.5.0.0 : New : VCL Styles support added
  // v1.5.1.0 : Improved : Changed Images type to allow to connect TVirtualImageList
...
begin
  ...
  begin
    ...
    if StyleServices.GetElementColor(lDetails, ecTextColor, clr) and (clr <> clNone) then
      lTextColor := clr
    else
      lTextColor := FHint.FHintFont.Color;
    ...
    begin
      FHint.FHintInfo.HintColor := clr;
      FHint.FHintColor := clr;
    end;
    ...
    // Border
    DCFrame3D(R, StyleServices.GetSystemColor(cl3DLight), StyleServices.GetSystemColor(cl3DDkShadow));
  end
  else
  {$ENDIF}
  begin
  ...
  HTMLDrawEx(Canvas,Caption,rd,FHint.FImages,0,0,-1,-1,FHint.FShadowOffset,False,False,False,False,True,False,not FHint.Ellipsis,true,
...
begin
  ...
  Canvas.Font.Height := Round(Canvas.Font.Height * Monitor.PixelsPerInch / 96);
  {$ENDIF}
  ...
  begin
    ...
    HTMLDrawEx(Canvas,AHint,Rect,FHint.FImages,0,0,-1,-1,1,False,True,False,False,True,False,not FHint.Ellipsis,true,
