...
const
  ...
  // 3.5.3.0 : Improved : Handling of tab edit & tab reorder simultaneously
  // 3.5.3.1 : Fixed : Issue with tab edit and AllowTabUndock = true
  // 3.5.3.2 : Improved : High-DPI handling with a fixed tab width is used
  // 3.5.3.3 : Improved : Drawing of larger badge text
  // 3.5.3.4 : Fixed : Issue for design-time tab height handling
  // 3.5.3.5 : Fixed : Color issue with Office 2016 style
  // 3.6.0.0 : New : AddAdvPage overload added
  //         : New : CloneAdvPage method added
  //         : New : Design-time cloning of a page added
  //         : Improved : Painting performance
  // 3.6.0.1 : Fixed : Regression with TabWidth handling
  // 3.6.0.2 : Fixed : Design-time issue with TabWidth setting
  // 3.6.0.3 : Improved : Transparent image drawing on tab
  // 3.6.0.4 : Improved : Badge painting
  // 3.6.0.5 : Fixed : Issue with erratic tab undock
  // 3.6.0.6 : Fixed : Issue with high-DPI handling
  //         : Fixed : Issue with text on tabs when ShowNonSelectedTabs = true
  // 3.6.0.7 : Improved : Multimonitor High DPI handling
  // 3.6.0.8 : Improved : High DPI handling when styler is assigned
  // 3.6.0.9 : Improved : Further improvement for text color on tabs when ShowNonSelectedTabs = true
  // 3.6.0.10: Fixed : Memory leak issue with insert button path
  // 3.6.0.11: Improved : Painting performance caused by superfluous WM_ERASEBKG calls from VCL
  // 3.7.0.0 : New : Support for VCL Styles
  // 3.7.1.0 : New : Added StyleElements property
  // 3.7.1.1 : Fixed : Issue with runtime VCL style switching
...
type
  ...
  protected
    ...
    property DPIScale: single read FDPIScale write FDPIScale;
    function RealButtonSize: integer;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  ...
  private
    ...
    FTabRowSize: Integer;
    FReorderStart: boolean;
    FUndockStart: boolean;
    FEditStart: boolean;
    FReorderPt: TPoint;
    FUndockPt: TPoint;
  ...
  protected
    ...
    procedure DrawDesignHelp; virtual;
    function CreateDesignPages: boolean; virtual;
  ...
  public
    ...
    function AddAdvPage: TAdvOfficePage; overload;
    function CloneAdvPage(AAdvPage: TAdvOfficePage): TAdvOfficePage;
...
begin
  ...
  begin
    ...
    try
      ...
      Images.GetIcon(ImageIndex, icon);
  ...
  // GetIcon failed, so extract as bitmap
  if not hasIcon then
  begin
    bmp := TBitmap.Create;
    try
  ...
  begin
    ...
    begin
      ...
      graphics.DrawImage(Img, MakeRect(P.X - 1, P.Y, Img.GetWidth, Img.Getheight),  // destination rectangle
...
function TPagerTabSettings.RealHeight: integer;
begin
  Result := Round(FHeight * Max(1,FDPIScale));
end;

function TPagerTabSettings.RealWidth: integer;
begin
  Result := Round(FWidth * Max(1,FDPIScale));
end;
...
procedure TAdvOfficePage.Assign(Source: TPersistent);
begin
  if (Source is TAdvOfficePage) then
  begin
    FBadge := (Source as TAdvOfficePage).FBadge;
    FBadgeColor := (Source as TAdvOfficePage).FBadgeColor;
    FBadgeTextColor := (Source as TAdvOfficePage).FBadgeTextColor;
    FCaption := (Source as TAdvOfficePage).Caption;
    FWideCaption := (Source as TAdvOfficePage).FWideCaption;
    FIDisabledPicture.Assign((Source as TAdvOfficePage).DisabledPicture);
    FImageIndex := (Source as TAdvOfficePage).ImageIndex;
    FPageAppearance.Assign((Source as TAdvOfficePage).PageAppearance);
    FUsePageAppearance := (Source as TAdvOfficePage).FUsePageAppearance;
    FIPicture.Assign((Source as TAdvOfficePage).FIPicture);
    FTabHint := (Source as TAdvOfficePage).FTabHint;
    FTabVisible := (Source as TAdvOfficePage).FTabVisible;
    FTabEnabled := (Source as TAdvOfficePage).FTabEnabled;
    FText := (Source as TAdvOfficePage).FText;
    FShowClose := (Source as TAdvOfficePage).FShowClose;
    FOfficeHint.Assign((Source as TAdvOfficePage).OfficeHint);
    FShortCutHintText := (Source as TAdvOfficePage).FShortCutHintText;
    FShortCutHintPos := (Source as TAdvOfficePage).FShortCutHintPos;
    FTabAppearance.Assign((Source as TAdvOfficePage).TabAppearance);
    FUseTabAppearance := (Source as TAdvOfficePage).FUseTabAppearance;
    FShowCheckBox := (Source as TAdvOfficePage).FShowCheckBox;
    FChecked := (Source as TAdvOfficePage).FChecked;
    FProgress.Assign((Source as TAdvOfficePage).Progress);
    FGlow := (Source as TAdvOfficePage).FGlow;
    FGlowColor := (Source as TAdvOfficePage).FGlowColor;
    FLocked := (Source as TAdvOfficePage).FLocked;
    PopupMenu := (Source as TAdvOfficePage).PopupMenu;
  end;
end;
...
begin
  ...
  begin
    ...
    if (FAdvOfficePager.TabSettings.RealHeight > 0) then
    begin
      // Attaching to Tab
      case (FAdvOfficePager.TabPosition) of
        tpTop:
...
procedure TAdvOfficePage.WndProc(var Msg: TMessage);
begin
  // paint performance improvement
...
begin
  ...
  begin
    ...
    TabSettings.DPIScale := FDPIScale;
    ButtonSettings.DPIScale := FDPIScale;
  end;

  if FDesignTime and (Name <> '') and not gotpages and CreateDesignPages then
  begin
    FDesignTime := false;
...
function TAdvOfficePager.CloneAdvPage(AAdvPage: TAdvOfficePage): TAdvOfficePage;
begin
  Result := AddAdvPage;
  if Assigned(AAdvPage) then
    Result.Assign(AAdvPage);
end;

//------------------------------------------------------------------------------

function TAdvOfficePager.AddAdvPage: TAdvOfficePage;
begin
  Result := TAdvOfficePage.Create(Self);
  AddAdvPage(Result);
end;
...
begin
  ...
  {$IFDEF DELPHIXE2_LVL}
  if not (csDesigning in ComponentState) and StyleServices.Enabled and (StyleServices.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements){$ENDIF} then
  begin
...
begin
  if (PageIndex < 0) or (PageIndex >= FAdvPages.Count) or (TabSettings.RealHeight <= 0) or (TabSettings.Width <= 0) then
    Exit;
  ...
  if CloseOnTab then
  begin
    CloseBtnSize.cx := ButtonSettings.RealButtonSize;
    CloseBtnSize.cy := ButtonSettings.RealButtonSize;
  end
  else
  begin
    CloseBtnSize.cx := 0;
    ...
    case TabSettings.Alignment of
      ...
      begin
        ...
        begin
          ...
          CloseBtnR.Right := CloseBtnR.Left + ButtonSettings.RealButtonSize;
        ...
        begin
          if (R.Left + ButtonSettings.RealButtonSize) > (R1.Left + tw) then
            CloseBtnR.Left := R.Right - ButtonSettings.RealButtonSize - 4
          else
  ...
  begin
    ...
    case TabSettings.Alignment of
      ...
      begin
        ...
        if ActivePg and CloseOnTab and (CloseOnTabPosition = cpLeft) then
        begin
          CloseBtnR.Top := R.Bottom - ButtonSettings.RealButtonSize;
          CloseBtnR.Bottom := CloseBtnR.Top + ButtonSettings.RealButtonSize;
        ...
        if ActivePg and CloseOnTab and (CloseOnTabPosition = cpRight) then
        begin
          if ((R.Bottom - ButtonSettings.RealButtonSize) < R1.Top) then
            CloseBtnR.Bottom := R1.Top + ButtonSettings.RealButtonSize + 4
          else
            CloseBtnR.Bottom := R.Bottom - 4;
          CloseBtnR.Top := CloseBtnR.Bottom - ButtonSettings.RealButtonSize;
        end;
      end;
      taRightJustify:
      begin
    ...
    case TabSettings.Alignment of
      ...
      begin
        ...
        begin
          CloseBtnR.Top := R.Top;
          CloseBtnR.Bottom := CloseBtnR.Top + ButtonSettings.RealButtonSize;
          R.Top := CloseBtnR.Top + ButtonSettings.RealButtonSize + 4;
        end;
        ...
        if ActivePg and CloseOnTab and (CloseOnTabPosition = cpRight) then
        begin
          if (R.Top + ButtonSettings.RealButtonSize > R1.Bottom) then
            CloseBtnR.Top := R1.Bottom - ButtonSettings.RealButtonSize - 4
          else
            CloseBtnR.Top := R.Top + 4;
          CloseBtnR.Bottom := CloseBtnR.Top + ButtonSettings.RealButtonSize;
        end;
...
var
  ...
  tl,tlt,d: integer;
begin
  if (PageIndex < 0) or (PageIndex >= FAdvPages.Count) or (TabSettings.RealHeight <= 0) then
    Exit;
  ...
  begin
    ...
    begin
      Canvas.Font.Size := 8;
      tl := Canvas.TextWidth(AdvPages[PageIndex].Badge);
      tlt := tl;
      ...
      begin
        ...
        gpgraphics.FillRectangle(gpbrush, badgexy.X + 2, badgexy.Y, tl, 14);
        gpgraphics.FillEllipse(gpbrush, badgexy.X + (d div 2) + 4, badgexy.Y, 14, 14);
        d := d div 2;
      end
      else
      ...
      if Length(AdvPages[PageIndex].Badge) > 2 then
        tl := 4
      else
        tl := 0;

      BR := Rect(badgexy.x - tlt + tl, badgexy.y, badgexy.X + 12 + tlt + d, badgexy.y + 14);
      ...
      begin
        ...
        begin
          if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex <> PageIndex) and AdvPages[PageIndex].ShowClose) then
            CapR.Left := CapR.Left + (ButtonSettings.RealButtonSize + 4 ) div 2
            ...
            CapR.Left := CapR.Left + ButtonSettings.RealButtonSize + 4;
        end
        else
        begin
//          if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex <> PageIndex) and AdvPages[PageIndex].ShowClose) then
//            CapR.Left := CapR.Left + (PAGEBUTTON_SIZE + 4 ) div 2
      ...
      begin
        ...
        begin
          ...
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex <> PageIndex) and AdvPages[PageIndex].ShowClose) then
              CapR.Bottom := CapR.Bottom - (ButtonSettings.RealButtonSize + 4) div 2
            else if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex = PageIndex) and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
              CapR.Bottom := CapR.Bottom - ButtonSettings.RealButtonSize - 4;
          end
          else
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
        ...
        begin
          ...
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex = PageIndex) and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
              ...
              CapR.Right := CapR.Right - (ButtonSettings.RealButtonSize + 4);
          end
          else
          begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex <> PageIndex) and AdvPages[PageIndex].ShowClose) then
              CapR.Top := CapR.Top + (ButtonSettings.RealButtonSize + 4) div 2
            else if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex = PageIndex) and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
              CapR.Top := CapR.Top + ButtonSettings.RealButtonSize + 4;
          end
          else
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
        ...
        begin
          ...
          begin
            if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex = PageIndex) and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
              CapR.Left := CapR.Left + ButtonSettings.RealButtonSize + 5
            else if (ButtonSettings.CloseButton and CloseOnTab and (ActivePageIndex = PageIndex) and AdvPages[PageIndex].ShowClose) and (CloseOnTabPosition = cpRight) and ((TabSettings.Width <= 0) or (TabSettings.Alignment <> taCenter)) then
              CapR.Right := CapR.Right - (ButtonSettings.RealButtonSize + 4);
          end
          else
          ...
          case TabSettings.ImagePosition of
            ...
            begin
              ...
              if (ButtonSettings.CloseButton and CloseOnTab and
                ...
                ImgX := ImgX - ButtonSettings.RealButtonSize;
    ...
    begin
      ...
      begin
        if CloseOnTabPosition = cpRight then
          CapR2.Right := CapR2.Right - ButtonSettings.RealButtonSize - 4;
      end;
...
procedure TAdvOfficePager.DrawDesignHelp;
var
  th: integer;
begin
  Canvas.Font.Assign(Font);
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvOfficePager.CMStyleChanged(var Message: TMessage);
begin
  inherited;

  if StyleServices.Enabled and (StyleServices.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements){$ENDIF} then
  begin
    SetColorTones(GetVCLStyleTones);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not FDragging and AllowTabUndock and not AdvPages[Tab].Locked then
        begin
          FUndockStart := true;
          FUndockPt := P;
        end;
...
begin
  ...
  begin
    ...
    if FReorderStart and TabReorder and ((Abs(FReorderPt.X - X) > 3) or (Abs(FReorderPt.Y - Y) > 3)) then
    begin
      FEditStart := false;
      FReorderStart := false;
      BeginDrag(true);
    ...
    if FUndockStart and AllowTabUndock and ((Abs(FUndockPt.X - X) > 3) or (Abs(FUndockPt.Y - Y) > 3)) and (Tab >= 0) then
    begin
      Allow := true;
...
begin
  ...
  begin
    ...
    if (Button = mbLeft) and (Tab >= 0) and AdvPages[Tab].TabEnabled and IsActive and FEditStart then
    begin
      if TabEdit then
      begin
        r := GetTabRect(ActivePageIndex);
...
begin
  ...
  case DockInfo.Dock.TabPosition of
    tpLeft: DockInfo.DockRect.Right := DockInfo.DockRect.Left + TabSettings.RealHeight;
    tpTop: DockInfo.DockRect.Bottom := DockInfo.DockRect.Top + TabSettings.RealHeight;
    tpRight: DockInfo.DockRect.Left := DockInfo.DockRect.Right - TabSettings.RealHeight;
    tpBottom: DockInfo.DockRect.Top := DockInfo.DockRect.Bottom - TabSettings.RealHeight;
  end;
  ...
  case TabPosition of
    ...
    begin
      if MultiLine then
        ...
        Result.Top := Result.Top + TabSettings.RealHeight;
      if not IsOnGlass then
      begin
    ...
    begin
      ...
      if MultiLine then
        ...
        Result.Bottom := Result.Bottom - TabSettings.RealHeight;
    ...
    begin
      ...
      if MultiLine then
        ...
        Result.Left := Result.Left + TabSettings.RealHeight;
      if not IsOnGlass then
        Result.Right := Result.Right-FPageMargin;
      Result.Bottom := Result.Bottom - FPageMargin-1;
    ...
    begin
      ...
      if MultiLine then
        ...
        Result.Right := Result.Right - TabSettings.RealHeight;
      Result.Bottom := Result.Bottom - FPageMargin-1;
...
begin
  ...
  case TabPosition of
    ...
    begin
      ...
      Result.Bottom := Result.Top + FTabSettings.RealHeight;
      Result.Top := Result.Bottom - ButtonSettings.RealButtonSize - 8;
      Result.Right := Result.Right - FTabSettings.EndMargin - FPageMargin - 3;
      Result.Left := Result.Right - 2;
      if FTabScroller.Visible then
        Result.Left := Result.Left -(ButtonSettings.RealButtonSize * cnt + cnt * 2);
      if (ButtonSettings.PageListButton) then
        Result.Left := Result.Left - ButtonSettings.RealButtonSize - Spc;
    ...
    tpBottom:
    begin
      Result.Top := Result.Bottom - FTabSettings.RealHeight - FTabOffSet;
      //Result.Bottom := Result.Bottom - FTabOffSet;
      Result.Bottom := Result.Top + ButtonSettings.RealButtonSize + 8;
      Result.Right := Result.Right - FTabSettings.EndMargin - FPageMargin-3;
      Result.Left := Result.Right - 2;
      if FTabScroller.Visible then
        Result.Left := Result.Left -(ButtonSettings.RealButtonSize*cnt + cnt * 2);
    ...
    begin
      ...
      Result.Right := Result.Left + FTabSettings.RealHeight;
      Result.Left := Result.Right - ButtonSettings.RealButtonSize - 8;
      if FTabScroller.Visible then
        Result.Top := Result.Top -(ButtonSettings.RealButtonSize*cnt + cnt * 2);
      if (ButtonSettings.PageListButton) then
        Result.Top := Result.Top - ButtonSettings.RealButtonSize - Spc;
      if (ButtonSettings.CloseButton and not CloseOnTab) then
        Result.Top := Result.Top - ButtonSettings.RealButtonSize - Spc;
      if (ButtonSettings.ClosedListButton and (FClosedPageList.Count > 0)) then
        Result.Top := Result.Top - ButtonSettings.RealButtonSize - Spc;
      ...
      if not RotateTabLeftRight then
        Result.Left := Result.Right - 5 - ButtonSettings.RealButtonSize - 3;
    end;
    tpRight:
    begin
      Result.Bottom := Result.Bottom - FTabSettings.EndMargin - FPageMargin-3;
      Result.Top := Result.Bottom -3;
      Result.Left := Result.Right - TabSettings.RealHeight - FTabOffSet;
      //Result.Right := Result.Right - FTabOffSet;
      Result.Right := Result.Left + ButtonSettings.RealButtonSize + 8;
      if FTabScroller.Visible then
        Result.Top := Result.Top -(ButtonSettings.RealButtonSize*cnt + cnt * 2);
      if (ButtonSettings.PageListButton) then
        Result.Top := Result.Top - ButtonSettings.RealButtonSize - Spc;
      ...
      if not RotateTabLeftRight then
        Result.Right := Result.Left + 5 + ButtonSettings.RealButtonSize + 3;
    end;
  end;
end;
  ...
  case TabPosition of
    ...
    begin
      ...
      if ButtonSettings.ShowInsertButton then
        Result.Right := Result.Right - ButtonSettings.RealButtonSize - 6;
    end;
    tpBottom:
    begin
      Result.Top := Result.Bottom - FTabSettings.RealHeight - FTabOffSet;
    ...
    begin
      ...
      if ButtonSettings.ShowInsertButton then
        Result.Bottom := Result.Bottom - ButtonSettings.RealButtonSize - 6;
      Result.Left := Result.Left + FTabOffSet;
      Result.Right := Result.Left + FTabSettings.RealHeight;
    end;
    tpRight:
      ...
      begin
        ...
        begin
          ...
          if (ButtonSettings.CloseButton and CloseOnTab and AdvPages[i].ShowClose) then
            TbW := Tbw + ButtonSettings.RealButtonSize + 4;

          if AdvPages[I].ShowCheckBox then
            TbW := TbW + 20;

          if (TabSettings.Width > 0) then
            TbW := TabSettings.RealWidth;
...
begin
  ...
  begin
    ...
    begin
      FTabRowSize := TabSettings.RealHeight;
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        ...
        Result.Right := Result.Left + ButtonSettings.RealButtonSize; // (Result.Right - Result.Left) div 2 - 1;
      end;
      tpLeft, tpRight:
      begin
        //Result.Bottom := Result.Top + (Result.Bottom - Result.Top) div 2 - 1;
        Result.Top := Result.Top + 3;
        Result.Bottom := Result.Top + ButtonSettings.RealButtonSize;
      end;
    end;
  end;
end;
    ...
    begin
      case TabPosition of
        ...
        begin
          ...
          begin
            if (CloseOnTabPosition = cpRight) then
              Result.Left := Result.Right - ButtonSettings.RealButtonSize - 4 - GetRightRoundingOffset
            else //CloseOnTabPosition = cpLeft
              Result.Left := Result.Left + 4 + GetLeftRoundingOffset
          end;
          Result.Right := Result.Left + ButtonSettings.RealButtonSize;
          Result.Bottom := Result.Bottom - 5;
          Result.Top := Result.Bottom - ButtonSettings.RealButtonSize;
        end;
        tpBottom:
        begin
          Result := GetTabRect(PageIndex);
        ...
        begin
          ...
          begin
            ...
            Result.Left := Result.Right - ButtonSettings.RealButtonSize;
            if (TabSettings.Width > 0) and (TabSettings.alignment = taCenter) and (PageIndex >= 0) then
            begin
              GetCloseBtnImageAndTextRect(PageIndex, cbr, tr, ip);
              Result.Top := cbr.Top;
            ...
            begin
              if (CloseOnTabPosition = cpRight) then
                ...
                Result.Top := Result.Bottom - ButtonSettings.RealButtonSize - 4 - GetLeftRoundingOffset;
            end;
            Result.Bottom := Result.Top + ButtonSettings.RealButtonSize;
          end
          else
          begin
            Result := GetTabRect(PageIndex);
            i := (Result.Bottom - Result.Top - ButtonSettings.RealButtonSize) div 2;
            ...
            begin
              if (CloseOnTabPosition = cpRight) then
                Result.Left := Result.Right - ButtonSettings.RealButtonSize - 5
              else //CloseOnTabPosition = cpLeft
        ...
        begin
          ...
          begin
            ...
            begin
              if (CloseOnTabPosition = cpRight) then
                ...
                Result.Bottom := Result.Top + ButtonSettings.RealButtonSize  + 4 + GetLeftRoundingOffset;
            end;
            Result.Top := Result.Bottom - ButtonSettings.RealButtonSize;
          end
          else
          begin
            ...
            begin
              if (CloseOnTabPosition = cpRight) then
                Result.Left := Result.Right - ButtonSettings.RealButtonSize - 3
              else //CloseOnTabPosition = cpLeft
                Result.Left := Result.Left + 5;
            end;
...
begin
  ...
  begin
    ...
    case TabPosition of
      tpTop:
      begin
        Result.Right := Result.Left + 3 +(ButtonSettings.RealButtonSize*cnt + 2+ 3);// SCROLLER_SIZE;
        Result.Bottom := Result.Bottom - 5;
        Result.Top := Result.Bottom - ButtonSettings.RealButtonSize;
      end;
      tpBottom:
      ...
      tpLeft:
      begin
        Result.Bottom := Result.Top + 3 +(ButtonSettings.RealButtonSize*cnt + 2+ 3);
        Result.Right := Result.Right - 5;
        Result.Left := Result.Right - ButtonSettings.RealButtonSize;
      end;
      tpRight:
...
begin
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        Result.Left := Result.Right + 2;
        Result.Right := Result.Left + ButtonSettings.RealButtonSize; // ((Result.Right - Result.Left) div 2);
      end;
      tpLeft, tpRight:
      begin
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        ...
        Result.Right := Result.Left + ButtonSettings.RealButtonSize + 6;
        Result.Top := R.Top + 5;
      ...
      begin
        ...
        begin
          ...
          Result.Bottom := Result.Top + ButtonSettings.RealButtonSize + 6;
        end
        else
        begin
          Result.Right := R.Right - 8;
          Result.Left := Result.Right - ButtonSettings.RealButtonSize - 1;
          Result.Top := R.Bottom + gap;
          Result.Bottom := Result.Top + ButtonSettings.RealButtonSize + 6;
        end;
      end;
      ...
      begin
        ...
        begin
          Result.Left := R.Left + 8;
          Result.Right := Result.Left + ButtonSettings.RealButtonSize + 1;
          Result.Top := R.Bottom + gap;
          Result.Bottom := Result.Top + ButtonSettings.RealButtonSize + 6;
        end;
      end;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvOfficePager.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  frm: TCustomForm;

{$ELSE}
procedure TAdvOfficePager.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
  FDPIScale := 1.0;
  ...
  begin
    FDPIScale := TForm(frm).Monitor.PixelsPerInch / 96;
  end
  else
  {$ENDIF}
    FDPIScale := GetDPIScale(FFormScaled, Canvas.Handle);
...
begin
  ...
  begin
    ...
    begin
      ...
      //j := ButtonSettings.RealButtonSize div 3;
...
begin
  ...
  FClosedListButton.Width := ButtonSettings.RealButtonSize;
  FClosedListButton.Height := ButtonSettings.RealButtonSize;
...
function TAdvOfficePager.CreateDesignPages: boolean;
begin
  Result := true;
end;
...
function TPageButtonSettings.RealButtonSize: integer;
begin
  Result := Round(ButtonSize * DPIScale);
end;
