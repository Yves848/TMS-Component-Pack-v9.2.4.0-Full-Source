{**************************************************************************}
{ TAdvSmoothDockDE DESIGN TIME EDITOR                                      }
...
uses
  Classes, AdvSmoothDock, DesignIntf, DesignEditors;

type
  TAdvSmoothDockEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothDockEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothDock).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothDock) then
         (Component as TAdvSmoothDock).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothDockEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothDockEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
