...
const
  ...
  // v1.3.1.0 : New : Support for multimonitor High DPI handling
  // v1.3.1.1 : Fixed : Issue with null date handling
  // v1.3.2.0 : New : Access to internal timepicker via public property
  // v1.3.2.1 : Improved : Behavior on high DPI
  // v1.3.2.2 : Fixed : Issue with rare constructor sequence on DB-aware version
  // v1.3.3.0 : Improved : Label positioning in high DPI
  // v1.3.4.0 : Improved : Time part hiding for null datetime values
  // v1.3.5.0 : Improved : VCL Styles support
  // v1.3.5.1 : Fixed : Issue with disabled border drawing
...
type
  ...
  public
    ...
    property TimePicker: TCustomDateTimePicker read FTimePicker;
  published
...
var
  ...
  LBorderColor: TColor;
begin
  if IsTimePicker and not Enabled then
    Exit;
  ...
  begin
    WinH := Parent.Handle;
    LHasFocus := (Parent as TCustomDateTimePicker).FHasFocus;
    LBorderColor := (Parent as TCustomDateTimePicker).FBorderColor;
  end;
  ...
  begin
    if not LHasFocus or (FFocusBorderColor = clNone) then
    begin
      BtnFaceBrush := CreateSolidBrush(ColorToRGB(LBorderColor));
      GetWindowRect(WinH, ARect);
      OffsetRect(ARect, -ARect.Left, -ARect.Top);
      FrameRect(DC, ARect, BtnFaceBrush);
      DeleteObject(BtnFaceBrush);
      Exit;
  ...
  begin
    ...
    begin
      ...
      if Assigned(frm) then
        ...
        DropW := DROPDOWNBTN_WIDTH;

      GetWindowText(FTimepicker.Handle, lpstr,255);
      i := GetTextSize(Self, strpas(lpstr), Font).cx + DropW + 10;
      ...
      begin
        ...
        begin
          ...
          begin
            if MetroStyleShowing then
              FTimePicker.SetBounds(Width - i - DropW - j, 1, i, Height - 2 - j)
            else
              FTimePicker.SetBounds(Width - i - DropW - 13 - j, 1, i, Height - 2 - j);
          end
          else  // for Windows standard scheme
            FTimePicker.SetBounds(Width - i - DropW - j, 2, i, Height - j - 4);
        end
        else
          FTimePicker.SetBounds(Width - i - DropW - 2 - j, -2, i, Height - j)
      end
      else
      begin
        if Ctl3D then
          FTimePicker.SetBounds(Width - i - DropW, -2, i, Height)
        else
          FTimePicker.SetBounds(Width - i - DropW, -2, i, Height);
      end;  
    end;
...
procedure TAdvDateTimePicker.WMNCPaint(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  FOriginalTimeFormat := TimeFormat;
