...
uses
  ...
  , TMSLogging
  {$ENDIF}
  , AdvScrollControl, AdvGraphicsTypes, AdvUtils, AdvRichEditorPic, AdvRichEditorRTF
  ;
...
type
  ...
  public
    ...
    function DrawSize: TSize; virtual;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    function Size: TSize; override;
    function DrawSize: TSize; override;
  ...
  THTMLImageGeneration = (igFile, igInline, igID, igNone, igReference);
  ...
  protected
    ...
    function GetContentAsHTML(UseSelection: boolean; ImgPath: string = ''; PlainHTML: boolean = false): string;
    ...
    function InsertLineBreak(Index: integer): TLineBreakElement; overload;
  ...
  public
    ...
    procedure GetWordAndIndexAtCaret(var AValue: string; var AIndex: integer; SpaceOnly: boolean = false); override;
    procedure UpdateWordAndIndexAtCaret(AValue: string; AIndex: integer; SpaceOnly: boolean = false); override;
    ...
    function InsertMultiLineText(AValue: string): TTextElement;
    ...
    function InsertTab: TTabElement; overload;
    function InsertTab(Index: integer): TTabElement; overload;
    ...
    procedure UnMerge;
    procedure GetMergeFields(AList: TStringList);
    ...
    function ContentAsPlainHTML(ImgPath: string = ''): string;
    ...
    procedure LoadFromStream(const AStream: TStream; Internal: boolean); overload;
  ...
  published
    property Author: string read FAuthor write FAuthor;
    property AllowFormatShortCuts: boolean read FAllowFormatShortCuts write FAllowFormatShortCuts default true;
    property Comments: string read FComments write FComments;
...
function TREElement.DrawSize: TSize;
begin
  Result := Size;
end;
...
function TAdvRichEditorBase.InsertMultiLineText(AValue: string): TTextElement;
var
  t: string;
  i,l,idx: integer;
  ch: string;
...
begin
  ...
  el := InsertText('');
  idx := FContext.Content.IndexOf(el);
  ...
  Result := InsertText(idx, t);

  Caret.Element := Result;
  Caret.CharIndex := Length(t);
  CaretToSelection;
...
function TAdvRichEditorBase.InsertLineBreak(Index: integer): TLineBreakElement;
begin
  Result := TLineBreakElement.Create;
  FContext.Content.Insert(Index, Result);
end;

function TAdvRichEditorBase.InsertTab(Index: integer): TTabElement;
begin
...
var
  ...
  begin
    ...
    if pel is TLineBreakElement then
      pel := PreviousElement(pel);

    if Assigned(pel) and (pel is TTextElement) and not FDefaultChanged then
    begin
      el.Font.Assign(pel.Font);
    end
    else
    begin
      el.FontStyle := GetDefaultFont.Style;
      el.FontSize := GetDefaultFontSize;
      el.FontName := GetDefaultFontName;
    end;
    Result := el;
  end;
...
begin
  ...
  KEY_DELETE:
    begin
      ...
      if IsInError and Assigned(Caret.Element) then
      begin
        S := Caret.Element.Text;
        ...
        if not Error and Assigned(Caret.Element) and (Caret.Element is TTextElement) then
        begin
          (Caret.Element as TTextElement).Error := false;
          Refresh;
        end;
  ...
  VK_V, KEY_INSERT:
    begin
      if (AChar = KEY_INSERT) and (ACommandState = ccCtrl) then
      begin
        s := SelectedText;
        if (Selected is TCustomGraphicElement) then
        begin
  ...
  KEY_UP:
    begin
      ...
      if not (Caret.XY.Y - SP.Y - 4 < 0) then
      begin
        XYToCaret(Caret.XY.X, Caret.XY.Y - SP.Y - 4,False,IsSel);
...
begin
  ...
  begin
    ...
    surl := GetWordAtCaret(pel, ci, nci, True, False);
    ...
    if (s <> '') and (Caret.CharIndex > 0) and not isURL(surl) then
    begin
      PrevError := false;
      ...
      begin
        ...
        if (pel is TTextElement) and not Error and ((pel as TTextElement).Error) then
          (pel as TTextElement).Error := false;
        ...
        begin
          ...
          // support for multiline text as auto-correct
          if pos(#13,s) = 0 then
          begin
            Caret.Element := pel;
            Caret.CharIndex := ci;
          ...
          begin
            ...
            begin
              ...
              if IsSeparator(Copy(s,Selection.ToChar + 1, 1)) then
...
begin
  ...
  begin
    ...
    if Assigned(PaintSelection.FromElement) and (PaintSelection.FromElement is TEmoticonPictureElement) then
      Selected := PaintSelection.FromElement;
...
procedure TAdvRichEditorBase.DoChanged;
begin
  if Assigned(OnChange) then
    OnChange(Self);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // single char predefined bullets all painted centered based on same size
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while (charidx <= len) and IsSeparator(Copy(s,charidx + 1, 1)) do
          inc(charidx);
      ...
      len := Length(s);
      repeat
        sep := IsSeparator(Copy(s,charidx + 1, 1));
        inc(charidx);
...
begin
  ...
  begin
    ...
    l := 1;
    // selection is within merged range, so offset for << indicator
    if PaintSelection.FFromElement.MergeRef <> '' then
      l := 2;

    if (PaintSelection.FromChar >= l) then
    begin
...
begin
  Result := HasTextSelection or
    (Assigned(Selection.FromElement) and (Selection.FromElement is TCustomGraphicElement) and (Selection.FromChar = 0) and (Selection.ToChar = 1));
end;
...
function TAdvRichEditorBase.InsertTab: TTabElement;
var
  el: TREElement;
  tel: TTextElement;
  s: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        pel := PreviousElement(Caret.Element);
        if (Caret.Element is TTextElement) and (Caret.CharIndex = 0) and Assigned(pel) and (pel is TTextElement) then
        begin
          Caret.Element := pel;
          Caret.CharIndex := Length(pel.Text);
        end;
      end;
      ...
      begin
        ...
        prevch := Copy(s, Caret.CharIndex, 1);
        ...
        begin
          ...
          if IsSeparator(su) and not IsSeparator(prevch) then
          begin
            // finish URL
            DoCorrectWord;
            DoEnterWord;
        ...
        if (URLAuto = uAuto) and IsURL(s) and (Caret.Element.URL = '') and not careturl then
        begin
          SelectWordAtCaret;
          SetSelectionHyperlink('#');
          if Selection.FromChar <> 0 then
          begin
            Caret.Element := Selection.ToElement;
          ...
          begin
            Caret.Element := Selection.FromElement;
            Caret.CharIndex := Length(Selection.FromElement.Text);
            CaretToSelection;
          end;
        end;
      end
...
begin
  ...
  begin
    ...
    if (s <> '') and (Caret.CharIndex > 0) and (Caret.Element is TTextElement) then
    begin
      ...
      begin
        w := Caret.Element.Text;
        if Assigned(OnCorrectWord) then
        begin
          if not ((Caret.CharIndex < Length(w)) and (w[Caret.CharIndex] <> ' ')) then
            DoCorrectWord
          else
            docorr := true;
        end;
  ...
  if docorr then
  begin
    // select element before linebreak
    Caret.Element := PreviousElement(PreviousElement(Caret.Element));
    Caret.CharIndex := Length(Caret.Element.Text);
    DoCorrectWord;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        el.Font.Assign(Caret.Element.Font);
...
begin
  Result := (ch = ' ') or (ch = ';') or (ch = '.') {or (ch = ':')} or (ch = ',')  or (ch = ')') or (ch = '(') or (ch = '[') or (ch = ']') or (ch = '}') or (ch = '{')  {or (ch = '''') } or (ch = '"');
end;
...
procedure TAdvRichEditorBase.LoadFromStream(const AStream: TStream; Internal: boolean);
var
  es: TElementSaver;
  esc: TElementClassSaver;
  el: TREElement;
...
begin
  ...
  if Internal or EditCanModify then
  begin
    FModified := true;
...
begin
  {$IFNDEF LCLLIB}
  {$IFDEF DELPHIXE2_LVL}
  AStream := TStringStream.Create('', Encoding);
  {$ELSE}
...
procedure TAdvRichEditorBase.ScrollToBegin;
begin
  SetCaret(cpBeginDoc);
  ScrollToCaret;
end;
...
procedure TAdvRichEditorBase.ScrollToEnd;
begin
  SetCaret(cpEndDoc);
  ScrollToCaret;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      while IsSeparator(Copy(s,vp + 1,1)) do
        inc(vp);
...
begin
{$IFDEF FMXLIB}
  Result := IsFocused or AlwaysFocus;
{$ENDIF}
{$IFNDEF FMXLIB}
...
procedure TAdvRichEditorBase.SetLineSpacing(const Value: integer);
begin
...
function TAdvRichEditorBase.LineCount: integer;
var
  re: TREElement;
begin
  Result := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if IsSeparator(Copy(s,i,1)) then
        begin
          while (i > 0) and IsSeparator(Copy(s,i,1)) do
          begin
            dec(i);
            inc(vp);
          end;
        ...
        while (i > 0) and not IsSeparator(Copy(s,i,1)) do
        begin
          dec(i);
          inc(vp);
        end;
...
procedure TAdvRichEditorBase.GetWordAndIndexAtCaret(var AValue: string; var AIndex: integer; SpaceOnly: boolean = false);
begin
  AValue := GetWordAtCaret(Caret.Element, Caret.CharIndex, AIndex, SpaceOnly, False);
end;

procedure TAdvRichEditorBase.UpdateWordAndIndexAtCaret(AValue: string; AIndex: integer; SpaceOnly: boolean = false);
var
  ci: integer;
  s: string;
  el: TREElement;
...
begin
  ci := 0;
  GetWordAtCaret(Caret.Element, Caret.CharIndex, ci, SpaceOnly, True);
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        if (idx = 0) and (elidx > 0) and not CheckEOW(Copy(s,idx,1)) then
        begin
          dec(elidx);
          ...
          if (nel is TTextElement) then
          begin
            if CheckEOW(Copy(s,Length(s),1)) then
              found := true
            else
            begin
              idx := Length(s);
      ...
      repeat
        while (idx <= Length(s)) and not CheckEOW(Copy(s,idx,1)) do
          inc(idx);
...
procedure TAdvRichEditorBase.GetMergeFields(AList: TStringList);
var
  i: integer;
  el: TREElement;
begin
...
begin
  ...
  if (FSelectionElements.Count = 0) or ((FSelectionElements.Count = 1) and (FSelectionElements[0] is TLineBreakElement)) then
  begin
    Color := clr;
    FDefaultChanged := true;
  end
...
begin
  ...
  begin
    ...
    if (cnt + l > Value) or ((cnt + l = Value) and (i = FContext.Content.Count - 1)) then
    begin
      Selection.FromElement := el;
...
{$ENDIF}
  ...
  function FontStyleAsString(AFontStyle: TFontStyles): string;
  begin
    Result := '';
    if TFontStyle.fsBold in AFontStyle then
      Result := 'fsBold';
    if TFontStyle.fsItalic in AFontStyle then
      Result := 'fsItalic';
    if TFontStyle.fsUnderline in AFontStyle then
      Result := 'fsUnderline';
    Result := '['+ Result +']';
  end;
...
begin
  ...
  begin
    ...
    TMSLogger.Info(inttostr(i)+'|'+el.ClassName+':'+al+':'+inttostr(el.Indent)+' ['+inttostr(el.XY.X)+','+inttostr(el.XY.Y)+'] {'+GetFontName(el.Font)+FontStyleAsString(el.FontStyle)+'-' + inttostr(el.GetFontSize)+'} |' + txt + '| -clr:'+inttohex(el.Color,6) );
    inc(i);
  end;
...
function TAdvRichEditorBase.ContentAsPlainHTML(ImgPath: string): string;
begin
  Result := GetContentAsHTML(false, ImgPath, True);
end;
...
function TAdvRichEditorBase.GetContentAsHTML(UseSelection: boolean; ImgPath: string = ''; PlainHTML: boolean = false): string;
var
  sl: TStringList;
  i,curindent: integer;
  ...
  didbold,diditalic,didunderline,didstrike,didclr: boolean;
...
//        s := s + '</DIV>';
    ...
    begin
      ...
      begin
        ...
        if ((bkclr and $FFFFFF) <> (Color and $FFFFFF)) and (bkclr <> clNone) then
        begin
          opentag := opentag + '<SPAN style="background-color:' + ColorToHTML(bkclr) + ';display:inline-block">';
          isbkclr := true;
        end;
      end;
      ...
      if not didclr or (clr <> (el as TTextElement).TextColor) or fntclose then
      begin
        if isfntclr then
        begin
          fntclose := true;
        ...
        if not didclr or (clr <> GetDefaultFontColor) then
        begin
          didclr := false;
      ...
      begin
        ...
        if Assigned(nel) and (nel is TLinebreakElement) then
          closetag := closetag + '<BR>';
      end;
    ...
    begin
      ...
      begin
        ...
        // export as reference name to bitmapcontainer image
        igReference:
          begin
...
begin
  ...
  {$IFNDEF LCLLIB}
  {$IFDEF DELPHIXE2_LVL}
  sb := TStringStream.Create('', TEncoding.UTF8);
  {$ELSE}
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (TFontStyle.fsStrikeOut in fs) and not (TFontStyle.fsStrikeOut in fsn) then
            SetSelectionStrikeOut(false);
          if not (TFontStyle.fsStrikeOut in fs) and (TFontStyle.fsStrikeOut in fsn) then
...
//          if FSelectionElements.Count > 0 then
        ...
        begin
          ...
          begin
            ...
            if (doc.ElementTable.Items[i].URL <> '') then
              txtEL.URL := doc.ElementTable.Items[i].URL;
...
begin
  ...
  begin
    ...
    if el is TTextElement then
    begin
      if el.Baseline <> bl then
      begin
        bl := el.Baseline;
        if bl = tbSubscript then
         FRTFEngine.AddSubScript;
        if bl = tbSuperscript then
         FRTFEngine.AddSuperScript;
        if bl = tbRegular then
         FRTFEngine.AddNormalScript;
      end;
...
function TPictureElement.DrawSize: TSize;
begin
  Result.cx := 0;
  Result.cy := 0;
...
begin
  inherited;
  Enabled := (Target is TAdvRichEditorBase);
end;
