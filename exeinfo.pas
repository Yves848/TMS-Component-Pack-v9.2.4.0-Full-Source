...
const
  ...
  // 1.5.0.0 : New : UserName property added
  // 1.5.1.0 : New : Changed GetVersionInfoOfApp to function that returns true when retrieval was successful
...
type
  ...
  public
    { Public declarations }
    function GetVersionInfoOfApp(const AAppName:String): boolean;
...
function TExeInfo.GetVersionInfoOfApp(const AAppName: string): boolean;
type
...
function TExeInfo.GetUserName: string;
var
   pcUser: PChar;
   dwCSize: DWORD;
begin
   dwCSize := 255;
   {$IFDEF DELPHI_UNICODE}
   GetMem( pcUser, dwCSize * 2); // allocate memory for the string
   {$ENDIF}
   {$IFNDEF DELPHI_UNICODE}
   GetMem( pcUser, dwCSize ); // allocate memory for the string
   {$ENDIF}
   try
      if Windows.GetUserName(pcUser, dwCSize ) then
         Result := StrPas(pcUser);
   finally
