...
uses
  Windows, Classes, Messages, Types, Graphics, Math, StdCtrls, Forms, Controls, SysUtils, ImgList,
  AdvStyleIF, Contnrs
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.0.1.1 : Fixed : Issue with reset of SelectedLookup
  // v1.0.2.0 : New : Windows 10, Office 2016 styles added
...
function TAdvLookupBar.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;

{$IFDEF DELPHIXE2_LVL}
procedure TAdvLookupBar.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(true);
end;
{$ENDIF}
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvLookupBar.InitVCLStyle(init: boolean);
var
...
begin
  ...
  if LStyle.Enabled and (LStyle.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements){$ENDIF} then
  begin
    ...
    if LStyle.GetElementColor(LDetails, ecFillColor, clr) and (clr <> clNone) then
    begin
      SelectedFont.Color := clr;
    end;

    ActiveFont.Color := LStyle.GetStyleFontColor(TStyleFont.sfButtonTextNormal);
    InActiveFont.Color := LStyle.GetStyleFontColor(TStyleFont.sfButtonTextDisabled);
  end;
end;
{$ENDIF}
 ...
 case AStyle of
   ...
   tsOffice2016White:
      begin
        ...
        Color := clWhite;
        ColorTo := Color;
        ActiveFont.Color := $505050;
        InActiveFont.Color := clSilver;

        ScrollColor := $F2E1D5;
        SelectedFont.Color := $505050;

        ScrollFont.Color := $505050;
      end;
    tsOffice2016Gray:
      begin
        BorderColor := $444444;
        Color := $B2B2B2;
        ColorTo := Color;
        ActiveFont.Color := $424242;
        InActiveFont.Color := clSilver;

        ScrollColor := $F2E1D5;
        SelectedFont.Color := $424242;

        ScrollFont.Color := $424242;
      end;
    tsOffice2016Black:
      begin
        BorderColor := $444444;
        Color := $363636;
        ColorTo := Color;
        ActiveFont.Color := $A6A6A6;
        InActiveFont.Color := clSilver;

        ScrollColor := $6A6A6A;
        SelectedFont.Color := $C8C8C8;

        ScrollFont.Color := $A6A6A6;
      end;
...
procedure TAdvLookupBar.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('TMSStyle',ReadTMSStyle, WriteTMSStyle, True);
end;

procedure TAdvLookupBar.ReadTMSStyle(Reader: TReader);
begin
  FTMSStyle := TTMSStyle(Reader.ReadInteger);
end;

procedure TAdvLookupBar.WriteTMSStyle(Writer: TWriter);
begin
