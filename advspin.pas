...
const
  ...
  // 1.7.4.0 : New : Property HexDigits added
  // 1.8.0.0 : New : Label positions lpTopRight, lpBottomRight, lpRightTop, lpRightCenter, lpRightBottom added
  // 1.8.0.1 : Improved : Behavior when Form.Scaled = false
  // 1.8.1.0 : New : Support for multimonitor High DPI handling
  // 1.8.2.0 : New : VCL styles support
  // 1.8.3.0 : Improved : Label positioning in high DPI
  // 1.8.4.0 : Improved : Flat button Support for High DPI (changed function CalculateFPI scale to GetDPIScale)
  // 1.9.0.0 : Improved : VCL Styles support
  // 1.9.0.1 : Fixed : Issue with IncrementFloat value initialization
...
{ TAdvSpinEdit }
  ...
  private
    ...
    procedure SetHexDigits(const Value: Byte);
  protected
...
begin
  ...
  sptHex:self.Text := '0x' + IntToHex(FHexValue,FHexDigits);
  end;
...
begin
  ...
  begin
    FLblUpdate := true;
    LabelFont.Height := Round(FLblFntHeight * GetDPIScale(Self, FLabel.Canvas));
    FLblUpdate := false;
  end;
...
begin
  if Assigned(FLabel) then
  begin
    lblmargin := Round(FLabelMargin *  GetDPIScale(Self, FLabel.Canvas));
...
procedure TAdvSpinEdit.SetHexDigits(const Value: Byte);
begin
  FHexDigits := Value;
...
begin
  ...
  newtxt := '0x'+IntToHex(newval, HexDigits);
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    LStyle.GetElementColor(LStyle.GetElementDetails(teEditTextNormal), ecTextColor, Arrowcolor);
    stylebkg := true;
    Glyph.Handle := 0;
    inherited;
  end;
{$ENDIF}
  ...
  if (TAdvSpinButton(Owner).ButtonColor <> clNone) or stylebkg or flat then
  begin
    if flat then
    begin
      if FState = bsDown then
        ...
        if FHasMouse then
          ...
          clr := (Parent.Parent as TAdvSpinEdit).Color
    end
    else
    begin
      if FState = bsDown then
    ...
    bsize := Round(2 * FDPIScale);

    case FButtonDirection of
    bdLeft:
      begin
        midB := Height div 2;
        sideB := (Width - bsize) div 2;

        pt[0].X := r.Left + sideB + bsize;
        pt[0].Y := r.Top + midB - bsize;
        pt[1].X := r.Left + sideB;
        pt[1].Y := r.Top + midB;
        pt[2].X := r.Left + sideB + bsize;
        pt[2].Y := r.Top + midB + bsize;
      end;
    bdRight:
      begin
        midB := Height div 2;
    ...
    bdUp:
      begin
        midB := Width div 2;
        sideB := (Height - bsize) div 2;

        pt[0].X := r.Left + midb - bsize;
        pt[0].Y := r.Top + sideB + bsize;
        pt[1].X := r.Left + midB + bsize;
        pt[1].Y := r.Top + sideB + bsize;
        pt[2].X := r.Left + midb;
        pt[2].Y := r.Top + sideB;
      end;
    bdDown:
  ...
  if DoVisualStyles and not stylebkg then
  begin
