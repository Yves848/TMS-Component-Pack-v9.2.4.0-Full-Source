...
const
  ...
  // 2.2.0.6 : Fixed : Regression with DropWidth
  // 2.2.1.0 : New : OnMouseEnter, OnMouseLeave events added
  // 2.2.2.0 : New : Support for LINE-HEIGHT attribute for <P> tag
  // 2.2.2.1 : Improved : Handling for high DPI with Form.Scaled = false
  // 2.2.2.2 : Improved : High-DPI handling
...
type
  ...
  private
    ...
    procedure SetEditHeight(const Value: Integer);
...
begin
  ...
  Offsetrect(r,2,0);
  HTMLDrawEx(Canvas,Items[Index],r,FImages,0,0,-1,-1,FShadowOffset,False,False,False,(odSelected in State),True,False,not Ellipsis,FFormScaled,
             1.0,URLCol,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode);
  ...
  begin
    ...
    try
      ...
      HTMLDrawEx(Canvas,Items[Index],r,FImages,0,0,-1,-1,FShadowOffset,True,True,False,True,True,False,not Ellipsis,
                 FFormScaled,1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode);
...
procedure THTMLComboBox.SetDropWidth(const Value: Integer);
begin
  FDropWidth := Value;
...
procedure THTMLComboBox.SetEditHeight(const Value: Integer);
begin
  FEditHeight := Value;
...
begin
  ...
  begin
    ...
    if HTMLDrawEx(Canvas,Items[res],r,FImages,msg.XPos,msg.YPos,-1,-1,FShadowOffset,True,False,False,True,True,False,not Ellipsis,
             FFormScaled,1.0,FURLColor,clNone,clNone,FShadowColor,anchor,stripped,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode) then
...
{$IFDEF DELPHIXE10_LVL}
procedure THTMLComboBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
{$IFNDEF DELPHIXE10_LVL}
procedure THTMLComboBox.ChangeScale(M, D: Integer);
{$ENDIF}
var
  frm: TCustomForm;
begin
  ...
  begin
    ...
    begin
      EditHeight := MulDiv(EditHeight, M, D);
    end;
  end;
end;
 ...
 if (res>=0) and (res<self.Items.Count) then
  begin
   ...
   canvas.handle := GetDC(self.handle);
    ...
    if HTMLDrawEx(Canvas,Items[res],r,FImages,X,Y,-1,-1,FShadowOffset,True,False,False,True,True,False,not Ellipsis, FFormScaled,
             1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode) then
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas,Items[index],r,FImages,0,0,-1,-1,FShadowOffset,True,True,False,True,True,False,not Ellipsis, FFormScaled,
               1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode);
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLComboBox.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
procedure THTMLComboBox.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF}
...
begin
  FDropWidth := Value;
  if Value > 0 then
    SendMessage(Handle, CB_SETDROPPEDWIDTH, FDropWidth, 0);
end;
end.
