...
const
  ...
  // v2.6.3.1 : Fixed : Issue with transparency on double buffered forms
  // v2.6.3.2 : Fixed : Issue with setting a high Subdivisions number in combination with a datetime range
  // v2.6.3.3 : Fixed : Issue changing indicator or section position without first selecting it in TDBAdvSmoothTimeLine
  // v2.6.4.0 : New : OnGetAnnotationFont event to customize annotation font for each annotation separately
  // v2.6.4.1 : Fixed : Issue with getting correct indenting
  // v2.6.4.2 : Fixed : Issue with not resetting indenting after recalculation
  // v2.6.4.3 : Fixed : Issue with hint window size to small with multiline text
  // v2.6.5.0 : Improved : Support for High DPI
  // v2.7.0.0 : New : VCL Styles support
...
type
  ...
  TAdvSmoothTimeLineGetAnnotationFont = procedure(Sender: TObject; indicator: TAdvSmoothTimeLineBarIndicator; AFont: TFont) of object;
  ...
  private
    ...
    FOnGetAnnotationFont: TAdvSmoothTimeLineGetAnnotationFont;
  ...
  published
    ...
    property OnGetAnnotationFont: TAdvSmoothTimeLineGetAnnotationFont read FOnGetAnnotationFont write FOnGetAnnotationFont;
...
begin
  ...
  begin
    ...
    f := TGPFont.Create(ff, AnnotationFont.Size * DPIScale, fs, UnitPoint);
  end;
  ...
  begin
    ...
    begin
      if Assigned(OnGetAnnotationFont) then
      begin
        ft := TFont.Create;
        try
          ft.Assign(TimeLineBar.AnnotationFont);
          OnGetAnnotationFont(Self, indicator, ft);
          if Assigned(ff) then
            ff.Free;
          ...
          f := TGPFont.Create(ff, ft.Size * DPIScale, fs, UnitPoint);
        finally
          ft.Free;
        end;
      end;
...
begin
  ...
  sf := TGPStringFormat.Create;
  f := TGPFont.Create(ff, FRangeAppearance.DivisionFont.Size * DPIScale, fs, UnitPoint);
  //
  ...
  fsub := TGPFont.Create(ffsub, FRangeAppearance.SubDivisionFont.Size * DPIScale, fs,
    UnitPoint);
  //
  Layr := MakeRect(0, 0, 10000, 10000);

  X := timeliner.X;
  Y := timeliner.Y + (2 + RangeAppearance.DivisionTickMarkSize) * DPIScale;
  ysub := timeliner.Y + (2 + RangeAppearance.SubDivisionTickMarkSize) * DPIScale;
...
procedure TAdvSmoothTimeLine.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      bmp := TGPBitmap.Create(Round(fillr.Width + 1), Round(fillr.Height + 1));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          p := TGPPen.Create(MakeColor(255, TickMarkColor), TickMarkWidth * DPIScale);
          g.DrawLine(p, fp.r.X + fp.r.Width / 2, fp.r.Y + fp.r.Height,
            fp.r.X + fp.r.Width / 2, fp.r.Y + fp.r.Height + (DPIScale * TickMarkSize));
          p.Free;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    sf := TGPStringFormat.Create;
    sf.SetLineAlignment(StringAlignmentCenter);
    f := TGPFont.Create(ff, SectionCaptionFont.Size * DPIScale, fs, UnitPoint);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        g.DrawLine(pdiv, X, Y, X, Y + Round(DPIScale * RangeAppearance.DivisionTickMarkSize));
        ...
        begin
          ...
          begin
            g.DrawLine(psubdiv, X + (wsubdiv * J), Y, X + (wsubdiv * J),
              Y + Round(DPIScale * RangeAppearance.SubDivisionTickMarkSize));
          end;
        end;
        X := X + wdiv;
      end;
...
begin
  ...
  begin
    ...
    begin
      pt := postext;
      pt.Y := pt.Y + (RangeAppearance.IndentSpacing * indent * DPIScale);
      case tp of
        vtDivision:
          begin
            if Assigned(FOnDivisionText) then
...
begin
  ...
  r := InsideRect;
  result := MakeRect(r.Left + cHM, r.Top + cVM,
    r.Right - r.Left - (cHM * 2), cH);
end;
...
begin
  ...
  r := InsideRect;
  result := MakeRect(r.Left + cHM,
    r.Top + cVM + cH, r.Right - r.Left -
      (cHM * 2), r.Bottom - r.Top - cH -
      (cVM * 2));
end;
...
begin
  ...
  begin
    case DivisionType of
      dtFixedNumber: d := Divisions;
      dtMilliSecond: d := (MilliSecondsBetween(RangeTo, RangeFrom) div Divisions);
      dtSecond:  d := (SecondsBetween(RangeTo, RangeFrom) div Divisions);
      dtMinute:  d := (MinutesBetween(RangeTo, RangeFrom) div Divisions);
      dtHour:  d := (HoursBetween(RangeTo, RangeFrom) div Divisions);
      dtDay:  d := (DaysBetween(RangeTo, RangeFrom) div Divisions);
      dtMonth:  d := (MonthsBetween(RangeTo, RangeFrom) div Divisions);
      dtYear:  d := (YearsBetween(RangeTo, RangeFrom) div Divisions);
      else
        d := 1;
    end;

    if d <> 0 then
      FSubDivisions := Min(Value, (FOwner.Width div 2) div d)
    else
      FSubDivisions := 0;
          ...
          if (((rind.X < rtime.X + rtime.Width / 2) and (AnnotationOrientation = aoAuto)) or (AnnotationOrientation = aoRight)) then
            result := MakeRect(rind.X + rind.Width / 2 + 5,
              rind.Y + rind.Height + 15 + ((FOwner.GetTotalIndent + 1)
                  * FOwner.RangeAppearance.IndentSpacing) * DPIScale, wtext, htext)
          else
...
begin
  ...
  if FOwner.FMouseMovedOnIndicator and FOwner.FMouseDownOnIndicator and
    ...
    result.X := FOwner.FPx - cSize / 2
  else
    result.X := FOwner.DateTimeToX(Position) - cSize / 2;

  result.X := max(r.X - Size / 2, min(result.X, r.X + r.Width - cSize / 2));

  result.Y := r.Y + r.Height / 2 - cSize / 2;
  result.Width := cSize;
  result.Height := cSize;
end;
