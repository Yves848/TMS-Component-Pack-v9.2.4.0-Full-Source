...
const
  ...
  // v2.5.6.0 : New : Added AddPanel / InsertPanel overload functions to allow to insert custom created panel classes
  // v2.5.6.1 : Fixed : Rare TAdvPanelGroup scrolling issue
  // v2.5.6.2 : Fixed : Issue with collaps on panel with client aligned controls
  // v2.5.7.0 : New : AppendPanel() overload to add custom panels
  // v2.5.7.1 : Fixed : TAdvPanelStyler settings update in particular circumstances
  // v2.5.7.2 : Fixed : Issue with high DPI and styler based font settings
  // v2.5.7.3 : Fixed : Issue with painting when BorderWidth > 1
  // v2.5.7.4 : Fixed : High DPI issue with Delphi Tokyo
  // v2.5.7.5 : Fixed : Issue with rotated caption font in high DPI
  // v2.5.7.6 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  //          : Fixed : Issue with drawing HR tag
  // v2.5.7.7 : Fixed : Issue with use of styler & high DPI
  // v2.5.7.8 : Fixed : Issue with caption font on high DPI
  // v2.5.7.9 : Fixed : Issue with VCL Styles
  // v2.5.8.0 : Improved : Per monitor high DPI support
...
//    property OnKeyUp;
  ...
  public
    ...
    function AddPanel: TCustomAdvPanel; overload; virtual;
    function AddPanel(APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel; overload; virtual;
    function AppendPanel: TCustomAdvPanel; overload; virtual;
    function AppendPanel(APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel; overload; virtual;
    function InsertPanel(Index: Integer): TCustomAdvPanel; overload; virtual;
    function InsertPanel(Index: Integer; APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel; overload; virtual;
...
begin
  ...
  if not (csDesigning in ComponentState) and (AOwner is TAdvPanel) then
    FOptimizePaint := true;
  {$ENDIF}
...
begin
  ...
  begin
    ...
    FShadedHeader.Height := FCapHeight;
  ...
  begin
    FShadedHeader.Width := FCapHeight;
  ...
  begin
    ...
    begin
      ...
      begin
        if FCaption.Position = cpTop then
        begin
          ind := Width - Round(FCaption.Background.Width * FDPIScale);
          if ind < 0 then ind := 0;
          if FDPIScale > 1 then
            Canvas.StretchDraw(Rect(r.Left + Ind, r.Top, Round(FCaption.Background.Width * FDPIScale), r.Bottom), FCaption.Background)
          else
        ...
        begin
          ...
          if ind < 0 then ind := 0;
          if FDPIScale > 1 then
            Canvas.StretchDraw(Rect(r.Left, r.Top, Round(FCaption.Background.Width * FDPIScale), r.Bottom), FCaption.Background)
          else
      ...
      begin
        if FCaption.Position = cpTop then
        begin
          ind := Round(FCaption.Background.Width * FDPIScale);
         ...
         begin
          ...
          if FDPIScale > 1 then
            Canvas.StretchDraw(Rect(r.Left, r.Top, Round(FCaption.Background.Width * FDPIScale), r.Bottom - Ind), FCaption.Background)
          else
...
begin
  ...
  begin
    ...
    begin
      ...
      BR := Rect(r.Left, r.Top, r.Right ,r.Bottom);
      InflateRect(BR, -(BorderWidth div 2), -(BorderWidth div 2));

      ACanvas.Rectangle(BR.Left, BR.Top, BR.Right ,BR.Bottom );
   end;
...
//  InflateRect(r,-1,-1);
  ...
  r.Left := r.Left + Round(Indent * FDPIScale);
  r.Top := r.Top + Round(TopIndent * FDPIScale);
  r.Bottom := r.Bottom - BottomIndent;

  if FStatusBar.Visible then
  begin
    if FCaption.Position = cpTop then
      R.Bottom := R.Bottom - FStatHeight
    else if FCaption.Position = cpLeft then
      R.Right := R.Right - FStatHeight
    else
      R.Left := R.Left + FStatHeight;
  end;
  ...
  begin
    ...
    HTMLDrawEx(ACanvas,Text,dr,FImages,0,0,-1,FHoverHyperLink,FShadowOffset,true,false,false,false,false,fHover,not FEllipsis,FFormScaled,
      1.0,fURLColor,fHoverColor,fHoverFontColor,fShadowColor,a,s,fa,
  ...
  HTMLDrawEx(ACanvas,Text,r,FImages,0,0,-1,FHoverHyperLink,FShadowOffset,false,false,false,false,false,FHover,not FEllipsis, FFormScaled,
  ...
  begin
    ...
    GW := Round(CAPTIONBTNSIZE * FDPIScale);
    ...
    begin
      ...
      r.Left := r.Left + Round(FCaption.Indent * FDPIScale) + 2;
      r.Top := r.Top + Round(FCaption.TopIndent * FDPIScale);

      HTMLDrawEx(ACanvas,FCaption.Text,r,fImages,0,0,-1,FCaptionHoverHyperLink,FShadowOffset,false,false,false,false,false,fHover,false, FFormScaled,
        1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing);
    end
    else if FCaption.Position = cpLeft then
    begin
      ...
      try
        ...
        tf.Assign(FCaption.Font);
        if not FWasLoaded then
          tf.Size := Round(tf.Size / CalcDPIScale(0, true));
  ...
  begin
    ...
    if FCaption.Position = cpTop then
      R2 := Rect(delta, Height - FStatHeight - 1, Width - delta, Height - delta)
    else if FCaption.Position = cpLeft then
      R2 := Rect(Width - FStatHeight - delta, 1, Width - delta, Height - delta)
    else  // FCaption.Position = cpRight
      R2 := Rect(delta, 1, delta + FStatHeight, Height - delta);
    ...
    begin
      ...
      ACanvas.Font.Assign(FStatusBar.Font);
      HTMLDrawEx(ACanvas, FStatusBar.Text, R2, FImages,0,0,-1,FCaptionHoverHyperLink,FShadowOffset,false,false,false,false,false,fHover,false,FFormScaled,
        1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing);
      R2.Left := R2.Left - 2;
      R2.Top := R2.Top - FStatusBar.TopIndent;
    end
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if FStatusBar.Visible then
              delta := (Height - FCapHeight - FStatHeight - 2 * BorderWidth) div CollapsSteps
            else
          ...
          if FStatusBar.Visible then
            Height := FCapHeight + FStatHeight - 2 + bdelta
          else
            Height := FCapHeight - 2 + bdelta;
        ...
        begin
          ...
          begin
            ...
            if FStatusBar.Visible then
              delta := (Width - FCapHeight - FStatHeight) div CollapsSteps
            else
          ...
          if FStatusBar.Visible then
            Width := FCapHeight + FStatHeight + bdelta
          else
      ...
      begin
        ...
        begin
          ...
          if FStatusBar.Visible then
            delta := (FullHeight - FCapHeight - FStatHeight) div CollapsSteps
          else
...
function TCustomAdvPanel.GetButtonSize: integer;
begin
  Result := Round(CAPTIONBTNSIZE * FDPIScale);

  if (not FCaption.CloseMinGlyph.Empty and not FCaption.CloseMaxGlyph.Empty) then
    Result := FCaption.CloseMinGlyph.Width;

  if (not FCaption.MaxGlyph.Empty and not FCaption.MinGlyph.Empty)  then
    Result := FCaption.MaxGlyph.Width;
...
begin
  ...
  HTMLDrawEx(Canvas,Text,r,FImages,0,0,-1,FHoverHyperLink,fShadowOffset,true,false,false,false,false,fHover,true,FFormScaled,
    1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing);
...
begin
  ...
  if FTextVAlign in [tvaCenter,tvaBottom] then
  begin
    HTMLDrawEx(Canvas,Text,r,FImages,0,0,-1,FHoverHyperLink,fShadowOffset,true,false,false,false,false,fHover,not FEllipsis, FFormScaled,
      1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing);
  end;
  ...
  begin
    ...
    begin
      ...
      DrwRes := HTMLDrawEx(Canvas,FCaption.Text,r,fImages,x,y,-1,-1,2,true,false,false,false,false,false,not FEllipsis, FFormScaled,
        1.0, FURLColor,clBlue,clRed,clgray,a,s,fa,xsize,ysize,hl,FCaptionHoverHyperlink,hoverrect,FImageCache,FContainer,FLineSpacing);
    end;
  end
  else
  begin
    ...
    DrwRes := HTMLDrawEx(Canvas,Text,r,FImages,x,y,-1,-1,2,true,false,false,false,false,false,not FEllipsis,FFormScaled,
      1.0, FURLColor,clBlue,clRed,clgray,a,s,fa,xsize,ysize,hl,FHoverHyperlink,hoverrect,FImageCache,FContainer,FLineSpacing);
  end;
...
begin
  ...
  FStatHeight := Round(FDPIScale * StatusBar.Height);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cpTop:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Top < d);
        end;
      cpLeft:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Left < d);
        end;
      cpRight:
        begin
          incaption := Caption.Visible and (Controls[i - 1].Left > Width - d);
        end;
      end;
...
begin
  ...
  begin
    if FCaption.Position = cpTop then
    begin
      if Rect.Bottom > Height - FStatHeight then
        Rect.Bottom := Height - FStatHeight;
    end
    else if FCaption.Position = cpLeft then
    begin
      if Rect.Right > Width - FStatHeight then
        Rect.Right := Width - FStatHeight;
    end
    else // FCaption.Position = cpRight
    begin
      if Rect.Left < FStatHeight then
        Rect.Left := FStatHeight;
    end;
  end;
...
begin
  ...
  HTMLDrawEx(Canvas,Text,r,FImages,0,0,-1,FHoverHyperLink,FShadowOffset,false,True,false,false,false,FHover,not FEllipsis, FFormScaled,
    1.0,FURLColor,FHoverColor,fHoverFontColor,fShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing);
  ...
  begin
    ...
    begin
      ...
      begin
        if FStatusBar.Visible then
          Width := XSize + FCapHeight + FStatHeight + 2 * BevelWidth + Indent
        else
      ...
      begin
        if FStatusBar.Visible then
          Width := XSize + FStatHeight + 2 * BevelWidth + Indent
        else
          Width := XSize + 2 * BevelWidth + Indent;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        if FStatusBar.Visible then
          Height := YSize + FCapHeight + FStatHeight + 2 * BevelWidth + TopIndent + BottomIndent + 2 * BorderWidth
        else
      ...
      begin
        if FStatusBar.Visible then
          Height := YSize + FStatHeight + 2 * BevelWidth + TopIndent + BottomIndent
        else
...
{$IFDEF DELPHIXE10_LVL}
procedure TCustomAdvPanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
...
begin
  ...
  {$ENDIF}
  Caption.FFont.Height := MulDiv(Caption.FFont.Height, M, D);
  StatusBar.FFont.Height := MulDiv(StatusBar.FFont.Height, M, D);

  FCapHeight := MulDiv(FCapHeight, M, D);
  FStatHeight := MulDiv(FStatHeight, M, D);
  FFullHeight := MulDiv(FFullHeight, M, D);

  if Assigned(FStyler) then
  begin
    {$IFNDEF DELPHIXE11_LVL}
    Font.Height := MulDiv(FStyler.Settings.Font.Height, M, D);
    {$ENDIF}
  end;
...
begin
  ...
  begin
    ...
    Font.Height := Round(Font.Height * CalcDPIScale(Canvas.Handle, True));
    Caption.Font.Assign(Settings.Caption.Font);
    Caption.Font.Height := Round(Caption.Font.Height * CalcDPIScale(Canvas.Handle, True));
  end
  else
    Caption.Font.Assign(Font);
...
procedure TPanelCaption.CaptionFontChanged(Sender: TObject);
begin
  Changed;
end;
...
function TAdvPanelGroup.AddPanel(APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel;
begin
   APanel.Parent := Self;
   if UseDefaultSettings then
   begin
     APanel.AssignSettings(DefaultPanel);
     APanel.Caption.Visible := DefaultPanel.Caption.Visible;
     APanel.Caption.MinMaxButton := DefaultPanel.Caption.MinMaxButton;
     APanel.Caption.CloseButton := DefaultPanel.Caption.CloseButton;
   end;
   APanel.Index := FPanels.Count;

   FPanels.Add(Pointer(APanel));
   UpdateScrollbar;
   ArrangeControls;
   UpdateScrollbar;
...
function TAdvPanelGroup.AppendPanel(APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel;
begin
  Result := AddPanel(APanel, UseDefaultSettings);
...
function TAdvPanelGroup.InsertPanel(Index: Integer; APanel: TCustomAdvPanel; UseDefaultSettings: boolean = true): TCustomAdvPanel;
var
  i: Integer;
begin
  Result := APanel;
