{**************************************************************************}
{ TAdvSmoothToggleButtonDE DESIGN TIME EDITOR                              }
...
uses
  Classes, AdvSmoothToggleButton,
  DesignIntf, DesignEditors;

type
  TAdvSmoothToggleButtonEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothToggleButtonEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothToggleButton).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothToggleButton) then
         (Component as TAdvSmoothToggleButton).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothToggleButtonEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothToggleButtonEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
