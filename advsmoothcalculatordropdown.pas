...
uses
  ...
  StdCtrls, ExtCtrls, AdvSmoothEditButton, AdvSmoothEdit, AdvSmoothCalculator,
  AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.0.0.4 : Fixed : Issue with closing calculator with keyboard
  // v1.0.0.5 : Added :  newest Windows 10 and Office 2016 styles in set component style list
  // v1.0.1.0 : Improved : Support for High DPI (drop down form and font size)
...
type
  ...
  protected
    ...
    procedure CalcParentClose(Sender: TObject);
...
type
  TAdvSmoothCalculatorOpen = class(TAdvSmoothCalculator);
...
begin
  DPIScale := GetDPIScale(Self,CalcParent.Canvas);
  ...
  Calc.Width := Round(Calculator.Width * DPIScale);
  Calc.Height := Round(Calculator.Height * DPIScale);
...
begin
  case AStyle of
    ...
    tsWindows8: Calc.SetComponentStyle(tsWindows8);
    tsWindows10: Calc.SetComponentStyle(tsWindows10);
    ...
    tsOffice2013White: Calc.SetComponentStyle(tsOffice2013White);
    tsOffice2013LightGray: Calc.SetComponentStyle(tsOffice2013LightGray);
    tsOffice2013Gray: Calc.SetComponentStyle(tsOffice2013Gray);
    tsOffice2016White: Calc.SetComponentStyle(tsOffice2016White);
    tsOffice2016Gray: Calc.SetComponentStyle(tsOffice2016Gray);
    tsOffice2016Black: Calc.SetComponentStyle(tsOffice2016Black);
  end;
...
begin
  ...
  TAdvSmoothCalculatorOpen(FCalc).OnCalcParentClose := CalcParentClose;
  FIgnoreSelect := false;
...
procedure TAdvSmoothCalculatorDropDown.CalcParentClose(Sender: TObject);
begin
  if CalcParent.Visible then
    HideParent;
end;
...
procedure TAdvSmoothCalculatorDropDown.CMStyleChanged(var Message: TMessage);
begin
  inherited;
