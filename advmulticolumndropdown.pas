...
type
  ...
  published
    ...
    property HoverSelect: boolean read FHoverSelect write FHoverSelect default false;
  ...
  TDropDownCellClickEvent = procedure(Sender: TObject; ACol, ARow: integer; AValue: string) of object;
  ...
  private
    ...
    FOrgIndex: integer;
    FHoverSelect: boolean;
    FOnDropDownCellClick: TDropDownCellClickEvent;
    FLookupIncr: boolean;
    procedure WMKeyDown(var Msg: TWMKeydown); message WM_KEYDOWN;
    ...
    procedure SetHoverSelect(const Value: boolean);
  protected
    procedure Notification(AComponent: TComponent; AOperation: TOperation); override;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
  ...
  published
    ...
    property HeaderHeight: Integer read FHeaderHeight write SetHeaderHeight;
    property HeaderFont: TFont read FHeaderFont write SetHeaderFont;
    property HoverSelect: boolean read FHoverSelect write SetHoverSelect default false;
...
{$ENDIF}
    ...
    property OnDropDownCellClick: TDropDownCellClickEvent read FOnDropDownCellClick write
      FOnDropDownCellClick;
...
begin
  ...
  FAdvColGrid.HoverSelect := FHoverSelect;
...
procedure TAdvMultiColumnDropDown.SetHoverSelect(const Value: boolean);
begin
  FHoverSelect := Value;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FOnDropDownCellClick) then
    begin
      r := ARow - FAdvColGrid.FixedRows;
      s := Items[r].Text[ACol];

      FOnDropDownCellClick(Self, ACol, ARow, s);
    end;
...
begin
  if (Key = #13) and ReturnIsTab then
    Key := #0
  else
...
var
  ...
  i: integer;
begin
  if not((Key = VK_RETURN) and (ReturnIsTab)) then
  ...
  if (Key > 31) and FLookupIncr then
  begin
    c1 := GetCharFromVirtualKey(Key);
    if c1 = '' then
      FLookupStr := ''
    else
      FLookupStr := FLookupStr + c1;

    c := upstr(FLookupStr, FCaseSensitive);
  end
  else
  begin
    FLookupStr := '';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (i + FAdvColGrid.FixedRows >= 0) and (i + FAdvColGrid.FixedRows < FAdvColGrid.RowCount) then
...
begin
  ...
  begin
    ...
    if (AdvDropDown.DropDownRowHeight > 0) and (h > AdvDropDown.DropDownRowHeight) then
      RowHeights[r] := h;
  end;
end;
...
procedure TAdvColGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  r,c: integer;
begin
  inherited;

  if FHoverSelect then
  begin
    MouseToCell(X,Y,C,R);
