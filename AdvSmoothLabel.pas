...
const
  ...
  // v1.6.0.3 : Fixed : memory leak when applying auto-sizing
  // v1.6.0.4 : Fixed : Strange issue with some specific TTF fonts and text size calculation
  // v1.6.1.0 : Improved : Support for High DPI
...
begin
  ...
  f := TGPFont.Create(ff, c.Font.Size * DPIScale, fs, UnitPoint);
  ...
  xs := x + Round(DPIScale * OffsetX);
  ys := y + Round(DPIScale * OffsetY);

  if Fill.ShadowType = stSurround then
  begin
    xs := xs + Round(DPIScale * Fill.ShadowOffset);
    ys := ys + Round(DPIScale * Fill.ShadowOffset);
  end;
  ...
  if c.GradientType <> gtTexture then
    g.DrawString(c.Text, Length(c.Text), f, MakeRect(gppointf.X, gppointf.Y, sizeRect.Width + 2, sizerect.Height), stringformat, textb);
...
begin
  ...
  begin
    ...
    begin
      ...
      DPIScale := GetDPIScale(Self, Canvas);
      Fill.BeginUpdate;
      Fill.ShadowOffset := Round(Fill.ShadowOffset * DPIScale);
      r := Fill.Fill(g, r);
      Fill.ShadowOffset := Round(Fill.ShadowOffset / DPIScale);
      Fill.EndUpdate;
      //////////////////////
...
begin
  ...
  f := TGPFont.Create(ff, c.Font.Size * GetDPIScale(Self, Canvas), fs, UnitPoint);
  try
    g.MeasureString(c.Text, Length(c.Text), f, r, stringFormat, sizerect);
