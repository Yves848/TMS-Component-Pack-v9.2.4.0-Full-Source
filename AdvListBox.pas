...
const
  ...
  // v1.1.6.0 : New : Exposed ListBox as public property
  // v1.1.7.0 : New : ListBox.ReadOnly property added
  // v1.2.0.0 : New : TAdvDBListBox variant added
  // v1.2.0.1 : Fixed : Issue with setting dataset active at design-time with TAdvDBListBox
  // v1.2.0.2 : Fixed : Memmory leak issue when control is recreated at runtime
  // v1.2.0.3 : Fixed : Issue with deleting items
  // v1.2.1.0 : New : Keyboard lookup on listbox items
  // v1.2.2.0 : Improved: Support for High DPI
  // v1.2.2.1 : Fixed : Case sensitive with filtering issue fixed
  // v1.3.0.0 : New : VCL Styles support added
  // v1.3.0.1 : Improved : VCL Style handling
...
type
  ...
  TListBoxItemDeleteEvent = procedure(Sender: TObject; Index: integer) of object;

  TListBoxItems = class(TCollection)
  private
    FOnChange: TNotifyEvent;
    FOnDelete: TListBoxItemDeleteEvent;
  ...
  published
    ...
    property OnDelete: TListBoxItemDeleteEvent read FOnDelete write FOnDelete;
  end;
  ...
  TAdvCustomListBox = class(TCustomControl)
  private
    ...
    function GetFilteredItemIndex: integer;
    procedure SetFilteredItemIndex(const Value: integer);
  protected
    ...
    procedure ItemDeleted(Sender: TObject; Index: integer); virtual;
    ...
    procedure DoInsertItem(Value: string); virtual;
  ...
  public
    ...
    property FilteredItemIndex: integer read GetFilteredItemIndex write SetFilteredItemIndex;
    ...
    property ListBox: TVisualListBox read FListBox;
  published
    property Align;
    property Anchors;
  ...
  TAdvListBox = class(TAdvCustomListBox)
  published
    property Items;
  end;
...
procedure TAdvCustomListBox.ListCheckBoxClick(Sender: TObject; Index: integer);
var
  li: TListBoxItem;
begin
  li := GetItem(Index);
...
procedure TAdvCustomListBox.CMColorChanged(var Message: TMessage);
begin
  inherited;
end;

procedure TAdvCustomListBox.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  FListBox.Enabled := Enabled;
end;

procedure TAdvCustomListBox.CMFontChanged(var Message: TMessage);
begin
  inherited;
...
procedure TAdvCustomListBox.CMSetCursor(var Message: TMessage);
begin
  inherited;
...
constructor TAdvCustomListBox.Create(AOwner: TComponent);
begin
  inherited;
...
function TAdvCustomListBox.CreateItems: TListBoxItems;
begin
  Result := TListBoxItems.Create;
end;

procedure TAdvCustomListBox.CreateWnd;
begin
  inherited;
  ...
  if not Assigned(FFilterIndexes) then
    FFilterIndexes := TIntList.Create;

  UpdateItems;
end;

destructor TAdvCustomListBox.Destroy;
begin
  FItems.Free;
  FFilterIndexes.Free;
  FSearchOptions.Free;
...
procedure TAdvCustomListBox.AlignmentChanged(Sender: TObject);
begin
  UpdateAlignment;
end;

procedure TAdvCustomListBox.BeginUpdate;
begin
  inc(FUpdateCount);
end;

function TAdvCustomListBox.BuildCondition(st: TFilterMethod; s: string): string;
begin
  if Pos(' ',s) > 0 then
    s := '"' + s + '"';
...
procedure TAdvCustomListBox.DoFilter;
var
  i: integer;
  f: boolean;
  szFilter: string;
...
begin
  ...
  begin
    f := false;
    if MatchStr(szFilter, Items[i].Text, SearchOptions.CaseSensitive) then
    begin
      inc(FFilterItems);
      f := true;
...
procedure TAdvCustomListBox.DoInsertItem(Value: string);
var
  li: TListBoxItem;
begin
  li := Items.Add;
...
procedure TAdvCustomListBox.DoItemHint(AItem: TListBoxItem; Index: integer;
  var HintStr: string);
begin
...
procedure TAdvCustomListBox.EndUpdate;
begin
...
function TAdvCustomListBox.GetDragModeEx: TDragMode;
begin
  Result := inherited DragMode;
end;

function TAdvCustomListBox.GetFilteredItemIndex: integer;
begin
  Result := GetItemIndex;
end;

function TAdvCustomListBox.GetItem(Index: integer): TListBoxItem;
begin
...
function TAdvCustomListBox.GetItemAtXY(X, Y: integer): TListBoxItem;
var
  idx: integer;
begin
  Result := nil;
...
function TAdvCustomListBox.GetItemIndex: integer;
begin
  Result := -1;
...
function TAdvCustomListBox.GetSelected(Index: integer): boolean;
begin
  Result := FListBox.Selected[Index];
end;

function TAdvCustomListBox.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TAdvCustomListBox.GetVersionNr: integer;
begin
  Result := MakeLong(MakeWord(BLD_VER, REL_VER), MakeWord(MIN_VER, MAJ_VER));
end;

procedure TAdvCustomListBox.HandleSearch(Filter: boolean);
var
  i,j: integer;
...
procedure TAdvCustomListBox.HandleShortCut(Key: TWMKey);
begin
...
procedure TAdvCustomListBox.InsertAddClick(Sender: TObject);
begin
...
procedure TAdvCustomListBox.InsertChanged(Sender: TObject);
var
  b: TEditButton;
begin
...
procedure TAdvCustomListBox.InsertCloseClick(Sender: TObject);
begin
...
procedure TAdvCustomListBox.InsertEditChanged(Sender: TObject);
begin
  if Assigned(OnInsertEditChange) then
    OnInsertEditChange(Self);
end;

procedure TAdvCustomListBox.InsertEditClick(Sender: TObject);
begin
  if Assigned(OnInsertEditClick) then
    OnInsertEditClick(Self);
end;

procedure TAdvCustomListBox.InsertEditDblClick(Sender: TObject);
begin
  if Assigned(OnInsertEditDblClick) then
    OnInsertEditDblClick(Self);
end;

procedure TAdvCustomListBox.InsertEditKeyPress(Sender: TObject; var Key: Char);
begin
  if Assigned(OnInsertEditKeyPress) then
    OnInsertEditKeyPress(Self, Key);
end;

procedure TAdvCustomListBox.InsertEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TAdvCustomListBox.InsertShow;
begin
  if Assigned(OnInsertShow) then
    OnInsertShow(Self);
end;

procedure TAdvCustomListBox.InsertEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
function TAdvCustomListBox.ItemCount: integer;
begin
  if FFilterActive then
    Result := FFilterItems
  else
...
procedure TAdvCustomListBox.ItemDeleted(Sender: TObject; Index: integer);
begin
  FListBox.Items.Delete(Index);
end;

procedure TAdvCustomListBox.ItemsChanged(Sender: TObject);
begin
  UpdateItems;
  Invalidate;
end;

procedure TAdvCustomListBox.ListClick(Sender: TObject);
begin
  if Assigned(OnListClick) then
    OnListClick(Self);
end;

procedure TAdvCustomListBox.ListDblClick(Sender: TObject);
begin
  if Assigned(OnListDblClick) then
    OnListDblClick(Self);
end;

procedure TAdvCustomListBox.ListDragDrop(Sender, Source: TObject; X, Y: Integer);
begin
  if Assigned(OnDragDrop) then
    OnDragDrop(Self, Source, X,Y);
end;

procedure TAdvCustomListBox.ListDragOver(Sender, Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
...
procedure TAdvCustomListBox.ListEndDrag(Sender, Target: TObject; X, Y: Integer);
begin
  if Assigned(OnEndDrag) then
    OnEndDrag(Self, Target, X, Y);
end;

procedure TAdvCustomListBox.ListKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TAdvCustomListBox.ListKeyPress(Sender: TObject; var Key: Char);
var
  i: integer;
  found: boolean;
begin
  ...
  for i := 0 to Items.Count - 1 do
  begin
    if Pos(Uppercase(FListbox.Lookup), Uppercase(Items[i].Text)) = 1 then
    begin
      ItemIndex := i;
      found := true;
      break;
...
procedure TAdvCustomListBox.ListKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TAdvCustomListBox.ListMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TAdvCustomListBox.ListMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TAdvCustomListBox.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TAdvCustomListBox.ListStartDrag(Sender: TObject;
...
procedure TAdvCustomListBox.Loaded;
var
  clr: TColor;
begin
  inherited;
...
procedure TAdvCustomListBox.Notification(AComponent: TComponent;
...
procedure TAdvCustomListBox.Paint;
{$IFDEF DELPHIXE2_LVL}
var
...
begin
  ...
  begin
    ...
    c := clNone;
    StyleServices.GetElementColor(LDetails, ecFillColor, c);
    if c <> clNone then
    begin
...
procedure TAdvCustomListBox.SearchChanged(Sender: TObject);
var
  b: TEditButton;
begin
...
procedure TAdvCustomListBox.SearchCloseClick(Sender: TObject);
begin
...
procedure TAdvCustomListBox.SearchEditChanged(Sender: TObject);
var
  eb: TEditButton;
begin
...
procedure TAdvCustomListBox.SearchEditClick(Sender: TObject);
begin
  if Assigned(OnSearchEditClick) then
    OnSearchEditClick(Self);
end;

procedure TAdvCustomListBox.SearchEditDblClick(Sender: TObject);
begin
  if Assigned(OnSearchEditDblClick) then
    OnSearchEditDblClick(Self);
end;

procedure TAdvCustomListBox.SearchEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TAdvCustomListBox.SearchEditKeyPress(Sender: TObject; var Key: Char);
begin
  if Assigned(OnSearchEditKeyPress) then
    OnSearchEditKeyPress(Self,Key);
end;

procedure TAdvCustomListBox.SearchEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TAdvCustomListBox.SearchFindClick(Sender: TObject);
begin
...
procedure TAdvCustomListBox.SearchShow;
begin
  if Assigned(OnSearchShow) then
    OnSearchShow(Self);
end;

procedure TAdvCustomListBox.SetAutoComplete(const Value: boolean);
begin
  FAutoComplete := Value;
  if Assigned(FListBox) then
...
procedure TAdvCustomListBox.SetAutoCompleteDelay(const Value: integer);
begin
...
procedure TAdvCustomListBox.SetBorderStyle(const Value: TBorderStyle);
begin
  FBorderStyle := Value;
  if Assigned(FListBox) then
...
procedure TAdvCustomListBox.SetDragModeEx(const Value: TDragMode);
begin
  inherited DragMode := Value;
  FListBox.DragMode := Value;
end;

procedure TAdvCustomListBox.SetFilterActive(const Value: boolean);
begin
...
procedure TAdvCustomListBox.SetFilterCondition(const Value: string);
begin
...
procedure TAdvCustomListBox.SetFilteredItemIndex(const Value: integer);
begin
  if FFilterActive then
  begin
    ItemIndex := Items[Value].FilterIndex;
  end
  else
  begin
    ItemIndex := Value;
...
procedure TAdvCustomListBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
end;

procedure TAdvCustomListBox.SetInsertOptions(const Value: TInsertOptions);
begin
  FInsertOptions.Assign(Value);
end;

procedure TAdvCustomListBox.SetItemHeight(const Value: integer);
begin
...
procedure TAdvCustomListBox.SetItemIndex(const Value: integer);
begin
  if Assigned(FListBox) then
    FListBox.ItemIndex := Value;
end;

procedure TAdvCustomListBox.SetItems(const Value: TListBoxItems);
begin
  FItems := Value;
end;

procedure TAdvCustomListBox.SetListColor(const Value: TColor);
begin
  FListColor := Value;
  if Assigned(FListBox) then
...
procedure TAdvCustomListBox.SetMultiSelect(const Value: boolean);
begin
  FMultiSelect := Value;
  if Assigned(FListBox) then
...
procedure TAdvCustomListBox.SetSearchOptions(const Value: TSearchOptions);
begin
  FSearchOptions.Assign(Value);
end;

procedure TAdvCustomListBox.SetSelected(Index: integer; const Value: boolean);
begin
  FListbox.Selected[Index] := Value;
end;

procedure TAdvCustomListBox.SetStyle(const Value: TAdvListBoxStyle);
begin
  FStyle := Value;
  Invalidate;
end;

procedure TAdvCustomListBox.UpdateAlignment;
begin
...
procedure TAdvCustomListBox.UpdateItems;
begin
  if FUpdateCount > 0 then
    Exit;
...
procedure TAdvCustomListBox.WMKeyDown(var Msg: TWMKeydown);
begin
  inherited;
  HandleShortCut(Msg);
end;

procedure TAdvCustomListBox.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
  FListBox.SetFocus;
end;
...
begin
  ...
  begin
    FChecked := Value;
    (Collection as TListBoxItems).Update(Self);
  end;
end;

procedure TListBoxItem.SetHasCheckBox(const Value: boolean);
begin
  if (FHasCheckBox <> Value) then
  begin
    FHasCheckBox := Value;
    (Collection as TListBoxItems).Update(Self);
  end;
...
procedure TListBoxItems.Delete(Index: integer);
begin
...
begin
  inherited;
  FOwner := TAdvCustomListBox(AOwner);
  FReadOnly := false;
end;
...
begin
  ...
  begin
    ...
    checkBoxScaleSize := Round(CHECKBOXSIZE* FDPIScale);
    Rect.Left := Rect.Left + checkBoxScaleSize;
  end;

  if Assigned(FOwner.Images) and (li.ImageIndex >= 0) then
  begin
    imgTop := Round(Rect.Bottom - Rect.Top - FOwner.Images.Height) div 2;

    FOwner.Images.Draw(Canvas, Rect.Left, Rect.Top + imgTop, li.ImageIndex);
  ...
  HTMLDrawEx(Canvas, s, Rect, FOwner.Images, -1, -1,-1,-1,2,false,true,false,
   (odSelected in State),false,false,false,FFormScaled,1.0,clBlue,clNone, clNone, clGray, a,s,fa,xs,ys,hl,ml,hr, nil, nil, 2);

  if (Rect.Bottom - Rect.Top > ys) then
    Rect.Top := Rect.Top + (Rect.Bottom - Rect.Top - ys) div 2;

  HTMLDrawEx(Canvas, s, Rect, FOwner.Images, -1, -1,-1,-1,2,false,false,false,
   (odSelected in State),false,false,false,FFormScaled,1.0,clBlue,clNone, clNone, clGray, a,s,fa,xs,ys,hl,ml,hr, nil, nil, 2, BiDiMode, clNone, clNone, [], FDPIScale);
end;

procedure TVisualListBox.KeyDown(var Key: Word; Shift: TShiftState);
begin
  if Key in [VK_HOME, VK_LEFT, VK_RIGHT, VK_END, VK_PRIOR, VK_NEXT, VK_END, VK_ESCAPE, VK_UP, VK_DOWN] then
    FLookup := '';

  inherited;
end;

procedure TVisualListBox.KeyPress(var Ch: Char);
begin
  FLookup := FLookup + Ch;
...
var
  FDPIScale: single;
  calcItemHeight: integer;
begin
  FDPIScale := GetDPIScale(Self,Canvas);
  calcItemHeight := Round(FOwner.FItemHeight * FDPIScale);

  if Assigned(FOwner.Images) then
    Height := Max(calcItemHeight, FOwner.Images.Height + 4)
  else
...
var
  FDPIScale: single;
  calcCheckboxSize: integer;
begin
  inherited;
  chk := -1;
  FDPIScale := GetDPIScale(Self, Canvas);
  calcCheckboxSize := Round(FDPIScale * CHECKBOXSIZE);

  if (FOwner.Style = stCheckList) and (x < calcCheckboxSize) then
  begin
...
begin
  ...
  if (FOwner.Style = stCheckList) and (x < calcCheckboxSize) and not FReadOnly then
  begin
...
var
  ...
  HTheme: THandle;
  FDPIScale: single;
  calcCheckSize: integer;
  top: integer;

begin
  FDPIScale := GetDPIScale(Self, Canvas);
  calcCheckSize := Round(FDPIScale * CHECKBOXSIZE);
  top := (R.Bottom - R.Top - calcCheckSize) div 2;
  r := Rect(R.Left, R.Top + top, R.Left + calcCheckSize, R.Top + top + calcCheckSize);
  ...
  csClassic:
    begin
      ...
      DrawRect.Left := R.Left + (R.Right - R.Left - calcCheckSize) div 2;
      DrawRect.Top:= R.Top + (R.Bottom - R.Top - calcCheckSize) div 2;
      DrawRect.Right := DrawRect.Left + calcCheckSize;
      DrawRect.Bottom := DrawRect.Top + calcCheckSize;
