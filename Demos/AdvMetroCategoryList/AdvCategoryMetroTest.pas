...
uses
  ...
  AdvPicturePane, PictureContainer, ShellAPI;
...
begin
  ...
  for j := (i*10) to (i*10)+4 do
  begin
    Item := Cat.Items.Add;
    Item.Text := 'New Item #' + inttostr(j);
    Item.Hint := 'Hint for #' + inttostr(j);
  end;
...
begin
  ...
  begin
    cat := AdvMetroCategoryList2.Categories.Add;
    cat.Text := 'Cat <b><a href="Category '+inttostr(i)+' Clicked">' + inttostr(i)+'</a></b>';
    cat.Hint := 'Hint #' + inttostr(i);
    for j := 0 to 4 do
    begin
      cat.Items.Add.Text := 'item <a href="Item ' + inttostr(j + i * 10) + ' Clicked">' + inttostr(j + i * 10) + '</a>';
...
procedure TForm96.Label4Click(Sender: TObject);
begin
