...
uses
  ...
  DBPlanner, Planner, Db, Grids, DBGrids, ComObj, StdCtrls, ActiveX,
...
//  DBDaySource1.Day := Now - 3;
  DBDaySource1.Day := EncodeDate(2001,3,1);
  DBDaySource2.Day := EncodeDate(2001,3,1);
...
begin
  ...
  begin
    DBPlanner1.Header.Captions.Add(FormatDateTime('dd/mm/yy',DBDaySource1.Day + i + Offset ));
  end;

  DBPlanner2.Header.Captions.Clear;
  DBPlanner2.Header.Captions.Add('');
  for i := 1 to DBPlanner2.Positions do
  begin
    DBPlanner2.Header.Captions.Add(FormatDateTime('dd/mm/yy',DBDaySource2.Day + i + Offset ));
  end;
