...
const
  ...
  // v1.0.2.0 : Improved : Support for High DPI
...
begin
  ...
  cSpacing := AItems.Appearance.Spacing;
  ...
  begin
    ...
    it := dr.Item;
    r := dr.DrawRect;
    r.Top := Round(DPIScale *(r.Top + Y - 1));
    r.Left := Round(DPIScale *(R.Left + X));
    r.Right := Round(DPIScale *(r.Right + X + 1));
    r.Bottom := Round(DPIScale *(r.Bottom + Y));
    ...
    Canvas.Font.Height := Round(DPIScale * Font.Height);

    Canvas.Rectangle(r.Left, r.Top, r.Right, r.Bottom);

    r.Left := r.Left + cSpacing;
    r.Right := r.Right - cSpacing;
...
begin
  ...
  begin
    DPIScale := GetDPIScale(Self, Canvas);
    cMX := FMouseX / DPIScale;
    cMY := FMouseY / DPIScale;

    col := items.XYToCollection(cMX, cMY);
    if Col <> nil then
    begin
      col.ResetAppearance;
      FActiveCollection := Col;
      colI := XToCollectionItem(cMX, col);
