{**************************************************************************}
{ TAdvSmoothExpanderGroup DESIGN TIME EDITOR                                       }
...
uses
  Classes, AdvSmoothFillPreview, AdvSmoothExpanderGroup, AdvSmoothFillEditor, GDIPFill,
  DesignIntf, DesignEditors;

type
  TAdvSmoothExpanderGroupEditor = class(TDefaultEditor)
  protected
...
procedure TAdvSmoothExpanderGroupEditor.EditProperty(const PropertyEditor: IProperty; var Continue: Boolean);
var
  PropName: string;
begin
...
procedure TAdvSmoothExpanderGroupEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothExpanderGroup).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothExpanderGroup) then
         (Component as TAdvSmoothExpanderGroup).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothExpanderGroupEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothExpanderGroupEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
