...
type
  // "friend class" so we can get access to protected methods
  TCustomAdvCardListFriend = class(TCustomAdvCardList);
...
begin
  {$IFDEF DELPHIXE2_LVL}
  { We don't want to process TMS styles when VCL styles are active. }
  if Assigned(CardList) and TCustomAdvCardListFriend(CardList).UseVCLStyles then
    Exit;
  {$ENDIF}
