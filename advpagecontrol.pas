...
uses
  ...
  Graphics, ImgList, ComCtrls, Buttons, Menus, ATXPVS, AxCtrls, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 2.0.1.0 : Improved : Multiline tab caption drawing when TabHeight > 0
  // 2.0.1.1 : Improved : High DPI handling
  // 2.0.1.2 : Improved : Width calculation for active tabs with bold active font
  // 2.0.1.3 : Fixed : Small painting issue in Tab style tsDelphi
  // 2.0.1.4 : Improved : Image drawing on tabs with tab height higher than image height
  // 2.0.1.5 : Fixed : Issue with tsDelphi tab style with hidden tabs
  // 2.0.1.6 : Fixed : Small display issue when used with no visible tabs
  // 2.0.1.7 : Fixed : Possible exception when clicking last tab
  // 2.0.2.0 : Improved : Per monitor high DPI handling
...
type
  ...
  private
    ...
    procedure DrawCloseButton(ARect: TRect; Active: Boolean);
    procedure DrawHoverCloseButton(ARect: TRect);
    procedure DrawDownCloseButton(ARect: TRect);
...
begin
  ...
  begin
    FTabCaption := Copy(Value, 1, length(Value) - 2) + 'W';
  end
  else
    FTabCaption := Value + ' ';
...
procedure TAdvPageControl.CreateWnd;
var
  p: TWinControl;
begin
  inherited;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvPageControl.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvPageControl.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
  FDPIScale := AdvStyleIF.GetDPIScale(Self, Canvas);
end;
...
procedure TAdvPageControl.DrawHoverCloseButton(ARect: TRect);
var
  P: TPoint;
  pw: integer;
  btnR: TRect;
  ...
  begin
    case TabPosition of
      ...
      begin
        ...
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + FLowerActive {2} {Active};
          ARect.Bottom := ARect.Bottom - FTabMargin.TopMargin;
          if not FTabBackGroundActive.Empty then
            ARect.Bottom := ARect.Bottom + TabOverlap;
        end
        else
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + FLowerActive {2} {Active};
          ARect.Bottom := ARect.Top + FTabMargin.RightMargin + GetCloseButtonHeight;
          if not FTabBackGroundActive.Empty then
            ARect.Bottom := ARect.Bottom + TabOverlap;
        end;
        //Rectangle(ARect.Left + 2, ARect.Bottom - 2, ARect.Left + GetCloseButtonWidth, ARect.Bottom - GetCloseButtonHeight);
        P.X:= ARect.Left + 2;
        P.Y:= ARect.Bottom - GetCloseButtonHeight;
      end;
      tpRight:
      begin
        if (ClosePosition in [cpLeft, cpLeftActive]) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin;
          ARect.Top := ARect.Top + FTabMargin.TopMargin;
        end
        else
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin;
          ARect.Top := ARect.Bottom - FTabMargin.RightMargin - GetCloseButtonHeight;
        end;
        //Rectangle(ARect.Left + 2, ARect.Top + 2, ARect.Left + GetCloseButtonWidth, ARect.Top + GetCloseButtonHeight);
        P.X:= ARect.Left + 2;
        P.Y:= ARect.Top + 2;
      end;
      tpTop:
      begin
        ...
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + {2} FLowerActive;
        end
        else
        begin
          ARect.Left := ARect.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + {2} FLowerActive;
        end;
        P.X:= ARect.Left + 2;
        P.Y:= ARect.Top + 2;
      ...
      begin
        ...
        begin
          ARect.Left := ARect.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + FLowerActive;
        end;
        P.X:= ARect.Left + 2;
        P.Y:= ARect.Top + 2;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        btnR := Rect(ARect.Left + 2, ARect.Bottom - 2, ARect.Left + GetCloseButtonWidth, ARect.Bottom - GetCloseButtonHeight);
        Rectangle(btnR);
        Pen.Color := clGray; //clBlack;

        pw := Round(2 * FDPIScale);
        Pen.Width := pw;

        MoveTo(btnR.Left + pw, btnR.Top + pw);
        LineTo(btnR.Right - 2*pw, btnR.Bottom - 2*pw);

        MoveTo(btnR.Right - 2*pw, btnR.Top + pw);
        LineTo(btnR.Left + pw, btnR.Bottom - 2*pw);
      end;
    end
    else
    begin
      ...
      begin
        Brush.Color := clWhite;
        Pen.Color := clBlack; //clGray;
        btnR := Rect(ARect.Left + 2, ARect.Top + 2, ARect.Left + GetCloseButtonWidth, ARect.Top + GetCloseButtonHeight);
        Rectangle(btnR);
        Pen.Color := clGray; //clBlack;
                      {/}
        pw := Round(2 * FDPIScale);
...
procedure TAdvPageControl.DrawDownCloseButton(ARect: TRect);
var
  P: TPoint;
  btnR: TRect;
  pw: integer;
  ...
  begin
    case TabPosition of
      ...
      begin
        if (ClosePosition in [cpLeft, cpLeftActive]) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + {2} FLowerActive {active};
          ARect.Bottom := ARect.Bottom - FTabMargin.TopMargin;
          if not FTabBackGroundActive.Empty then
            ARect.Bottom := ARect.Bottom + TabOverlap;
        end
      ...
      begin
        if (ClosePosition in [cpLeft, cpLeftActive]) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin - FLowerActive + 2;
          ARect.Top := ARect.Top + FTabMargin.TopMargin;
        end
        else
        begin
...
begin
  ...
  begin
    GlRgn := CreateRectRgn(P.X, P.Y, P.X + GetCloseButtonWidth-2, P.Y + GetCloseButtonHeight - 2);
    ...
    Canvas.Rectangle(P.X,P.Y,P.X+14,P.Y+14);
...
procedure TAdvPageControl.DrawCloseButton(ARect: TRect; Active: Boolean);
var
  a: integer;
  P: TPoint;
  btnR: TRect;
  ...
  begin
    case TabPosition of
      ...
      begin
        if (ClosePosition in [cpLeft, cpLeftActive]) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + a;
          ARect.Bottom := ARect.Bottom - FTabMargin.TopMargin;
          if not FTabBackGroundActive.Empty then
            ARect.Bottom := ARect.Bottom + TabOverlap;
        end
      ...
      begin
        if (ClosePosition in [cpLeft, cpLeftActive]) then
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin + 2 - a;
          ARect.Top := ARect.Top + FTabMargin.TopMargin;
        end
        else
        begin
      ...
      begin
        ...
        begin
          ARect.Left := ARect.Left + FTabMargin.LeftMargin;
          ARect.Top := ARect.Top + FTabMargin.TopMargin + a;
        end
        else
        begin
          ARect.Left := ARect.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
...
begin
  ...
  begin
    ...
    begin
      r.Left := r.Left + FTabMargin.LeftMargin {Active};
      r.Bottom := r.Top + FTabMargin.RightMargin + GetCloseButtonHeight;
    end;
    if not FTabBackGroundActive.Empty then
    begin
      r.Left := r.Left + 2;
    ...
    r := Rect(r.Left + 2, r.Bottom - GetCloseButtonHeight, r.Left + GetCloseButtonWidth, r.Bottom - 2);
  ...
  begin
    ...
    begin
      ...
      r := Rect(r.Left + 2, r.Top + 2, r.Left + GetCloseButtonWidth, r.Top + GetCloseButtonHeight);
    end
    else
    begin
      case TabPosition of
        tpTop, tpBottom:
        begin
          r.Left := r.Right - FTabMargin.RightMargin - GetCloseButtonWidth;
          r.Top := r.Top + FTabMargin.TopMargin;
          r := Rect(r.Left, r.Top, r.Left + GetCloseButtonWidth, r.Top + GetCloseButtonHeight);
        end;
        tpRight:
        begin
          r.Left := r.Left + FTabMargin.LeftMargin;
          r.Top := r.Bottom - FTabMargin.RightMargin - GetCloseButtonHeight;
          r := Rect(r.Left, r.Top, r.Left + GetCloseButtonWidth, r.Top + GetCloseButtonHeight);
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    if (tabIndex>=0) and (TAdvTabSheet(Pages[TabIndex]).ShowClose) {and (TabIndex = FOldActiveIndex)} and IsOnButton(tabIndex, X, Y) then
    begin
    ...
    case TabPosition of
      tpLeft:
        begin
          ...
          if not hasTabs then
          begin
            Canvas.MoveTo(MaxRight + 1, 0);
            Canvas.LineTo(MaxRight + 1, Height);
            Canvas.MoveTo(MaxRight, 1);
      ...
      tpRight:
        begin
          ...
          if not HasTabs then
          begin
            Canvas.MoveTo(1, ActBottom);
            Canvas.LineTo(1, Height);
            Canvas.MoveTo(0, ActBottom+1);
            Canvas.LineTo(Width, ActBottom+1);
          end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (ClosePosition in [cpLeft, cpLeftActive]) then
              TextY := Rect.Bottom - 4 - GetCloseButtonHeight
            else
              TextY := Rect.Bottom - 4;
          end
          else
        ...
        begin
          ...
          begin
            if (ClosePosition in [cpLeft, cpLeftActive]) then
              TextY := Rect.Top + 4 + GetCloseButtonHeight
            else
              TextY := Rect.Top + 4;
          end
          else
        ...
        begin
          ...
          begin
            if (ClosePosition in [cpLeft, cpLeftActive]) then
              TextX := Rect.Left + 4 + GetCloseButtonWidth
            else
              TextX := Rect.Left + 4;
          end
          else
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (i < PageCount - 1) and not (Pages[i + 1].TabVisible) then
                  isNextSelected := true;
        ...
        begin
          ...
          begin
            ...
            if (TabHeight > 0) and (TabHeight > Images.Height) then
            begin
              ImgY := TextY + (TabHeight - Images.Height) div 2;
            end;
            ...
            if (TabPosition = tpLeft) then
              ...
              if (TabPosition = tpRight) then
                ...
                Images.Draw(Canvas, TextX - 2, ImgY, TAdvTabSheet(FPages[TabIndex]).ImageIndex);
          ...
          begin
            ...
            if (Pos(#13,TAdvTabSheet(FPages[TabIndex]).Caption) > 0) and (TabHeight > 0) then
            begin
              wwstyle := DT_WORDBREAK or DT_VCENTER;

              th := DrawText(Canvas.Handle, Pchar(TAdvTabSheet(FPages[TabIndex]).Caption), Length(TAdvTabSheet(FPages[TabIndex]).Caption), TextR, DT_LEFT or DT_NOCLIP or wwstyle or DT_CALCRECT);
              TextR.Top := TextR.Top + (TabHeight - th) div 2;
              TextR.Bottom := TextR.Top + TabHeight;
             end
            else
              wwstyle := DT_SINGLELINE or DT_VCENTER;

            if not Assigned(FOnDrawTab) then
              DrawText(Canvas.Handle, Pchar(TAdvTabSheet(FPages[TabIndex]).Caption), Length(TAdvTabSheet(FPages[TabIndex]).Caption), TextR, DT_LEFT or DT_NOCLIP or wwstyle);
          end
          else
          begin
            if not Assigned(FOnDrawTab) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Pos(#13,TAdvTabSheet(FPages[ActiveTabIndex]).Caption) > 0) and (TabHeight > 0) then
        begin
          wwstyle := DT_WORDBREAK or DT_VCENTER;

          th := DrawText(Handle, PChar(TAdvTabSheet(FPages[ActiveTabIndex]).Caption), Length(TAdvTabSheet(FPages[ActiveTabIndex]).Caption), TextR, DT_LEFT or DT_NOCLIP or wwstyle or DT_CALCRECT);
          TextR.Top := TextR.Top + (TabHeight - th) div 2;
          TextR.Bottom := TextR.Top + TabHeight;
        end
        else
        ...
        DrawText(Handle, PChar(TAdvTabSheet(FPages[ActiveTabIndex]).Caption), Length(TAdvTabSheet(FPages[ActiveTabIndex]).Caption), TextR, DT_LEFT or DT_NOCLIP or wwstyle);
      end
      else
      begin
...
function TAdvPageControl.GetCloseButtonHeight: integer;
begin
  Result := Round(CloseButtonHeight * FDPIScale);
end;

function TAdvPageControl.GetCloseButtonWidth: integer;
begin
  Result := Round(CloseButtonWidth * FDPIScale);
end;
...
begin
  ...
  begin
    FDummyImages.Width := ImgW + TabMargin.RightMargin + TabMargin.LeftMargin + Round(4 * FDPIScale);
  end;
end;
