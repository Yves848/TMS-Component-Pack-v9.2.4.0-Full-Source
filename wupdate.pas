...
const
  ...
  // 2.4.1.0 : New : Delphi 10.2 Tokyo & C++Builder 10.2 Tokyo support
  // 2.4.2.0 : New : Event with info on new vs local file version comparison
  // 2.4.3.0 : New : Extra logging for handling of deleting & renaming temp files
  // 2.4.4.0 : New : Support for use of Unicode text in update control files
  // 2.4.4.1 : Fixed : Issue with non-Unicode encoded .INF files
...
begin
  ...
  DoStatus(979,'Version compare:'+fvn+' to local:'+ fvl, WebUpdateInformation, 0);
...
begin
  ...
  begin
    if not  sysutils.DeleteFile(strpas(TmpName)) then
      DoStatus(972,'Delete error '+inttostr(GetLastError)+ ' for ' + strpas(TmpName),WebUpdateReplaceError,ErrCannotDeleteFile);
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Result := False;
          DoStatus(972,'Delete error ' + inttostr(GetLastError)+' for ' +tgt,WebUpdateReplaceError,ErrCannotDeleteFile);
          Log('Unable to delete file '+ tgt);
          Exit;
        end;
      end;
    end;
    if not sysutils.RenameFile(strpas(TmpName),tgt) then
      DoStatus(972,'Rename file error ' + inttostr(GetLastError)+' for ' +strpas(TmpName),WebUpdateReplaceError,ErrCannotDeleteFile);
    Result := True;
  end;
end;
...
begin
  ...
  {$IFDEF DELPHIXE_LVL}
  try
    IniFile := TMemIniFile.Create(ControlFilename, TEncoding.UTF8);
  except
...
begin
  ...
  {$IFDEF FREEWARE}
  ShowMessage('Starting application update with'#13#10'TWebUpdate © 1998-2019 by tmssoftware.com');
  {$ENDIF}
...
begin
  ...
  {$IFDEF DELPHIXE_LVL}
  try
    IniFile := TMemIniFile.Create(FControlFilename, TEncoding.UTF8);
  except
