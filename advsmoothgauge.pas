...
const
  ...
  // v2.3.0.2 : Fixed : Issue with transparency on double buffered forms
  // v2.3.1.0 : New : OnAfterDraw event added
  // v2.3.2.0 : Improved: Support for High DPI (font size)
  // v2.3.2.1 : Change needle color on tsOffice2016Black
  // v2.4.0.0 : New : VCL Styles support
  // v2.4.0.1 : Improved : Font color & arc color handling with VCL styles
...
type
  ...
  TAdvSmoothGaugeAfterDraw = procedure(Sender: TObject; ACanvas: TCanvas; ARect: TRect) of object;
  ...
  private
    ...
    FOnAfterDraw: TAdvSmoothGaugeAfterDraw;
...
//    procedure CreateParams(var Params: TCreateParams); override;
  ...
  published
    ...
    property OnAfterDraw: TAdvSmoothGaugeAfterDraw read FOnAfterDraw write FOnAfterDraw;
...
var
  r: TRect;
begin
  r := Bounds(0, 0, GetWidth, GetHeight);
  DrawGauge(Canvas, r);
  if Assigned(OnAfterDraw) then
    OnAfterDraw(Self, Canvas, r);
end;
...
begin
  ...
  Digit.Color := ATones.Foreground.TextColor;
  ...
  ValueFont.Color := ATones.Foreground.TextColor;
...
procedure TAdvSmoothGauge.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
    ...
    ftgp := TGPFont.Create(ftfam, Round(ft.Size * DPIScale) , fs, UnitPoint);
