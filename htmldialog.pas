...
const
  ...
  // 1.5.0.0 : New : Support for per monitor High DPI
  //         : New : Support for VCL styles
  // 1.5.0.1 : Fixed : Issue with label font initialization
...
type
  ...
  private
    ...
    {$ENDIF}
  protected
    property Owner: THTMLDialog read FOwner write FOwner;
  end;
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLDialog.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
begin
  InitVCLStyle(True);
  Form := THTMLDialogForm.CreateNew(Application);
  Form.FOwner := Self;
  with Form do
    try
      if Assigned(Self.Owner) and (Self.Owner is TControl) then
        FDPIScale := GetDPIScale(TControl(Self.Owner), Canvas)
      else
        FDPIScale := GetDPIScale(Form, Form.Canvas);

      //Per monitor High DPI properties
      Scaled := true;
      FPrevPPI := Round(FDPIScale * 96);
      if(FDPIScale > 1) then
        First := true
      else
        First := false;
      ...
      Canvas.Font.Height := Round(FDPIScale * FFont.Height);
      ...
      ClientWidth := Round(FWidth * FDPIScale);
      ClientHeight := Round(FHeight * FDPIScale);
      Position := FPosition;
      ...
      HTMLabel.SetDPIScale(FDPIScale);
      ...
      begin
        Font.Assign(FFont);
        Font.Height := Round(FDPIScale * FFont.Height);
...
procedure THTMLDialog.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
{$ENDIF}
...
{$IFDEF DELPHIXE10_LVL}
procedure THTMLDialogForm.WMDpiChanged(var Message: TWMDpi);
begin
  inherited;

  if First then
  begin
    ClientWidth := Round((Owner as THTMLDialog).FWidth * Message.YDpi / 96);
    ClientHeight := Round((Owner as THTMLDialog).FHeight * Message.YDpi / 96);

    ScaleControls(message.YDpi, FPrevPPI);
