...
const
  ...
  // v1.4.0.1 : Fixed : Issue with pressing operator repeatedly
  // v1.4.0.2 : Fixed : Issue with missing events
  //          : Fixed : Issue with focus not being set when clicking on calculator
  // v1.4.0.3 : Fixed : Issue with closing calculator with keyboard
  // v1.4.1.0 : Improved : Support for High DPI (Button font wasn't scaled and font changes weren't handled)
...
type
  ...
  private
    ...
    FOnCalcParentClose: TNotifyEvent;
  ...
  protected
    ...
    property OnCalcParentClose: TNotifyEvent read FOnCalcParentClose write FOnCalcParentClose;
  public
    procedure Assign(Source: TPersistent); override;
...
begin
  ...
  begin
    if ((key = VK_DOWN) and (ssAlt in Shift)) or ((key = VK_F4) and not (ssAlt in Shift) and not (ssCtrl in Shift)) or (Key = VK_ESCAPE) or (Key = VK_TAB) then
    begin
      if Assigned(OnCalcParentClose) then
        OnCalcParentClose(Self);
    end;
  end;
  Changed;
end;
...
procedure TAdvSmoothCalculator.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
...
begin
  ...
  begin
   case ButtonIdx of
   10:begin if lastop = -1 then doplus; prevop:=0; lastop:=0; end;
   11:begin if lastop = -1 then domin; prevop:=2; lastop:=0; end;
   12:begin if lastop = -1 then domul; prevop:=1; lastop:=0; end;
   13:begin if lastop = -1 then dodiv; prevop:=3; lastop:=0; end;
   14:FFloat := FFloat * -1;
   15:begin
       FDecim := 10;
       if newval then
...
begin
  ...
  begin
    ...
    CalculatorLook.Font.Height := Round(CalculatorLook.Font.Height * DPIScale);
...
procedure TAdvSmoothCalculator.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    ...
    Font.Assign((Source as TCalculatorLook).Font);
    Changed;
  end
  else
    Inherited;
