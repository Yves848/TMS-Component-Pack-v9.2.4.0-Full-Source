{**************************************************************************}
{ TAdvSmoothButtonDE DESIGN TIME EDITOR                                    }
...
uses
  Classes, AdvSmoothButton, DesignIntf, DesignEditors;

type
  TAdvSmoothButtonEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothButtonEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothButton).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothButton) then
         (Component as TAdvSmoothButton).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothButtonEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothButtonEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
