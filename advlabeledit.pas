...
const
  ...
  // v1.1.3.0 : Improved : Behavior when EditType = etPassword
...
function TAdvLabelEdit.Makepassword(s: string): string;
var
  i: integer;
begin
  Result := '';
  for i := 1 to Length(s) do
    Result := Result + chr($2022);
end;
...
begin
  if FEditType = etPassword then
    FLabel.Caption := Makepassword(Value)
  else
