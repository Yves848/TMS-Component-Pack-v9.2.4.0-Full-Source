...
const
  ...
  // v2.1.1.0 : New : Per monitor support for high DPI
  // v2.1.1.1 : Deprecated : TCalComp
...
begin;
  ...
  FDPIScale := GetDPIScale(Owner as TControl, CalForm.Canvas);

  CalForm.Font.Assign(Font);
  CalForm.Font.Height := Round(FDPISCale * Font.Height);
  CalForm.DPIScale := FDPIScale;
  ...
  begin
    pt.x := Round(FDPIScale * Faligncontrol.left);
    pt.y := Round(FDPIScale * FAlignControl.top);;
    clienttoscreen(FAlignControl.Parent.handle,pt);
    CalForm.Left:=pt.x;

    case FShowMethod of
      smBottom:CalForm.Top:= pt.y + Round(FDPIScale * FAlignControl.Height);
      smTop:CalForm.Top:= pt.y - CalForm.height;
      smDependent:
        begin
          if (pt.y + Round(FDPIScale * FAlignControl.height) + Calform.height > getsystemmetrics(SM_CYSCREEN)) then
            CalForm.top := pt.y - CalForm.height
          else
            CalForm.top := pt.y + Round(FDPIScale * FAlignControl.Height);
        end;
     end;
     {end of case}
  end
  ...
  begin
    CalForm.top := Round(FDPIScale * FTop);;
    CalForm.left := Round(FDPIScale * FLeft);
  end;

  CalForm.Width := Round(136 * FDPIScale);
  CalForm.Height := Round(139 * FDPIScale);
