...
const
  ...
  // v1.5.1.1 : Fixed: Issue with changing focus and double-clicking tile
  // v1.5.1.2 : Fixed: Issue with XYToTile in combination with move timer
  // v1.5.1.3 : Fixed : Issue with transparency on double buffered forms
  // v1.5.1.4 : Fixed : Issue displaying maximized tile while resizing
  // v1.5.1.5 : Improved : Skip animation when PageAnimationFactor = 0
  // v1.5.1.6 : Fixed : Issue with resizing and maximized tiles
...
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            dx := Abs(rTo.X - rFrom.X) / Max(1, AnimationFactor);
            dy := Abs(rTo.Y - rFrom.Y) / Max(1, AnimationFactor);
            dw := Abs(rTo.Width - rFrom.Width) / Max(1, AnimationFactor);
            dh := Abs(rTo.Height - rFrom.Height) / Max(1, AnimationFactor);
            dtox := rFrom.X;
            dtoy := rFrom.Y;
            dwto := rFrom.Width;
            dhto := rFrom.Height;
  ...
  begin
    ...
    d := Abs(FTilePos - FTilePosTo) / Max(1, anim);
    pos := FTilePos;
...
begin
  ...
  begin
    ...
    w := TileRectangle.Width - Round(DPIScale * TileAppearance.HorizontalSpacing);
    TileW := w;
    if Columns > 0 then
     TileW := (w - (Columns * Round(DPIScale * TileAppearance.HorizontalSpacing))) / Columns;

    h := TileRectangle.Height - Round(DPIScale * TileAppearance.VerticalSpacing);
    TileH := h;
    if Rows > 0 then
     TileH := (h - (Rows * Round(DPIScale * TileAppearance.VerticalSpacing))) / Rows;

    TileX := ((PageIndexForTileIndex(TileIndex) - PageIndex) * TileRectangle.Width) + TileRectangle.X + Round(DPIScale * TileAppearance.HorizontalSpacing);
    TileY := TileRectangle.Y + Round(DPIScale * TileAppearance.VerticalSpacing);
    for Row := 0 to Rows - 1 do
    begin
      for Column := 0 to Columns - 1 do
      begin
        ...
        TileX := TileX + Round(DPIScale * TileAppearance.HorizontalSpacing) + TileW;
        Inc(TileIndex);
      end;
      TileX := ((PageIndexForTileIndex(TileIndex) - PageIndex) * TileRectangle.Width) + TileRectangle.X + Round(DPIScale * TileAppearance.HorizontalSpacing);
      TileY := TileY + Round(DPIScale * TileAppearance.VerticalSpacing + TileH);
    end;
  end;
end;
...
procedure TAdvSmoothTileList.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  if not Header.Float then
    hh := Round(DPIScale * Header.GetHeight);

  if not Footer.Float then
    fh := Round(DPIScale * Footer.GetHeight);
...
begin
  ...
  begin
    DPIScale := GetDPIScale(Self, Canvas);
    Result := MakeRect(r.X, r.Y + r.Height - Round(DPISCale * Footer.GetHeight), r.Width, Round(DPISCale * Footer.GetHeight));
  end
  else
...
begin
  ...
  if Header.Visible then
    Result := MakeRect(r.X, r.Y, r.Width, Round(GetDPIScale(Self, Canvas) * Header.GetHeight))
  else
    Result := MakeRect(0, 0, 0, 0);
end;
...
begin
  r := GetBoundsRectangle;
  DPIScale := GetDPIScale(Self, Canvas);
  Result := MakeRect(r.X +  Round(DPIScale * TileMargins.Left), r.Y + Round(DPIScale * TileMargins.Top), r.Width - Round(DPIScale * (TileMargins.Right + TileMargins.Left)), r.Height - Round(DPIScale * (TileMargins.Bottom + TileMargins.Top)))
end;
...
procedure TAdvSmoothTileList.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
...
var
  ...
  proceed, proceedmax: Boolean;
begin
  if (FMode = tmView) or (FMode = tmDelete) then
  begin
    FMoveCount := 0;
    FMove.Enabled := False;
  end;
  ...
  begin
    ...
    if (FBulletIdx <> -1) then
    begin
      if PageAnimationFactor = 0 then
        PageIndex := FBulletIdx
      else
        GoToPage(FBulletIdx);
    end;
...
begin
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (TileIndex >= 0) and (TileIndex <= FWorkTiles.Count - 1) then
            begin
              dx := Abs(FWorkTiles[TileIndex].TileRectangle.X - FWorkTiles[TileIndex].OrigTileRectangle.X) / Max(1, AnimationFactor);
              dy := Abs(FWorkTiles[TileIndex].TileRectangle.Y - FWorkTiles[TileIndex].OrigTileRectangle.Y) / Max(1, AnimationFactor);
    ...
    begin
      dx := Abs(MoveTile.TileRectangle.X - MoveTile.OrigTileRectangle.X) / Max(1, AnimationFactor);
      dy := Abs(MoveTile.TileRectangle.Y - MoveTile.OrigTileRectangle.Y) / Max(1, AnimationFactor);
...
begin
  ...
  begin
    FPageAnimationFactor := Max(0, Value);
    Changed;
  end;
end;
  ...
  begin
    ...
    begin      
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            xs := r.X - GetWidth / 2;
            ys := r.Y - GetHeight / 2;
            xs := xs + Round(DPIScale * DeleteIndicatorLeft);
            ys := ys + Round(DPIScale * DeleteIndicatorTop);
            Draw(g, SaveRound(xs), SaveRound(ys), 0, 0, true, cap, DPIScale);
          end;
        end;
      end;
      ...
      begin
        ...
        begin
          CalculateSize(g, cap, DPIScale);
          xs := r.X + R.Width - GetWidth / 2;
          ys := r.Y - GetHeight / 2;
          xs := xs + Round(DPIScale * StatusIndicatorLeft);
          ys := ys + Round(DPIScale * StatusIndicatorTop);
          Draw(g, SaveRound(xs), SaveRound(ys), 0, 0, true, cap, DPIScale);
        end;
      end;
    end;  
...
begin
  ...
  begin
    ...
    begin
      ...
      st := Round(DPIScale * BulletSize);
      si := st - 3;
      if ArrowNavigation then
      begin
        sz := Round(DPIScale * ArrowRectangleSize);
        rs := r;
        rs.X := rs.X + sz;
    ...
    begin
      ...
      begin
        ...
        HTMLDrawGDIP(g, FFont, Caption,htmlr,FOwner.ImageList, 0, 0, -1, -1, 0, False, true, false, false,
          False, False, true, 1.0, clBlue, clNone, clNone, clGray, a, ms, k, XSize, YSize, l, m, hr, nil, FOwner.PictureContainer, 2, DPIScale);
        ...
        begin
          x := Round(DPIScale * CaptionLeft);
          y := Round(DPIScale * CaptionTop);
        end;
      ...
      begin
        ft := g.MakeFont(Font, Round(DPIScale * Font.Size));
        ...
        g.DrawString(Caption, Length(Caption), ft, MakePoint(x + r.X + Round(DPIScale * CaptionLeft), y + r.Y + Round(DPIScale * CaptionTop)), b);
        b.Free;
        sf.Free;
        ft.Free;
      end;
...
begin
  ...
  sz := Round(GetDPIScale(FOwner, FOwner.Canvas) * ArrowRectangleSize);
...
begin
  ...
  f := g.MakeFont(ft, Round(ft.Size * GetDPIScale(Tile.FOwner, Tile.FOwner.Canvas )));
  sf := TGPStringFormat.Create;
