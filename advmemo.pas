...
const
  ...
  // 3.5.4.1  : Fixed : Issue with SetCursor() and selection update
  // 3.5.4.2  : Fixed : Issue with OnChange event when TAB key is pressed
  //          : Fixed : Issue with ReadOnly = true and TAB block indent
  // 3.5.4.3  : Fixed : Issue with loading files that contains tabs
  // 3.5.4.4  : Fixed : Issue with ReplaceAll in Selection
  // 3.5.4.5  : Fixed : Issue with inserting large multiline text
  // 3.5.4.6  : Fixed : Issue with styler manager & file extensions in specific cases
  // 3.5.4.7  : Fixed : Issue with assigning sorted stringlists to the AdvMemo.Lines
  // 3.5.5.0  : New : Public property FileName added
  //          : Fixed : Initialization issue for variable used in number rendering
  // 3.6.0.0  : New : Memo toolbar & ribbon controls
  //          : New : Memo file actions
  //          : Improved : Handling of readonly in relationship to popup menu
  // 3.6.0.1  : Fixed : PopupMenu SelectAll not disabled when ReadOnly = true
  // 3.6.0.2  : Fixed : Issue with removing indexed bookmarks
  // 3.6.0.3  : Fixed : Issue with TAdvFindDialog.OnFindPrevious event
  // 3.6.1.0  : Fixed : OnChange() event not triggered when calling BlockIndent()
  ...
  //          : New : Tag property added to TLineProp
  // 3.6.1.1  : Improved : Text & scrollbar updating when text is programmatically added with wordwrap
  // 3.6.1.2  : Fixed : Issue with whole-word searches
  // 3.6.2.0  : Improved : Support for nested BeginUpdate/EndUpdate
  // 3.6.2.1  : Fixed : Issue with OnCorrectWord when '.' is typed.
  // 3.6.2.2  : Fixed : Issue with font size handling on high DPI
  //          : Fixed : Issue with URL as last element of text
  // 3.6.2.3  : Improved : Gutter size handling for high DPI
  // 3.6.2.4  : Improved : Further improvements for gutter size on high DPI
  // 3.6.2.5  : Improved : Font handling under high DPI
  //          : Fixed : Issue with autocompletionlist display when KeepLastSize = true
  // 3.6.2.6  : Fixed : Issue with auto completion in specific circumstances
  // 3.6.2.7  : Fixed : Issue with hyperlinks & wordwrap
  // 3.6.2.8  : Fixed : Issue with number highlighting
  // 3.6.2.9  : Fixed : Issue with PageDown in specific circumstances with code-folding
  // 3.6.2.10 : Improved : Line number drawing in gutter for high DPI
  //          : New : function IsLastChild(lineindex) added to check for last line of code folding region
  // 3.6.2.11 : Improved : High DPI handling for runtime created control
  // 3.6.2.12 : Fixed : Issue with number syntax coloring
  // 3.6.2.13 : Fixed : Issue with automatic code folding and programmatically set code folding regions
  // 3.6.2.14 : Fixed : Issue with hex number drawing
  // 3.6.2.15 : Fixed : Issue with triggering OnWordComplete()
  // 3.6.2.16 : Improved : Handling of special keys with form.KeyPreview = true
  // 3.6.2.17 : Fixed : Issue with inserting & deleting lines and markers
  //          : Improved : Auto completion list disappears automatically when no more matching items are found
  // 3.6.2.19 : Fixed : Issue with default font with fsBold style
  // 3.6.2.20 : Fixed : Issue with TDBAdvMemo programmatic text updating
  // 3.6.2.21 : Fixed : Issue with OnClipboardAction event for cut to clipboard
  // 3.6.2.22 : Fixed : Issue with TDBAdvMemo in master/detail scenario
  // 3.6.2.23 : Improved : Printing with text highlighting enabled
  // 3.6.2.24 : Fixed : Rare issue with Escape char
  //          : Fixed : AutoIndent issue
  // 3.6.2.25 : Improved : Handling of URLs in bracket segments
  // 3.6.2.26 : Improved : Make auto parameter hint not active in comment blocks
  // 3.6.3.0  : New : Styler.KeywordStyle property added
  // 3.6.4.0  : New : Extended Styler.KeywordStyle with ksUppercase, ksLowercase
  //          : Fixed : Issue with multi comment LUA styler
  //          : Fixed : Issue with Modified state and Ctrl-A, Ctrl-C
  // 3.6.4.1  : Fixed : High DPI issue Font size when changing screens.
  // 3.6.5.0  : Improved : High DPI per monitor behavior
  // 3.6.5.1  : Fixed : Issue with runtime VCL style change
  // 3.6.6.0  : New : Property CloseOnFind property added to TAdvMemoFindDialog
  //          : New : Event OnCloseOnFind event added to TAdvMemoFindDialog
...
//------------------------------------------------------------------------------
  ...
  TMemoFileNameChangeEvent = procedure(Sender: TObject; AFileName: string) of object;
...
//------------------------------------------------------------------------------
//        TAdvCustomMemoStylerManager
//------------------------------------------------------------------------------

  {$IFDEF DELPHIXE2_LVL}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvCustomMemoStylerManager = class(TComponent)
...
//------------------------------------------------------------------------------
  ...
  private
    ...
    FOnFileNameChange: TMemoFileNameChangeEvent;
    FAutoExpand: Boolean;
    ...
    FDPIScale: single;
    FFileName: string;
    FAlwaysFocus: boolean;
    FWWURLEnd: boolean;
    FUpdateCount: integer;
    FStylerManager: TAdvCustomMemoStylerManager;
    ...
    function GetLineStyle(Index: Integer; var LineStyle: TStyle): Boolean; overload;
  ...
  protected
    ...
    procedure GetLines;
    function AddCodeFoldingInt(StartLineIndex, EndLineIndex: Integer; ByCode: boolean): Boolean;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    ...
    property AlwaysFocus: boolean read FAlwaysFocus write FAlwaysFocus;
    ...
    function WordBeforeCursor: string;
    ...
    function GetLineStyle(ARow: integer): TStyle; overload;
    ...
    function IsLastChild(LineIndex: Integer): Boolean;
    ...
    property ContextMenuImages: TImageList read FContextMenuImages write FCOntextMenuImages;
    property MemoLines: TMemoLines read FMemoLines;
    property StylerManager: TAdvCustomMemoStylerManager read FStylerManager write FStylerManager;
    property OnFileNameChange: TMemoFileNameChangeEvent read FOnFileNameChange write FOnFileNameChange;
  published
...
//------------------------------------------------------------------------------
  ...
  TKeywordStyle = (ksUserCase, ksListCase, ksUppercase, ksLowercase);
  ...
  public
    ...
    property KeywordStyle: TKeywordStyle read FKeywordStyle write FKeywordStyle default ksUserCase;
  ...
  TAdvCustomMemoStylerItem = class(TCollectionItem)

  end;

  TAdvCustomMemoStylerList = class(TCollection)
  ...
  private
    ...
    FOnCloseOnFind: TNotifyEvent;
    FCloseOnFind: boolean;
  ...
  published
    ...
    property CloseOnFind: boolean read FCloseOnFind write FCloseOnFind default false;
    ...
    property OnCloseOnFind: TNotifyEvent read FOnCloseOnFind write FOnCloseOnFind;
  end;
...
type
  ...
  TAdvFindDialogOpen = class(TAdvFindDialog);
...
begin
  ...
  Buff := StringReplace(buff, #13, #13#10, [rfReplaceAll]);
...
begin
  ...
  begin
    ...
    tl := tr + dP * LinesPerPage;
    if tl < VisibleLineCount - LinesPerPage then
      TopLine := tl;
  end;
...
begin
  ...
  for i := 0 to FMarkerList.FMarkers.Count - 1 do
  begin
    if FMarkerList.FMarkers[i].LineNumber >= CurY then
      FMarkerList.FMarkers[i].LineNumber := FMarkerList.FMarkers[i].LineNumber - 1;
  end;

  ClearSelection;
  Invalidate;
  if not InternalLines.FDeleting then
  ...
  case Key of
    ...
    VK_TAB:
      begin
        ...
        if (FSelStartY <> sy) and not ReadOnly then
        begin
...
begin
  ...
  if Ord(Key) in [13, 32, 9,ord('.'),ord('('),ord(','),ord('['),ord('='),ord(';'),ord(':'),ord('"'),ord('?'),ord('!')] then
  begin
    if Ord(Key) = 13 then
    begin
    ...
    if (Ord(Key) in [9,32,ord('.'),ord('('),ord(','),ord('['),ord('='),ord(';'),ord(':'),ord('"'),ord('?'),ord('!')]) then
    begin
      wrd := WordTillCursor;
      newword := wrd;
...
begin
  ...
  begin
    ...
    if (WordWrap <> wwNone) and (newpos.Y > 0) then
    begin
       urls := TStringList.Create;

       ExtractURL(InternalLines[newPos.y - 1], urls);

       if urls.Count > 0 then
       begin
         if WordIsURL(urls.Strings[urls.Count - 1]) then
         begin
           ustr := InternalLines[newPos.y];
           ExtractURL(ustr, urls);
           if (urls.Count > 0) and (Pos(' ', urls.Strings[0]) = 0) then
             prevurl := newPos.X <= Length(urls.Strings[0]);
         end;
       end;
       urls.Free;
    end;

    if TestforURLMove(InternalLines[newPos.y], newPos.x) or prevurl then
    begin
...
procedure TAdvCustomMemo.SetFileName(const Value: string);
begin
  FFileName := Value;
  if Assigned(OnFileNameChange) then
    OnFileNameChange(Self, FFileName);
end;
          ...
          begin
            ...
            begin
              if (InternalStyles.FAllStyles.Items[allb].FStyleType = stBracket) and
                (InternalStyles.FAllStyles.Items[allb].BracketStart <> #0) then
              begin
                if not bracket then
                begin
...
var
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            LineCanvas.Pen.Style := psSolid;

            if DM in [dmPrinter, dmPrintPreview] then
            begin
              LineCanvas.Rectangle(rct1.Left + 0, rct1.Top, rct1.Left + FCellSize.W * ls1, rct1.top + FCellSize.H );
              LineCanvas.Brush.Color := Self.BkColor;
              LineCanvas.Pen.Color := Self.BkColor;
              LineCanvas.Rectangle(rct1.Left + 0, rct1.Top + FCellSize.H, rct1.Left + FCellSize.W * ls1, rct1.top + FCellSize.H + 2);
            end
            else
  ...
  begin
    ...
    if (SegmentStyle.isURL) and FURLAware {and not SegmentStyle.isBracket} then
    begin
      BufferingDraw(s, SegmentStyle);
      Exit;
    end;
    ...
    begin
      ...
      if validno then
        ...
        begin
          ...
          if ((toEnd <= Len) and (AnsiPos(UpCase(S[toEnd]), numsallowed) = 0)) and (pos(s[toEnd], FTempdelimiters) = 0) then
            numberbreak := true;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  case InternalStyles.KeywordStyle of
                  ksListCase:  BufferingDraw(InternalStyles.FAllStyles.Items[i].FKeyWords[keyidx], SegmentStyle);
                  ksUpperCase: BufferingDraw(UpperCase(part), SegmentStyle);
                  ksLowerCase: BufferingDraw(LowerCase(part), SegmentStyle);
                  ksUserCase:  BufferingDraw(part, SegmentStyle);
                  end;
                end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (WordWrap <> wwNone) and (i = 0) and FWWURLEnd then
          begin
            if pos(' ',urls.Strings[i]) = 0 then
              style.isURL := true;
          end;

          DrawSegments(urls.Strings[i], rct1, style);

          if goturl and (i = urls.Count - 1) and (WordWrap <> wwNone) then
          begin
            FWWURLEnd := true;
          end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      if (WordWrap <> wwNone) and not GetWrapped(i) then
        FWWURLEnd := false;
...
{$IFDEF FREEWARE}
    ...
    Canvas.TextOut(eRect.Left, eRect.Bottom - 14, 'Copyright © 2019 tmssoftware.com');
...
{$ENDIF}
      ...
      begin
        ...
        sbVert.PageSize := VisibleLineCount;
        sbVert.Width := Round(16* FDPIScale);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sbHorz.Height := Round(16 * FDPIScale);
        Position := FLeftCol;
...
begin
  ...
  if (Operation = opRemove) and (AComponent = FStylerManager) then
    FStylerManager := nil;
...
procedure TAdvCustomMemo.DoChange;
begin
...
function TAdvCustomMemo.GetFocused: boolean;
begin
...
begin
  Result := AddCodeFoldingInt(StartLineIndex, EndLineIndex, true);
end;

function TAdvCustomMemo.AddCodeFoldingInt(StartLineIndex, EndLineIndex: Integer; ByCode: boolean): Boolean;
var
...
begin
  ...
  begin
    ...
    if Assigned(Tlp) and (Tlp is TLineProp) and not Tlp.ByCode then
    begin
      Tlp.Expanded := True;
      //Tlp.LastChild := False;
...
function TAdvCustomMemo.IsLastChild(LineIndex: Integer): Boolean;
var
  Tlp: TlineProp;
begin
  Result := False;
...
function TAdvCustomMemo.WordBeforeCursor: string;
var
  s: string;
  i: integer;
  fb, fe: integer;
...
begin
  ...
  {$ENDIF}
  {$IFDEF DELPHI_UNICODE}
  Result := (ch = #32) or (ch = #39) or (ch = '"') or (ch = ',') or (ch = ';') or (ch = '.') or (ch = '?') or (ch = '!');
  {$ENDIF}
end;
...
function TAdvCustomMemo.GetLineStyle(ARow: integer): TStyle;
begin
  Result := GetUpStyle(ARow);
end;
...
begin
  ...
  begin
    ...
    mnu.Tag := $70000002;
    mnu.ImageIndex := 0;
    mnu.Enabled := (SelLength > 0) and not ReadOnly;
    FStdMenu.Items.Add(mnu);
...
begin
  ...
  FFont := TFont.Create;
  FFont.Name := 'Courier New';
  FFont.Size := Round(10 / CalcDPIScale(0, true));
...
begin
  ...
  if not (csLoading in memo.ComponentState) and not (csReading in memo.ComponentState) and not (csDesigning in memo.ComponentState) then
  begin
//    FWidth :=  Round(Value * memo.FDPIScale);
//    FGutterWidth := Round(Value * memo.FDPIScale);
  end;
...
begin
  if Value < FGutterWidth div 2 then
    Value := FGutterWidth div 2;

  if not (csLoading in memo.ComponentState) and not (csReading in memo.ComponentState) and not (csDesigning in memo.ComponentState) then
  begin
    //Value := Round(Value * memo.FDPIScale);
  end;
...
begin
  ...
  for I := 0 to Memo.FMarkerList.FMarkers.Count - 1 do
  begin
    if Memo.FMarkerList.FMarkers.Items[i].LineNumber >= idx then
    begin
      Memo.FMarkerList.FMarkers.Items[i].LineNumber := Memo.FMarkerList.FMarkers.Items[i].LineNumber + ofs;
    end;
  end;
...
//  Canvas.Font.Size := Round(Canvas.Font.Size * FDPIScale);
  ...
  // avoid that gutter text is partially drawn
  if Position + cl * cw > FGutter.GutterWidth then
    Position := FGutter.GutterWidth - cl * cw - 2;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Brush.Color := ActiveLineSettings.ActiveLineColor;
        Pen.Color := clGray;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        isSorted := FInternalStyles.FAllStyles[i - 1].KeyWords.Sorted;
        FInternalStyles.FAllStyles[i - 1].KeyWords.Sorted := False;
        FInternalStyles.FAllStyles[i - 1].KeyWords.Sorted := isSorted;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        // url is on prev line
        if (i = 0) and (WordWrap <> wwNone) and (cury > 0) and (Pos(' ', urls[i]) = 0) then
        begin
          str := InternalLines[cury - 1];

          ExtractURL(str, temp);

          if (temp.Count > 0) then
          begin
            // last word of prev line is URL
            if WordIsURL(temp[temp.Count - 1]) then
            begin
              url := temp[temp.Count - 1] + urls[i];
        ...
        begin
          ...
          // get part of URL on next line
          if (CurY < InternalLines.Count) and (WordWrap <> wwNone) then
          begin
            if cury + 1 < InternalLines.Count then
            begin
               str := InternalLines[cury + 1];
               ExtractURL(str, temp);
               if (temp.Count > 0) and (pos(' ', temp.Strings[0]) = 0) then
                 url := url + temp.Strings[0];
            end;
          end;
...
var
  ...
  begin
    worddelim := ['a'..'z','0'..'9','_','á','ä','â','à','é','è','ê','ë','ó','ò','ô','ö','ú','ù','û','ü','í','ì','ï','î'];
    ...
    begin
      ...
      begin
        ...
        {$IFDEF DELPHI_UNICODE}
        if not CharInSet(sc[position - 1], worddelim) then
          Exit;
        {$ENDIF}

        {$IFNDEF DELPHI_UNICODE}
        if not (sc[position - 1] in worddelim) then
          Exit;
        {$ENDIF}
      end;

      {$IFDEF DELPHI_UNICODE}
      if (position = 1) and not CharInSet(sc[length(SearchStr) + 1],worddelim) then
        Exit;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if (position = 1) and not (sc[length(SearchStr) + 1] in worddelim) then
        Exit;
      {$ENDIF}

      {$IFDEF DELPHI_UNICODE}
      if not (CharInSet(sc[position - 1], worddelim)) or CharInSet(sc[length(SearchStr) + position],worddelim) then
        Exit;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if not (sc[position - 1] in worddelim) or (sc[length(SearchStr) + position] in worddelim) then
        Exit;
      {$ENDIF}
      Result := 0;
    end;
...
var
  ...
  begin
    ...
    begin
      ...
      {$IFDEF DELPHI_UNICODE}
      if not (CharInSet(sc[position - 1], worddelim)) and not CharInSet(sc[length(SearchStr) + position],worddelim) then
        Exit;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if not (sc[position - 1] in worddelim) and not (sc[length(SearchStr) + position] in worddelim) then
        Exit;
      {$ENDIF}
      Result := 0;
    end;
...
begin
  ...
  if freSelection in Options then
    OrderPos(FSelStartX, FSelStartY, FSelEndX, FSelEndY);
  ...
  repeat
    ...
    if (rz > -1) then
    begin
      if FSelEndY = sey then
        dx := dx + Length(NewStr) - Length(Selection)
      else
        dx := 0;
  ...
  if (rz = -1) and ((freReplaceAll in Options) or (freWrapAtEnd in Options)) and (freDown in Options) and (sey > 0) then
  begin
    CurX := 0;
    CurY := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      TAdvFindDialogOpen(FFindDialogEx).OnInternalFindPrevious := FindPrevious;
...
begin
  ...
  if Assigned(FFindDialog) and CloseOnFind then
  begin
    FFindDialog.CloseDialog;
    if Assigned(OnCloseOnFind) then
      OnCloseOnFind(Self);
  end;
end;
...
//    begin
//      Strings[i] := StringReplace(Strings[i], #9, stringofchar(#32, memo.tabsize), [rfreplaceall]);
//    end;
//
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      FListCompletion.BorderStyle := bsSingle;
    end
    else
    begin
      FormAutoCompletion.BorderStyle := bsSizeToolWin;
  ...
  FormAutoCompletion.Top := P.Y;
  FormAutocompletion.Width := dw;
  FormAutocompletion.Height := Round(FAutoCompletion.Height * FDPIScale);
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomMemo.ChangeScale(M, D: Integer; isDpiChange: boolean);
{$ELSE}
procedure TAdvCustomMemo.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;

  Gutter.GutterWidth := MulDiv(Gutter.GutterWidth, M, D);
  Gutter.GutterMargin := MulDiv(Gutter.GutterMargin, M, D);
  Font.Size := CalcFontSize(Canvas, Font, M/D);
  Gutter.Font.Size := CalcFontSize(Canvas, Gutter.Font, M/D);
...
begin
  ...
  if GetUpStyle(CurY).isComment > 0 then
    Exit;
...
begin
  ...
  // remove the indexed bookmarks
  for i := 0 to Lines.Count - 1 do
  begin
    if GetBookmarkIndex(i) <> -1 then
      BookmarkIndex[i] := -1;
  end;
end;
...
procedure TAutoCompletionListBox.SetIdentifierWidth;
var
  i, idw, iw, vp: integer;
  s: string;
begin
...
begin
  ...
  Canvas.Font.Height := Round(Canvas.Font.Height * FDPIScale);
  ...
  if iw > Round(16 * FDPIScale) then
    ItemHeight := iw
  else
    ItemHeight := Round(16 * FDPIScale);
...
begin
  Result := ((Control <> nil) and (Target = Control) or
    (Control = nil) and (Target is TAdvCustomMemo)) and TAdvCustomMemo(Target).GetFocused;
end;
