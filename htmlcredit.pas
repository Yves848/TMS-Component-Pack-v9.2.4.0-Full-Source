...
uses
  ...
  PictureContainer, ImgList, Types, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.2.1.0 : New : Support for LINE-HEIGHT attribute for <P> tag
  // 1.2.1.1 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // 1.3.0.0 : New : Support for per monitor DPI
...
begin
  ...
  HTMLDrawEx(Canvas,s,r,FImages,pt.x,pt.y,fhl,FHoverHyperlink,FShadowOffset,True,True,False,False,FBlinking,FHover,not FEllipsis,FFormScaled,1.0,
    FURLColor,FHoverColor,FHoverFontColor,FShadowColor,Anchor,Stripped,FocusAnchor,x,y,hl,ml,HR,FImageCache,FContainer,FLineSpacing,BidiMode, -16777203, -16777202, [], FDPIScale);
  ...
  begin
    ...
    HTMLDrawEx(Canvas,s,r,nil,0,0,fhl,FHoverHyperlink,FShadowOffset,False,False,false,False,FBlinking,FHover,not FEllipsis,FFormScaled,1.0,
      clWhite,clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,x,y,hl,ml,HR,FImageCache,FContainer,FLineSpacing,BidiMode, -16777203, -16777202, [], FDPIScale);
    ...
    HTMLDrawEx(Canvas,s,r,FImages,0,0,fhl,FHoverHyperlink,FShadowOffset,False,False,false,False,FBlinking,FHover,not FEllipsis,FFormScaled,1.0,
      clGray,clNone,clNone,FShadowColor,Anchor,Stripped,FocusAnchor,x,y,hl,ml,HR,FImageCache,FContainer,FLineSpacing,BidiMode, -16777203, -16777202, [], FDPIScale);
...
procedure THTMLCredit.InitVCLStyle(init: boolean);
begin
  FUseVCLStyles := False;
...
begin
  ...
  if HTMLDrawEx(Canvas,s,r,FImages,x,y,-1,-1,FShadowOffset,True,False,False,False,false,FHover,not FEllipsis,FFormScaled,1.0,
    clWhite,clNone,clNone,clNone,Anchor,Stripped,FocusAnchor,xsize,ysize,hl,FHoverHyperlink,HoverRect,FImageCache,FContainer,FLineSpacing,BidiMode) then
  begin
    Result := Anchor;
  end
  else
...
{$IFDEF DELPHIXE2_LVL}
procedure THTMLCredit.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
  Invalidate;
end;
