{**************************************************************************}
{ TAdvSmoothSliderDE DESIGN TIME EDITOR                                    }
...
uses
  Classes, AdvSmoothSlider, DesignIntf, DesignEditors;

type
  TAdvSmoothSliderEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothSliderEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothSlider).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothSlider) then
         (Component as TAdvSmoothSlider).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothSliderEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothSliderEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
