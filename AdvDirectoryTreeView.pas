...
const
  ...
  // v1.0.1.1 : Fixed: Issue with directories also showing file size
  // v1.0.1.2 : Fixed: Issues with symbolic link directories also showing file size on Mac OSX
...
uses
  ...
  {$ENDIF}
  {$IFDEF LINUX}
  IOUtils, FMUX.API, FMX.Canvas.Linux, Posix.SysStat,
  {$ENDIF}
  {$IFDEF MACOS}
  {$IFNDEF IOS}
...
begin
  ...
  begin
    ...
    else if Assigned(ft) and not (UTF8ToString(ft.UTF8String) = 'NSFileTypeDirectory') and FileExists(AFile) then
    begin
  ...
  {$IFDEF LINUX}
  fst := 0;
  dir := TDirectory.Exists(AFile);
  if dir then
  begin
    cd := DateTimeToStr(TDirectory.GetCreationTime(AFile));
    md := DateTimeToStr(TDirectory.GetLastWriteTime(AFile));
  end
  else
  begin
    cd := DateTimeToStr(TFile.GetCreationTime(AFile));
    md := DateTimeToStr(TFile.GetLastWriteTime(AFile));
    if stat(M.AsAnsi(AFile).ToPointer, LRec) = 0 then
      fst := LRec.st_size;
  end;

  dn := FmuxGetFileType(PChar(AFile));
  n := ExtractFileName(AFile);

  ic := FmuxGetFileIcon(PChar(AFile), 16);
  if ic <> nil then
  try
    bmp := CreateBitmapFromHandle(ic);
  finally
    FmuxBitmapDestroy(ic);
  end;
  {$ENDIF}
