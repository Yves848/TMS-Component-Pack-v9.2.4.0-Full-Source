...
type
  TContainerEditor = class(TForm)
    ...
    procedure edLookupChange(Sender: TObject);
  private
...
procedure TContainerEditor.Button1Click(Sender: TObject);
begin
  if HTMListBox.ItemIndex >= 0 then
  begin
    if SaveDialog1.Execute then
    begin
      PictureContainer.Items.Items[HTMListBox.ItemIndex].Picture.SaveToFile(SaveDialog1.FileName);
    end;
  end;
end;
...
procedure TContainerEditor.edLookupChange(Sender: TObject);
var
  i: integer;
begin
  //
  ...
  begin
    if pos(edLookup.Text, PictureContainer.Items[i].Name) > 0 then
    begin
      HTMListBox.ItemIndex := i;
      Break;
    end;
  end;
end;
