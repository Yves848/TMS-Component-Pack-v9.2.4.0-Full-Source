...
const
  ...
  // 1.5.0.1 : Improved : Handling for high DPI with Form.Scaled = false
  // 1.5.1.0 : Improved : Compatibility with VCL styles
  // 1.5.2.0 : New : Per monitor support for high DPI
  // 1.5.2.1 : Fixed : Issue with runtime style change
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    LStyle.GetElementColor(LStyle.GetElementDetails(tpPanelBackground), ecFillColor, clr);
    Color := clr;
    LStyle.GetElementColor(LStyle.GetElementDetails(tpPanelBackground), ecTextColor, clr);
    FTxtColor := clr;
  end;
{$ENDIF}
end;
...
begin
  ...
  case Panel.Style of
    psHTML:
      begin
        dr := drect;
        dr.Left := dr.Left + Round(FDPIScale * Panel.HTMLOffsetX);
        dr.Top := dr.Top + Round(FDPIScale * Panel.HTMLOffsetY);
        HTMLDrawEx(Canvas, panel.Text, dr, fImages, 0, 0, -1, -1, 1, false, false, false, false, (fTimerCount > 5), false,
        true, FFormScaled, 1.0, URLColor, clNone, clNone, clGray, anchor, stripped, focusanchor, xsize, ysize,
        hyperlinks, mouselink, re, nil , FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale);
      end;
    psEllipsText:
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Inc(PanelPos, Round(FDPIScale * Panels[I].Width));
...
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure THTMLStatusBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  Anchor := '';
  if HTMLDrawEx(Canvas, Panels.Items[idx].Text, r, FImages, x, y, -1, -1, 1, true, false, false, true, true, false, true, FFormScaled,
     1.0, FURLColor, clNone, clNone, clGray, anchor, stripped, focusanchor, xsize, ysize, hyperlinks,
     mouselink, re, nil, FContainer, 0, BiDiMode, clHighlight, clHighlightText, [], FDPIScale) then
