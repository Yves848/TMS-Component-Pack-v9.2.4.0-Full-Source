...
const
  ...
  // v1.1.2.0 : Improved : Support for High DPI (button size, text size, expand and collapse size)
  // v1.2.0.0 : New : VCL Styles support
...
procedure TAdvSmoothExpanderPanel.AfterExpand;
begin
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    hto := Height;
    d := Abs(DPIScale * FExpHeightTo - hto) / AnimationFactor ;
    a := AnimateDouble(hto, Round(DPISCale * FExpHeightTo), d, 0.1);
    if a then
    begin
      FEnableResize := false;
      newheight := Round(hto);
    ...
    begin
      case ExpanderShape of
        ...
        begin
          ...
          fl.Rounding := Round(ExpanderSize * DPIScale / 2);
        end;
        esRoundRect:
        begin
          fl.RoundingType := rtBoth;
          fl.Rounding := Round(ExpanderSize * DPIScale / 4);
        end;
      end;
    end
    else
...
begin
  ...
  if ExpanderLocation <> plCustom then
    GetPosition(x, y, MakeRect(r.Left, r.Top, r.Right, r.Bottom), Round(ExpanderSize * DPIScale), Round(ExpanderSize * DPIScale), ExpanderLocation)
  else
  begin
    x := Round(ExpanderLeft * DPIScale);
    y := Round(ExpanderTop * DPIScale);
  end;

  Result := MakeRect(X, y, Round(ExpanderSize * DPIScale), Round(ExpanderSize * DPIScale));
end;
...
procedure TAdvSmoothExpanderPanel.CMStyleChanged(var Message: TMessage);
begin
  inherited;
