{**************************************************************************}
{ TAdvSmoothGauge DESIGN TIME EDITOR                                       }
...
uses
  Classes, AdvSmoothGauge, DesignIntf, DesignEditors;

type
  TAdvSmoothGaugeEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothGaugeEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothGauge).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothGauge) then
         (Component as TAdvSmoothGauge).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothGaugeEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothGaugeEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
