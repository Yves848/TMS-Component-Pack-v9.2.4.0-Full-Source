{**************************************************************************}
{ TAdvSmoothMenuDE DESIGN TIME EDITOR                                      }
...
uses
  Classes, AdvSmoothMenu, DesignIntf, DesignEditors;

type
  TAdvSmoothMenuEditor = class(TDefaultEditor)
  public
...
procedure TAdvSmoothMenuEditor.ExecuteVerb(Index: integer);
var
...
begin
  ...
  if (Index = 0) then
  begin
    style := (Component as TAdvSmoothMenu).GetComponentStyle;
    ...
    begin
      ...
      if (Component is TAdvSmoothMenu) then
         (Component as TAdvSmoothMenu).SetComponentStyle(style);
         Designer.Modified;
    end;
    psf.Free;
  end;
...
function TAdvSmoothMenuEditor.GetVerb(index: integer): string;
begin
  if index = 0 then
    Result := 'Styles';
end;

function TAdvSmoothMenuEditor.GetVerbCount: integer;
begin
  Result := 1;
end;
