...
const
  ...
  // v1.8.4.3 : Fixed : Issue with transparency on double buffered forms
  // v1.8.5.0 : New : Exposed AllowAutoZoom property, AutoZoom and Zoom functionality
  // v1.8.5.1 : Fixed : Issue with CTRL + X, C and V combination
  // v1.8.5.2 : Fixed : Issue with internal edit control consuming tab
...
{TAdvSmoothTouchKeyCollection}
  ...
  private
    ...
    function GetAllowAutoZoom: Boolean;
    procedure SetAllowAutoZoom(const Value: Boolean);
  protected
    procedure SetComponentStyle(AStyle: TTMSStyle);
    procedure SetColorTones(ATones: TColorTones);
  ...
  published
    property AllowAutoZoom: Boolean read GetAllowAutoZoom write SetAllowAutoZoom default True;
...
procedure TAdvSmoothPopupTouchKeyBoard.AutoZoom;
begin
  FKbd.AutoZoom;
end;
...
procedure TAdvSmoothPopupTouchKeyBoard.SetAllowAutoZoom(const Value: Boolean);
begin
  FKbd.AllowAutoZoom := Value;
end;
...
function TAdvSmoothPopupTouchKeyBoard.GetAllowAutoZoom: Boolean;
begin
  Result := FKbd.AllowAutoZoom;
end;
...
procedure TAdvSmoothPopupTouchKeyBoard.Zoom(fhorz, fvert: double; keysonly,
  absoluteZoom: boolean);
begin
  FKbd.Zoom(fhorz, fvert, keysonly, absoluteZoom);
end;
