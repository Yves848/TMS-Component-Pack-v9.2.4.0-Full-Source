...
const
  ...
  // v1.4.0.3 : Fixed : Issue with transparency on double buffered forms
  // v1.4.1.0 : New : Public property AlwaysToggle: boolean added
  // v1.4.2.0 : Improved : Per monitor high DPI support
  // v1.4.2.1 : Improved : Added High DPI support for button width
...
type
  ...
  public
    ...
    property AlwaysToggle: boolean read FAlwaysToggle write FAlwaysToggle;
  published
...
begin
  ...
  begin
    ...
    begin
      ...
      f := TGPFont.Create(ff, Round(GetDPIScale(Self, Canvas) * FFont.Size), fs, UnitPoint);
...
begin
  ...
  begin
    ...
    r := MakeRect(GetButtonRect.X + DPIScale * ButtonAppearance.Size / 2, InsideRect.Top, InsideRect.Right - GetButtonRect.X - DPIScale * ButtonAppearance.Size / 2, InsideRect.Bottom - InsideRect.Top);
    if Enabled then
      FFill.Fill(g, r)
    else
...
begin
  ...
  begin
    ...
    if (FFill.BorderColor <> clNone) and (FFill.BorderWidth <> 0) then
      result := MakeRect(InsideRect.Right - Round(DPISCale * Size), InsideRect.Top, Round(DPISCale * Size), InsideRect.Bottom - InsideRect.Top)
    else
      result := MakeRect(InsideRect.Right - Round(DPISCale * Size), InsideRect.Top, Round(DPISCale * Size), InsideRect.Bottom - InsideRect.Top);
