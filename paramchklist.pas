...
const
  ...
  // 1.3.4.0 : New : OnMouseWheel* events exposed
...
procedure TParamCheckList.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
const
...
{$IFDEF DELPHIXE2_LVL}
procedure TParamCheckList.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(true);
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      paramcolor         := StyleServices.GetSystemColor(clHighlightText);
      {$ENDIF xe2}
    end
    else
    begin
      ...
      paramcolor         := FSelectionFontColor;
    end;
  end
  else
  begin
  ...
  { paints or clears the selection rectangle }
  ...
  HTMLDrawEx(Canvas,Items[index],hrect,FImages,pt.x,pt.y,fl,FHyperLink,FShadowOffset,
    False,False,False,(odSelected in State) and FShowSelection,False,fHover,WordWrap,FMouseDown,False,1.0, ParamColor,
...
procedure TParamCheckList.Loaded;
begin
  inherited;
  if not (csDesigning in ComponentState) then
    InitVCLStyle(true);
