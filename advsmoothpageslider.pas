...
const
  ...
  // v1.2.0.2 : Improved : Handling of situation with zero pages
  // v1.2.1.0 : New : Per monitor support for high DPI
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothPage.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothPage.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        clNone, clNone, clNone, a, s, f, xs, ys, hl, ml, hr, nil, FPictureContainer, 0, FDPIScale);
      ...
      begin
        ft := g.MakeFont(AFont, Round(FDPIScale * AFont.Size));
...
begin
  ...
  begin
    if Assigned(FPageSlider) then
    begin
      if Left < Max(150, Round(FDPIScale * MinimumWidth) / 2) then
        FPageSlider.NextPage
      else if Left > FPageSlider.Width - Max(150, Round(FDPIScale * MinimumWidth)) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          page.CurrentPos := nextpage.Left - Round(FDPIScale * page.MinimumWidth);
...
begin
  ...
  if ShowFooter then
    Result := MakeRect(0, Height - 1 - Round(FDPIScale *  FooterHeight), Width - 1, Round(FDPIScale *  FooterHeight));
end;
...
begin
  Result := MakeRect(0, 0, 0, 0);
  if ShowHeader then
    Result := MakeRect(0, 0, Width - 1, Round(FDPIScale *  HeaderHeight));
end;
...
begin
  if FPageSlider.ActivePageIndex = PageIndex then
  begin
    if FLeftTemp < Round(FDPIScale * MinimumWidth) * 2  then
      Result := Round(FDPIScale * MinimumWidth)
    else
      Result := Round(FDPIScale * MinimumWidth) * 2;
  end
  else if FPageSlider.ActivePageIndex > PageIndex then
  begin
    if FLeftTemp < Round(FDPIScale * MinimumWidth) then
      Result := 0
    else
      Result := Round(FDPIScale * MinimumWidth)
  end
...
begin
  ARect.Bottom := ARect.Bottom - Round(GetFooterRect.Height) - HeaderFill.BorderWidth;
  ARect.Top := ARect.Top + Round(GetHeaderRect.Height) + HeaderFill.BorderWidth;
  ARect.Right := Round(FDPIScale * MinimumWidth);
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothPageSlider.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothPageSlider.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      begin
        if csDesigning in ComponentState then
        begin
          APage.SetBounds(r.Left + Round(APage.FDPIScale * APage.MinimumWidth), R.Top, R.Right - r.Left - Round(APage.FDPIScale * APage.MinimumWidth),
            R.Bottom - R.Top);
        end
        else
        begin
          ...
          APage.SetBounds(r.Left + APage.GetCurrentPos, R.Top, R.Right - r.Left - Round(APage.FDPIScale * APage.MinimumWidth),
            R.Bottom - R.Top);
        end;
      end;
    end;
