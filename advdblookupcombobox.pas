...
const
  ...
  // 1.9.4.1 : Fixed : Issue with VCL styles and control label
  // 1.9.4.2 : Fixed : Issue with item select after item clear
  // 1.9.4.3 : Improved : VCL styles support for dropdown button in control
  // 1.9.4.4 : Improved : Triggering of OnLookupError
  // 1.9.4.5 : Fixed : Issue with KeyValue property access
  // 1.9.5.0 : New : RefreshDropDownList public method added
  // 1.9.5.1 : Fixed : Issue with destroying TAdvDBLookupComboBox when anchoring & label is used
  // 1.9.6.0 : Improved : Support for High DPI.
...
type
  ...
  TLabelPosition = (lpLeftTop,lpLeftCenter,lpLeftBottom,lpTopLeft,lpBottomLeft,
                    ...
                    lpBottomCenter, lpRightTop,lpRightCenter,lpRightBottom, lpTopRight, lpBottomRight, lpRighBottom);
...
begin
  ...
  begin
    if  not Enabled then
      l := StyleServices.GetElementDetails(tcDropDownButtonDisabled)
    else
    begin
      if Down then
        l := StyleServices.GetElementDetails(tcDropDownButtonPressed)
      else
       if Hover then
         l := StyleServices.GetElementDetails(tcDropDownButtonHot)
       else
         l := StyleServices.GetElementDetails(tcDropDownButtonNormal);
    end;

    ARect := ClientRect;

    LStyle.DrawElement(Canvas.Handle, l, ARect, ARect);
...
begin
  ...
  DPIScale := GetDPIScale(FLabel, FLabel.Canvas);

  FLabel.Font.Assign(FLabelFont);
  FLabel.Font.Height := Round(DPIScale * FLabelFont.Height);
  ...
  begin
    if (LabelField <> '') and not FLookupError then
    begin
      if (Text = '') then
        FLabel.Caption := '';
    end;
...
procedure TAdvDBLookupComboBox.RefreshDropDownList;
begin
  LoadFromListSource;
end;
...
begin
  ...
  DPIScale := GetDPIScale(Self, FChkForm.Canvas);

  FChkForm.Font.Assign(Self.Font);
  FChkForm.Font.Height := Round(DPIScale * Self.Font.Height);
  ...
  FChkForm.Height := Round(DPIScale * FDropHeight);
  ...
  begin
    ...
    begin
      ...
      FStringGrid.RowHeights[0] := Round(DPIScale * Height);
      k := 1;
    end;

    FStringGrid.ColWidths[i] := Round(DPIScale * FColumns.Items[i].FWidth);
    ...
    begin
      ...
      begin
        ...
        FStringGrid.Canvas.Font.Height := Round(DPIScale * FColumns.Items[i].Font.Height);
      ...
      FStringGrid.RowHeights[j] := Round(DPIScale * Height);
    ...
    begin
      ...
      FStringGrid.Canvas.Font.Size := Round(DPIScale * FColumns.Items[i].TitleFont.Size); 
  ...
  FStringGrid.Font.Size := Round(DPIScale * FDropFont.Size);
  ...
  FStringGrid.DefaultRowHeight := Round(DPIScale * FGridRowHeight);

  if FShowGridTitleRow then
  begin
    if GridHeaderAutoSize then
      FStringGrid.RowHeights[0] := Round(DPIScale * HeaderHeight)
    else
      FStringGrid.RowHeights[0] := Round(DPIScale * FGridHeaderHeight);
  end;
...
begin
  ...
  begin
    for i := 0 to FStringGrid.ColCount -1 do
    begin
      Columns[i].Width := Round(FStringGrid.ColWidths[i] / GetDPIScale(Self, FStringGrid.Canvas));
    end;
  end;
...
begin
  ...
  Loc.Right := ClientWidth - Round(GetDPIScale(FButton, FButton.Canvas) * FButton.Width) - 3;
...
begin
  ...
  DPIScale := GetDPIScale(FButton, FButton.Canvas);
  ...
  begin
    if NewStyleControls and Ctl3D then
      FButton.SetBounds(Width - Round(DPIScale * 17) - Dist , 0, Round(DPIScale * 17), Height - Dist)
    else
      FButton.SetBounds (Width - Round(DPIScale * FButton.Width) - d, 1, Round(DPIScale * FButton.Width), Height - 3);
    SetEditRect;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // require exact match when programmatically setting
      if (txt = s) and (fndmatch = -1) then
      begin
        fndmatch := I;
        break;
      end;
    ...
    begin
      //FLookupError := true;
      if Assigned(OnLookupError) then
        OnLookupError(Self, Text);
    end;
...
var
  ...
  DPIScale: single;
begin
  DPIScale := GetDPIScale(Self, FStringGrid.Canvas);
  ...
  begin
    ...
    begin
      ...
      FStringGrid.Canvas.Font.Size := Round(DPIScale * FColumns.Items[ACol].TitleFont.Size);
    end
  else
  begin
      FStringGrid.Canvas.Font.Assign(Self.Font);
      FStringGrid.Canvas.Font.Size := Round(DPIScale * Self.Font.Size);
  end;
  end
  else
  begin
    ...
    begin
      ...
      FStringGrid.Canvas.Font.Height := Round(DPIScale * FColumns.Items[ACol].Font.Height);
    end
    else if Self.ParentFont then
    begin
      FStringGrid.Canvas.Font.Assign(Self.Font);
      FStringGrid.Canvas.Font.Height := Round(DPIScale * Self.Font.Height);
    end
    else
    begin
      FStringGrid.Canvas.Font.Assign(FDropFont);
      FStringGrid.Canvas.Font.Height := Round(DPIScale * FDropFont.Height)
    end;
  end;
...
begin
  ...
  if Assigned(DataSource) and Assigned(DataSource.DataSet) and (DataField <> '') and DataSource.DataSet.Active then
  begin
    fld := DataSource.DataSet.FieldByName(DataField);
    if Assigned(fld) then
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (LabelField <> '') then
        begin
          if (Text = '') and not FLookupError then
            LabelCaption := ''
          else
          begin
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvDBLookupComboBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvDBLookupComboBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  begin
    OldSearch := FCurrentSearch;
    Insert(Key, FCurrentSearch, SelStart +  1);
    FCurrentSearch := AnsiUpperCase(FCurrentSearch);
  end;
  ...
  if FLookupError then
  begin
    if Assigned(OnLookupError) then
      OnLookupError(self, FCurrentSearch);
  end;
