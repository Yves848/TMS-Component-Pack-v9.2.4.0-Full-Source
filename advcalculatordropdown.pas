...
begin
  ...
  begin
   case (btn.tag) of
   10:begin if Flastop = -1 then doplus; FPrevop:=0; FLastOp := 0; end;
   11:begin if Flastop = -1 then domin; FPrevop:=2; FLastOp := 0; end;
   12:begin if Flastop = -1 then domul; FPrevop:=1; FLastOp := 0; end;
   13:begin if Flastop = -1 then  dodiv; FPrevop:=3; FLastOp := 0; end;
...
begin
  ...
  if Key = FormatSettings.DecimalSeparator then
    if (pos(FormatSettings.DecimalSeparator,Text)=0) then Text:=Text+FormatSettings.DecimalSeparator;
...
//  if FCalcClosed or (Assigned(FDropDownForm) and (not FDropDownForm.Visible)) then
  ...
  if not (IsNumChar(Key) or (Key = FormatSettings.DecimalSeparator) or (Key = #8) or (Key = '-')) then
    key := #0;

  if ((Text='0') or (FNewval)) and (Text <> '-') and (IsNumChar(key)) then
  begin
    Text := Key;
    Key := #0;
  ...
  if (Key = FormatSettings.DecimalSeparator) and (Pos(Key,Text)>0) then
  begin
    Key := #0;
  end;
