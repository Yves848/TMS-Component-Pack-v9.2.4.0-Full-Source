...
const
  ...
  // v1.4.0.1 : Fixed : Issue with initializing datetime selection
  // v1.4.0.2 : Fixed : Issue with transparency on double buffered forms
  // v1.4.0.3 : Fixed : Issue with cyclic and keyboard support
  // v1.4.1.0 : Improved : Support for High DPI
    ...
    begin
      ...
      begin
        ...
        begin
          if Cyclic then
          begin
            if FScPosTo >= (GetRangeCount + 1) * cTextSpacing then
            begin
              FCurrentScPos := FCurrentScPos - (GetRangeCount * cTextSpacing) - cTextSpacing;
              FScPosTo := FScPosTo - (GetRangeCount * cTextSpacing) - cTextSpacing;
              SetAnimatedValue;
            end
            else if FScPosTo <= -cTextSpacing then
            begin
              FCurrentScPos := FCurrentScPos + (GetRangeCount * cTextSpacing) +  cTextSpacing;
              FScPosTo := FScPosTo + (GetRangeCount * cTextSpacing) +  cTextSpacing;
              SetAnimatedValue;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothSpinner.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothSpinner.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
var
  I: Integer;
begin
  ...
  for I := 0 to Columns.Count - 1 do
  begin
    Columns[I].ScrollToValue(Columns[I].GetSelectedValue, False);
    Columns[I].FMouseUp := False;
  end;
...
procedure TAdvSmoothSpinner.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  hs := Round(GetDPISCale(Self, Canvas) * SelectedHeight) div 2;
...
begin
  ...
  begin
    ...
    if Cyclic then
    begin
      if sel = IncSteps(GetRangeTo, 1) then
        sel := GetRangeFrom
...
procedure TAdvSmoothSpinner.Loaded;
begin
  inherited;
  {$IFDEF DELPHIXE2_LVL}
...
begin
  ...
  begin
    ...
    begin
      ...
      if FMouseDown then
      begin
        if ((FDragY < y-cTextSpacing / 2) or (FDragY > Y+cTextSpacing / 2)) or SmoothScrolling then
        begin
          FSp := 2;
          FAnimate := false;
          ...
          begin
            ...
            begin
              while not ((pos mod cTextSpacing) = 0) do
                pos := pos - 1;
            end;
          end
          else
          ...
          if Cyclic then
          begin
            if pos = (GetRangeCount + 1) * cTextSpacing then
              pos := 0
            else if pos = -cTextSpacing then
              pos := GetRangeCount * cTextSpacing;
          end;

          if (SmoothScrolling and not Cyclic) or not SmoothScrolling then
            pos := Max(0, Min(cTextSpacing * GetRangeCount, pos));

          if not SmoothScrolling then
          begin
            FSelectedValue := IncSteps(GetRangeFrom, pos div cTextSpacing);
          ...
          begin
            if (Y - FDragY) > 0 then
            begin
              while not ((FScPosTo mod cTextSpacing) = 0) do
                FScPosTo := FScPosTo - 1;
            end
            else
            begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if not Cyclic then
              FScPosTo := Max(0, Min(cTextSpacing * GetRangeCount, FScPosTo));
          end;
        end;
      end;
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FScPosTo := FScPosTo + Round(DPIScale * ColumnAppearance.TextSpacing)
          end
          else
          begin
            FSp := FAnimationFactor;
          ...
          if not Cyclic then
            FScPosTo := Max(0, Min(Round(DPIScale * ColumnAppearance.TextSpacing) * GetRangeCount, FScPosTo));
          FAnimate := true;
        end;
      end;
    end;
...
begin
  ...
  rgn := CreateRoundRectRgn(rc.Left, rc.Top, rc.Right + 2, rc.Bottom + 2, Round(DPIScale * rnd), Round(DPIScale * rnd));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FCurrentScPos := StepsFromTo(GetRangeFrom, FSelectedValue) * Round(DPIScale * FOwner.ColumnAppearance.TextSpacing);
        FScPosTo := FcurrentScPos;
      end;
    end;
    Changed;
...
begin
  ...
  begin
    ...
    f := TGPFont.Create(ff, fnt.Size * DPIScale, fs, UnitPoint);
    b := TGPSolidBrush.Create(ColorToARGB(fnt.Color));
    g.MeasureString('gh', length('gh'), f, MakeRect(0, 0, 10000, 10000), sf, sri);

    cTextSpacing := Round(DPIScale * TextSpacing);

    cnt := Round((FOwner.InsideRect.Bottom - FOwner.InsideRect.Top) / ((cTextSpacing / 2) + sri.Height)) + Round(DPIScale * 5);
    i := (FCurrentScPos div cTextSpacing) - (cnt div 2);
    j := 0;
    si := -1;
    siw := 0;
    sih := 0;
    ...
    begin
      ...
      begin
        ...
        if s <> '' then
        begin
          ypostext := -FCurrentScPos + (i * cTextSpacing) + (FOwner.InsideRect.Bottom - (sri.Height)) / 2;
        ...
        yposimg := -FCurrentScPos + (i * cTextSpacing) + (FOwner.InsideRect.Bottom - (sih)) / 2;
        if s <> '' then
...
begin
  ...
  begin
    ...
    begin
      s := (r.Right - r.Left - ((cc + 1) * ColumnAppearance.Spacing * DPIScale)) / cc;
      st := r.Left + (FDrawIndex * s) + (FDrawIndex + 1) * ColumnAppearance.Spacing * DPIScale;
    end
    else
    begin
      s := Self.Width * DPIScale;
      i := FDrawIndex;
      st := r.Left;
      while (i > 0) do
      begin
        st := st + (Columns[i - 1].Width + ColumnAppearance.Spacing) * DPIScale;
        Dec(i);
      end;
    end;
...
//  if (FSelectedValue <> value) and Enabled and Visible then
  if Enabled and Visible and Assigned(FOwner) and not (csLoading in FOwner.ComponentState) then
  begin
    DPIScale := GetDPIScale(FOwner, FOwner.Canvas);
    cTextSpacing := Round(DPIScale * Fowner.ColumnAppearance.TextSpacing);
    ...
    if Value < GetRangeFrom then
      FScPosTo := -cTextSpacing
    else
      FScPosTo := StepsFromTo(GetRangeFrom, Value) * cTextSpacing;
    ...
    begin
      ...
      begin
        if FScPosTo = (GetRangeCount + 1) * cTextSpacing then
          ...
          FScPosTo := GetRangeCount * cTextSpacing;
      end;

      FScPosTo := Max(0, Min(cTextSpacing * GetRangeCount, FScPosTo));
      FSelectedValue := IncSteps(GetRangeFrom, FScPosTo div cTextSpacing);
...
begin
  FSelectedValue := Max(GetRangeFrom, Min(IncSteps(GetRangeFrom, FScPosTo div Round(GetDPISCale(FOwner, FOwner.Canvas) * FOwner.ColumnAppearance.TextSpacing)), GetRangeTo));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FCurrentScPos := StepsFromTo(GetRangeFrom, FSelectedValue) * Round(GetDPIScale(FOwner, FOwner.Canvas) * FOwner.ColumnAppearance.TextSpacing);
        FScPosTo := FcurrentScPos;
      end;
    end;
    Changed;
