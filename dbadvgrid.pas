...
const
  ...
  // 2.4.1.4  : Fixed : Issue with dataset scroll and scrolling in a dtSequenced grid
  // 2.4.1.5  : Fixed : Issue with style updating for black styles
  // 2.4.1.6  : Fixed : Issue with high DPI initialization when ColCount is used directly
  // 2.4.1.7  : Improved : high DPI handling when used on a form with Scaled = false
  // 2.4.1.8  : Fixed : Issue with custom inplace editors and editing with EditPostMode = epRow
  // 2.4.1.9  : Fixed : Issue with grouping and column coloring
  // 2.4.1.10 : Fixed : Issue with high DPI and use of grid on frames
  // 2.4.2.0  : New : Event OnNeedData added
  // 2.4.2.1  : Fixed : Rare issue with scrolling for specific fixed row heights
  // 2.4.2.2  : Fixed : Rare issue when editing in epCell mode and clearing a cell
  // 2.4.2.3  : Fixed : Issue with hiding columns & grouping columns
  // 2.4.2.4  : Fixed : Issue with multi row selection and setting up grid columns at runtime
  // 2.4.2.5  : Fixed : Issue with moving columns and using custom imagelists per column
  // 2.4.2.6  : Fixed : Side-effect of accessing cells via column rows in a grouped grid
  // 2.4.2.7  : Fixed : Issue with clipboard copy handling
  // 2.4.3.0  : Improved : Support for datasets that have no built-in bookmark support & grid.PageMode = true
  // 2.4.3.1  : Fixed : Rare issue with blocked SelectOnRightClick
  // 2.4.4.0  : New : Automatic button column type when setting Columns[x].Editor := edButton
  // 2.4.4.1  : Fixed : Issue with setting DB field visibility at runtime with connected DB grid
  // 2.4.4.2  : Fixed : Issue with fixed column updates during specific scroll operations
  // 2.4.4.3  : Fixed : Issue with specific inplace editors and epRow EditPostMode
  // 2.4.4.4  : Fixed : Issue with setting column index in grid with hidden columns
  // 2.4.4.5  : Fixed : Rare issue with AppendOnArrowDown = true and arrow key use on inplace editors
  // 2.4.4.6  : Fixed : Small potential memory leak with LoadColumnPositions()
  // 2.4.4.7  : Fixed : Issue with AppendOnArrowDown = true
  // 2.4.4.8  : Fixed : Issue with ColumnPopup
  // 2.4.4.9  : Fixed : Issue when FixedCols > 1 for refresh of fixed columns during scroll
  //          : Fixed : Issue with SelectAll
  // 2.4.4.10 : Fixed : Issue with DB fields used in fixed cells
  // 2.4.4.11 : Fixed : Issue with inplace combobox editor and ESC key
  // 2.4.4.12 : Fixed : Issue with hidden columns and close/open dataset
  // 2.4.5.0  : New : TGridColumn.DisplayIndex to retrieve display column index from column
  // 2.4.5.1  : Fixed : Issue with updating DefaultRowHeight when grid has data
  // 2.4.5.2  : Fixed : Issue with checkbox handling in fixed cells
  // 2.4.5.3  : Improved : Behavior when changing row heights with mouse
  // 2.4.5.4  : Improved : Runtime floating footer visibility change handling
  // 2.4.5.5  : Fixed : Issue with floating footer row for fsCustomPreview style
  // 2.4.6.0  : New : Per monitor high DPI support
  // 2.4.6.1  : Fixed : Small issue with VCL styles and selection colors
  // 2.4.6.2  : Fixed : Regression with use of OnGetCellColor
  // 2.4.6.3  : Fixed : Issue with column based background color settings
...
type
  ...
  protected
    procedure SetDefRowHeightEx(const Value: Integer); override;
   ...
   // procedure GetCellReadOnly(ACol,ARow: Integer;var IsReadOnly: Boolean); override;
    ...
    property ShowDefinedFields: boolean read FShowDefinedFields write FShowDefinedFields;
  public
...
begin
  ...
  begin
    ...
    FFont.Height := Round(FFont.Height / GetGrid.DPIScale);
    ...
    FImages := TDBGridColumnItem(Source).Images;
    FButtonWidth := TDBGridColumnItem(Source).ButtonWidth;
    FButtonHeight := TDBGridColumnItem(Source).ButtonHeight;
    FButtonCaption := TDBGridColumnItem(Source).ButtonCaption;
  end;
end;
...
function TDBGridColumnItem.GetDisplayIndex: integer;
begin
  Result := Index;

  if TDBGridColumnCollection(Collection).FOwner.GroupColumn > Index then
    Result := Index - 1;
end;
...
procedure TDBGridColumnItem.SetButtonCaption(const Value: string);
begin
...
procedure TDBGridColumnItem.SetButtonHeight(const Value: integer);
begin
  FButtonHeight := Value;
  TDBGridColumnCollection(Collection).Update(Self);
end;
...
procedure TDBGridColumnItem.SetButtonWidth(const Value: integer);
begin
  FButtonWidth := Value;
  TDBGridColumnCollection(Collection).Update(Self);
end;
...
begin
  ...
  for i := 0 to (Collection as TDBGridColumnCollection).Count - 1 do
  begin
    (Collection as TDBGridColumnCollection).FOwner.AllColWidths[i] := il.Items[i];
  end;
...
begin
  ...
  if Grid.CheckDataSet and (Value = '')  then
    Grid.ShowDefinedFields := true;
end;
       ...
       if not FShowDBIndicator or (FixedCols <= 0) then
        ...
        begin
          if (c = 0) and (FixedCols > 0) and FShowDBIndicator {or FDataLink.DataSet.Eof or FDataLink.DataSet.Bof} then
            Exit;
          if (r <= FixedRows - 1) or (FloatingFooter.Visible and (r >= RowCount - 1) and (FloatingFooter.FooterStyle in [fsCustomPreview, fsFixedLastRow])) then
          begin
            Exit;
          end;
          ...
          if (DataSetType = dtSequenced) then
          begin
            if (FDataLink.DataSet.State = dsInsert) or (FCacheRecNo < 0) then
            begin
        ...
        //FDataLink.DataSet.MoveBy(-1);
          ...
          SB_PAGEUP:
            begin
              if (DataSetType = dtSequenced) then
              begin
                if FloatingFooter.Visible and (FloatingFooter.FooterStyle in [fsCustomPreview, fsFixedLastRow]) then
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  ase := FDataLink.DataSet.AfterScroll;
                  bse := FDataLink.DataSet.BeforeScroll;
                  FDataLink.DataSet.AfterScroll := nil;
                  FDataLink.DataSet.BeforeScroll := nil;
                  ...
                  FDataLink.DataSet.AfterScroll := ase;
                  FDataLink.DataSet.BeforeScroll := bse;
                end;
              end
              else
                ds := MoveDBCursor(VisibleRowCount);
    ...
    if Assigned(FDataLink.DataSet) then
      ...
      begin
        ...
        begin
          ...
          FEmptyDataSet := Result = 0;
          if (FloatingFooter.Visible) and PageMode and (FloatingFooter.FooterStyle in [fsCustomPreview, fsFixedLastRow]) then
          begin
            if (Result = 0) then
              Inc(Result, 2)
            else
...
begin
  ...
  CallBU := (LeftCol > FixedCols) or (FixedCols > 1);

  if not CallBU then
  begin
    CallBU := (Msg.CharCode = VK_UP) and (Row = TopRow);
  end;

  if not CallBU then
  begin
    CallBU := (Msg.CharCode = VK_DOWN) and (Row >= TopRow + VisibleRowCount - 1);
  end;
...
begin
  ...
  if FixedRowHeight <= DefaultRowHeight then
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(TDBGridColumnItem(FColumnCollection.Items[ACol]).BorderPen) then
...
begin
  ...
  begin
    if (ACol >= FixedCols) and
      (ACol < ColCount - FixedRightCols + nh) and
      (ARow < RowCount - FixedFooters) then
    begin
      if not StyleBkg then
      begin
        if TDBGridColumnItem(FColumnCollection.Items[ACol]).Fixed then
          ABrush.Color := FixedColor
        else
      ...
      if ARow < FixedRows then
      begin
        if Assigned(TDBGridColumnItem(FColumnCollection.Items[ACol]).HeaderFont) then
          AFont.Assign(TDBGridColumnItem(FColumnCollection.Items[ACol]).HeaderFont)
      end
      else
        if Assigned(TDBGridColumnItem(FColumnCollection.Items[ACol]).Font) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if TDBGridColumnItem(FColumnCollection.Items[ACol]).Editor = edButton then
      begin
        FCellGraphic.CellType := ctVirtButton;
        FCellGraphic.CellTransparent := ControlLook.ControlStyle = csFlat;
        FCellGraphic.CellIndex := MakeLong(TDBGridColumnItem(FColumnCollection.Items[ACol]).ButtonWidth,TDBGridColumnItem(FColumnCollection.Items[ACol]).ButtonHeight);
        FCellGraphic.CellText := TDBGridColumnItem(FColumnCollection.Items[ACol]).ButtonCaption;
        Result := FCellGraphic;
      end;
...
// CheckBox for String Fields
  if (Result = nil) and (FColumnCollection.Items[ACol].CheckBoxField) and (ARow >= FixedRows) and (ACol >= IndCol) and
     not ((ARow = RowCount - 1) and (FloatingFooter.Visible and (FloatingFooter.FooterStyle in [fsCustomPreview, fsFixedLastRow]))) then
  begin
    Fld := GetDBFieldAtColumn(ACol);
...
procedure TDBAdvGrid.UpdateBufferCount;
begin
      ...
      begin
        ...
        begin
          ...
          case FColumnCollection.Items[ACol].Editor of
            ...
            , edImagePickerDropDown, edAdvGridDropDown, edCustom
            {$IFNDEF DELPHI_UNICODE}
...
begin
  ...
  begin
    if not (frm as TForm).Scaled then
     Exit;
  end;
...
begin
  ...
  for i := 0 to Columns.Count - 1 do
  begin
//    if Assigned(Columns[i].Font) then
//      Columns[i].Font.Height := Round(Columns[i].Font.Height * DPIScale);
//
//    if Assigned(Columns[i].HeaderFont) then
...
procedure TDBAdvGrid.StyleUpdated;
var
  i: integer;
begin
  inherited;
...
//  OldActiveRecord:  Integer;
  ...
  ae: boolean;
begin
  if (Row <> ARow) and (ACol >= FixedCols) then
  begin
    Row := ARow;
  end;
...
begin
  ...
  begin
    if FRecordChanged and (FEditPostMode = epRow) and not IsEditHiding then
    begin
      FRecordChanged := false;
      FDataLink.DataSet.Post;
    end;
    ...
    begin
      ...
      begin
        ...
        if (AEditor in [edButton, edNormal, edNumeric, edFloat, edValidChars, edPositiveNumeric, edPositiveFloat, edUppercase, edMixedCase, edLowerCase]) then
          Result := GridCells[RealColIndex(Col), Row];
      end;
...
begin
  ...
  if (Button = mbRight) and (MouseActions.SelectOnRightClick) then
    Exit;
...
procedure TDBAdvGrid.SetDefRowHeightEx(const Value: Integer);
begin
  inherited;
  UpdateBufferCount;
end;
...
procedure TDBAdvGrid.RowHeightsChanged;
begin
  inherited;
  FRowHeightChanged := true;
end;
...
begin
  ...
  if (Button <> mbLeft) or (MouseActions.SelectOnRightClick) then
    Exit;
  ...
  if (R <> Row) and (R >= 0) and PageMode and (LastRowClicked or (ShowDBIndicator and (FixedCols > 0) and (C = 0))) and not FRowHeightChanged then
  begin
    if Assigned(DataSource) then
      if Assigned(DataSource.Dataset) then
      begin
