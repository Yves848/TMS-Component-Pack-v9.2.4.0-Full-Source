...
const
  ...
  // v1.0.3.0 : Improved : Support for High DPI
  // v1.1.0.0 : New : VCL Styles support added
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvWordCloud.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
begin
  ...
  try
    ...
    begin
      ...
      dsDown:
        begin
          LDetails := LStyle.GetElementDetails(tbPushButtonLast);
          {$IFDEF DELPHIXE6_LVL}
          if (seBorder in StyleElements) then
          {$ENDIF}
          begin
            if LStyle.GetElementColor(LDetails, ecBorderColor, clr) then
              clrbrdr := clr
            else
              clrbrdr := LStyle.GetStyleColor(scBorder);
          end;
          {$IFDEF DELPHIXE6_LVL}
          if (seClient in StyleElements) then
          {$ENDIF}
          begin
            if LStyle.GetElementColor(LDetails, ecFillColor, clr) then
            begin
              clrfrom := clr;
              clrto := clr;
            end
            ...
            begin
              clrfrom := LStyle.GetStyleColor(scButtonPressed);
              clrto := clrfrom;
            end
          end;
        end;
      dsHover:
        begin
          ...
          begin
            ...
            begin
              clrfrom := LStyle.GetStyleColor(scButtonHot);
              clrto := clrfrom;
            end
          end;
        end;
      dsSelected:
        begin
          ...
          begin
            ...
            begin
              clrfrom := LStyle.GetStyleColor(scButtonFocused);
              clrto := clrfrom;
            end
          end;
        end
        ...
        begin
          ...
          begin
            ...
            begin
              clrfrom := LStyle.GetStyleColor(scEdit);
              clrto := clrfrom;
              clrbrdr := clrfrom;
            end
          end;
    ...
    dsNormal:
      begin
        ...
        if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
          ...
          Canvas.Font.Color := LStyle.GetStyleFontColor(sfButtonTextNormal);
      end;
    dsDown:
      begin
        LDetails := LStyle.GetElementDetails(tbPushButtonLast);
        if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
          ...
          Canvas.Font.Color := LStyle.GetStyleFontColor(sfButtonTextPressed);
      end;
    dsHover:
      begin
        LDetails := LStyle.GetElementDetails(tbPushButtonHot);
        if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
          ...
          Canvas.Font.Color := LStyle.GetStyleFontColor(sfButtonTextHot);
      end;
    dsSelected:
      begin
        LDetails := LStyle.GetElementDetails(tbPushButtonPressed);
        if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
          ...
          Canvas.Font.Color := LStyle.GetStyleFontColor(sfButtonTextFocused);
      end;
    end;
  end;
  {$ENDIF}
...
procedure TAdvWordCloud.InitVCLStyle(Init: Boolean);
{$IFDEF DELPHIXE2_LVL}
var
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
  ...
  begin
    ...
    {$ENDIF}
      if LStyle.GetElementColor(LDetails, ecTextColor, clr) and (clr <> clNone) then
        ...
        FStyledFontColor := LStyle.GetStyleFontColor(sfWindowTextNormal);
    {$IFDEF DELPHIXE6_LVL}
    if (seClient in StyleElements) then
    {$ENDIF}
...
begin
  ...
  fh := Round(FDPISCale * ItemHeight);

  dx := Round(FDPISCale * LEFTOFFSET);
  dy := Round(FDPISCale * TOPOFFSET);
  ...
  Canvas.Pen.Color := CurClientColor;

  Canvas.Rectangle(ClientRect);

  for i := 0 to Words.Count - 1 do
  begin
    dw := Words[i].GetWidth(Canvas) + Round(FDPISCale * ITEMMARGIN);

    if dx + dw - Round(FDPISCale * ITEMMARGIN) + 4 >= Width then
    begin
      dx := Round(FDPISCale * LEFTOFFSET);
      dy := dy + fh + Round(FDPISCale * LINEMARGIN);
    end;
    ...
    DrawItem(Canvas, Rect(dx, dy, dx + dw - Round(FDPISCale * ITEMMARGIN) + 4, dy + fh + 2), Words[i], state);
  ...
  begin
    ...
    R.Right := R.Right - Round(FDPISCale * ITEMMARGIN);
...
begin
  ...
  begin
    ...
    begin
      dx := Round(FDPISCale * LEFTOFFSET);
      dy := dy + th + Round(FDPISCale * LINEMARGIN);
    end;

    if (x >= dx) and (x <= dx + dw - 4) and (y > dy) and (y < dy + Round(FDPISCale * ITEMHEIGHT)) then
    begin
      // hit an item
      Result := i;
      Exit;
...
begin
  ...
  dx := Round(FDPISCale * LEFTOFFSET);
  dy := Round(FDPISCale * TOPOFFSET) + (dt div 2);
  ...
  begin
    ...
    begin
      tw := Words[j].GetWidth(Canvas) + Round(FDPISCale * ITEMMARGIN);

      if dx + tw - Round(FDPISCale * ITEMMARGIN) + 4 >= Width then
      //if dx + tw >= Width then
      begin
        dx := Round(FDPISCale * LEFTOFFSET) + tw;
        dy := dy + th + Round(FDPISCale * LINEMARGIN);
      end
      else
        dx := dx + tw;
...
begin
  ...
  begin
    ...
    if i = Index then
    begin
      Result := Rect(dx, dy, dx + dw, dy + th + Round(FDPISCale * LINEMARGIN));
      break;
    end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvWordCategoryCloud.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
procedure TAdvWordCategoryCloud.CreateWnd;
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
end;
      ...
      dsSelected:
        ...
        begin
          ...
          begin
            ...
            begin
              clrfrom := LStyle.GetStyleColor(scButtonNormal);
              clrto := clrfrom;
              clrbrdr := clrfrom;
            end
          end;
...
procedure TAdvWordCategoryCloud.InitVCLStyle(Init: Boolean);
{$IFDEF DELPHIXE2_LVL}
var
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
...
var
  ...
  CurBorderColor: TColor;
  {$IFDEF DELPHIXE2_LVL}
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
  clr: TColor;
  CurClientColor, CurFontColor: TColor;
...
begin
  ...
  CurClientColor := Color;
  CurFontColor := Font.Color;
  HeaderFontColor := FHeader.Font.Color;
  if FUseVCLStyles then
  begin
    {$IFDEF DELPHIXE6_LVL}
    if (seBorder in StyleElements) then
    ...
    begin
      ...
      if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
        ...
        HeaderFontColor := LStyle.GetStyleFontColor(sfButtonTextNormal);
    end;
  end;
  {$ENDIF}
  ...
  dx := Round(FDPIScale * LEFTOFFSET);
  dy := Round(FDPIScale * (TOPOFFSET + FHeader.Height));

  CR.Bottom := CR.Top + Round(FDPIScale * FHeader.Height);
  Canvas.Brush.Style := bsSolid;
  {$IFDEF DELPHIXE2_LVL}
  if FUseVCLStyles then
  begin
    ...
    try
      ...
      if LStyle.DrawElement(bmp.Canvas.Handle, LDetails, Rect(0, 0, bmp.Width, bmp.Height)) then
        Canvas.CopyRect(CR, bmp.Canvas, Rect(0, 0, bmp.Width, bmp.Height));
    finally
      FreeAndNil(bmp);
    end;
  end
  ...
  {$ENDIF}
  Canvas.Font.Height := Round(FDPIScale * FHeader.Font.Height);
  ...
  begin
    ...
    if i = CategoryIndex then
      tri := DR.Left + Round(FDPIScale * ITEMMARGIN) div 2;

    DR.Left := DR.Right + Round(FDPIScale * ITEMMARGIN);
    DR.Right := Width;
    DR.Top := CR.Top;
    DR.Bottom := CR.Bottom;
  end;
  ...
  CR.Top := CR.Top + Round(FDPIScale * FHeader.Height);
  ...
  begin
    ...
    begin
      dw := Categories[CategoryIndex].Words[i].GetWidth(Canvas) + Round(FDPIScale * ITEMMARGIN);

      if dx + dw - Round(FDPIScale * ITEMMARGIN) + 6 >= Width then
      begin
        dx := Round(FDPIScale * LEFTOFFSET);
        dy := dy + fh + Round(FDPIScale * LINEMARGIN);
      end;
      ...
      DrawItem(Canvas, Rect(dx, dy, dx + dw - Round(FDPIScale * ITEMMARGIN) + 4, dy + fh + 2), Categories[CategoryIndex].Words[i], state);
    ...
    {$IFDEF DELPHIXE2_LVL}
    if FUseVCLStyles then
      gpbrush := TGPSolidBrush.Create(MakeColor(255, HeaderFontColor))
    else
    {$ENDIF}
    ...
    try
      ...
      pts[0].X := tri;
      pts[0].Y := Round(FDPIScale * (FHeader.Height + 1));
      pts[1].X := tri + 3;
      pts[1].Y := Round(FDPIScale * (FHeader.Height - 6 + 1));
      pts[2].X := tri + 6;
      pts[2].Y := Round(FDPIScale * (FHeader.Height + 1));
...
begin
  ...
  dx := Round(FDPIScale * LEFTOFFSET);
  dy := Round(FDPIScale * (TOPOFFSET + FHeader.Height)) + (dt div 2);
  ...
  begin
    ...
    begin
      tw := Categories[CategoryIndex].Words[j].GetWidth(Canvas) + Round(FDPIScale * ITEMMARGIN);

      if dx + tw - Round(FDPIScale * ITEMMARGIN) + 6 >= Width then
      begin
        dx := Round(FDPIScale * LEFTOFFSET) + tw;
        dy := dy + th + Round(FDPIScale * LINEMARGIN);
      end
