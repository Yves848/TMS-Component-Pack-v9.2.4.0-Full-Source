...
const
  ...
  // v1.4.0.4 : Fixed : Issue with transparency on double buffered forms
  // v1.4.0.5 : Fixed : Issue with text anti-aliasing on tab
  // v1.4.0.6 : Fixed : Issue with drawing tab borders on active tab
  // v1.4.0.7 : Fixed : Issue with text rendering hint for tabs
  // v1.5.0.0 : New : VCL Styles support
  // v1.5.0.1 : Fixed : Design-time issue with style select
...
procedure TAdvSmoothTabPager.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('TMSStyle',ReadTMSStyle, WriteTMSStyle, True);
end;

procedure TAdvSmoothTabPager.ReadTMSStyle(Reader: TReader);
begin
  FTMSStyle := TTMSStyle(Reader.ReadInteger);
end;

procedure TAdvSmoothTabPager.WriteTMSStyle(Writer: TWriter);
begin
  Writer.WriteInteger(integer(FTMSStyle));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.Font.Height := Round(FDPIScale * tf.Height);
      Canvas.Brush.Style := bsClear;
      tw := Canvas.TextWidth(AdvSmoothTabPages[PageIndex].Caption);
      th := Canvas.TextHeight('gh');

      textr := Rect(R.Left + Round(FDPIScale * hs), r.Top + Round(FDPIScale * vs), r.Right - Round(FDPIScale * hs), r.Bottom - Round(FDPIScale * vs));
    ...
    FAppearance.Draw(g, AdvSmoothTabPages[PageIndex].Caption, R.Left - hs, R.Top - vs, R.Right - R.Left + (hs * 2), R.Bottom - R.Top + (vs * 2), hs, vs,
      c, ColorDown, bc, tc, Shadow, d, Bevel, vertical, righttoleft, roundingtype, Picture, th, tw, true, picidx, picname, TextRenderingHintClearTypeGridFit);
    ...
    begin
      ...
      if Status.Visible then
        Appearance.Draw(g, Status.OffsetLeft + x, y, 0, 0, true,Status.Caption, FDPIScale);
    end;
...
begin
  ...
  if (ActivePageIndex >= 0) and (ActivePageIndex <= FAdvSmoothTabPages.Count - 1) then
...
function TAdvSmoothTabPager.GetComponentStyle: TTMSStyle;
begin
  Result := FTMSStyle;
end;
...
begin
  ...
  begin
    case TabPosition of
      ...
      begin
        Result.Top := Result.Top + Round(FDPIScale * TabSettings.GetHeight(ActivePageIndex));
        Result.Left := Result.Left + Round(FDPIScale * FPageMargin);
        Result.Right := Result.Right - Round(FDPIScale * FPageMargin);
        Result.Bottom := Result.Bottom - Round(FDPIScale * FPageMargin) - 1;
      end;
      tpBottomLeft, tpBottomRight, tpBottomCenter:
      begin
        Result.Top := Result.Top + Round(FDPIScale * FPageMargin) + 1;
        Result.Left := Result.Left + Round(FDPIScale * FPageMargin);
        Result.Right := Result.Right - Round(FDPIScale * FPageMargin);
        Result.Bottom := Result.Bottom - Round(FDPIScale * TabSettings.GetHeight(ActivePageIndex));
      end;
      tpLeftTop, tpLeftBottom, tpLeftCenter:
      begin
        Result.Top := Result.Top + Round(FDPIScale * FPageMargin) + 1;
        Result.Left := Result.Left + Round(FDPIScale * TabSettings.GetHeight(ActivePageIndex));
        Result.Right := Result.Right - Round(FDPIScale * FPageMargin);
        Result.Bottom := Result.Bottom - Round(FDPIScale * FPageMargin) - 1;
      end;
      ...
      begin
        Result.Top := Result.Top + Round(FDPIScale * FPageMargin) + 1;
        Result.Left := Result.Left + Round(FDPIScale * FPageMargin);
        Result.Right := Result.Right - Round(FDPIScale * TabSettings.GetHeight(ActivePageIndex));
        Result.Bottom := Result.Bottom - Round(FDPIScale * FPageMargin) - 1;
      end;
    end;
  end;
...
begin
  Result := ClientRect;
  case TabPosition of
    tpTopLeft, tpTopRight, tpTopCenter: Result.Bottom := Result.Top + Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
    tpBottomLeft, tpBottomRight, tpBottomCenter: Result.Top := Result.Bottom - Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
    tpLeftTop, tpLeftBottom, tpLeftCenter: Result.Right := Result.Left + Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
    tpRightTop, tpRightBottom, tpRightCenter: Result.Left := Result.Right - Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
  end;
end;
...
begin
  ...
  case TabPosition of
    tpTopLeft, tpTopRight, tpTopCenter:
    begin
      Result.Top := Result.Top + Round(FDPIScale * FTabOffSet);
      Result.Bottom := Result.Top + Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
      Result.Left := Result.Left + Round(FDPIScale * (FTabSettings.StartMargin + FPageMargin));
      Result.Right := Result.Right - Round(FDPIScale * FTabSettings.EndMargin);
    end;
    tpBottomLeft, tpBottomRight, tpBottomCenter:
    begin
      Result.Top := Result.Bottom - Round(FDPIScale * (FTabSettings.GetHeight(ActivePageIndex) + FTabOffSet));
      Result.Bottom := Result.Bottom - Round(FDPIScale * FTabOffSet);
      Result.Left := Result.Left + Round(FDPIScale * (FTabSettings.StartMargin + FPageMargin));
      Result.Right := Result.Right - Round(FDPIScale * FTabSettings.EndMargin);
    end;
    tpLeftTop, tpLeftBottom, tpLeftCenter:
    begin
      Result.Top := Result.Top + Round(FDPIScale * (FTabSettings.StartMargin + FPageMargin));
      Result.Bottom := Result.Bottom - Round(FDPIScale * FTabSettings.EndMargin);
      Result.Left := Result.Left + Round(FDPIScale * FTabOffSet);
      Result.Right := Result.Left + Round(FDPIScale * FTabSettings.GetHeight(ActivePageIndex));
    end;
    tpRightTop, tpRightBottom, tpRightCenter:
    begin
      Result.Top := Result.Top + Round(FDPIScale * (FTabSettings.StartMargin + FPageMargin));
      Result.Bottom := Result.Bottom - Round(FDPIScale * FTabSettings.EndMargin);
      Result.Left := Result.Right - Round(FDPIScale * (TabSettings.GetHeight(ActivePageIndex) + FTabOffSet));
      Result.Right := Result.Right - Round(FDPIScale * FTabOffSet);
    end;
  end;
end;
...
begin
  ...
  totals := -1;
  case TabPosition of
    tpBottomCenter, tpTopCenter, tpBottomRight, tpTopRight: totals := (rlast.Right - r.left) + Round(FDPIScale * TabSettings.Spacing);
    tpLeftCenter, tpLeftBottom, tpRightCenter, tpRightBottom: totals := (rlast.Bottom - r.Top) + Round(FDPIScale * TabSettings.Spacing);
  end;
...
begin
  Result := Rect(-1, -1, -1, -1);
  Sp := Round(FDPIScale * FTabSettings.Spacing);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          TbW := Round(FDPIScale * TabSettings.GetWidth(i));

          if not AdvSmoothTabPages[i].TabAppearance.Picture.Empty then
          begin
            AdvSmoothTabPages[i].TabAppearance.Picture.GetImageSizes;
            R := Rect(CR.Left, CR.Top, CR.Left + TbW + Sp + Round(FDPIScale * AdvSmoothTabPages[i].TabAppearance.Picture.Width) + Round(FDPIScale * AdvSmoothTabPages[i].TabAppearance.Appearance.Spacing), CR.Bottom);
            CR.Left := CR.Left + TbW + Sp + Round(FDPIScale * AdvSmoothTabPages[i].TabAppearance.Picture.Width) + Round(FDPIScale * AdvSmoothTabPages[i].TabAppearance.Appearance.Spacing);
          end
          else
          begin
...
procedure TAdvSmoothTabPager.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothTabPager.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothTabPager.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);
