...
const
  ...
  // 3.0.2.1 : Fixed : Issue with number highlighting
  // 3.0.2.2 : Fixed : Issue with reparenting
  // 3.1.0.0 : New : VCL Styles support 
...
procedure TAdvCodeList.CMRecreateWnd(var Message: TMessage);
begin
  inherited;
  outputdebugstring('recreate');
end;

{$IFDEF DELPHIXE2_LVL}
procedure TAdvCodeList.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      bmp.Canvas.Brush.Color := LStyle.GetStyleColor(scListBox);
      bmp.Canvas.Brush.Style := bsSolid;
      bmp.Canvas.Pen.Color := LStyle.GetStyleColor(scListBox);
      bmp.Canvas.Rectangle(rcItem);
    end;
    {$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          bmp.Canvas.Brush.Color := LStyle.GetStyleColor(scButtonFocused)
        end
        else
        begin
          bmp.Canvas.Brush.Color := LStyle.GetStyleColor(scButtonNormal)
        end
      end
      else
      {$ENDIF}
      ...
      rct2 := rcItem;
      rct2.Top := rct2.Top + 1;
      rct2.Left := rct2.Left + 3;
      rct2.Right := rct2.Right - 2;
      ...
      begin
        ...
        LStyle.DrawElement(bmp.Canvas.Handle, LDetails, rct2);
      end
      else
      {$ENDIF}
      begin
        if (Integer(itemID) >= 0) and (odSelected in State) then
        begin
          clrFrom := FCodeBlockSelectColor;
          clrTo := FCodeBlockSelectColorTo;
        end
        else
        begin
          clrFrom := FCodeBlockColor;
          clrTo := FCodeBlockColorTo;
        end;

        if (clrFrom <> clNone) and (clrTo <> clNone) then
          DrawGradient(bmp.Canvas, ColorToRGB(clrFrom), ColorToRGB(clrTo), 64, rct2, false);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          LDetails := LStyle.GetElementDetails(tlListItemNormal);
        ...
        begin
          ...
          StyleServices.DrawElement(bmp.Canvas.Handle, LDetails, Rect(rcItem.Left + 2, rcItem.Top + 1, rcItem.Right - 1, rcItem.Top + th + 4));
        end
        else
        {$ENDIF}
        begin
        ...
        begin
          ...
          if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
            bmp.Canvas.Font.Color := clr
          else
            bmp.Canvas.Font.Color := LStyle.GetStyleFontColor(sfButtonTextPressed);
        end
        else
        {$ENDIF}
        begin
    ...
    begin
      ...
      begin
        ...
        if (odSelected in State) then
          LStyle.DrawEdge(bmp.Canvas.Handle, LDetails, Rect(rcItem.left + 2, rcItem.Top, rcItem.Right - 1, rcItem.Bottom - 1), [TElementEdge.eeRaised], [TElementEdgeFlag.efRect])
        else
          LStyle.DrawEdge(bmp.Canvas.Handle, LDetails, Rect(rcItem.left + 2, rcItem.Top, rcItem.Right - 1, rcItem.Bottom - 1), [TElementEdge.eeSunken], [TElementEdgeFlag.efRect]);
      end
      else
      {$ENDIF}
      begin
...
begin
  ...
  begin
    if odSelected in State then
      LDetails := LStyle.GetElementDetails(tgCellLast)
    else
      LDetails := LStyle.GetElementDetails(tgCellNormal);
    if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
      ACanvas.Font.Color := clr
    else
      ACanvas.Font.Color := LStyle.GetStyleFontColor(sfGridItemNormal);
  end;
  {$ENDIF}
...
//          if (lc > 0) and ((lc < rc) or (rc = 0)) and (not SegmentStyle.isBracket) then
      ...
      if validno then
        ...
        begin
          ...
          if (toEnd = len) or (UpCase(S[toEnd+1]) = #32) then
          begin
...
procedure TAdvCodeList.InitVCLStyle(Init: Boolean);
{$IFDEF DELPHIXE2_LVL}
var
  LStyle: TCustomStyleServices;
{$ENDIF}
