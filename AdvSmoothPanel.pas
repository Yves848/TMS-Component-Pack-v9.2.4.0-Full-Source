...
const
  ...
  // v1.5.2.2 : Fixed : Issue with child control handling in header when CanMove is true
  // v1.5.2.3 : Fixed : Issue with transparency on double buffered forms
  // v1.5.2.4 : Fixed : Issue with caption text size calculation
  // v1.5.3.0 : Improved : Support for High DPI
  // v1.6.0.0 : New : VCL Styles support
  // v1.6.0.1 : Fixed : Able to turn off VCL styles with the Style Elements seClient
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothPanel.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothPanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;

  FFill.Rounding :=  MulDiv(FFill.Rounding, M, D);
end;

{$IFDEF DELPHIXE2_LVL}

procedure TAdvSmoothPanel.CMStyleChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    ...
    f := TGPFont.Create(ff, Caption.Font.Size * DPIScale, fs, UnitPoint);
    ...
    try
      ...
      // fix for size calculation rounding
      sizerect.Width := sizerect.Width + 1;
    except
    end;
    ...
    begin
      ...
      try
        ...
        AFill.Rounding := Round(DPIScale * Fill.Rounding);
  ...
  begin
    ...
    if Caption.HTMLLocation = plCustom then
      htmlr := Rect(0, ch, Width - GetShadowSize - (2 * Fill.BorderWidth) - Round(Caption.HTMLLeft * DPIScale), 10000)
    else
      htmlr := Rect(Round(Caption.HTMLLeft * DPIScale), ch, Width - GetShadowSize - (Fill.BorderWidth * 2), 10000);
    ...
    HTMLDrawGDIP(g, Caption.HTMLFont, Caption.HTMLText, htmlr, FImages, 0, 0, -1, -1, Caption.HTMLShadowOffset, False, True, False, False,
      False, False, Caption.HTMLWordWrapping, 1.0, Caption.HTMLURLColor, clNone, clNone, Caption.HTMLShadowColor, a, s, k, XSize, YSize, l, m, hr, nil, FContainer, 2, DPIScale);
    ...
    begin
      x := Round(Caption.HTMLLeft * DPIScale);
      y := Round(Caption.HTMLTop * DPIScale);
    ...
    if Caption.HTMLLocation <> plCustom then
      ...
      htmlr := Bounds(x, y, Width - GetShadowSize - 2 * Fill.BorderWidth - Round(Caption.HTMLLeft * DPIScale) , YSize);
...
procedure TAdvSmoothPanel.Loaded;
begin
  inherited;
...
begin
  if FFill <> value then
  begin
    if (FFill.Rounding <> Value.Rounding) then
    begin
      DPIScale := GetDPIScale(Self, Canvas);
      FFill.Rounding := Round(Value.Rounding * DPIScale);
    end;
    FFill.Assign(Value);
...
begin
  ...
  if InMoveArea(pt) and (Msg.Result = htClient) and FCanMove {$IFDEF DELPHIXE2_LVL}and not Assigned(ControlAtPos(pt, True, True, True)){$ENDIF} then
  begin
    MouseMove([],pt.X,pt.Y);
