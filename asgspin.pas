...
begin
  ...
  Result := (Key = FormatSettings.DecimalSeparator) or (Key = FormatSettings.ThousandSeparator) or (Key = FormatSettings.TimeSeparator) or (Key = FormatSettings.DateSeparator)
    or (CheckSignedNum(Key)) or ((Key < #32) and (Key <> Chr(VK_RETURN)));

  if ((key = FormatSettings.TimeSeparator) and (FSpinType <> sptTime) and
    not (((FormatSettings.TimeSeparator = FormatSettings.DecimalSeparator) or (FormatSettings.TimeSeparator = FormatSettings.ThousandSeparator) or (FormatSettings.TimeSeparator = '-') or (FormatSettings.TimeSeparator = '+')) and (FSpinType in [sptNormal, sptFloat]))) then
    Result := False;

  if ((key = FormatSettings.DateSeparator) and (FSpinType <> sptDate) and
    not (((FormatSettings.DateSeparator = FormatSettings.DecimalSeparator) or (FormatSettings.DateSeparator = FormatSettings.ThousandSeparator) or (FormatSettings.DateSeparator = '-') or (FormatSettings.DateSeparator = '+')) and (FSpinType in [sptNormal, sptFloat]))) then
    Result := False;
  ...
  begin
    ...
    if ((Key = FormatSettings.DecimalSeparator) and
      ((pos(FormatSettings.DecimalSeparator, Text) > 0) and (pos(FormatSettings.DecimalSeparator, SelText) = 0) )) then
      Result := False;

    dp := pos(FormatSettings.Decimalseparator, Text);
  ...
  begin
    ...
    if (key = FormatSettings.TimeSeparator) and (Pos(FormatSettings.TimeSeparator,s) > 0) then
    begin
      Delete(s,Pos(FormatSettings.TimeSeparator,s),1);
      if Pos(FormatSettings.TimeSeparator,s) > 0 then
        Result := False;
    end;
  end;
...
begin
  if ReadOnly then MessageBeep(0)
    ...
    sptTime:
      begin
        if (selstart >= Pos(FormatSettings.TimeSeparator, Text)) then
        begin
          if ShowSeconds and (selstart>=posfrom(FormatSettings.TimeSeparator,text,pos(FormatSettings.TimeSeparator,text))) then
    ...
    sptDate:
      begin
        FUSDates := Pos('M',Uppercase(FormatSettings.ShortDateFormat)) < Pos('D',Uppercase(FormatSettings.ShortDateFormat));

        if FUsDates then
        begin
          if selstart >= pos(FormatSettings.DateSeparator,text) then
          begin
            if selstart >= posfrom(FormatSettings.DateSeparator,text,pos(FormatSettings.DateSeparator,text)) then
