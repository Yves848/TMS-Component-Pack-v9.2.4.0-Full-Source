...
uses
  Forms, Windows, Types, SysUtils, Classes, Graphics, Menus, Controls, AdvGDIP, AdvStyleIF
...
const
  ...
  // 2.6.4.1 : Fixed : Issue with handling of high DPI when application is not high DPI enabled
  // 2.6.5.0 : New : PopupAtControl() method added
  // 2.6.5.1 : Improved : High DPI handling in Delphi 10.2 Tokyo
  // 2.6.5.2 : Improved : High DPI handling with menus with system fonts
  // 2.6.5.3 : Improved : Disabled font color handling
  // 2.6.5.4 : Improved : Drawing of default menu item
  // 2.6.5.5 : Fixed: Disabled menu item drawing when hovering
  // 2.6.5.6 : Improved : Iconbar size handling in high DPI mode
  //         : Improved : Menu checkmarks & radiobutton in high DPI
  // 2.6.5.7 : Improved : Changing menu root item font at runtime
  // 2.6.5.8 : Improved : Drawing subnode menu triangle when VCL styles are used
  // 2.6.5.9 : Fixed : Memory leak on DPI changing
  // 2.6.5.10: Fixed : Regression with glyph resource handling
  // 2.6.5.11: Fixed : Issue with drawing large imagelist items in TAdvPopupMenu
  // 2.6.5.12: Improved : High DPI handling
  // 2.6.5.13: Improved : Shortcut drawing right-aligned in menu items
  // 2.6.5.14: Fixed : Issue with height calculation on high DPI of menu items with Notes
  // 2.6.6.0 : New : Per monitor support for high DPI
...
type
  ...
  TPopopControlPosition = (pcpBottomLeft, pcpBottomRight, pcbTopLeft, pcpTopRight, pcpCenter);
...
type
  ...
  TResType = (rtx1, rtx15, rtx2, rtundefined);
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    property Menu: TMenu read FMenu;
    procedure UpdateResources(dpi: single);
  published
  ...
  public
    ...
    procedure PopupAtControl(AControl: TControl; Position: TPopopControlPosition = pcpCenter; DeltaX: integer = 0; DeltaY: integer = 0);
...
begin
  ...
  if Assigned(Application.MainForm) and Application.MainForm.HandleAllocated then
    hwin := Application.MainFormHandle
  else
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      if not TopLevel then
        Width := Round(IconBar.Size * FDPIScale);
      ...
      begin
        ...
        begin
          ...
          {$IFDEF DELPHIXE11_LVL}
          imgh := ImageList.Height + 4;
          {$ENDIF}
        end;

        if ImageList.Width > IconBar.Size then
          IconBar.Size := ImageList.Width + Round(4 * FDPIScale);
      end
      ...
      begin
        ...
        begin
          ...
          ACanvas.Font.Height := Round(ACanvas.Font.Height * FDPIScale);
        end;
      end
      else
      begin
      ...
      begin
        ...
        begin
          tw := ACanvas.TextWidth(AdvWinKeyString);
          if not customfont then
            tw := Round(tw / FDPIScale);
        ...
        if SideBarVisible and not IsAfterMenuBreak(MenuItem) then
          Inc(Width, Round(SideBar.Size * FDPIScale) + SideBar.SplitterWidth);
  ...
  {$IFDEF DELPHIXE11_LVL}
  if not customfont then
    Height := Max(imgh, Round(Height / FDPIScale));
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    begin
      ItemRect.Left := ItemRect.Left + Round(SideBar.Size * FDPIScale) + SideBar.SplitterWidth;
      { remove bar width }
      if AdvMenu.CurrentMenuStyler.SideBar.Position = spRight then
        OffsetRect(ItemRect, -Round(SideBar.Size * FDPIScale) - SideBar.SplitterWidth, 0);
    end;
    ...
    begin
      ...
      begin
        ...
        if AdvMenu.CurrentMenuStyler.SideBar.Position = spLeft then
          SidebarRect.Right := SidebarRect.Left + Round(SideBar.Size * FDPIScale) +
            SideBar.SplitterWidth
        else
          SidebarRect.Left := SidebarRect.Right - Round(SideBar.Size * FDPIScale) -
            SideBar.SplitterWidth;
      ...
      IconBarRect := Rect(ItemRect.Left, MenuRect.Top,
        ItemRect.Left + Round(IconBar.Size * FDPIScale), MenuRect.Bottom);
...
function GetDisabledFontColor(AState: TOwnerDrawState): TColor;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    Result := GetColor(LDetails, clBtnShadow);
  end
  else
{$ENDIF}
    Result := clBtnShadow;
...
begin
  ...
  if GetMenuItemRect(0, AdvMenu.Self.Handle, MenuItem.MenuIndex, dr) then
    menuwidth := dr.Right - dr.Left;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Font.Height := Round(Font.Height / GetDPIScale * FDPIScale);  //MenuFont height isn't scaled when changing screen.
        end
        else
        begin
      ...
      begin
        ...
        begin
          Font.Assign(Screen.MenuFont);
          Font.Height := Round(Font.Height / GetDPIScale * FDPIScale);
        end
      ...
      begin
        ...
        begin
          if not Enabled then
            Font.Color := GetDisabledFontColor(State);
        ...
        begin
          ...
          if not Enabled then
            fclr := GetDisabledFontColor(State);
      ...
      begin
        ...
        begin
          ...
          begin
            Rect.Right := menuwidth - 20;
            s := Copy(Text, 1, Pos(#9, Text) - 1);
            DrawText(Handle, PChar(s), Length(s), Rect, Flags);
            s := Copy(Text, Pos(#9, Text) + 1, Length(Text));
            Flags := Flags AND (NOT DT_LEFT);
            Flags := Flags or DT_RIGHT or DT_SINGLELINE or DT_VCENTER;
...
var
  ...
  begin
    ...
    if Alignment <> paRight then
      ...
      R := Rect(ARect.Left + 5, ARect.Top, ARect.Left + 8, ARect.Bottom);
  ...
  begin
    ...
    begin
      ...
      BorderRect := ARect;
      BorderRect.Right :=
        BorderRect.Left + Round(AdvMenu.CurrentMenuStyler.IconBar.Size * FDPIScale) - 1;
      ...
      begin
{$IFDEF DELPHIXE_LVL}
        if ((MenuItem.Action is TCustomAction) and ((MenuItem.Action as TCustomAction).ActionList is TActionManager)) then
        HaveActionImage := Assigned(MenuItem.Action) and
          (MenuItem.Action is TCustomAction) and Assigned
          ((MenuItem.Action as TCustomAction).ActionList) and Assigned
...
{$ENDIF}
      ...
      if not TopLevel then
        Inc(ARect.Left, Round(AdvMenu.CurrentMenuStyler.IconBar.Size * FDPIScale) + ItemTextOffset)
...
begin
  AdvMenu.CurrentMenuStyler.Glyphs.UpdateResources(GetDPIScale);
...
begin
  ...
  begin
    Inc(ARect.Left, Round(IconBar.Size * GetDPIScale));
...
begin
  ...
  begin
    ...
    X := Round((Round(CurrentMenuStyler.SideBar.Size * GetDPIScale) - TextSize.cy) / 2 - 0.5);
...
procedure TAdvPopupMenu.PopupAtControl(AControl: TControl; Position: TPopopControlPosition = pcpCenter; DeltaX: integer = 0; DeltaY: integer = 0);
var
  pt: TPoint;
begin
  ...
  case Position of
    pcpBottomLeft:
      pt.Y := pt.Y + AControl.Height;
    pcpBottomRight:
      begin
        pt.X := pt.X + AControl.Width;
        pt.Y := pt.Y + AControl.Height;
      end;
    pcpTopRight:
      pt.X := pt.X + AControl.Width;
    pcpCenter:
      begin
        pt.X := pt.X + AControl.Width div 2;
        pt.Y := pt.Y + AControl.Height div 2;
      end;
  end;
...
procedure TMenuGlyphs.UpdateResources(dpi: single);
var
  res: string;
  newres: TResType;

  function GetResName(AResourceName: string): string;
  begin
    Result := AResourceName;
    case FResType of
    rtx15: Result := AResourceName + 'x15';
    rtx2: Result := AResourceName + 'x20';
    end;
  end;

begin
  newres := rtx1;
  if (dpi >= 1.5) and (dpi < 2) then
    newres := rtx15
  else
    if (dpi >= 2) then
      newres := rtx2;

  if (newres = FResType) and Assigned(FSubMenu) then
    Exit;

  FResType := newres;

  res := GetResName(AdvResourceSubMenuItem);
  ...
  FSubMenu.Handle := LoadBitmap(HInstance, PChar(res));
  FSubMenu.Transparent := True;

  res := GetResName(AdvResourceCheckItem);

  if Assigned(FCheck) then
    FreeAndNil(FCheck);

  FCheck := TBitmap.Create;
  FCheck.Handle := LoadBitmap(HInstance, PChar(res));
  FCheck.Transparent := True;

  res := GetResName(AdvResourceRadioItem);

  if Assigned(FRadio) then
    FreeAndNil(FRadio);

  FRadio := TBitmap.Create;
  FRadio.Handle := LoadBitmap(HInstance, PChar(res));
  FRadio.Transparent := True;
end;
