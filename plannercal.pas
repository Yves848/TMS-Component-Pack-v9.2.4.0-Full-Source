...
const
  ...
  // 2.3.1.3 : Improved : Handling of tab key
  // 2.3.1.4 : Fixed : High DPI handling in Delphi Tokyo
  // 2.3.1.5 : Fixed : Issue with handling OnMouseUp
  // 2.3.2.0 : New : New types of event shapes added
  // 2.3.3.0 : New : Per monitor high DPI support
...
type
  ...
  TEventShape = (evsRectangle, evsCircle, evsSquare, evsTriangle, evsNone, evsAMPMRectangle,
                 evsReverseTriangle,
                 evsTriangleSummRight, evsTriangleSummLeft, evsMiddleLeftUpRec,
                 evsMiddleLeftDownRec, evsMiddleRightUpRec, evsMiddleRightDownRec,
                 evsLeftUpCorner, evsLeftDownCorner, evsRightUpCorner, evsRightDownCorner,
                 evsStar, evsReverseSquare, evsLeftUpCircle, EvsLeftDownCircle,
                 evsRightUpCircle, EvsRightDownCircle, evsRecLeftUpCircle,
                 evsRecLeftDownCircle, evsRecRightUpCircle, evsRecRigthDownCircle);
  ...
  protected
    ...
    procedure ChangeScale(M, D: Integer); override;
    {$ENDIF}
    function DisplCalWidth: integer;
    function DisplCalHeight: integer;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Loaded; override;
...
{$IFDEF DELPHIXE10_LVL}
procedure TPlannerCalendar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
...
begin
  if Browsers.PrevYear and FGlyphs.FPrevMonth.Empty then
    xoffs := Round(FDPIScale * XOffset) + 20
  else
    xoffs := Round(FDPIScale * XOffset);
    ...
    begin
      ...
      begin
        if FGlyphs.FPrevYear.Empty then
          FCanvas.Draw(xoffs + Round(FDPIScale * 15), 1 + BorderWidth , FGlyphs.FPrevMonth)
        else
          FCanvas.Draw(xOffs + Round(FDPIScale * 10) + FGlyphs.FPrevYear.Width, 1 + BorderWidth, FGlyphs.FPrevMonth);
      end;
    end
    else
    begin
      ...
      if FGlyphs.FPrevYear.Empty or not Browsers.PrevYear then
        Polygon([Point(xoffs + Round(FDPIScale * 10), Round(FDPIScale) + BorderWidth + 2), Point(xoffs + Round(FDPIScale * 5), Round(FDPIScale * 6) + BorderWidth + 2), Point(xoffs + Round(FDPIScale * 10), Round(FDPIScale * 11) + BorderWidth + 2)])
      else
        Polygon([Point(xoffs + Round(FDPIScale * 5) + FGlyphs.FPrevYear.Width, Round(FDPIScale) + BorderWidth), Point(xoffs + FGlyphs.FPrevYear.Width, Round(FDPIScale * 6) + BorderWidth), Point(xoffs + Round(FDPIScale * 5) + FGlyphs.FPrevYear.Width, Round(FDPIScale * 11) + BorderWidth)]);
...
begin
  if Browsers.NextYear then
    xoffs := Round(FDPIScale * 25)
  else
    xoffs := Round(FDPIScale * 5);
  ...
  begin
    ...
    begin
      ...
      if FGlyphs.FNextYear.Empty or not Browsers.NextYear then
        FCanvas.Draw(Width - (xoffs + FGlyphs.FNextMonth.Width), Round(FDPIScale) + BorderWidth, FGlyphs.NextMonth)
      else
        FCanvas.Draw(Width - (FGlyphs.FNextMonth.Width + Round(FDPIScale * 10) + FGlyphs.FNextYear.Width), Round(FDPIScale) + BorderWidth, FGlyphs.NextMonth);
    end
    else
    begin
      if flgr then
      ...
      if FGlyphs.FNextYear.Empty or not Browsers.NextYear then
        Polygon([Point(Width - Round(FDPIScale * 5) - xoffs, Round(FDPIScale) + BorderWidth + 2), Point(Width - Round(FDPIScale * 5) - xoffs, Round(FDPIScale * 11) + BorderWidth + 2), Point(Width - xoffs, Round(FDPIScale * 6) + BorderWidth + 2)])
      else
        Polygon([Point(Width - (Round(FDPIScale * 15) + FGlyphs.FNextYear.Width), Round(FDPIScale) + BorderWidth), Point(Width - (Round(FDPIScale * 15) + FGlyphs.FNextYear.Width), Round(FDPIScale * 11) + BorderWidth), Point(Width - (Round(FDPIScale * 10) + FGlyphs.FNextYear.Width), Round(FDPIScale * 6) + BorderWidth)]);
...
begin
  ...
  begin
    ...
    begin
      ...
      FCanvas.Draw(xoffset + Round(FDPIScale * 5), 1 + BorderWidth, FGlyphs.FPrevYear);
    end
    else
    begin
      if dflgl then
      ...
      Polygon([Point(xoffset + Round(FDPIScale * 10), Round(FDPIScale) + BorderWidth + 2), Point(xoffset + Round(FDPIScale * 5), Round(FDPIScale * 6) + BorderWidth + 2), Point(xoffset + Round(FDPIScale * 10), Round(FDPIScale * 11) + BorderWidth + 2)]);
      Polygon([Point(xoffset + Round(FDPIScale * 15), Round(FDPIScale) + BorderWidth + 2), Point(xoffset + Round(FDPIScale * 10), Round(FDPIScale * 6) + BorderWidth + 2), Point(xoffset + Round(FDPIScale * 15), Round(FDPIScale * 11) + BorderWidth + 2)]);
...
begin
  ...
  begin
    ...
    begin
      ...
      FCanvas.Draw(Width - (FGlyphs.FNextYear.Width + Round(FDPIScale * 5)), 1 + BorderWidth, FGlyphs.FNextYear);
    end
    else
    begin
      if dflgr then
      ...
      Polygon([Point(Width - Round(FDPIScale * 10), Round(FDPIScale) + BorderWidth + 2), Point(Width - Round(FDPIScale * 10), Round(FDPIScale * 11) + BorderWidth + 2), Point(Width - Round(FDPIScale * 5), Round(FDPIScale * 6) + BorderWidth + 2)]);
      Polygon([Point(Width - Round(FDPIScale * 15), Round(FDPIScale) + BorderWidth + 2), Point(Width - Round(FDPIScale * 15), Round(FDPIScale * 11) + BorderWidth + 2), Point(Width - Round(FDPIScale * 10), Round(FDPIScale * 6) + BorderWidth + 2)]);
...
begin
  ...
  case AShape of
    ...
    begin
      wi := (r.Right - r.Left) shr 1;
      FCanvas.Polygon([Point(r.Left + wi, r.Bottom), Point(r.Right, r.top),
        Point(r.Left, r.top)]);
    end;
    evsTriangleSummRight:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.Right, r.Top + wi), Point(r.Left, r.Top),
        Point(r.Left, r.Bottom)]);
    end;
    evsTriangleSummLeft:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.left, r.Top + wi), Point(r.Right, r.Top),
        Point(r.Right, r.Bottom)]);
    end;
    evsMiddleLeftUpRec:
    begin
      FCanvas.Polygon([Point(r.Right, r.Top), Point(r.Left, r.Top),
        Point(r.Left, r.Bottom)]);
    end;
    evsMiddleLeftDownRec:
    begin
      FCanvas.Polygon([Point(r.Left, r.Top), Point(r.Left, r.Bottom),
        Point(r.Right, r.Bottom)]);
    end;
    evsMiddleRightUpRec:
    begin
      FCanvas.Polygon([Point(r.left, r.Top), Point(r.Right, r.Top),
        Point(r.Right, r.Bottom)]);
    end;
    evsMiddleRightDownRec:
    begin
      FCanvas.Polygon([Point(r.Right, r.Top), Point(r.Right, r.Bottom),
        Point(r.Left, r.Bottom)]);
    end;
    evsLeftUpCorner:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.left + wi, r.Top), Point(r.Left, r.Top + wi),
        Point(r.Left, r.Top)]);
    end;
    evsLeftDownCorner:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.left, r.Top + wi), Point(r.Left + wi, r.Bottom),
        Point(r.Left, r.Bottom)]);
    end;
    evsRightUpCorner:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.Right - wi, r.Top), Point(r.Right, r.Top + wi),
        Point(r.Right, r.Top)]);
    end;
    evsRightDownCorner:
    begin
      wi := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.Right - wi, r.Bottom), Point(r.Right, r.Bottom - wi),
        Point(r.Right, r.Bottom)]);
    end;
    evsReverseSquare:
    begin
      wi := (r.Right - r.Left) shr 1;
      he := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.left + wi, r.Top), Point(r.Left, r.Top + he),
        Point(r.Left, r.Top)]);
      FCanvas.Polygon([Point(r.left, r.Top + he), Point(r.Left + wi, r.Bottom),
        Point(r.Left, r.Bottom)]);
      FCanvas.Polygon([Point(r.Right - wi, r.Top), Point(r.Right, r.Top + he),
        Point(r.Right, r.Top)]);
      FCanvas.Polygon([Point(r.Right - wi, r.Bottom), Point(r.Right, r.Bottom - he),
        Point(r.Right, r.Bottom)]);
    end;
    evsLeftUpCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Ellipse(r.Left, r.Top, r.Left + wi, r.Top + wi);
    end;
    EvsLeftDownCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Ellipse(r.Left, r.Bottom, r.Left + wi, r.Bottom - wi);
    end;
    evsRightUpCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Ellipse(r.Right, r.Top, r.Right - wi, r.Top + wi);
    end;
    EvsRightDownCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Ellipse(r.Right, r.Bottom, r.Right - wi, r.Bottom - wi);
    end;
    evsStar:
    begin
      wi := (r.Right - r.Left) shr 1;
      he := (r.Bottom - r.Top) shr 1;
      FCanvas.Polygon([Point(r.Left + wi, r.Top), Point(r.right, r.Bottom),
                    point(r.Left, r.Top + he), point(r.Right, r.Top + he),
                    Point(r.left, r.Bottom), Point(r.Left + wi, r.Top)]);
    end;
    evsRecLeftUpCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Polyline([Point(r.Left, r.Top), Point(r.Right, r.Top), Point(r.Right, r.Bottom),
                  Point(r.Left, r.Bottom), Point(r.Left, r.Top)]);
      FCanvas.Ellipse(r.Left, r.Top, r.Left + wi, r.Top + wi);
    end;
    evsRecLeftDownCircle:
    begin
      wi := (r.Right - r.Left) shr 1;
      wi := wi Shr 1;
      FCanvas.Polyline([Point(r.Left, r.Top), Point(r.Right, r.Top), Point(r.Right, r.Bottom),
...
begin
  ...
  begin
    ...
    FCanvas.Font.Height := Round(FCanvas.Font.Height * FDPIScale);
    ...
    if UsedVCLStyles then
      FCanvas.Pen.Color := FTxtColor
    else
      FCanvas.Pen.Color := FLineColor;

    FCanvas.MoveTo(BorderWidth,r.Top + wfh + 2);
    FCanvas.LineTo(Width - BorderWidth,r.Top + wfh + 2);

    if FLine3D and not UsedVCLStyles then
    begin
      FCanvas.Pen.Color := clWhite;
      FCanvas.MoveTo(BorderWidth, r.Top + wfh + 3);
      FCanvas.LineTo(Width - BorderWidth, r.Top + wfh + 3);
    end;

    if FShowWeeks then
    begin
      FCanvas.Pen.Color := FLineColor;
      FCanvas.MoveTo(dx,dy + wfh);
      FCanvas.LineTo(dx, Height - 2);

      if Line3D then
      begin
        FCanvas.Pen.Color := clwhite;
        FCanvas.MoveTo(dx + 1,dy + wfh + 1);
        FCanvas.LineTo(dx + 1, Height - 2);
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if inlist then
            begin
              FCanvas.Brush.Color := FDateSelectColor;
              if FDateSelectColor = clNone then
                FCanvas.Brush.Style := bsClear;
              FCanvas.Pen.Color := FDateSelectBorderColor;
              FCanvas.Font.Color := FSelectFontColor;
              FCanvas.Rectangle(r);
            end
            else
              FCanvas.Brush.Color := self.Color;
            ...
            begin
              if inlist then
                FCanvas.Brush.Color := FDateDownColor
              else
                FCanvas.Brush.Color := FDateHoverColor;
            ...
            FCanvas.Font.Color := FInversColor;

            if IsEvent then
            begin
              FCanvas.Font.Assign(FEventFont);
              FCanvas.Font.Height := Round(FCanvas.Font.Height * FDPIScale);

              FCanvas.Font.Color := FEventDayColor;
            ...
            if not CheckDateRange(d) then
              FCanvas.Font.Color := FInactiveColor;
        ...
        begin
          FCanvas.Pen.Color := FDateSelectColor;
          FCanvas.Font.Color := FDateSelectColor;
        ...
        begin
          ...
          FCanvas.MoveTo(xoffset + (i - 1) * dx + 4, (j + 1) * dy - 3 + yoffset);
          FCanvas.LineTo(xoffset + (i - 1) * dx + 4, j * dy - 2 + yoffset);
          FCanvas.LineTo(xoffset + i * dx + 2, j * dy - 2 + yoffset);
          ...
          FCanvas.LineTo(xoffset + i * dx + 2, (j + 1) * dy - 3 + yoffset);
          FCanvas.LineTo(xoffset + (i - 1) * dx + 4, (j + 1) * dy - 3 + yoffset);
...
begin
  ...
  if (x >= lblx1) and (x <= lblx3) and (y > 0) and (y < Round(FDPIscale * 15)) and FMonthSelect
  ...
  if ShowGotoToday and (y > dy * 8 - (dy div 2)) and (x > Round(FDPIscale * XOffset))
    and (x < Round(FDPIscale * XOffset) + Canvas.TextWidth(FormatDateTime(TodayFormat,Now))) and Enable then
  begin
    flgt := True;
    PaintToday;
    Cursor := crHandPoint;
  ...
  begin
    ...
    begin
      if FGlyphs.FPrevYear.Empty then
      begin
        WidthX1 := Round(FDPIscale * 20);
        WidthX2 := Round(FDPIscale * 35);
      end
      else
      begin
        WidthX1 := Round(FDPIscale * 20);
        WidthX2 := Round(FDPIscale * 22) + FGlyphs.FPrevYear.Width;
      end;
      HeightY1 := 0;
      HeightY2 := Round(FDPIscale * 15);
    end
    else
    begin
      if FGlyphs.FPrevYear.Empty then
      begin
        WidthX1 := Round(FDPIscale * 20);
        WidthX2 := Round(FDPIscale * 22) + FGlyphs.FPrevMonth.Width;
      end
      else
      begin
        WidthX1 := Round(FDPIscale * 10) + FGlyphs.FPrevYear.Width;
        WidthX2 := Round(FDPIscale * 10) + FGlyphs.FPrevYear.Width + FGlyphs.FPrevMonth.Width;
      end;
    ...
    begin
      WidthX1 := WidthX1 - Round(FDPIscale * 20);
      WidthX2 := WidthX2 - Round(FDPIscale * 20);
    end;
  ...
  begin
    if FGlyphs.FPrevYear.Empty then
    begin
      WidthX1 := Round(FDPIScale * 5);
      WidthX2 := Round(FDPIScale * 15);
      HeightY1 := 0;
      HeightY2 := Round(FDPIScale * 15);
    end
    else
    begin
      WidthX1 := Round(FDPIScale * 5);
      WidthX2 := Round(FDPIScale * 5) + FGlyphs.FPrevYear.Width;
      HeightY1 := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        WidthX1 := Round(FDPIscale * 25) + FGlyphs.FNextMonth.Width;
        WidthX2 := Round(FDPIscale * 25);
      end
      else
      begin
        WidthX1 := Round(FDPIscale * 10) + FGlyphs.FNextYear.Width + FGlyphs.FNextMonth.Width;
        WidthX2 := Round(FDPIscale * 10) + FGlyphs.FNextYear.Width;
      end;
    ...
    begin
      if FGlyphs.FNextYear.Empty then
      begin
        WidthX1 := Round(FDPIscale * 35);
        WidthX2 := Round(FDPIscale * 25);
      end
      else
      begin
        WidthX1 := Round(FDPIscale * 15) + FGlyphs.FNextYear.Width;
        WidthX2 := Round(FDPIscale * 10) + FGlyphs.FNextYear.Width;
      end;
  ...
  begin
    if FGlyphs.FNextYear.Empty then
    begin
      WidthX1 := Round(FDPIscale * 15);
      WidthX2 := Round(FDPIscale * 5);
      HeightY1 := 0;
      HeightY2 := Round(FDPIscale * 15);
    end
    else
    begin
      WidthX1 := FGlyphs.FNextYear.Width + Round(FDPIscale * 5);
      WidthX2 := Round(FDPIscale * 5);
      HeightY1 := 0;
...
begin
  inherited;
  if Msg.CharCode in [VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT,VK_TAB] then
    Msg.Result := 1;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(cf) then
      begin
        if (cf as TForm).Visible then
          cf.Perform(WM_NEXTDLGCTL,nxt,0);
      end;
    end;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    FResizeVertical.Width := FColCount * DisplCalWidth + 2 * (BevelWidth + BorderWidth);
    FResizeVertical.Height := FSpliterV;
    FResizeVertical.Left := 0;
    FResizeVertical.Top := FRowCount * DisplCalHeight + 2 * (BevelWidth + BorderWidth);
    Cursor := crVSplit;
    Caption := '';
    Color := clScrollBar;
    Visible := True;
  ...
  begin
    Align := alNone;
    Height := FRowCount * DisplCalHeight + 2 * (BevelWidth + BorderWidth);
    Width := FSpliterH;
    Cursor := crHSplit;
    Color := clScrollBar;
    Left := FColCount * DisplCalWidth + 2 * (BevelWidth + BorderWidth);
    Top := 0;
    Caption := '';
    Visible := True;
  ...
  for i := 1 to FRowCount do
    ...
    begin
      ...
      Fpl.Left := (j - 1) * DisplCalWidth;
      Fpl.Top := (i - 1) * DisplCalHeight;
      Fpl.Width := DisplCalWidth;
      Fpl.Height := DisplCalHeight;
      Fpl.Color := clWhite;
      Fpl.IsGrouped := true;
  ...
  Width := DisplCalWidth * FColCount + FSpliterH + (BevelWidth + BorderWidth) * 2;
  Height := DisplCalHeight * FRowCount + FSpliterV + (BevelWidth + BorderWidth) * 2;
...
function TPlannerCalendarGroup.DisplCalHeight: integer;
begin
  Result := Round(FDPIScale * FCalendarHeight);
end;

function TPlannerCalendarGroup.DisplCalWidth: integer;
begin
  Result := Round(FDPIScale * FCalendarWidth);
end;
...
begin
  FColCount := Cols;
  FRowCount := Rows;
  FCalendarWidth := Round(CalendarWidth / FDPIScale);
  FCalendarHeight := Round(CalendarHeight / FDPIScale);
  ShowPlanners;
end;
...
begin
  if Value < 1 then
    Exit;
  FCalendarHeight := Round(Value / FDPIScale);
  if not (csLoading in ComponentState) then
    ShowPlanners;
end;
...
begin
  if Value < 1 then
    Exit;
  FCalendarWidth := Round(Value / FDPIScale);
  if not (csLoading in ComponentState) then
    ShowPlanners;
end;
...
begin
  ...
  FResizeHorizontal.Left := DisplCalWidth * FColCount + (BevelWidth + BorderWidth) * 2;
  FResizeHorizontal.Top := 0;
  FResizeHorizontal.Height := Height;

  FResizeVertical.Width := Width;
  FResizeVertical.Left := 0;
  FResizeVertical.Top := DisplCalHeight * FRowCount + (BevelWidth + BorderWidth) * 2;
  ...
  for i := 1 to FRowCount do
    ...
    begin
      ...
      pl.Left := (j - 1) * DisplCalWidth + (BevelWidth + BorderWidth);;
      pl.Top := (i - 1) * DisplCalHeight + (BevelWidth + BorderWidth);;
      pl.Width := DisplCalWidth;
      pl.Height := DisplCalHeight;
      pl.Day := 1;
      pl.Month := mo;
      pl.Year := ye;
...
begin
  if Value > DisplCalWidth div 2 then
    Exit;
  FSpliterH := Value;
  Refresh;
...
begin
  if Value > Round(FCalendarHeight * FDPIScale) div 2 then
    Exit;
  FSpliterV := Value;
  Refresh;
  Height := DisplCalHeight * FRowCount + FSpliterV + (BevelWidth + BorderWidth) * 2;
...
begin
  ...
  if FLetResize or FScaleChanging then
    Exit;
...
var
  ...
  ftmps: TSelDateItems;
begin
  x := Width div DisplCalWidth;

  if x < 1 then
    x := 1;

  y := Height div DisplCalHeight;
...
{$IFDEF DELPHIXE10_LVL}
procedure TPlannerCalendarGroup.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
