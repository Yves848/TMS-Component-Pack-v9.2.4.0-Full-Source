...
uses
  ...
  Menus, ShellAPI, AdvStyleIF, Types, AdvGDIP
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
const
  ...
  // v1.4.1.0   : New : AdvTreeNode.GetNext function added
  // v1.4.2.0   : Improved : Support for High DPI
...
procedure DrawArrow(Canvas: TCanvas; R: TRect; ArClr, ArShad: TColor; LeftDir: Boolean; ADPIScale: single = 1);  // Dir : true = Left; False = down
var
  g: TGPGraphics;
...
begin
  ...
  ArrowMid.X := R.Left + (R.Right - R.Left) div 2;
  ArrowMid.Y := R.Top + (R.Bottom - R.Top) div 2;
  ArrowSize := Round (ADPIScale * 2);

  if not LeftDir then      // Down direction
  begin
    p := TGPPen.Create(MakeColor(255, ArClr), ADPIscale * 2);

    Pts[0].X := ArrowMid.X - ArrowSize * 2;
    Pts[0].Y := ArrowMid.Y - ArrowSize;
    Pts[1].X := ArrowMid.X;
    Pts[1].Y := ArrowMid.Y + ArrowSize;
    Pts[2].X := ArrowMid.X + ArrowSize * 2;
    Pts[2].Y := ArrowMid.Y - ArrowSize;

  end
  else
  begin
    p := TGPPen.Create(MakeColor(255, ArClr), ADPIscale);

    Pts[0].X := ArrowMid.X - ArrowSize;
    Pts[0].Y := ArrowMid.Y - ArrowSize - 1;
    Pts[1].X := ArrowMid.X + ArrowSize;
    Pts[1].Y := ArrowMid.Y;
    Pts[2].X := ArrowMid.X - ArrowSize;
    Pts[2].Y := ArrowMid.Y + ArrowSize + 1;
  end;
...
procedure DrawScrollArrow(Canvas: TCanvas; R: TRect; ArClr: TColor; ADPIScale: single = 1);
var
  g: TGPGraphics;
  path: TGPGraphicsPath;
  ArrowSize: integer;
...
begin
  ...
  ArrowSize := Round (ADPIScale * 2);
  ArrowMid.X := (R.Left + (R.Right - R.Left) div 2) - ArrowSize;
  ArrowMid.Y := R.Top + (R.Bottom - R.Top) div 2;

    Pts[0].X := ArrowMid.X - ArrowSize;
    Pts[0].Y := ArrowMid.Y;
    Pts[1].X := ArrowMid.X + ArrowSize - 1;
    Pts[1].Y := ArrowMid.Y - ArrowSize - 1;
    Pts[2].X := ArrowMid.X + ArrowSize - 1;
    Pts[2].Y := ArrowMid.Y + ArrowSize + 1;

  path.AddLine(Pts[0],Pts[1]);
  path.AddLine(Pts[0],Pts[2]);
  g.DrawPath(p, path);
...
function TAdvTreeNode.GetNext: TAdvTreeNode;
var
  tn: TAdvTreeNode;
begin
  ...
  begin
    ...
    begin
      tn  := GetParent;
      while not Assigned(Result) and Assigned(tn) do
      begin
        Result := tn.GetNextSibling;
        if not Assigned(Result) then
          tn := tn.GetParent;
      end;
    end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TCustomExplorerTreeview.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TCustomExplorerTreeview.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  PostMessage(Self.Handle, WM_ET_SETEDITRECT, 0, 0);
end;
...
begin
  ...
  begin
    DPIScale := GetDPIScale(FDropDownButton, FDropDownButton.Canvas);
    R := GetNodeButtonsRect;
    s := R.Right - R.Left;  // available space for buttons
    if (S < Round(DPIScale * DwBUTTON_WIDTH)) then  // not enough space to even display scroll button
      Exit;
    ...
    begin
      ...
      FNodeButtons.Add(NB);
      if (NB.Width > s) or (Assigned(N) and (NB.Width > s - Round(DPIScale * DwBUTTON_WIDTH))) then
      begin
...
begin
  inherited;
  if (Message.Msg = WM_ET_SETEDITRECT) then
  begin
    SetEditRect;
    SendMessage(Handle, EM_SETSEL, Length(inherited Text), Length(inherited Text));
    Invalidate;
...
var
  DPIScale: single;
begin
  DPIScale := GetDPIScale(Self, FDropDownButton.Canvas);

  if RefreshButton then
  begin
    Result := GetRefreshButtonRect;
    Result.Right := Result.Left;
    Result.Left := Result.Right - Round(DPIScale * DROPDOWNBTN_WIDTH);
  end
  else
    Result := Rect(Width - Round(DPIScale * DROPDOWNBTN_WIDTH) - 2, GetBorderWidth, Width - 2, Height - GetBorderWidth);
...
var
  ...
  DPIScale: single;
begin
  DPIScale := GetDPIScale(Self, FRefreshBtn.Canvas);
  i := GetBorderWidth;
  if RefreshButton then
    Result := Rect(Width - Round(DPIScale * REFRESHBTN_WIDTH) - 2, i, Width - 2, Height - i)
  else
    Result := Rect(-1, -1, -1, -1);
end;
...
{$ENDIF}
  ...
  try
    ...
    begin
      if FDown then
        R.Top := R.Top + 2;
      DrawArrow(aCanvas, R, ArClr, ArShad, False, GetDPIScale(Self, Canvas));
    end;
...
begin
  ...
  begin
    ...
    if ((X > Width - Round(DPIScale * DwBUTTON_WIDTH)) and IsSplitButton) or ScrollButton then
      DropDownPress;
  end;
end;
...
begin
  inherited;
  DPIScale := GetDPIScale(Self, Canvas);
  if ((X < (Width - Round(DPIScale * DwBUTTON_WIDTH))) and ShowText) or not IsSplitButton then
  begin
    if not FHot then
    begin
      FHot := True;
...
begin
  ...
  if (X < (Width - Round(GetDPIScale(Self, Canvas) * DwBUTTON_WIDTH))) or not IsSplitButton then
  begin
    ButtonClick;
  end;
end;
...
begin
  ...
  if ScrollButton or not ShowText then
    Width := Round(DPIScale * DwBUTTON_WIDTH)
  else
  begin
    w := 0;
    if (Node.Text <> '') then
    begin
      ...
      Canvas.Font.Height := Round(DPIScale * ExplorerTreeview.Appearance.ButtonAppearance.Font.Height);
      R := Rect(0, 0, 1000, 100);
    ...
    if IsSplitButton then
      w := w + Round(DPIScale * DwBUTTON_WIDTH);

    Width := Max(Round(DPIScale * DwBUTTON_WIDTH), w);
  end;
end;
...
{$ENDIF}
  ...
  try
    ...
    begin
      R2.Left := R2.Right - Round(DPIScale * DwBUTTON_WIDTH);
      R1.Right := R2.Left;
    end;
    ...
    begin
      ...
      aCanvas.Font.Height := Round(DPIScale * ExplorerTreeview.Appearance.ButtonAppearance.Font.Height);
    ...
    begin
      DrawScrollArrow(aCanvas, ClientRect, ArClr, DPIScale);
    end
    else if not ScrollButton and IsSplitButton and ShowText then
    begin
      if FDown then
        R2.Top := R2.Top + 2;
      DrawArrow(aCanvas, R2, ArClr, ArShad, not FDown, DPIScale);
    end
...
begin
  ...
  begin
    ...
    R := ClientRect;
    FHot := PtInRect(r, p);
    R.Left := Width - Round(GetDPIScale(Self, Canvas) * DwBUTTON_WIDTH);
...
initialization
  ...
  WM_ET_SETEDITRECT := RegisterWindowMessage('ET_SETEDITRECT');
end.
