...
interface
  ...
  // 1.7.4.2 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // 1.7.4.3 : Fixed : Issue with unthemed ownerdraw drawing
...
const
  ...
  BLD_VER = 3; // Build nr.
  DATE_VER = 'Jan, 2019'; // Month version
...
{$ENDIF}
    ...
    function GetOwnerDrawEx: boolean;
    procedure SetOwnerDrawEx(const Value: boolean);
  protected
...
{$ENDIF}
    ...
    property OwnerDraw: boolean read GetOwnerDrawEx write SetOwnerDrawEx default false;
...
function TAdvListView.GetOwnerDrawEx: boolean;
begin
...
begin
  ...
  begin
    ...
    if not HTMLDrawEx(Canvas, s, r, TImageList(SmallImages), x, y, -1, -1, 1, True, False, False, False, True, False, WordWrap, FFormScaled, 1.0,
...
begin
  ...
  begin
      ...
      if Pos('{\', s) = 1 then
        ...
        begin
          ...
          begin
            ...
            begin
              HTMLDrawEx(FCanvas, s, DrwRect, TImageList(SmallImages), 0, 0, -1, -1, 1, False, True, False, False, True, False, WordWrap, FFormScaled, 0.0,
            ...
            HTMLDrawEx(FCanvas, s, DrwRect, TImageList(SmallImages), 0, 0, -1, -1, 1, False, False, False, False, True, False, WordWrap, FFormScaled, 0.0,
              FURLColor, clNone, clNone, clGray, Anchor, Stripped, FA, XSize, YSize, ml, hl, hr, FImageCache, FContainer,0);
          end
          else
          begin
    ...
    begin
      ...
      if pos('</', s) > 0 then
      begin
        HTMLDrawEx(FCanvas, s, TmpRct, TImagelist(SmallImages), 0, 0, -1, -1, 1, False, False, False, False, True, False, WordWrap, FFormScaled, 0.0,
          FURLColor, clNone, clNone, clGray, Anchor, Stripped, FA, XSize, YSize, ml, hl, hr, FImageCache, FContainer,0);
...
begin
  ...
  begin
    ...
    if not HTMLDrawEx(Canvas, s, r, TImageList(smallimages), msg.XPos, msg.YPos, -1, -1, 1, True, False, False, False, True, False, WordWrap, FFormScaled, 0.0,
 ...
 {-------------------------------------------------
  ...
  begin
    ...
    begin
      ...
      HTMLDrawEx(Canvas, tmpstr, r, TImageList(SmallImages), 0, 0, -1, -1, 1, False, True, False, False, True, False, WordWrap, FFormScaled, 1.0,
    ...
    begin
      ...
      if pos('{\', s) > 0 then
        ...
        begin
          ...
          SetMapMode(Canvas.Handle, mm_text);
          Canvas.Font.size := fprintsettings.Font.size;
          HTMLDrawEx(Canvas, s, tr, timagelist(smallimages), 0, 0, -1, -1, 1, False, False, True, False, True, False, WordWrap, FFormScaled, ResFactor,
            FURLColor, clNone, clNone, clGray, Anchor, Stripped, FA, XSize, YSize, ml, hl, hr, FImageCache, FContainer,0);
          SetMapMode(Canvas.Handle, mm);
        end
        else
...
procedure TAdvListView.SetOwnerDrawEx(const Value: boolean);
begin
  if SubItemEdit and not Value then
    Exit;

  inherited OwnerDraw := Value;
end;
...
begin
  ...
  for i := 0 to ColCount - 1 do
    Columns[i].Caption := 'Col '+inttostr(i);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if not HTMLDrawEx(Canvas, s, r, TImageList(SmallImages), hintinfo.cursorpos.x, hintinfo.cursorpos.y, -1, -1, 1, True, False, False, False, True, False, WordWrap, FFormScaled, 1.0,
            FURLColor, clNone, clNone, clGray, Anchor, Stripped, fa, XSize, YSize, hl, ml, hr, FImageCache, FContainer,0) then
          Anchor := '';
