...
const
  ...
  // 1.1.1.5 : Fixed : Issue with using StateImages
  // 1.1.1.6 : Fixed : Issue with drawing regular imagelist images
...
type
  ...
  private
    ...
    procedure HeaderClick(Sender: TObject; ASection:integer);
    procedure HeaderRightClick(Sender: TObject; ASection:integer);
...
begin
  ...
  if Assigned(StateImages) then
    FIndent := FIndent + StateImages.Width;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (TVcd.nmcd.rc.left + FImgWidth < TVcd.nmcd.rc.left + GetColWidth(0)) then
            begin
          ...
          if Assigned(StateImages) and GetColImage(0) and
            ((tn.StateIndex >= 0) or (tn.SelectedIndex >= 0)) then
          begin
            FImgWidth := StateImages.Width;
            ...
            begin
              if (TVcd.nmcd.uItemState and CDIS_SELECTED = CDIS_SELECTED) and
                (tn.SelectedIndex >= 0) then
                StateImages.Draw(ACanvas, TVcd.nmcd.rc.left, nr.Top {TVcd.nmcd.rc.top}, tn.SelectedIndex)
              else
                if (tn.StateIndex >= 0) then
                  StateImages.Draw(ACanvas, TVcd.nmcd.rc.left, nr.Top {TVcd.nmcd.rc.top}, tn.StateIndex);
            end;

            if tn.OverlayIndex >= 0 then
              StateImages.Draw(ACanvas,TVcd.nmcd.rc.left,nr.Top {TVcd.nmcd.rc.top},tn.OverlayIndex);
          end;
