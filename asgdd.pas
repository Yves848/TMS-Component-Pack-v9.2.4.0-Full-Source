...
begin
  ...
  try
    ...
    if (HasURL and FAcceptURLs) then FDropFormats := FDropFormats + [dfURL];
    if (HasURI and FAcceptURLs) then FDropFormats := FDropFormats + [dfURI];
...
begin
  ...
  begin
    with TEnumFormats.Create(DataObj) do
    try
      if HasURI and FAcceptURLs then
      begin
        DropURL(pt, Text);
      end
      else
