...
\**************************************************************************)
  ...
  public
    ...
    constructor Create(hRgn: HRGN; {%H-}Dummy1: integer = 0; {%H-}Dummy2: integer = 0); reintroduce; overload;
...
//--------------------------------------------------------------------------
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
  public
    constructor Create(ANativeImage: GpImage; status: TStatus); reintroduce; overload;
  ...
  public
    ...
    constructor Create(hicon: HICON;
      {%H-}Dummy1: integer = 0); reintroduce; overload;
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
  public
    constructor Create(ANativeCap: GpCustomLineCap; status: TStatus); reintroduce; overload;
    constructor Create; reintroduce; overload;
...
\**************************************************************************)
  ...
  public
    constructor Create(ANativeMatrix: GpMatrix); reintroduce; overload;
    ...
    function Translate(offX, offY: Single; order: TMatrixOrder = MatrixOrderPrepend): TStatus;      // ok
...
\**************************************************************************)
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
  public
    constructor Create(ANativeBrush: GpBrush; status: TStatus); overload;
...
//--------------------------------------------------------------------------
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
  public
    constructor Create(ANativePen: GpPen; status: TStatus); reintroduce; overload;
...
\**************************************************************************)
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
    constructor Create(ANativePath: GpPath); reintroduce; overload;
  public
    constructor Create(path: TGPGraphicsPath); reintroduce; overload;
...
\**************************************************************************)
  ...
  public
    ...
    constructor Create(hdc: HDC; {%H-}Dummy1: integer = 0; {%H-}Dummy2: integer = 0); reintroduce; overload;
    ...
    // DrawString
    function DrawString(string_: string; font: TGPFont;
...
\********************************************************************F******)
  ...
  var c: GpImageAttributes;
  begin
    SetStatus(GdipCloneImageAttributes(nativeImageAttr, c));
    result := TGPImageAttributes.Create(c, lastResult);
  end;
  ...
  procedure TGPImageAttributes.SetNativeImageAttr(ANativeImageAttr: GpImageAttributes);
  begin
    self.nativeImageAttr := ANativeImageAttr;
  end;
...
\**************************************************************************)
  ...
  function TGPMatrix.Translate(offX, offY: Single; order: TMatrixOrder = MatrixOrderPrepend): TStatus;
  begin
    result := SetStatus(GdipTranslateMatrix(nativeMatrix, offX, offY, order));
  end;
  ...
  constructor TGPMatrix.Create(ANativeMatrix: GpMatrix);
  begin
    lastResult := Ok;
    SetNativeMatrix(ANativeMatrix);
  end;

  procedure TGPMatrix.SetNativeMatrix(ANativeMatrix: GpMatrix);
  begin
    self.nativeMatrix := ANativeMatrix;
  end;
...
\**************************************************************************)
  ...
  constructor TGPRegion.Create(ANativeRegion: GpRegion);
  begin
    SetNativeRegion(ANativeRegion);
  end;

  procedure TGPRegion.SetNativeRegion(ANativeRegion: GpRegion);
  begin
...
\**************************************************************************)
  ...
  constructor TGPCustomLineCap.Create(ANativeCap: GpCustomLineCap; status: TStatus);
  begin
    lastResult := status;
    SetNativeCap(ANativeCap);
  end;

  procedure TGPCustomLineCap.SetNativeCap(ANativeCap: GpCustomLineCap);
  begin
    self.nativeCap := ANativeCap;
  end;
...
\**************************************************************************)
  ...
  constructor TGPPen.Create(ANativePen: GpPen; status: TStatus);
  begin
    lastResult := status;
    SetNativePen(ANativePen);
  end;

  procedure TGPPen.SetNativePen(ANativePen: GpPen);
  begin
    self.nativePen := ANativePen;
  end;
...
//--------------------------------------------------------------------------
  ...
  constructor TGPBrush.Create(ANativeBrush: GpBrush; status: TStatus);
  begin
    lastResult := status;
    SetNativeBrush(ANativeBrush);
  end;

  procedure TGPBrush.SetNativeBrush(ANativeBrush: GpBrush);
  begin
    self.nativeBrush := ANativeBrush;
  end;
...
//--------------------------------------------------------------------------
  ...
  constructor TGPImage.Create(ANativeImage: GpImage; status: TStatus);
  begin
    SetNativeImage(ANativeImage);
    lastResult := status;
  end;

  procedure TGPImage.SetNativeImage(ANativeImage: GpImage);
  begin
    self.nativeImage := ANativeImage;
  end;
...
\**************************************************************************)
  ...
  begin
    Result := CompositingQualityDefault;
  ...
  function TGPGraphics.DrawString( string_: string; font: TGPFont;
...
\**************************************************************************)
  ...
  constructor TGPGraphicsPath.Create(ANativePath: GpPath);
  begin
    lastResult := Ok;
    SetNativePath(ANativePath);
  end;

  procedure TGPGraphicsPath.SetNativePath(ANativePath: GpPath);
  begin
    self.nativePath := ANativePath;
  end;
...
//--------------------------------------------------------------------------
  ...
  procedure TGPGraphicsPathIterator.SetNativeIterator(ANativeIterator: GpPathIterator);
  begin
    self.nativeIterator := ANativeIterator;
  end;
