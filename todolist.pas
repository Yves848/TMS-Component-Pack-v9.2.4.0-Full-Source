...
{$ENDIF}
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Jan, 2018'; // Month version
  ...
  // 1.5.4.0 : New : Enabled  property added at item level
  // 1.5.5.0 : New : GridLinePen property added
...
{$ENDIF}
  ...
  published
    ...
    property Completion: TCompletion read FCompletion write SetCompletion default 0;
    ...
    property Priority: TTodoPriority read FPriority write SetPriority default tpLowest;
    property Project: string read FProject write SetProject;
    property Resource: string read FResource write SetResource;
    property Status: TTodoStatus read FStatus write SetStatus default tsNotStarted;
...
//    procedure WMPaint(var Message: TWMPaint); message WM_PAINT;
    ...
    procedure SetGridLinePen(const Value: TGridLinePen);
  protected
  ...
  published
    ...
    property GridLinePen: TGridLinePen read FGridLinePen write SetGridLinePen default lpDotted;
...
{$ENDIF}
    ...
    procedure SetGridLinePen(const Value: TGridLinePen);
    function GetGridLinePen: TGridLinePen;
...
{$ENDIF}
    ...
    property GridLinePen: TGridLinePen read GetGridLinePen write SetGridLinePen default lpDotted;
...
begin
  ...
  begin
    ...
    if not TodoItems[Index].Enabled then
  ...
  begin
    ...
    if GridLinePen = lpDotted then
      HPen := ExtCreatePen(PS_COSMETIC or PS_ALTERNATE,1,lb,0,nil)
    else
      HPen := ExtCreatePen(PS_SOLID, 1, lb, 0, nil);
...
procedure TTodoListBox.SetGridLinePen(const Value: TGridLinePen);
begin
  FGridLinePen := Value;
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    Enabled := TTodoItem(Source).Enabled;
  end;
end;
...
procedure TTodoItem.SetEnabled(const Value: boolean);
begin
...
function TCustomTodoList.GetGridLinePen: TGridLinePen;
begin
  Result := FTodoListBox.GridLinePen;
end;
...
procedure TCustomTodoList.SetGridLinePen(const Value: TGridLinePen);
begin
  FTodoListBox.GridLinePen := Value;
end;
