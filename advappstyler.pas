...
const
  ...
  // 2.2.1.1 : Fixed : Issue with setting AppColor at TAdvAppStyler level
  // 2.3.0.0 : New : AutoThemeAdapt takes Windows version in account when Office not found
...
type
  ...
  private
    ...
    procedure SetAppColor(const Value: TColor);
  protected
  ...
  published
    property AppColor: TColor read FAppColor write SetAppColor default clNone;
  ...
  published
    property AppColor: TColor read FAppColor write SetAppColor default clBlack;
    property AutoThemeAdapt: boolean read FAutoThemeAdapt write SetAutoThemeAdapt default false;
    property Enabled: boolean read FEnabled write SetEnabled default true;
    property Metro: boolean read FMetro write SetMetro default false;
    property MetroColor: TColor read FMetroColor write SetMetroColor default clHighlight;
...
procedure TAdvFormStyler.SetAppColor(const Value: TColor);
begin
  if (FAppColor <> Value) then
  begin
    FAppColor := Value;

    if (Style <> tsCustom) and not (csLoading in ComponentState) then
    begin
      ApplyStyle;
    end;
  end;
...
procedure TAdvFormStyler.SetComboBox(const Value: TCustomComboBox);
var
  sl: TStringList;
  i: integer;
begin
  ...
  begin
    ...
    sl.Free;
    TCustomComboBoxEx(FComboBox).OnSelect := HandleStyleSelect;
    TCustomComboBoxEx(FComboBox).Style := csDropDownList;
...
begin
  ...
  otUnknown:
    begin
      ...
      winxp: Style := tsWindowsXP;
      win7: Style := tsWindows7;
      win8: Style := tsWindows8;
      win10: Style := tsWindows10;
      end;
    end;
...
procedure TAdvAppStyler.SetAppColor(const Value: TColor);
var
  i: integer;
begin
  FAppColor := Value;
  ...
  begin
    ...
    begin
      ...
      TAdvFormStyler(FForms[i]).AppColor := FAppColor;
    end;
  end;
  DoChange;
end;
...
begin
  ...
  for i := 0 to FForms.Count - 1 do
  begin
    TAdvFormStyler(FForms[i]).Metro := FMetro;
    TAdvFormStyler(FForms[i]).MetroStyle := FMetroStyle;
    TAdvFormStyler(FForms[i]).MetroColor := FMetroColor;
    TAdvFormStyler(FForms[i]).MetroTextColor := FMetroTextColor;
    TAdvFormStyler(FForms[i]).AppColor := FAppColor;
    TAdvFormStyler(FForms[i]).Style := FStyle;
    TAdvFormStyler(FForms[i]).AppColor := FAppColor;
  end;
