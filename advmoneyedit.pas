...
const
  ...
  // v1.1.2.7 : Fixed : Issue with Signed handling and leaving focus
  // v1.1.2.8 : Improved : Behavior of control when Button.Visible = false
  // v1.1.2.9 : Improved : Positioning of dropdown button
...
begin
  ...
  begin
     ...
     FDPIScale := GetDPIScale(Self, FButton.Canvas);
     BtnW := Round(18 * FDPIScale);

     if NewStyleControls and Ctl3D then
       FButton.SetBounds(Width - BtnW - Dist - Offs - 1, -1, BtnW, Height - Dist)
     else
       FButton.SetBounds(Width - BtnW - Offs, 1, BtnW, Height - 1);
...
begin
  ...
  case msg.CharCode of
  VK_F4:
    if Button.Visible then ShowCalculator;
...
begin
  ...
  if not Signed and (prevop = 2) then
    Exit;
