...
const
 ...
 //  1.9.1.0 : New : High DPI support added
 //  1.9.2.0 : New : Public Object property added in TMsgCollectionItem
 //  1.9.2.1 : Improved : Hiding TAdvAlertWindow on citrix
 //  1.9.2.2 : Improved : HTML engine drawing in high DPI mode with form.Scaled = false
 //  2.0.0.0 : Improved : High DPI support
 //  2.0.0.1 : Fixed : Rare issue when creating & destroying alert window before parent form is shown
 //  2.1.0.0 : New : VCL Styles support added
 //  2.1.0.1 : Improved : VCL Styles support
...
begin
  ...
  begin
    if HTMLDrawEx(Canvas, AlertMessages.Strings[FMsgScroller.Position - 1], rd, FImages, pt.X, pt.Y + FScrollPos, -1, -1, 1, True, False, False, False, False, False, True, True,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //FillRect(R);
        R.Bottom := R.Bottom - 1;
        DrawGradient(Canvas, FBtnDownColor, FBtnDownColorTo, btnsize, R, false);
        R.Bottom := R.Bottom + 1;
        Pen.Color := clBlack;
        Brush.Style := bsClear;
    ...
    begin
      ...
      begin
        R.Bottom := R.Bottom - 1;
        DrawGradient(Canvas, FBtnHoverColor, FBtnHoverColorTo, btnsize, R, false);
        R.Bottom := R.Bottom + 1;
        Pen.Color := clBlack;
        Brush.Style := bsClear;
        Rectangle(R.Left, R.Top, R.Right, R.Bottom);
    ...
    begin
      ...
      if LStyle.GetElementColor(LDetails, ecTextColor, clr) then
        Font.Color := clr;
    end
    else
    {$ENDIF DELPHIXE2_LVL}
    ...
    begin
      ...
      Canvas.StretchDraw(Rect(R.left + 2, R.Top + 2, R.left + 2 + btnsize, R.Top + 2 + btnsize), FGlyphClose)
    end
    else
    begin
      Brush.Style := bsClear;
      ...
      DrawText(Canvas.Handle, 'X', 1, r, DT_SINGLELINE + DT_VCENTER + DT_CENTER);
...
var
  ...
  R: TRect;
begin
  R := Rect(1, 1, width - 1, Round(7*FDPIScale));
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, AlertMessages.Strings[FMsgScroller.Position - 1], TextR, FImages, 0, 0, -1, FHoverLink, 1, False, False, False, False, True, FHover, True, True,
...
begin
  ...
  HTMLDrawEx(Canvas, AlertMessages.Strings[Index], rd, FImages, 0, 0, -1, FHoverLink, 1, False, True, False, False, True, FHover, True, True,
    1.0, FURLColor, clNone, clNone, clGray, Anchor, Stripped, FocusAnchor, XSize, YSize,
    HyperLinks, MouseLink, hr, FImageCache, FContainer, 0, BidiMode);
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, AlertMessages.Strings[i], rd, FImages, 0, 0, -1, FHoverLink, 1, False, False, False, False, True, FHover, True, True,
      1.0, FURLColor, clNone, clNone, clGray, Anchor, Stripped, FocusAnchor, XSize, YSize,
      HyperLinks, MouseLink, hr, FImageCache, FContainer, 0, BidiMode);
...
begin
  Result := Rect(Width - Round(CloseBtnSize * FDPIScale) - 1, 8, Width - 1, 8 + Round(CloseBtnSize * FDPIScale));
end;
...
procedure TAlertWindow.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  FUseVCLStyles := LStyle.Enabled and (LStyle.Name <> 'Windows');
...
{$IFDEF DELPHIXE2_LVL}
procedure TAlertWindow.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
begin
  ...
  ImgW := 0;
  if FImages <> nil then
    ImgW := FImages.Width + Round(5 * FDPIScale);

  SH := ScrollBtnHeight;
  if not ShowDelete and (AlertMessages.Count <= 1) then
    SH := 0;

  Result := Rect(R.Left + Imgw + MarginX, R.Top + Round(CloseBtnSize * FDPIScale) + MarginY, R.Right - Round(CloseBtnSize * FDPIScale) - MarginX, R.Bottom - MarginY - SH);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          LDetails := LStyle.GetElementDetails(tgCellNormal);
          if LStyle.GetElementColor(LDetails, ecShadowColor, clr) then
            Font.Color := clr
          else
            Font.Color := clGray;
        end
      ...
      begin
        ...
        if FMsgScroller.CanGoBack then
          Canvas.StretchDraw(Rect(R.left + 2, R.Top + 4, R.left + 2 + btnsize, R.Top + 4 + btnsize), FGlyphPrev)
        else
        begin
          Canvas.StretchDraw(Rect(R.left + 2, R.Top + 4, R.left + 2 + btnsize, R.Top + 4 + btnsize), FGlyphPrevDisabled);
        end;
      end
      else
      begin
        ...
        Font.Size := 10;
        SetBkMode(Canvas.Handle, TRANSPARENT);
        DrawText(Canvas.Handle, '<',1, r, DT_SINGLELINE + DT_VCENTER + DT_CENTER);
        Font.Assign(Self.Font);
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FMsgScroller.CanGoForward then
          Canvas.StretchDraw(Rect(R.left + 2, R.Top + 4, R.left + 2 + btnsize, R.Top + 4 + btnsize), FGlyphNext)
        else
        begin
          Canvas.StretchDraw(Rect(R.left + 2, R.Top + 4, R.left + 2 + btnsize, R.Top + 4 + btnsize), FGlyphNextDisabled);
        end;
      end
      else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      {$ENDIF DELPHIXE2_LVL}
      Font.Size := Round(Font.Size * FDPIScale);

      x := TextWidth(S);
      x := (Width div 2) - (x div 2);
      Brush.Style := bsClear;
      TextOut(x, Height - TextHeight('gt') - Round(5 * FDPIScale), S);
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Canvas.StretchDraw(Rect(R.left + 2, R.Top + 4, R.left + 2 + btnsize, R.Top + 4 + btnsize), FGlyphDelete);
      end
      else
      begin
        Brush.Style := bsClear;
...
begin
  lf := Width div 2;
  lf := lf - Round(ScrollBtnWidth * FDPIScale) - (ScrollTextWidth div 2);
  Result := Rect(lf, Height - Round(ScrollBtnHeight * FDPIScale) - 1, lf + Round(ScrollBtnWidth * FDPIScale), Height - 1);
end;
...
begin
  lf := Width div 2;
  if FMsgScroller.Visible then
    lf := lf + (ScrollTextWidth div 2) + Round(ScrollBtnWidth * FDPIScale)
  else
...
begin
  Result := PtInRect(Rect(1, 1, width - 1, Round(7*FDPIScale)), Point(X, Y));
end;
...
begin
  ...
  {$IFDEF DELPHIXE6_LVL}
  FAlertWindow.StyleElements := FStyleElements;
  {$ENDIF}
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvAlertWindow.CMStyleChanged(var Message: TMessage);
begin
  if Assigned(FAlertWindow) then
  begin
    if StyleServices.Enabled and (StyleServices.Name <> 'Windows') then
    begin
      FAlertWindow.InitVCLStyle(True);
      FAlertWindow.RecreateWnd;
    end;
  end;
end;
{$ENDIF}
...
begin
  ...
  begin
    if not FShowFullAlpha then
    begin
      if FAlertWindow.HandleAllocated then
        ShowWindow(FAlertWindow.Handle, SW_HIDE);
      FAlertWindow.Visible := false;
    end; 
  end;
  FFadeThread := nil;
...
{$IFDEF DELPHIXE6_LVL}
procedure TAdvAlertWindow.SetStyleElements(const Value: TStyleElements);
begin
  if FStyleElements <> Value then
  begin
    FStyleElements := Value;
    if Assigned(FAlertWindow) then
      FAlertWindow.StyleElements := Value;
  end;
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHIXE_LVL}
    &Object :=  (Source as TMsgCollectionItem).&Object;
    {$ENDIF}
  end;
end;
