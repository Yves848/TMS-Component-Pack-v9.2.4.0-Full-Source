...
const
  ...
  // v1.3.0.0 : New : Property ContentType added
  // v1.3.1.0 : New : Property EncodeSpaces added

type
  TWebPostContentType = (ctFormURLEncoded, ctMultiPartForm);
  ...
  private
    ...
    FContentType: TWebPostContentType;
    FEncodeSpaces: boolean;
  ...
  published
    ...
    property ContentType: TWebPostContentType read FContentType write FContentType default ctFormURLEncoded;
    property EncodeSpaces: boolean read FEncodeSpaces write FEncodeSpaces default true;
...
begin
  ...
  ctFormURLEncoded:  hdr := 'Content-Type: application/x-www-form-urlencoded';
  ctMultiPartForm: hdr := 'Content-Type: multipart/form-data';
  end;
  ...
  begin
    ...
    if FEncodeSpaces then
      while (pos(' ',string(value)) > 0) do
        value[pos(' ',string(value))] := '+';

    case FContentType of
      ctFormURLEncoded: data := data + FItems.Items[i - 1].Name + '=' + ansistring(URLEncode(string(value)));
      ctMultiPartForm: data := data + FItems.Items[i - 1].Name + '=' + ansistring(UTF8Encode(string(value)));
    end;
