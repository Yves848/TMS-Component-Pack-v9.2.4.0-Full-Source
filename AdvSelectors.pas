...
const
  ...
  // 1.4.1.0 : Improved : Support for High DPI
  // 1.4.1.1 : Fixed: Stretch drawing issue with Pen Width selector
  // 1.4.2.0 : New : PopupMenu exposed on TAdvToolSelector
...
begin
  ...
  if not (csDesigning in ComponentState) then
  begin
    FDropDownWindow.FDPIScale := FDPIScale;
  end;
...
begin
  ...
  FMouseDown := true;
  if (Style in [ssButton {, ssCheck}]) and (Button = mbLeft) then
  begin
    if FDropDownButton and (X > (width - Round(FDPIScale * FDropDownBtnWidth))) then
    begin
      PopupBtnDown;
    end
    else
...
var
  ...
  begin
    ...
    pt[0].X := ArP.X;
    pt[0].Y := ArP.Y;
    pt[1].X := ArP.X + Round(FDPIScale * 4);
    pt[1].Y := ArP.Y;
    pt[2].X := ArP.X + Round(FDPIScale * 4 / 2);
    pt[2].Y := ArP.Y + Round(FDPIScale * 2);

    Canvas.Brush.Color := ArClr;
...
begin
  ...
  begin
    ...
    begin
      R2 := Rect(R.Left, R.Top, R.Right - Round(FDPIScale * FDropDownBtnWidth), R.Bottom);
      CapR := Rect(R.Left + 2, R.Top, R2.Right, R.Bottom);
      BtnR := Rect(R.Right - Round(FDPIScale * FDropDownBtnWidth), R.Top, R.Right, R.Bottom);
    end
    else
    begin
      R2 := R;
    ...
    AP.X := BtnR.Left + ((BtnR.Right - BtnR.Left - Round(FDPIScale * 4)) div 2);
    AP.Y := BtnR.Top + ((BtnR.Bottom - BtnR.Top - Round(FDPIScale * 2)) div 2);
...
var
  ...
  begin
    ...
    Inc(ARect.Left, ClientWidth - Round(FDPIScale * FDropDownBtnWidth));
...
begin
  ...
  begin
    R2 := Rect(R.Left, R.Top, R.Right - Round(FDPIScale * FDropDownBtnWidth), R.Bottom);
    CapR := Rect(R.Left + 2, R.Top, R2.Right, R.Bottom);
    BtnR := Rect(R.Right - Round(FDPIScale * FDropDownBtnWidth) - 1, R.Top + 1, R.Right - 1, R.Bottom - 1);
...
begin
  ...
  w := FDropDownWindow.Width;
  h := FDropDownWindow.Height;
  ...
  if Assigned(FSelectorPanel) then
    FSelectorPanel.Width := w;
   FDropDownWindow.Width := w;
  if Assigned(FSelectorPanel) then
    FSelectorPanel.Height := h;
...
begin
  ...
  FSelectorPanel.FDPIScale := GetDPIScale(Self, Canvas);
...
begin
  if Assigned(FSelectorPanel) then
  begin
    Height := FSelectorPanel.GetVisibleHeight;
    Width := FSelectorPanel.Width;
  end;
end;
...
procedure TAdvCustomColorSelector.DoDropDown;
begin
  inherited;

  if Assigned(FColorCubePanel) and FColorCubePanel.Visible then
  begin
    FColorCubePanel.Width := FDropDownWindow.Width;
    FColorCubePanel.Height := FDropDownWindow.Height;
  end;

  if Assigned(FSpectrumPanel) and FSpectrumPanel.Visible then
  begin
    FSpectrumPanel.Width := FDropDownWindow.Width;
    FSpectrumPanel.Height := FDropDownWindow.Height;
    FSpectrumPanel.Left := 0;
    FSpectrumPanel.Top := 0;
    FSpectrumPanel.FSpectrumImage.Height := 128;
    FSpectrumPanel.FSpectrumImage.Width := 128;
    FSpectrumPanel.FSpectrumImage.Left := (FSpectrumPanel.Width - FSpectrumPanel.FSpectrumImage.Width) div 2;
    FSpectrumPanel.FSpectrumImage.Top := 10;
  end;
end;
...
var
  ...
  R2: TRect;
  P: TPoint;
  cCellSize, cCellSpace: integer;
  cLabelHeight, cTopMargin, cLeftMargin: integer;
begin
  inherited;
  cCellSize := Round(FDPIScale * FCellSize);
  cCellSpace := Round(FDPIScale * FCellSpace);
  cLabelHeight := Round(FDPIScale * FLabelHeight);
  cTopMargin := Round(FDPIScale * FTopMargin);
  cLeftMargin := Round(FDPIScale * FLeftMargin);
  ...
  if TSelectorDropDownWindow(FOwner).ShowAbove then
  begin
    ay := cLabelHeight + cTopMargin;
    ay := ay + ((cCellSize + cCellSpace) * (RowCount - 1));

    for r := 1 to RowCount do
    begin
      for c := 1 to ColCount do
      begin
        if PtInRect(Rect(ax, ay, ax + cCellSize, ay + cCellSize), Point(X, Y)) then
        begin
        ...
        ax := ax + cCellSize + cCellSpace;
      end;

      if CellFound then
        break;
      ax := cLeftMargin;
      ay := ay - cCellSize - cCellSpace;
    end;
  end
  else
  begin
  ...
  if TSelectorDropDownWindow(FOwner).ShowAbove then
  begin
    if (Y < cLabelHeight + cTopMargin) and not CellFound then
    begin
      FSelectedCols := -1;
      FSelectedRows := -1;
  ...
  begin
    if (X > Width - cLeftMargin - 2) and not CellFound then
    begin
      ...
      if (P.X + Width + cCellSize + 2) < R2.Right then
      begin
        ColCount := ColCount + 1;
        SetPanelSize;
    ...
    begin
      ...
      begin
        ...
        if (P.Y - cCellSize - 2) > R2.Top then
        begin
          RowCount := RowCount + 1;
          SetPanelSize;
          TSelectorDropDownWindow(FOwner).SetWindowSize;
          TSelectorDropDownWindow(FOwner).Top := TSelectorDropDownWindow(FOwner).Top - cCellSize - cCellSpace;
        end;
      end;
    end
    else // if not TSelectorDropDownWindow(FOwner).ShowAbove then
    begin
      if (Y > Height - cLabelHeight - 2) then
      begin
        SystemParametersInfo(SPI_GETWORKAREA, 0, @R2, 0);
        ...
        if (P.Y + Height + cCellSize + 2) < R2.Bottom then
        begin
          RowCount := RowCount + 1;
          SetPanelSize;
          TSelectorDropDownWindow(FOwner).SetWindowSize;
...
begin
  ...
  if TSelectorDropDownWindow(FOwner).ShowAbove then
    R := Rect(Round(FDPISCale * FLeftMargin), 1, Width - Round(FDPISCale * FLeftMargin), Round(FDPISCale * FLabelHeight))
  else
    R := Rect(Round(FDPISCale * FLeftMargin), Height - Round(FDPISCale * FLabelHeight), Width - Round(FDPISCale * FLeftMargin), Height - Round(FDPISCale * FTopMargin));
...
var
  c, r, x, y: integer;
  cCellSize, cCellSpace, cLabelHeight: integer;
begin
  x := Round(FDPIScale * FLeftMargin);
  cLabelHeight := Round(FDPIScale * FLabelHeight);
  cCellSize := Round(FDPISCale * FCellSize);
  cCellSpace := Round(FDPISCale * FCellSpace);
  ...
  if TSelectorDropDownWindow(FOwner).ShowAbove then
  begin
    y := cLabelHeight + Round(FDPISCale * FTopMargin);
    y := y + ((cCellSize + cCellSpace) * (RowCount - 1));
    ...
    begin
      ...
      begin
        ...
        if (c <= FSelectedCols) and (r <= FSelectedRows) then
          ...
          Canvas.Brush.Color := clWhite;
        Canvas.Rectangle(x, y, x + cCellSize, y + cCellSize);

        x := x + cCellSize + cCellSpace;
      end;
      x := Round(FDPISCale * FLeftMargin);
      y := y - cCellSize - cCellSpace;
    end;
  end
  else
  begin
    y := Round(FDPISCale * FTopMargin);
    for r := 1 to RowCount do
    begin
      for c := 1 to ColCount do
      begin
...
begin
  ...
  for c := 1 to ColCount do
  begin
    x := x + Round(FDPIScale * FCellSize) + Round(FDPIScale * FCellSpace);
  end;
  x := x + Round(FDPIScale * FLeftMargin) - Round(FDPIScale * FCellSpace);

  for r := 1 to RowCount do
  begin
    y := y + Round(FDPIScale * FCellSize) + Round(FDPIScale * FCellSpace);
  end;
  y := y + Round(FDPIScale * FTopMargin) - Round(FDPIScale * FCellSpace);

  Height := y + Round(FDPIScale * FLabelHeight);
...
var
  ...
  cCellSize, cCellSpace: integer;
  cLeftMargin, CTopMargin, cLabelHeight: integer;
begin
  Rc := Rect(0, 0, 0, 0);
  cCellSize := Round(FDPIScale * FCellSize);
  cCellSpace := Round(FDPIScale * FCellSpace);
  ...
  begin
    ...
    if TSelectorDropDownWindow(FOwner).ShowAbove then
    begin
      y := cLabelHeight + cTopMargin;
      y := y + ((cCellSize + cCellSpace) * (RowCount - 1));

      Rc.Bottom := y + (cCellSize + cCellSpace);
  ...
  begin
    ...
    begin
      y := cLabelHeight + cTopMargin;
      y := y + ((cCellSize + cCellSpace) * (RowCount));
  ...
  if TSelectorDropDownWindow(FOwner).ShowAbove then
    RgnL := CreateRectRgn(cLeftMargin, 1, Width - cLeftMargin, cLabelHeight)
  else
    RgnL := CreateRectRgn(cLeftMargin, Height - cLabelHeight, Width - cLeftMargin, Height - cTopMargin);
...
begin
  ...
  FTableSelectorPanel.FDPIScale := FDPIScale;
...
{  if j >= 0 then
  ...
  FSelectorPanel.Canvas.Font.Height := Round(FDPIScale * FCharFont.Height);
  FSelectorPanel.Font.Assign(FCharFont);
  FSelectorPanel.Font.Height := Round(FDPIScale * FCharFont.Height);
  FSelectorPanel.SetItemsPosition;
end;
