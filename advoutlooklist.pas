...
const
  ...
  // 1.7.10.2: Improved : HTML engine drawing in high DPI mode with form.Scaled = false
  // 1.7.10.3: Improved : OnSelectionChange for select all is only triggered once
  // 1.7.10.4: Fixed : Issue with Ctrl-A handling when multiselect is disabled
  // 1.8.0.0 : New : VCL styles support added
  //         : New : POGLItem.YTop property added
  // 1.8.0.1 : Fixed : Issue for Win64 for C++Builder
  // 1.8.0.2 : Fixed : Issue with VCL Styles & inherited forms 
            ...
            case ColumnType of
              ...
              begin
                ...
                begin
                  ...
                  {$IFDEF DELPHIXE2_LVL}
                  if StyleServices.Enabled and (StyleServices.Name <> 'Windows') {$IFDEF DELPHIXE6_LVL}and (seFont in StyleElements){$ENDIF} then
                  begin
                    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgCellNormal), ecTextColor, clr);
                    HTMLTextColor := clr;
                    StyleServices.GetElementColor(StyleServices.GetElementDetails(tgCellNormal), ecTextColor, clr);
                    ItemCanvas.Font.Color := clr;
                  end
                  else
                  {$ENDIF}
                  begin
              ...
              begin
                ...
                begin
                  ...
                  StyleServices.DrawElement(ItemCanvas.Handle, lDetails, DrawRect);
                end
                else
                {$ENDIF}
...
function TAdvOutlookList.UseVCLStyles: boolean;
begin
...
begin
  ...
  begin
    ...
    begin
    {$IFDEF DELPHIXE2_LVL}
      clr := StyleServices.GetStyleColor(scBorder);
...
begin
  Result := HTMLDrawEx(Canvas,s,fr,FImages,xpos,ypos,-1,HoverLink,ShadowOffset,checkhotspot,checkheight,print,selected,Blink,
                       Hoverstyle,not FEllipsis,FFormScaled,Resfactor,urlcolor,hovercolor,hoverfontColor,shadowcolor,anchorval,stripval,focusanchor,
...
//------------------------------------------------------------------------------
procedure TAdvOutlookList.InitVCLStyle(init: boolean);
{$IFDEF DELPHIXE2_LVL}
var
  clr: TColor;
  i: Integer;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    SelectionColor     := StyleServices.GetSystemColor(clHighlight);
    SelectionTextColor := StyleServices.GetSystemColor(clHighlightText);
    ...
    {$ENDIF}
      ...
      begin
        if Columns[i].Color <> clNone then
          Columns[i].Color := clr;
      end;
    {$IFDEF DELPHIXE6_LVL}
    end;
...
//------------------------------------------------------------------------------
{$IFDEF DELPHIXE2_LVL}
procedure TAdvOutlookList.CMStyleChanged(var Message: TMessage);
begin
  InitVCLStyle(True);
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if not FOutLookList.FUseVCLStyles then
    begin
    ...
    begin
      ...
      begin
        ...
        if FOutLookList.FUseVCLStyles then
        begin
          {$IFDEF DELPHIXE2_LVL}
          lDetails := StyleServices.GetElementDetails(thHeaderItemNormal);
          StyleServices.DrawElement(Canvas.Handle, lDetails, SecRect);
          {$ENDIF}
        end
        else
        begin
        ...
        begin
          if FOutLookList.FUseVCLStyles {$IFDEF DELPHIXE6_LVL}and (seFont in StyleElements){$ENDIF} then
          begin
            {$IFDEF DELPHIXE2_LVL}
            lDetails := StyleServices.GetElementDetails(tgFixedCellNormal);
            StyleServices.DrawText(Canvas.Handle, lDetails, S, TxtRect, TTextFormatFlags(DT_SINGLELINE or DT_NOPREFIX or DT_VCENTER or format));
            {$ENDIF}
          end
          else
        ...
        begin
          ...
          begin
            {$IFDEF DELPHIXE2_LVL}
            lDetails := StyleServices.GetElementDetails(tgFixedCellNormal);
            StyleServices.DrawText(Canvas.Handle, lDetails, Sections[i], TxtRect, TTextFormatFlags(DT_SINGLELINE or DT_NOPREFIX or DT_VCENTER or format or DT_CALCRECT));
            {$ENDIF}
          end
          else
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvOutLookHeader.CMStyleChanged(var Message: TMessage);
begin
  Invalidate;
end;
{$ENDIF}
